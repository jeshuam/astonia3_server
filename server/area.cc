...
static struct sector sector1[] = {{73, 147, 91, 165, 45},    // thieves house
...
static struct sector sector2[] = {{1, 1, 85, 70, 4},
                                  {123, 2, 142, 57, 15},
                                  {127, 58, 142, 102, 15},
                                  {118, 102, 142, 124, 15},
                                  {118, 124, 128, 183, 16},
...
static struct sector sector4[] = {{1, 1, 254, 254, 63}, {0, 0, 0, 0, 0}};

static struct sector sector5[] = {{55, 70, 134, 119, 29},  // sewer1
...
static struct sector sector6[] = {{1, 1, 254, 254, 64}, {0, 0, 0, 0}};

static struct sector sector7[] = {{1, 1, 254, 254, 63}, {0, 0, 0, 0}};

static struct sector sector8[] = {{1, 1, 254, 254, 65}, {0, 0, 0, 0}};

static struct sector sector9[] = {{1, 1, 254, 254, 63}, {0, 0, 0, 0}};

static struct sector sector10[] = {{1, 1, 254, 254, 66}, {0, 0, 0, 0}};

static struct sector sector11[] = {{1, 1, 254, 254, 67}, {0, 0, 0, 0}};

static struct sector sector12[] = {{1, 1, 254, 254, 68}, {0, 0, 0, 0}};

static struct sector sector13[] = {{1, 1, 254, 254, 69}, {0, 0, 0, 0}};

static struct sector sector14[] = {{1, 1, 254, 254, 70}, {0, 0, 0, 0}};

static struct sector sector15[] = {{1, 1, 254, 254, 71}, {0, 0, 0, 0}};

static struct sector sector16[] = {{92, 76, 116, 96, 73},
                                   {42, 229, 86, 255, 74},
                                   {114, 193, 134, 226, 75},
                                   {1, 1, 254, 254, 72},
                                   {0, 0, 0, 0}};
...
static struct sector sector19[] = {{1, 1, 255, 255, 108}, {0, 0, 0, 0}};

static struct sector sector20[] = {
    {230, 233, 255, 255, 112}, {1, 1, 255, 255, 109}, {0, 0, 0, 0}};

static struct sector sector21[] = {{1, 1, 255, 255, 110}, {0, 0, 0, 0}};

static struct sector sector22[] = {{224, 199, 255, 255, 111},
                                   {134, 141, 223, 254, 113},
                                   {134, 1, 254, 140, 114},
                                   {224, 140, 254, 198, 114},
                                   {21, 171, 133, 254, 115},
...
static struct sector sector23[] = {{1, 1, 255, 255, 117}, {0, 0, 0, 0}};

static struct sector sector24[] = {{1, 1, 255, 255, 118}, {0, 0, 0, 0}};

static struct sector sector25[] = {{211, 132, 254, 254, 119},  // green
...
static struct sector sector27[] = {{0, 0, 0, 0}};

static struct sector sector28[] = {{0, 0, 0, 0}};
...
static struct sector sector30[] = {{0, 0, 0, 0}};

static struct sector sector31[] = {{1, 1, 255, 255, 137},  // grimroot
                                   {0, 0, 0, 0}};

static struct sector sector32[] = {{1, 1, 255, 255, 138},  // jobbington
                                   {0, 0, 0, 0}};

static struct sector sector33[] = {{1, 1, 255, 255, 139},  // long tunnel
                                   {0, 0, 0, 0}};
...
static struct sector sector35[] = {{0, 0, 0, 0}};

static struct sector sector36[] = {{207, 167, 221, 194, 146},
                                   {207, 138, 221, 165, 147},
                                   {207, 109, 221, 136, 148},
                                   {100, 175, 128, 227, 149},
                                   {142, 176, 170, 228, 150},
...
struct sector *area_sector[] = {
    NULL,     sector1,  sector2,  sector3,  sector4,  sector5,  sector6,
    ...
    sector28, sector29, sector30, sector31, sector32, sector33, sector34,
    sector35, sector36};
...
int get_section(int x, int y) {
  ...
  for (n = 0; sec[n].fx; n++) {
    if (x >= sec[n].fx && x <= sec[n].tx && y >= sec[n].fy && y <= sec[n].ty)
      break;
  }
...
const char *get_section_name(int x, int y) {
  struct sector *sec;
  int n;
...
void show_section(int x, int y, int cn) {
  ...
  // log_char(cn,LOG_SYSTEM,0,"If the following message seems wrong, please
  // report these numbers to Ishtar: %d,%d - %d",x,y,n);
  ...
  if (diff < -5) {
    log_char(cn, LOG_SYSTEM, 0,
             "%s. This area is way too easy for you. (%d,%d)", sec->name, x, y);
    return;
  } else if (diff > 5) {
    log_char(cn, LOG_SYSTEM, 0,
             "%s. This area is way too dangerous for you. (%d,%d)", sec->name,
             x, y);
    return;
  }
  switch (diff) {
    case -5:
      log_char(cn, LOG_SYSTEM, 0, "%s. This area is too easy for you. (%d,%d)",
               sec->name, x, y);
      return;
    case -4:
      log_char(cn, LOG_SYSTEM, 0, "%s. This area is easy for you. (%d,%d)",
               sec->name, x, y);
      return;
    case -3:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is fairly easy for you. (%d,%d)", sec->name, x,
               y);
      return;
    case -2:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is rather easy for you. (%d,%d)", sec->name, x,
               y);
      return;
    case -1:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is just about right for you. (%d,%d)", sec->name,
               x, y);
      return;
    case 0:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is just right for you. (%d,%d)", sec->name, x, y);
      return;
    case 1:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is slightly dangerous for you. (%d,%d)",
               sec->name, x, y);
      return;
    case 2:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is a bit dangerous for you. (%d,%d)", sec->name,
               x, y);
      return;
    case 3:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is somewhat dangerous for you. (%d,%d)",
               sec->name, x, y);
      return;
    case 4:
      log_char(cn, LOG_SYSTEM, 0, "%s. This area is dangerous for you. (%d,%d)",
               sec->name, x, y);
      return;
    case 5:
      log_char(cn, LOG_SYSTEM, 0,
               "%s. This area is very dangerous for you. (%d,%d)", sec->name, x,
               y);
      return;
  }
}
...
int register_kill_in_section(int cn, int co) {
  ...
  int t, s, index, bit, n, have = 0, need = 0, exp, diff, new_;

  if (!(ch[cn].flags & CF_PLAYER)) return 0;

  if (!(dat = (struct section_data *)set_data(cn, DRD_SECTION_DATA,
                                              sizeof(struct section_data))))
    return 0;  // OOPS
  ...
  if (new_ && exp && diff && ((diff % 10) == 0 || diff < 6 || have == 1)) {
  ...
  if (have >= need) {
    ...
    if (exp) {
      log_char(cn, LOG_SYSTEM, 0,
               "Strike! You killed all %d enemies in this section. Good work!",
               need);
...
void walk_section_msg(int cn) {
  ...
  if (s != dat->current_section) {
    if (s) {
      log_char(cn, LOG_SYSTEM, 0, "Now entering %s.", section[s].name);
    } else {
      log_char(cn, LOG_SYSTEM, 0, "Now leaving %s.",
               section[dat->current_section].name);
    }
...
void area_sound_wet_dungeon(int cn) {
  ...
  if (r == 10 || r == 20)
    player_special(cn, 14, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 30 || r == 40)
    player_special(cn, 15, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 50 || r == 60)
    player_special(cn, 16, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
}
...
void area_sound_dry_dungeon(int cn) {
  ...
  if (r == 10)
    player_special(cn, 36, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 20)
    player_special(cn, 37, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 30)
    player_special(cn, 38, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 40)
    player_special(cn, 39, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 50)
    player_special(cn, 40, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
}
...
void area_sound_woods(int cn) {
  ...
  if (hour > 6 && hour < 22) {
    if (r == 10)
      player_special(cn, 10, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 20)
      player_special(cn, 11, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 30)
      player_special(cn, 12, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 40)
      player_special(cn, 19, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 50)
      player_special(cn, 20, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 60)
      player_special(cn, 21, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 70)
      player_special(cn, 22, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 80)
      player_special(cn, 24, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 90)
      player_special(cn, 25, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  } else {
    if (r == 10)
      player_special(cn, 17, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 20)
      player_special(cn, 18, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 30)
      player_special(cn, 26, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 40)
      player_special(cn, 27, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 50)
      player_special(cn, 28, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
    if (r == 60)
      player_special(cn, 23, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  }
}
...
void area_sound_underwater(int cn) {
  ...
  if (r == 10)
    player_special(cn, 47, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 20)
    player_special(cn, 48, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
  if (r == 30)
    player_special(cn, 49, -(RANDOM(1000) + 100), 5000 - RANDOM(10000));
}
