...
// goto item and use it, returns 0 if not possible. check co if there is an
// enemy
int use_and_attack_driver(int cn, int in, int spec, int *co) {
  ...
  if (it[in].flags & IF_FRONTWALL) {
    ...
    if (!(map[it[in].x + it[in].y * MAXMAP + 1].flags &
      ...
      dir = pathfinder(ch[cn].x, ch[cn].y, it[in].x + 1, it[in].y, 0,
                       attack_check_target, 0);
    if (dir == -1 &&
        !(map[it[in].x + it[in].y * MAXMAP + MAXMAP].flags &
          (MF_MOVEBLOCK | MF_TMOVEBLOCK)))
      dir = pathfinder(ch[cn].x, ch[cn].y, it[in].x, it[in].y + 1, 0,
                       attack_check_target, 0);
  } else
    dir = pathfinder(ch[cn].x, ch[cn].y, it[in].x, it[in].y, 1,
                     attack_check_target, 0);
...
int qsortproc_gnometorch(const void *a, const void *b) {
  ...
  da = (it[*((int *)(a))].x - __gnomex) * (it[*((int *)(a))].x - __gnomex) +
       (it[*((int *)(a))].y - __gnomey) *
           (it[*((int *)(a))].y - __gnomey);  // short2int
  db = (it[*((int *)(b))].x - __gnomex) * (it[*((int *)(b))].x - __gnomex) +
       (it[*((int *)(b))].y - __gnomey) *
           (it[*((int *)(b))].y - __gnomey);  // short2int
  return db - da;
}
...
void scan_gnometorches(int cn, struct labgnome_driver_data *dat) {
  ...
  for (y = sy; y < ey && dat->numtorch < MAX_GNOMETORCH; y++) {
    for (x = sx; x < ex && dat->numtorch < MAX_GNOMETORCH; x++) {
      m = x + y * MAXMAP;
      if (map[m].it && it[map[m].it].driver == IDR_LABTORCH &&
          los_can_see(cn, ch[cn].x, ch[cn].y, x, y,
                      15)) {  //&& char_see_item(cn,map[m].it)*/) {
        if (pathfinder(ch[cn].x, ch[cn].y, x, y, 1,
                       scan_gnometorch_check_target, 0) == -1)
          continue;
  ...
  __gnomex = ch[cn].x;
  __gnomey = ch[cn].y;
  qsort(dat->torch, dat->numtorch, sizeof(int),
        qsortproc_gnometorch);  // short2int
}
...
void labgnome_driver_parse(int cn, struct labgnome_driver_data *dat) {
  ...
  for (ptr = nextnv(ch[cn].arg, name, value); ptr;
    ...
    if (!strcmp(name, "fighter"))
      dat->fighter = atoi(value);
    else if (!strcmp(name, "text"))
      dat->text = atoi(value);
    else if (!strcmp(name, "master"))
      dat->master = atoi(value);
    else if (!strcmp(name, "aggressive"))
...
void labgnome_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct labgnome_driver_data *)set_data(cn, DRD_LABGNOMEDRIVER,
                                                sizeof(*dat));
  if (!dat) return;  // oops...
    ...
    if (msg->type == NT_NPC && msg->dat1 == NTID_LABGNOMETORCH) {
      ...
      for (i = 0; i < dat->numtorch; i++) {
        if (in == dat->torch[i] && fight_driver_add_enemy(cn, co, 1, 1)) {
          shout(cn,
                "Hurgha. Master me told protecting torch. Prepare to die %s!",
                ch[co].name);
          break;
        }
      }
  ...
  if (dat->usetarget) {
    ...
    if (co)
      if (fight_driver_add_enemy(cn, co, 1, 1))
        shout(cn, "You're in my way %s! Die!", ch[co].name);
  }
  ...
  if (!dat->master) {
    switch (RANDOM(500)) {
      ...
      case 2:
        if (dat->numtorch)
          say(cn, "Me have to protect %d torch. Hungrfa.", dat->numtorch);
        break;
      case 3:
...
void labgnome_died_driver(int cn, int co) {
  ...
  if (dat->text)
    say(cn,
        "Arrrggh. %s me killed, but %s never kills master behind door. Master "
        "can be killed only by Deathfibrin.",
...
struct deathfibrin_data {
  ...
  int tickervanish;  // used to remove the staff from the map if it's to long
                     // alone
};
...
int deathfibrin_scan(int cn) {
  ...
  for (y = sy; y < ey; y++) {
    for (x = sx; x < ex; x++) {
      m = x + y * MAXMAP;
      if ((co = map[m].ch) && ch[co].driver == CDR_LABGNOMEDRIVER &&
          !strcmp(ch[co].name, "Immortal Master") && char_see_char(cn, co))
        return co;
    }
  }
  return 0;
...
int deathfibrin_check(int in, struct deathfibrin_data *dat) {
  ...
  sprintf(it[in].description, "Staff containing %d%% Deathfibrin",
          dat->amount / 100);
...
void deathfibrin(int in, int cn) {
  ...
  if (it[in].sprite == 10428) {
    ...
    if (ch[cn].citem) {
      log_char(cn, LOG_SYSTEM, 0,
               "The Shrine of Deathfibrin seems to ignore everything. It may "
  ...
  if (cn) {
    ...
    if (!co) {
      log_char(cn, LOG_SYSTEM, 0,
               "Nothing happens. There is no immortal close enough. %d",
               map[ch[cn].x + ch[cn].y * MAXMAP].light);
      return;
    }
    ...
    if (map[mn = x + y * MAXMAP].light > 4 && ticker > dat->tickerused) {
      dat->amount =
          max(0, dat->amount -
                     (map[mn].light - 3) * (map[mn].light * map[mn].light - 3));
