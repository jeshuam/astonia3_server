...
Game time and date. Also calculates stuff like sunrise/sunset, solstice
(longest/shortest
day of the year), equinox (day and night of same length), the state of the moon
and the
daylight created by sun and moon. It updates a bunch of global variables which
can be used
...
void tick_date(void) {
  ...
  if (daytime < sunrise)
    sunlight = 0;
  else if (daytime < sunrise + HOURLEN)
    sunlight = (daytime - sunrise) * 255 / HOURLEN;
  else if (daytime <= sunset)
    sunlight = 255;
  else if (daytime > sunset + HOURLEN)
    sunlight = 0;
  else if (daytime > sunset)
    sunlight = 255 - (daytime - sunset) * 255 / HOURLEN;

  if (moonrise < moonset) {
    if (daytime < moonrise)
      moonlight = 0;
    else if (daytime < moonrise + HOURLEN)
      moonlight = (daytime - moonrise) * 255 / HOURLEN;
    else if (daytime <= moonset)
      moonlight = 255;
    else if (daytime > moonset + HOURLEN)
      moonlight = 0;
    else if (daytime > moonset)
      moonlight = 255 - (daytime - moonset) * 255 / HOURLEN;
  } else {
    if (daytime <= moonset)
      moonlight = 255;
    else if (daytime < moonset + HOURLEN)
      moonlight = 255 - (daytime - moonset) * 255 / HOURLEN;
    else if (daytime < moonrise)
      moonlight = 0;
    else if (daytime < moonrise + HOURLEN)
      moonlight = (daytime - moonrise) * 255 / HOURLEN;
    else if (daytime >= moonrise)
...
void showtime(int cn) {
  log_char(cn, LOG_SYSTEM, 0,
           "It's %02d:%02d on the %d/%d/%d. Sunrise is at %02d:%02d, sunset at "
           "%02d:%02d. Moonrise is at %02d:%02d, moonset at %02d:%02d.",
           hour, minute, month + 1, mday + 1, year, sunrise / HOURLEN,
           (sunrise % HOURLEN) / MINLEN, sunset / HOURLEN,
           (sunset % HOURLEN) / MINLEN, moonrise / HOURLEN,
           (moonrise % HOURLEN) / MINLEN, moonset / HOURLEN,
           (moonset % HOURLEN) / MINLEN);

  if (!fullmoon && !newmoon) {
    if (moonsize < 3)
      log_char(cn, LOG_SYSTEM, 0, "Quarter Moon.");
    else if (moonsize < 10)
      log_char(cn, LOG_SYSTEM, 0, "Half Moon.");
    else
      log_char(cn, LOG_SYSTEM, 0, "Three Quarter Moon.");
  }

  if (newmoon) log_char(cn, LOG_SYSTEM, 0, "Be careful, New Moon tonight!");
  if (fullmoon)
    log_char(cn, LOG_SYSTEM, 0, "It's a fine day, Full Moon tonight!");

  if (summer_solstice)
    log_char(cn, LOG_SYSTEM, 0,
             "It's a great day, it's Summer Solstice today!");
  if (winter_solstice)
    log_char(cn, LOG_SYSTEM, 0,
  ...
  if (spring_equinox)
    log_char(cn, LOG_SYSTEM, 0,
             "Everything is in balance, it's Spring Equinox today!");
  if (fall_equinox)
    log_char(cn, LOG_SYSTEM, 0,
             "Everything is in balance, it's Fall Equinox today!");
  ...
  if (yday < 90) {
    log_char(cn, LOG_SYSTEM, 0, "Spring Equinox will be in %d days.",
             90 - yday);
  } else if (yday < 180) {
    log_char(cn, LOG_SYSTEM, 0, "Summer Solstice will be in %d days.",
             180 - yday);
  } else if (yday < 270) {
    log_char(cn, LOG_SYSTEM, 0, "Fall Equinox will be in %d days.", 270 - yday);
  } else {
    log_char(cn, LOG_SYSTEM, 0, "Winter Solstice will be in %d days.",
             360 - yday);
  }
}
