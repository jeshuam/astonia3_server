...
void create_special_execs(struct rune_ppd *ppd) {
  ...
  static int badlist[] = {555, 55,  5,  666, 66,  6,  777, 77,
                          7,   888, 88, 8,   999, 99, 9};
...
void bonebridge(int in, int cn) {
  ...
  if (!it[in].drdata[0] || it[in].drdata[1]) {  // basis or bones on ground
    if (!(in2 = ch[cn].citem) || it[in2].ID != IID_AREA18_BONE ||
        it[in2].drdata[0] != 5) {
...
void rune_raise(int cn, int war_skill, int mage_skill) {
  if (ch[cn].flags & CF_WARRIOR) {
    if (raise_value_exp(cn, war_skill))
      log_char(cn, LOG_SYSTEM, 0, "You gained %s.", skill[war_skill].name);
  } else {
    if (raise_value_exp(cn, mage_skill))
      log_char(cn, LOG_SYSTEM, 0, "You gained %s.", skill[mage_skill].name);
  }
}
...
void exec_rune(int cn, int nr, int lastholder) {
  ...
  ppd = (struct rune_ppd *)set_data(cn, DRD_RUNE_PPD, sizeof(struct rune_ppd));
  if (!ppd) return;
  ...
  switch (nr) {
    ...
    case 111:
      log_char(cn, LOG_SYSTEM, 0,
               "You hear a sinister laugh echo through the emptiness.");
    ...
    case 222:
      ...
      break;
    case 212:
      if (raise_value_exp(cn, V_ENDURANCE))
        log_char(cn, LOG_SYSTEM, 0, "You gained endurance.");
      flag = 1;
      break;
    ...
    case 333:
      ...
      break;
    case 231:
      if (raise_value_exp(cn, V_HP))
        log_char(cn, LOG_SYSTEM, 0, "You gained hitpoints.");
      flag = 1;
      break;
    case 133:
      if (raise_value_exp(cn, V_MANA))
        log_char(cn, LOG_SYSTEM, 0, "You gained mana.");
      flag = 1;
      break;
    ...
    case 444:
      ...
      break;
    case 143:
      if (raise_value_exp(cn, V_PARRY))
        log_char(cn, LOG_SYSTEM, 0, "You gained parry.");
      flag = 1;
      break;
    case 442:
      if (raise_value_exp(cn, V_MAGICSHIELD))
        log_char(cn, LOG_SYSTEM, 0, "You gained magic shield.");
      flag = 1;
      break;
    case 241:
      if (raise_value_exp(cn, V_IMMUNITY))
        log_char(cn, LOG_SYSTEM, 0, "You gained immunity.");
      flag = 1;
      break;
        ...
        if (ppd->special_exec[n] == nr) {
          switch (n) {
            ...
            case 0:
              ...
              break;
            case 1:
              rune_raise(cn, V_SWORD, V_PULSE);
              flag = 1;
              break;
            case 2:
              rune_raise(cn, V_TWOHAND, V_FIREBALL);
              flag = 1;
              break;
            case 3:
            ...
            case 13:
              ...
              break;
            case 14:
              rune_raise(cn, V_BODYCONTROL, V_STAFF);
              flag = 1;
              break;
            ...
            case 20:
              if (!lastholder)
                log_char(cn, LOG_SYSTEM, 0,
                         "This combination seems to be right, but it does not "
                         "work here.");
              else {
...
void boneholder(int in, int cn) {
  ...
  if (it[in].drdata[1] == 2 || it[in].drdata[1] == 3) {  // activation holder
    ...
    if (in2 && it[in2].drdata[0] &&
        *(unsigned int *)(it[in2].drdata + 8) == charID(cn)) {
    ...
    if (!nr) {
      log_char(cn, LOG_SYSTEM, 0,
               "You sense that you must place something on the stand before "
               "you can activate it.");
      return;
    }
  ...
  if (!cn) {  // expire runes after 120 seconds
    ...
    *(unsigned int *)(it[in].drdata + 12) = ticker;
    ...
    if (*(unsigned int *)(it[in].drdata + 8) != charID(cn)) {
      log_char(cn, LOG_SYSTEM, 0,
               "This rune does not belong to you. You cannot take it.");
      return;
    }
...
void bonewall(int in, int cn) {
  ...
  if (it[in].drdata[0] == 0) {
    if ((in2 = map[it[in].x + it[in].y * MAXMAP + 1].it) &&
        it[in2].driver == IDR_BONEWALL && it[in2].drdata[0] == 0)
      call_item(it[in2].driver, in2, 1, ticker + 4);
    if ((in2 = map[it[in].x + it[in].y * MAXMAP - 1].it) &&
        it[in2].driver == IDR_BONEWALL && it[in2].drdata[0] == 0)
      call_item(it[in2].driver, in2, 1, ticker + 4);
    if ((in2 = map[it[in].x + it[in].y * MAXMAP + MAXMAP].it) &&
        it[in2].driver == IDR_BONEWALL && it[in2].drdata[0] == 0)
      call_item(it[in2].driver, in2, 1, ticker + 4);
    if ((in2 = map[it[in].x + it[in].y * MAXMAP - MAXMAP].it) &&
        it[in2].driver == IDR_BONEWALL && it[in2].drdata[0] == 0)
...
void bonehint(int in, int cn) {
  ...
  static const char *rune_name[10] = {"none",   "Ansuz", "Berkano", "Dagaz",
                                      "Ehwaz",  "Fehu",  "Hagalaz", "Isa",
                                      "Ingwaz", "Raidho"};

  static const char *pos_name[3] = {"first", "second", "third"};
  ...
  res = buf[pos] - '0';
  if (res < 0 || res > 9) {
    log_char(cn, LOG_SYSTEM, 0, "You found bug #197-%d-%d-%d-%d", level, nr,
             pos, val);
    return;
  }

  log_char(cn, LOG_SYSTEM, 0, "Rune Diary, Page %d:", level * 10 + nr);
  log_char(cn, LOG_SYSTEM, 0, "Used the rune %s in the %s position.",
           rune_name[res], pos_name[pos]);
}
