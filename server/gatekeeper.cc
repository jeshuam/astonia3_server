...
static int enter_room(int cn, int cls, int xs, int ys) {
  int x, y, m, in, n, co;
  struct gate_ppd *ppd;

  ppd = (struct gate_ppd *)set_data(cn, DRD_GATE_PPD, sizeof(struct gate_ppd));
  if (!ppd) return 0;
  ...
  log_char(cn, LOG_SYSTEM, 0,
           "Once you are ready for the test, use the door to the south-west to "
           "enter the room containing your opponent. You have ten minutes from "
           "now on.");
  ...
  ppd->target_class = cls;
  ppd->step = 1;

  return 1;
}

static int enter_test(int cn, int cls) {
  static int room_start[] = {186, 196, 194, 196, 202, 196, 178,
                             212, 186, 212, 194, 212, 202, 212};
  int n, cnt;
  ...
  if (!(ch[cn].flags & CF_GOD)) {
    ...
    if (cnt > 3) {
      log_char(cn, LOG_SYSTEM, 0,
               "Sorry, you may not enter while you are carrying more than "
               "three items. You currently have %d items.",
               cnt);
      return 1;
    }
  }
  ...
  for (n = 0; n < ARRAYSIZE(room_start); n += 2) {
    if (enter_room(cn, cls, room_start[n + 0], room_start[n + 1])) break;
  }
...
void gate_welcome_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct gate_welcome_driver_data *)set_data(
      cn, DRD_GATE_WELCOME, sizeof(struct gate_welcome_driver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      ppd = (struct gate_ppd *)set_data(co, DRD_GATE_PPD,
                                        sizeof(struct gate_ppd));

      if (ppd) {
        switch (ppd->welcome_state) {
          case 0:
            say(cn,
                "Be greeted, %s. These are the halls of Ishtar. Only the "
                "greatest fighters and magic users come here, to take the "
                "final test and fight the Gatekeeper.",
                ch[co].name);
            ppd->welcome_state++;
            didsay = 1;
            break;
          case 1:
            say(cn,
                "Those who succeed in this test will be able to enhance their "
                "abilities further. They may either choose to learn more about "
                "their profession than any other mortal being, or to start "
                "again as one who can learn all arts.");
            ppd->welcome_state++;
            didsay = 1;
            break;
          case 2:
            if (teleport_next_lab(co, 0)) {
              say(cn,
                  "Before thou mayest engage the Gatekeeper, thou must solve "
                  "the Labyrinth built by Ishtar. Thou canst enter the "
                  "labyrinth through the door to the east.");
              ppd->welcome_state++;
              didsay = 1;
            } else {
              ppd->welcome_state = 4;
          ...
          case 3:
            if (!teleport_next_lab(co, 0))
              ppd->welcome_state++;  // fall through intended
            else
              break;
          ...
          case 4:
            ...
            if (ch[co].flags & CF_ARCH) {  // arch-XXX
              say(cn,
                  "There is nothing I can do for thee, %s, though, since thou "
                  "art already an Arch-%s.",
                  ch[co].name,
                  (ch[co].flags & CF_WARRIOR)
                      ? (ch[co].flags & CF_MAGE) ? "Seyan'Du" : "Warrior"
                      : "Mage");
              ppd->welcome_state = 6;
              didsay = 1;
            } else if ((ch[co].flags & CF_MAGE) &&
                       (ch[co].flags & CF_WARRIOR)) {  // seyan, non-arch
              say(cn,
                  "Since thou art already a Seyan'Du, thy only choice is to "
                  "become Arch-Seyan'Du.");
              ppd->welcome_state++;
              didsay = 1;
            } else {
              say(cn,
                  "The choice is hard, and so is the test. If thou wishest to "
                  "take the test, decide which path to follow. That of the "
                  "Arch-%s, or that of the Seyan'Du.",
          ...
          case 5:
            say(cn,
                "Name the class thou wishest to become to begin the test. Each "
                "try will cost thee 100 gold coins.");
            ppd->welcome_state++;
            didsay = 1;
            break;
...
void gate_fight_dead(int cn, int co) {
  ...
  ppd = (struct gate_ppd *)set_data(co, DRD_GATE_PPD, sizeof(struct gate_ppd));
  if (!ppd) return;
  ...
  switch (ppd->target_class) {
    case 5:  // arch warrior
      ...
      sprintf(buf, "0000000000°c10Grats: %s is an Arch-Warrior now!",
              ch[co].name);
      server_chat(6, buf);
      break;
    case 6:  // arch mage
    ...
    case 7:  // arch seyan'dr
      ...
      sprintf(buf, "0000000000°c10Grats: %s is an Arch-Seyan'Du now!",
              ch[co].name);
      server_chat(6, buf);
      break;
    case 8:  // seyan'du
...
void labentrance(int in, int cn) {
  ...
  if (ret == 0)
    log_char(cn, LOG_SYSTEM, 0,
             "You have solved all existing labyrinths already. You can now "
             "fight the gatekeeper.");
  else if (ret == -1)
    log_char(cn, LOG_SYSTEM, 0,
             "°c3The area containing the next labyrinth part is down. Please "
             "try again soon.");
  else if (ret < -1)
    log_char(cn, LOG_SYSTEM, 0,
             "°c3You may not enter before reaching level %d.", -ret);
}
