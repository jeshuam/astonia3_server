...
void gate_welcome_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct gate_welcome_driver_data*)set_data(cn, DRD_GATE_WELCOME, sizeof(struct gate_welcome_driver_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct gate_ppd*)set_data(co, DRD_GATE_PPD, sizeof(struct gate_ppd));
...
void gate_fight_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct gate_fight_driver_data*)set_data(cn, DRD_GATE_FIGHT, sizeof(struct gate_fight_driver_data));
  if (!dat) return;	// oops...
