 ...
 * reduced most rewards prices by a factor of 2; increased rewards for higher
 *difficulties by about 50%
 */
...
int analyse_text_driver(int cn, int type, char *text, int co) {
  ...
  if (w) {
    ...
    for (q = 0; q < sizeof(mis_rew) / sizeof(mis_rew[0]); q++) {
      ...
      if (!strcmp("show", wordlist[0]) &&
          !strcasecmp(mis_rew[q].code, wordlist[1]))
        return q + 1000;
      if (!strcmp("ibuy", wordlist[0]) &&
          !strcasecmp(mis_rew[q].code, wordlist[1]))
        return q + 2000;
    }
  }
...
static const char *misstypetext[] = {"NULL", "Kill"};
...
struct mission_data {
  ...
  const char *bossname;
  const char *bossdesc;
  const char *bigbossname;
  const char *bigbossdesc;
  int sprite;
  int bosssprite;
  const char *strengthname[3];
  const char *temp;
  const char *bosstemp;
  const char *itemtemp;
  const char *itemname;
...
struct mission_data thief_data = {
    "Stolen Documents",
    "A bold thief named 'Sacewan' from the famous gang 'The Pickers' has "
    "stolen some vitally important documents from a friend of mine. Please "
    "retrieve the documents, and kill Sacewan and his whole gang.",
    MISS_TYPE_KILL,
    "Thief",
...
struct mission_data spy_data = {
    "Silence the Spy",
    "A spy has stolen a important secret from my boss. He's fled into the "
    "wilderness while his messengers are trying to find a buyer for the "
    "information. See to it the deal is never made.",
    MISS_TYPE_KILL,
    "Bodyguard",
...
struct mission_data beast_data = {
    ...
    "A group of swamp beasts has invaded one of our outposts. Please free the "
    "outpost of their presence.",
    MISS_TYPE_KILL,
    "Swamp Beast",
...
struct mission_data ruffian_data = {
    "Dispatch Ruffians",
    "A group of ruffians has taken camp in my superior's summer house. Please "
    "see to it that they leave. Be careful, their boss, one 'Gorinion', is "
    "said to be a powerful mage.",
    MISS_TYPE_KILL,
    "Ruffian",
...
struct mission_data vampire_data = {
    "Vampire Infection",
    "There have been rumors about a group of vampires in a town nearby. Please "
    "investigate the town's graveyard for any signs of vampires. Make all "
    "vampires you find 'see the light'.",
    MISS_TYPE_KILL,
    "Vampire",
...
struct mission_data graverobber_data = {
    "Graverobber",
    "This time I have a delicate mission for you. I have discovered the "
    "location of an ancient tomb in old writings, which is said to hold the "
    "'Magical Spoon of Doom'. Please enter the tomb and retrieve the spoon.",
    MISS_TYPE_KILL,
    "Zombie",
    "Rotten flesh, decaying bones, weird grin. Looks like a Zombie.",
    "Zombie Boss",
    "Rotten flesh, decaying bones, weird grin, lots of muscle. Looks like a "
    "Zombie Boss.",
    "Zombie Lord",
    "Rotten flesh, decaying bones, weird grin, lots of muscle. Looks like the "
    "Zombie Bosses boss.",
    9,
    9,
    {" Slave", "", " Master"},
...
struct mission_data hideseek_data = {
    "Hide and Seek",
    "A wizard-friend of my boss has hidden a ring in a dungeon which my boss "
    "is to retrieve. Some kind of bet I heard. Go there, get the ring, and "
    "kill anything that comes into your way.",
    MISS_TYPE_KILL,
    "Skeleton",
...
struct mission_data *mdtab[] = {&thief_data,   &spy_data,     &beast_data,
                                &ruffian_data, &vampire_data, &graverobber_data,
                                &hideseek_data};

void offer_mission_sub(int cn, int co, int idx, struct mission_ppd *ppd) {
  static const char *missname[3] = {"Alpha", "Beta", "Gamma"};

  log_char(co, LOG_SYSTEM, 0, " ");
  log_char(co, LOG_SYSTEM, 0, "°c2Job %s \006%s \020%s \024Difficulty %d°c0",
           missname[idx], mdtab[ppd->sm[idx].mdidx]->title,
           misstypetext[ppd->sm[idx].type], ppd->sm[idx].difficulty);
  log_char(co, LOG_SYSTEM, 0, " ");
  log_char(co, LOG_SYSTEM, 0, "%s", mdtab[ppd->sm[idx].mdidx]->description);
  log_char(co, LOG_SYSTEM, 0, " ");
  log_char(co, LOG_SYSTEM, 0,
           "Do you °c4accept Job %s°c0? Accepting the job will teleport you to "
           "the job area immediately. Or do you want to look at °c4Job %s°c0 "
           "or °c4Job %s°c0 instead?",
           missname[idx], idx == 0 ? missname[1] : missname[0],
           idx == 2 ? missname[1] : missname[2]);
}
...
void offer_mission(int cn, int co, struct mission_ppd *ppd) {
  ...
  quiet_say(cn,
            "Please choose one of the following jobs (this will display job "
            "details, you get a chance to accept or refuse the job later):");
  ...
  for (n = 0; n < 3; n++) {
    if (!ppd->sm[n].type) {
      ...
      } while (ppd->sm[n].mdidx == ppd->sm[n == 0 ? 1 : 0].mdidx ||
               ppd->sm[n].mdidx == ppd->sm[n == 1 ? 2 : 1].mdidx);

      switch (ppd->sm[n].type) {
        case MISS_TYPE_KILL:
          ppd->sm[n].difficulty = ppd->dif_kill + RANDOM(10);
          break;
      }
    }
    log_char(co, LOG_SYSTEM, 0, "°c4Job %s°c0 \006%s \020%s \024Difficulty %d",
             missname[n], mdtab[ppd->sm[n].mdidx]->title,
             misstypetext[ppd->sm[n].type], ppd->sm[n].difficulty);
  }
}

int build_fighter(int x, int y, int diff, int keyID, const char *keyname,
                  const char *name, const char *temp, const char *desc, int fID,
                  int sprite, int hasspecialitem, long long extra_flags) {
  int cn, n, val, in;
  // char buf[80];
  ...
  if ((diff > 100 && ch[cn].level < 10) || ch[cn].level > 200)
    ch[cn].level = 200;

  ch[cn].sprite = sprite;
  ch[cn].flags |= extra_flags;
  strncpy(ch[cn].name, name, sizeof(ch[cn].name));
  ch[cn].name[sizeof(ch[cn].name) - 1] = 0;
  strncpy(ch[cn].description, desc, sizeof(ch[cn].description));
  ...
  if (hasspecialitem) {
    ...
    if (ch[cn].level < 10) {
      str = 3;
      base = 1;
    } else if (ch[cn].level < 17) {
      str = 4;
      base = 10;
    } else if (ch[cn].level < 24) {
      str = 6;
      base = 20;
    } else if (ch[cn].level < 31) {
      str = 7;
      base = 30;
    } else if (ch[cn].level < 38) {
      str = 8;
      base = 40;
    } else if (ch[cn].level < 45) {
      str = 10;
      base = 50;
    } else if (ch[cn].level < 52) {
      str = 12;
      base = 60;
    } else if (ch[cn].level < 60) {
      str = 14;
      base = 70;
    } else if (ch[cn].level < 66) {
      str = 16;
      base = 80;
    } else if (ch[cn].level < 74) {
      str = 18;
      base = 90;
    } else {
      str = 20;
...
void mission_status(int cn, struct mission_ppd *ppd) {
  ...
  if (ppd->kill_easy[1] > ppd->kill_easy[0])
    log_char(cn, LOG_SYSTEM, 0, "#%d0- Kill %d %s%s%s", line++,
             ppd->kill_easy[1] - ppd->kill_easy[0], md->basename,
             md->strengthname[0],
             ppd->kill_easy[1] - ppd->kill_easy[0] != 1 ? "s" : "");
  if (ppd->kill_normal[1] > ppd->kill_normal[0])
    log_char(cn, LOG_SYSTEM, 0, "#%d0- Kill %d %s%s%s", line++,
             ppd->kill_normal[1] - ppd->kill_normal[0], md->basename,
             md->strengthname[1],
             ppd->kill_normal[1] - ppd->kill_normal[0] != 1 ? "s" : "");
  if (ppd->kill_hard[1] > ppd->kill_hard[0])
    log_char(cn, LOG_SYSTEM, 0, "#%d0- Kill %d %s%s%s", line++,
             ppd->kill_hard[1] - ppd->kill_hard[0], md->basename,
             md->strengthname[2],
             ppd->kill_hard[1] - ppd->kill_hard[0] != 1 ? "s" : "");
  if (ppd->kill_boss[1] > ppd->kill_boss[0])
    log_char(cn, LOG_SYSTEM, 0, "#%d0- Kill %d %s%s", line++,
             ppd->kill_boss[1] - ppd->kill_boss[0], md->bossname,
             ppd->kill_boss[1] - ppd->kill_boss[0] != 1 ? "s" : "");
  if (ppd->find_item[1] > ppd->find_item[0])
    log_char(cn, LOG_SYSTEM, 0, "#%d0- Find %s", line++, md->itemname);
...
void mission_done(int cn, struct mission_ppd *ppd) {
  ...
  if (ppd->active) {
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "You've finished the job. Good work, %s. Now talk to Mr. Jones "
             "for your reward.",
             ch[cn].name);
  }
}
...
void start_mission(int cn, int co, int idx, struct mission_ppd *ppd) {
  ...
  char buf[256];
  int easy = 0, normal = 0, hard = 0, boss = 0, chest = 0, keyID, ex = 0,
      ey = 0, busy;
  ...
  if (shutdown_at && realtime - shutdown_at < 10) {
    quiet_say(cn,
              "I'm sorry, %s, but a shutdown is in progress. Please come back "
              "after the shutdown.",
              ch[co].name);
    return;
  }
  ...
  if (busy) {
    quiet_say(cn,
              "I'm sorry, %s, but it appears that this job is unavailable "
              "right now. Please choose a different one.",
              ch[co].name);
    return;
  }
    ...
    for (y = fy; y <= ty; y++) {
      ...
      if ((in = map[x + y * MAXMAP].it)) {
        ...
        if (it[in].ID == IID_MISSIONFIGHTER) {
          ...
          switch (it[in].drdata[0]) {
            case 1:
              sprintf(buf, "%s%s", md->basename, md->strengthname[0]);
              build_fighter(it[in].x, it[in].y, (ppd->sm[idx].difficulty / 3),
                            0, "", buf, md->temp, md->basedesc, 1, md->sprite,
                            0, md->char_flags);
              easy++;
              break;
            case 2:
              sprintf(buf, "%s%s", md->basename, md->strengthname[1]);
              build_fighter(it[in].x, it[in].y,
                            (ppd->sm[idx].difficulty / 3) + 1, 0, "", buf,
                            md->temp, md->basedesc, 2, md->sprite, 0,
                            md->char_flags);
              normal++;
              break;
            case 3:
              sprintf(buf, "%s%s", md->basename, md->strengthname[2]);
              build_fighter(it[in].x, it[in].y,
                            (ppd->sm[idx].difficulty / 3) + 2, 0, "", buf,
                            md->temp, md->basedesc, 3, md->sprite, 0,
                            md->char_flags);
              hard++;
              break;
            case 4:
              if (RANDOM(10)) {
                ...
                build_fighter(it[in].x, it[in].y,
                              (ppd->sm[idx].difficulty / 3) + 3, 0, "", buf,
                              md->bosstemp, md->bossdesc, 4, md->bosssprite, 0,
                              md->char_flags);
              } else {
                sprintf(buf, "%s", md->bigbossname);
                build_fighter(it[in].x, it[in].y,
                              (ppd->sm[idx].difficulty / 3) + 5, 0, "", buf,
                              md->bosstemp, md->bigbossdesc, 4, md->bosssprite,
                              1, md->char_flags);
              }
              boss++;
              break;
            case 5:
              ...
              key1--;
              build_fighter(it[in].x, it[in].y,
                            (ppd->sm[idx].difficulty / 3) + 1,
                            key1 == 0 ? keyID : 0, "Door Key I", buf, md->temp,
                            md->basedesc, 2, md->sprite, 0, md->char_flags);
              normal++;
              break;
            case 6:
              sprintf(buf, "%s%s", md->basename, md->strengthname[1]);
              key2--;
              build_fighter(
                  it[in].x, it[in].y, (ppd->sm[idx].difficulty / 3) + 1,
                  key2 == 0 ? keyID + 1 : 0, "Door Key II", buf, md->temp,
                  md->basedesc, 2, md->sprite, 0, md->char_flags);
              normal++;
              break;
            case 7:
              ...
              build_fighter(
                  it[in].x, it[in].y, (ppd->sm[idx].difficulty / 3) + 1,
                  md->itemname && key3 == 0 ? keyID + 2 : 0, "Chest Key", buf,
                  md->temp, md->basedesc, 2, md->sprite, 0, md->char_flags);
              normal++;
              break;
          }
        }
  ...
  ppd->kill_easy[0] = 0;
  ppd->kill_easy[1] = easy;
  ppd->kill_normal[0] = 0;
  ppd->kill_normal[1] = normal;
  ppd->kill_hard[0] = 0;
  ppd->kill_hard[1] = hard;
  ppd->kill_boss[0] = 0;
  ppd->kill_boss[1] = boss;
  ppd->find_item[0] = 0;
  ppd->find_item[1] = chest;
...
void mission_reward_list(int cn, int co, struct mission_ppd *ppd) {
  ...
  if (!init_done) {
    qsort(mis_rew, sizeof(mis_rew) / sizeof(mis_rew[0]), sizeof(mis_rew[0]),
          mr_cmp);
    init_done = 1;
  }
  ...
  for (n = first; n < last; n++) {
    log_char(co, LOG_SYSTEM, 0, "°c4show %s°c0 \010%d \012%s", mis_rew[n].code,
             mis_rew[n].value, mis_rew[n].desc);
  }
  log_char(co, LOG_SYSTEM, 0, "You have: \010%d \012points.", ppd->points);
  log_char(co, LOG_SYSTEM, 0,
           "You'll get a more detailed description of the offer when you "
           "choose it.");
}
...
void mission_give_reward(int cn, int co, int nr, struct mission_ppd *ppd) {
  ...
  if (mis_rew[nr].value > ppd->points) {
    quiet_say(cn, "%s costs %d points, but you only have %d points.",
              mis_rew[nr].code, mis_rew[nr].value, ppd->points);
    return;
  }
  if (strcmp(mis_rew[nr].itmtmp, "MEXP") == 0) {
    ...
    dlog(co, 0,
         "took %s from mission offer for %d points (new total %d points)",
         mis_rew[nr].itmtmp, mis_rew[nr].value, ppd->points);
    ...
    quiet_say(cn,
              "One custom stat potion coming up. Do you want it to hold °c4one "
              "skill°c0 or °c4two skills°c0 or °c4three skills°c0?");
    dlog(co, 0,
         "took %s from mission offer for %d points (new total %d points)",
         mis_rew[nr].itmtmp, mis_rew[nr].value, ppd->points);
    return;
  } else {
    if (strcmp(mis_rew[nr].itmtmp, "RNORB") == 0)
      in = create_orb();
    else
      in = create_item(mis_rew[nr].itmtmp);
    if (!in) {
      quiet_say(cn,
                "Oops. I've ran out of stock. Please choose something else.");
      elog("mission_give_reward: item template %s (%d) not found.",
           mis_rew[nr].itmtmp, nr);
      return;
    }
    ...
    if (!give_char_item(co, in)) {
      destroy_item(in);
      quiet_say(cn,
                "Hey, sleepy head, there's no room in your hand or inventory "
                "to give you an item!");
      return;
    }
    ppd->points -= mis_rew[nr].value;
    dlog(co, in, "took from mission offer for %d points (new total %d points)",
         mis_rew[nr].value, ppd->points);
  }

  quiet_say(cn,
            "Here you go, %s, one %s (%s) for %d points. You now have %d "
            "points left.",
            ch[co].name, mis_rew[nr].code, mis_rew[nr].desc, mis_rew[nr].value,
            ppd->points);
}
...
void mission_show_reward(int cn, int co, int nr, struct mission_ppd *ppd) {
  ...
  if (strcmp(mis_rew[nr].itmtmp, "CTPOT") == 0) {
    quiet_say(cn,
              "A custom potion which will enhance one of your stats by 50 or "
              "two of your stats by 30 or three of your stats by 20.");
  } else if (strcmp(mis_rew[nr].itmtmp, "RNORB") == 0) {
    quiet_say(cn,
              "A randomly chosen orb, used to enhance one modifier on an item "
              "by one.");
  } else if (strcmp(mis_rew[nr].itmtmp, "MEXP") == 0) {
    if (mis_rew[nr].value == 100)
      quiet_say(cn, "The equivalent of a two Privates in Military Experience.");
    else if (mis_rew[nr].value == 1000)
      quiet_say(cn,
                "The equivalent of a Lance Corporal in Military Experience.");
    else if (mis_rew[nr].value == 10000)
      quiet_say(cn,
                "The equivalent of a Staff Sergeant in Military Experience.");
    // else quiet_say(cn,"The equivalent of a Second Lieutenant in Military
    // Experience."); //100000
  } else if (strcmp(mis_rew[nr].itmtmp, "GOLD") == 0) {
    if (mis_rew[nr].value == 10)
      quiet_say(cn, "50 gold coins, fresh from the press.");
    else if (mis_rew[nr].value == 100)
      quiet_say(cn, "500 gold coins, fresh from the press.");
    else if (mis_rew[nr].value == 1000)
      quiet_say(cn, "5000 gold coins, fresh from the press.");
    else if (mis_rew[nr].value == 10000)
      quiet_say(cn, "50000 gold coins, fresh from the press.");
  ...
  quiet_say(cn,
            "This could be yours for %d points (you have %d points). Say "
            "°c4ibuy %s°c0 to buy it.",
            mis_rew[nr].value, ppd->points, mis_rew[nr].code);
}
...
void mission_giver_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct mission_giver_data *)set_data(
      cn, DRD_MISSIONGIVEDRIVER, sizeof(struct mission_giver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (ticker < dat->last_talk + TICKS * 3) {
        remove_message(cn, msg);
        continue;
      }

      if (ticker < dat->last_talk + TICKS * 5 && dat->current_victim != co) {
        remove_message(cn, msg);
        continue;
      }
      ...
      ppd = (struct mission_ppd *)set_data(co, DRD_MISSION_PPD,
                                           sizeof(struct mission_ppd));

      if (ppd) {
        if (realtime - ppd->lastseenmissiongiver > 30)
        ...
        switch (ppd->missiongive_state) {
          case 0:
            if (ppd->solved) {
              ...
              pts = (int)((13 + (ppd->sm[ppd->solved - 1].difficulty / 6)) *
                          md->diff_mult);
              ppd->points += pts;
              ppd->solved = 0;
              dlog(co, 0, "solved mission, got %d points (new total %d points)",
                   pts, ppd->points);
              ...
              quiet_say(cn,
                        "Congratulations on doing a good job, %s. You earned "
                        "%d brownie points in my book, for a total of %d "
                        "points. Feel free to ask me for an °c4offer°c0 "
                        "anytime. You can also ask me to °c4increase°c0 or "
                        "°c4decrease°c0 the difficulty of your jobs (this must "
                        "be done before asking for a new job, otherwise you'll "
                        "be changing the difficulty of the following job "
                        "offer, not the current one; multiple commands will "
                        "stack). Or do you want a °c4new job°c0?",
                        ch[co].name, pts, ppd->points);
              ...
              quiet_say(cn,
                        "You still have a job. Do you want to °c4fail°c0 it?");
              ppd->missiongive_state = 2;
              didsay = 1;
            } else {
              quiet_say(
                  cn, "Hello %s. Looking for a job, I bet. Well, let me see...",
                  ch[co].name);
              ppd->missiongive_state++;
              didsay = 1;
            }
            break;
          case 1:
            offer_mission(cn, co, ppd);
            ppd->missiongive_state++;
            didsay = 1;
            break;
          case 2:
            break;  // waiting for player
        }
        if (didsay) {
        ...
        switch ((didsay = analyse_text_driver(cn, msg->dat1, (char *)msg->dat2,
          ...
          case 3:
            if (ch[co].flags & CF_GOD) {
              say(cn, "reset done");
              bzero(ppd, sizeof(struct mission_ppd));
              dat->last_talk = 0;
            }
            break;
          case 4:
            if (!ppd->active && !ppd->solved)
              offer_mission_sub(cn, co, 0, ppd);
            else
              quiet_say(cn,
                        "You still have a job. Do you want to °c4fail°c0 it?");
            break;
          ...
          case 6:
            if (!ppd->active && !ppd->solved)
              offer_mission_sub(cn, co, 2, ppd);
            else
              quiet_say(cn,
                        "You still have a job. Do you want to °c4fail°c0 it?");
            break;
          case 7:
            if (ppd->sm[0].type && !ppd->active && !ppd->solved)
              start_mission(cn, co, 0, ppd);
            else
              quiet_say(cn, "I haven't offered you that job yet.");
            break;
          case 8:
            if (ppd->sm[1].type && !ppd->active && !ppd->solved)
              start_mission(cn, co, 1, ppd);
            else
              quiet_say(cn, "I haven't offered you that job yet.");
            break;
          case 9:
            if (ppd->sm[2].type && !ppd->active && !ppd->solved)
              start_mission(cn, co, 2, ppd);
            else
              quiet_say(cn, "I haven't offered you that job yet.");
            break;
          case 10:
            if (ppd->active) {
              ...
              dlog(co, 0,
                   "failed mission, lost %d points (new total %d points)", pts,
                   ppd->points);
              if (pts) {
                quiet_say(
                    cn,
                    "You lost %d brownie points for a new total of %d points.",
                    pts, ppd->points);
              }
              log_char(co, LOG_SYSTEM, 0, "#30");
              log_char(co, LOG_SYSTEM, 0, "#40");
          ...
          case 11:
            ...
            quiet_say(cn, "I also have a °c4special offer°c0, for %d points.",
                      dat->spec_cost);
            break;
          case 12:
          ...
          case 13:
            ...
            if (ch[co].flags & CF_FEMALE) {
              quiet_say(cn,
                        "Oops. Sorry. Old habit from my military days, ma'am. "
                        "Alright, lady, you'll get the easy ones now.");
            }
            break;
          case 14:
            if (ch[co].flags & CF_GOD) {
              say(cn, "I hate freeloaders!");
              ppd->points += 100;
            }
            break;
          case 15:
          ...
          case 18:
            ...
            log_char(co, LOG_SYSTEM, 0, "Price: %d points (you have %d points)",
                     dat->spec_cost, ppd->points);
            log_char(co, LOG_SYSTEM, 0,
                     "Do you want to °c4buy the special offer°c0 (offer "
                     "guaranteed for 5 minutes, unless someone else buys it; "
                     "might change anytime after that)?");
          ...
          case 19:
            ...
            dat->next_spec = 0;
            dlog(co, in,
                 "took from mission offer for %d points (new total %d points)",
                 dat->spec_cost, ppd->points);
            break;
          case 20:
            if (ppd->statowed < 1) {
            ...
            ppd->statcnt = 1;
            quiet_say(cn,
                      "Alright, a one-stat potion it will be. Please name the "
                      "skill you'd like to have (ie. °c4attack skill°c0, "
                      "°c4immunity skill°c0 etc.).");
            break;
          case 21:
            if (ppd->statowed < 1) {
              quiet_say(cn, "You did not buy a stat potion.");
            ...
            ppd->statcnt = 2;
            quiet_say(cn,
                      "Alright, a two-stat potion it will be. Please name the "
                      "skills you'd like to have (ie. °c4attack skill°c0, "
                      "°c4immunity skill°c0 etc.), one skill per line.");
            break;
          case 22:
            if (ppd->statowed < 1) {
              quiet_say(cn, "You did not buy a stat potion.");
            ...
            ppd->statcnt = 3;
            quiet_say(cn,
                      "Alright, a three-stat potion it will be. Please name "
                      "the skills you'd like to have (ie. °c4attack skill°c0, "
                      "°c4immunity skill°c0 etc.), one skill per line.");
            break;
          default:
            if (didsay >= 2000)
              mission_give_reward(cn, co, didsay - 2000, ppd);
            else if (didsay >= 1000)
              mission_show_reward(cn, co, didsay - 1000, ppd);
            break;
        }
        if (!didsay && ppd->statowed &&
            (nr = find_skill_text(cn, msg->dat1, (char *)msg->dat2, co)) &&
            nr > 0 && nr < V_MAX) {
          didsay = 1;
          ...
          if (n >= ppd->statcnt) {
            ...
            if (in) {
              ...
              if (ppd->statcnt == 1) {
                it[in].mod_value[0] = 50;
                it[in].mod_value[1] = 0;
                it[in].mod_value[2] = 0;
              }
              if (ppd->statcnt == 2) {
                it[in].mod_value[0] = 30;
                it[in].mod_value[1] = 30;
                it[in].mod_value[2] = 0;
              }
              if (ppd->statcnt == 3) {
                it[in].mod_value[0] = 20;
                it[in].mod_value[1] = 20;
            ...
            quiet_say(cn, "Very well, the %s skill will be %s.",
                      n == 1 ? "first" : "second", skill[nr].name);
        }
        if (didsay) {
          talkdir = offset2dx(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y);
          dat->current_victim = co;
...
void missionchest_driver(int in, int cn) {
  ...
  ppd = (struct mission_ppd *)set_data(cn, DRD_MISSION_PPD,
                                       sizeof(struct mission_ppd));
  if (!ppd) return;
...
void mission_fighter_dead(int cn, int co) {
  ...
  switch (nr) {
    case 1:
      ppd->kill_easy[0] = min(ppd->kill_easy[0] + 1, ppd->kill_easy[1]);
      break;
    case 2:
      ppd->kill_normal[0] = min(ppd->kill_normal[0] + 1, ppd->kill_normal[1]);
      break;
    case 3:
      ppd->kill_hard[0] = min(ppd->kill_hard[0] + 1, ppd->kill_hard[1]);
      break;
    case 4:
      ppd->kill_boss[0] = min(ppd->kill_boss[0] + 1, ppd->kill_boss[1]);
      break;
  }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
  switch (nr) {
    case CDR_MISSIONGIVE:
      mission_giver_driver(cn, ret, lastact);
      return 1;
    case CDR_MISSIONFIGHT:
      mission_fighter_driver(cn, ret, lastact);
      return 1;
...
int it_driver(int nr, int in, int cn) {
  switch (nr) {
    case IDR_MISSIONCHEST:
      missionchest_driver(in, cn);
      return 1;
    default:
      return 0;
  }
