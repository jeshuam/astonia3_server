...
static const char *ing_name[] = {
    "Adygalah",    "Bhalkissa", "Chrysado",    "Domari",    "Elithah",
    "Firuba",      "Ghethiye",  "Akond",       "Barun",     "Chylmoth",
    "Dizul",       "Edyak",     "Forud",       "Ghestroz",  "Hangot",
    "Ivnan",       "Azmey",     "Beelough",    "Ciuba",     "Dyelshi",
    "Fiery Stone", "Icy Stone", "Earth Stone", "Hell Stone"};
...
int mixer_power(int in, int cn) {
  ...
  stone = it[in].drdata[31] + it[in].drdata[32] + it[in].drdata[33] +
          it[in].drdata[34];

  if (cnt == 2 && pw1 == 1 && pw2 == 1) {
    if (solstice || (fullmoon && ch[cn].prof[P_ALCHEMIST] >= 30) ||
        ch[cn].prof[P_ALCHEMIST] >= 50)
      return 16;
    if (equinox || (fullmoon && ch[cn].prof[P_ALCHEMIST] >= 20) ||
        ch[cn].prof[P_ALCHEMIST] >= 40)
      return 12;
  ...
  if (solstice || (fullmoon && ch[cn].prof[P_ALCHEMIST] >= 30) ||
      ch[cn].prof[P_ALCHEMIST] >= 50)
    good = 8;
  else if (equinox || (fullmoon && ch[cn].prof[P_ALCHEMIST] >= 20) ||
           ch[cn].prof[P_ALCHEMIST] >= 40)
    good = 4;
  else if (fullmoon || ch[cn].prof[P_ALCHEMIST] >= 10)
    good = 2;
  else if (hour == 12)
    good = 1;
...
int mixer_mix(int cn, int in, int power, int duration, double divi) {
  ...
  if (cnt == 5 && wis == 1 && inu == 1 && agi == 2 &&
      str == 1) {  // sword attack parry
    skl[0] = V_SWORD;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && wis == 1 && inu == 1 && agi == 1 &&
             str == 2) {  // two-handed attack parry
    skl[0] = V_TWOHAND;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_ATTACK;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && agi == 1 && str == 2 && lfe == 1 &&
             spr == 1) {  // attack parry immunity
    skl[0] = V_ATTACK;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_PARRY;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && inu == 1 && str == 1 && lfe == 2 &&
             spr == 1) {  // flash magic-shield immunity
    skl[0] = V_FLASH;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_MAGICSHIELD;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && str == 2 && lfe == 2 &&
             spr == 1) {  // fire magic-shield immunity
    skl[0] = V_FIRE;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_MAGICSHIELD;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && inu == 1 && lfe == 2 && spr == 1 &&
             end == 1) {  // flash magic-shield pulse
    skl[0] = V_FLASH;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_MAGICSHIELD;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && str == 1 && lfe == 2 && spr == 1 &&
             end == 1) {  // fire magic-shield pulse
    skl[0] = V_FIRE;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_MAGICSHIELD;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && inu == 1 && str == 2 && spr == 1 &&
             end == 1) {  // flash immunity pulse
    skl[0] = V_FLASH;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_IMMUNITY;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 5 && str == 3 && spr == 1 &&
             end == 1) {  // fire immunity pulse
    skl[0] = V_FIRE;
    if ((val[0] = power / divi / 4)) flag = 1;
    skl[1] = V_IMMUNITY;
    val[1] = power / divi / 4;
    ...
    val[2] = power / divi / 4;
    value = 10;
  } else if (cnt == 4 && wis == 1 && inu == 1 && agi == 1 &&
             str == 1) {  // attack parry
    skl[0] = V_ATTACK;
    ...
    val[2] = 0;
    value = 8;
  } else if (cnt == 4 && inu == 1 && str == 1 && lfe == 1 &&
             spr == 1) {  // flash immunity
    skl[0] = V_FLASH;
    if ((val[0] = power / divi / 3)) flag = 1;
    skl[1] = V_IMMUNITY;
    val[1] = power / divi / 3;
    ...
    val[2] = 0;
    value = 8;
  } else if (cnt == 4 && str == 1 && lfe == 2 &&
             spr == 1) {  // magic shield immunity
    skl[0] = V_MAGICSHIELD;
    if ((val[0] = power / divi / 3)) flag = 1;
    skl[1] = V_IMMUNITY;
    val[1] = power / divi / 3;
    ...
    val[2] = 0;
    value = 10;
  } else if (cnt == 4 && agi == 1 && end == 1 && lfe == 1 &&
             spr == 1) {  // dagger flash
    skl[0] = V_DAGGER;
    if ((val[0] = power / divi / 3)) flag = 1;
    skl[1] = V_FLASH;
    ...
    val[2] = 0;
    value = 8;
  } else if (cnt == 4 && agi == 1 && str == 1 && end == 1 &&
             spr == 1) {  // dagger fire
    skl[0] = V_DAGGER;
    if ((val[0] = power / divi / 3)) flag = 1;
    skl[1] = V_FIRE;
    ...
    val[2] = 0;
    value = 8;
  } else if (cnt == 4 && agi == 1 && str == 1 && lfe == 1 &&
             spr == 1) {  // staff flash
    skl[0] = V_STAFF;
    if ((val[0] = power / divi / 3)) flag = 1;
    skl[1] = V_FLASH;
    ...
    for (n = 0; n < 3; n++) {
      ...
      if (end) {
        skl[n] = V_ENDURANCE;
        if ((val[n] = power * end / divi / cnt)) flag = 1;
        end = 0;
        continue;
      }
      skl[n] = -1;
...
int mixer(int cn, int in) {
  ...
  for (n = 11; n < 40; n++) {
    if (it[in].drdata[n]) {
      log_char(cn, LOG_SYSTEM, 0, "Contains %d parts %s.", it[in].drdata[n],
               ing_name[n - 11]);
    }
  }
  ...
  // log_char(cn,LOG_SYSTEM,0,"Power=%d, Duration=%d,
  // Divisor=%f",power,duration,divi);
...
void flask_driver(int in, int cn) {
  ...
  if (shaken && ch[cn].citem) {
    log_char(cn, LOG_SYSTEM, 0,
             "This potion is finished. You cannot add more ingredients.");
    return;
  }
  ...
  if (!(in2 = ch[cn].citem)) {
    ...
    if (it[in].drdata[1] == 0) {  // shaking empty potion
      log_char(cn, LOG_SYSTEM, 0,
               "You shake the empty bottle, but nothing happens.");
      return;
    }

    if (mixer(cn, in)) {
      log_char(cn, LOG_SYSTEM, 0,
               "You shake the bottle and create a magical potion.");

      switch (it[in].drdata[0]) {
        case 1:
          it[in].sprite = 50213;
          strcpy(it[in].description,
                 "A small flask containing a magical liquid.");
          break;
        case 2:
          it[in].sprite = 50214;
        ...
        case 3:
          it[in].sprite = 50253;
          strcpy(it[in].description,
                 "A big flask containing a magical liquid.");
          break;
      ...
      log_char(cn, LOG_SYSTEM, 0,
               "You shake the bottle and create a stinking liquid which you "
               "throw away.");
      strcpy(it[in].name, "Empty Potion");
      switch (size) {
        case 1:
  ...
  if (it[in2].ID != IID_ALCHEMY_INGREDIENT) {
    log_char(cn, LOG_SYSTEM, 0,
             "That's not an ingredient you can use in a flask.");
    return;
  }
  ...
  switch (size) {
    case 1:
      ...
      strcpy(it[in].description,
             "A small flask containing some strange liquid.");
      break;
    case 2:
