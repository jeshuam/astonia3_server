...
void nomad(int cn, int ret, int lastact)
{
  ...
  dat = (struct nomad_data*)set_data(cn, DRD_NOMADDRIVER, sizeof(struct nomad_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      co = msg->dat1;
      if ((ch[co].flags & CF_PLAYER) && ticker - dat->lasttalk > TICKS * 5 && char_dist(cn, co) < 12 && char_see_char(cn, co) && (ppd = (struct nomad_ppd*)set_data(co, DRD_NOMAD_PPD, sizeof(struct nomad_ppd)))) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      if ((ppd = (struct nomad_ppd*)set_data(co, DRD_NOMAD_PPD, sizeof(struct nomad_ppd)))) {
    ...
    if (msg->type == NT_NPC) {
      if (msg->dat1 == NTID_DICE) {
        co = msg->dat2;
        val = msg->dat3;
        if (co == dat->play_with && (ppd = (struct nomad_ppd*)set_data(co, DRD_NOMAD_PPD, sizeof(struct nomad_ppd)))) nomad_roll(cn, dat, co, val, ppd, dat->nr);
      }
    }
