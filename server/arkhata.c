...
void rammy_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct std_npc_driver_data*)set_data(cn, DRD_STDNPCDRIVER, sizeof(struct std_npc_driver_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      ppd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));


      if (ppd) {
        switch (ppd->rammy_state) {
        case 0:		sppd = (struct staffer_ppd*)set_data(co, DRD_STAFFER_PPD, sizeof(struct staffer_ppd));
        ...
        case 5:		say(cn, "And come back here afterwards, I'm in need of thy help with another problem.");
          ppd->rammy_state++; didsay = 1;
          break;
        case 6:		sppd = (struct staffer_ppd*)set_data(co, DRD_STAFFER_PPD, sizeof(struct staffer_ppd));
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_NPC && msg->dat1 == NTID_GLADIATOR) {
      ...
      if (cc && (ch[cc].flags & CF_PLAYER)) {
        ppd = (struct arkhata_ppd*)set_data(cc, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
        ...
        break;
      case 5:		ppd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
...
void fortressguard_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct fortressguard_driver_data*)set_data(cn, DRD_SIMPLEBADDYDRIVER, sizeof(struct fortressguard_driver_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
        ...
        break;
      case 6:		ppd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
        ...
        break;
      case 7:		ppd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
...
void stopwatch_driver(int in, int cn)
{
  ...
  ppd = (struct arkhata_ppd*)set_data(cn, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
