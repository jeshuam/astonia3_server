...
int do_walk(int cn, int dir) {
  ...
  if (!(ch[cn].flags & CF_EDEMON)) {  // slowdown for earth demon mud spell
    for (n = 0; n < 4; n++) {
      if ((fn = map[m].ef[n]) && ef[fn].type == EF_EARTHMUD)
        cost += edemon_reduction(cn, ef[fn].strength) * 2;
    }
  }
  ...
  if (diag) {
    if (map[ch[cn].x + dx + (ch[cn].y) * MAXMAP].flags &
        (MF_MOVEBLOCK | MF_TMOVEBLOCK)) {
      error = ERR_BLOCKED;
      return 0;
    }
    if (map[ch[cn].x + (ch[cn].y + dy) * MAXMAP].flags &
        (MF_MOVEBLOCK | MF_TMOVEBLOCK)) {
      error = ERR_BLOCKED;
      return 0;
    }
  ...
  if (map[m].ch) {
    elog(
        "do_walk(): map.ch at position %d,%d is set, but MF_TMOVEBLOCK is not.",
        x, y);
    error = ERR_CONFUSED;
    return 0;
  }
  ...
  ch[cn].duration = speed(ch[cn].value[0][V_SPEED], ch[cn].speed_mode,
                          cost + diag * cost / 2);
...
int do_take(int cn, int dir) {
  ...
  if (in < 1 || in >= MAXITEM) {
    elog("do_take(): item number on map position %d,%d is out of bounds.", x,
         y);
    map[m].it = 0;
    error = ERR_CONFUSED;
    return 0;
  ...
  ch[cn].act1 = in;
  ch[cn].duration =
      speed(ch[cn].value[0][V_SPEED], ch[cn].speed_mode, DUR_MISC_ACTION);
  if (ch[cn].speed_mode == SM_FAST) ch[cn].endurance -= end_cost(cn);
...
int do_use(int cn, int dir, int spec) {
  ...
  ch[cn].act2 = spec;
  ch[cn].duration =
      speed(ch[cn].value[0][V_SPEED], ch[cn].speed_mode, DUR_USE_ACTION);
  if (ch[cn].speed_mode == SM_FAST) ch[cn].endurance -= end_cost(cn);
...
int do_drop(int cn, int dir) {
  ...
  if (in < 1 || in >= MAXITEM) {
    elog("do_drop(): item number %d in citem of %s (%d) is out of bounds.", in,
         ch[cn].name, cn);
    ch[cn].citem = 0;
    error = ERR_CONFUSED;
    return 0;
  ...
  if (!(it[in].flags & IF_TAKE)) {
    elog(
        "do_drop(): character %s (%d) is dropping item %s (%d) which doesn't "
        "have IF_TAKE set",
...
int do_attack(int cn, int dir, int co) {
  ...
  ch[cn].act1 = co;
  ch[cn].duration =
      speed(ch[cn].value[0][V_SPEED], ch[cn].speed_mode, DUR_COMBAT_ACTION);
...
int do_give(int cn, int dir) {
  ...
  if (in < 1 || in >= MAXITEM) {
    elog("do_give(): item number %d in citem of %s (%d) is out of bounds.", in,
         ch[cn].name, cn);
    ch[cn].citem = 0;
    error = ERR_CONFUSED;
    return 0;
  ...
  if ((it[in].flags & IF_BONDTAKE) &&
      ((ch[cn].flags & CF_GOD) || (ch[co].flags & CF_GOD))) {
  ...
  if ((ch[cn].flags & CF_PLAYER) && (ch[co].flags & CF_PLAYER) &&
      ...
      realtime - ppd->swapped < 20) {
    log_char(cn, LOG_SYSTEM, 0,
             "°c3Give canceled: Your target has swapped recently.");
...
int do_bless(int cn, int co) {
  ...
  if ((ch[cn].flags & CF_PLAYER) &&
      !(ch[co].flags & (CF_PLAYER | CF_PLAYERLIKE))) {
    error = ERR_NOT_PLAYER;
    return 0;
  }

  if (!may_add_spell(co, IDR_BLESS)) {
  ...
  if (cn == co) {
    ...
    ch[cn].act1 = co;
    ch[cn].duration =
        speed(ch[cn].value[0][V_SPEED], ch[cn].speed_mode, DUR_MAGIC_ACTION);
    if (ch[cn].speed_mode == SM_FAST) ch[cn].endurance -= end_cost(cn);
  } else {
...
int do_heal(int cn, int co) {
  ...
  str = min(str, (ch[co].value[0][V_HP] * POWERSCALE) -
                     ch[co].hp);  // but only up to max hp
  str =
      min(str, ch[cn].mana * 2);  // but only with as much mana as we have left
...
int do_fireball(int cn, int x, int y) {
  ...
  if (!dir) {
    if (!may_add_spell(cn, IDR_FIRERING)) {
      error = ERR_ALREADY_WORKING;
      return 0;
    }
...
int do_magicshield(int cn) {
  ...
  str =
      min(str, ch[cn].value[0][V_MAGICSHIELD] * POWERSCALE - ch[cn].lifeshield);
...
int do_flash(int cn) {
  ...
  if (ch[cn].x < 1 || ch[cn].x >= MAXMAP || ch[cn].y < 1 ||
      ch[cn].y >= MAXMAP) {
    error = ERR_ILLEGAL_COORDS;
    return 0;
  }
...
int do_warcry(int cn) {
  ...
  if (ch[cn].endurance < ch[cn].value[0][V_WARCRY] * POWERSCALE / 3) {
    error = ERR_MANA_LOW;
    return 0;
  }
...
int container(int cn, int pos, int flag, int fast) {
  ...
  if (in < 1 || in >= MAXITEM) {
    elog("container(): found illegal item %d on map pos %d,%d, removing.", in,
         x, y);
    map[x + y * MAXMAP].it = 0;
  ...
  if (ct < 1 || ct >= MAXCONTAINER) {
    elog("container(): found illegal container %d in item %s (%d), removing.",
         ct, it[in].name, in);
    it[in].content = 0;
  ...
  if (con[ct].owner == charID(cn) && con[ct].owner_not_seyan == 1 &&
  ...
  if (flag) {  // swap
    ...
    if (ch[cn].flags & CF_PLAYER) {
      if (in) {
        if (con[ct].in)
          dlog(cn, in, "took %s from container (item %d/%s/%s)", it[in].name,
               con[ct].in, it[con[ct].in].name, it[con[ct].in].description);
        else if (con[ct].cn)
          dlog(cn, in, "took %s from container (char %d/%s/%s)", it[in].name,
               con[ct].cn, ch[con[ct].cn].name, ch[con[ct].cn].description);
        else
          dlog(cn, in, "took %s from container (unspec?)", it[in].name);
      }

      if (in2) {
        if (con[ct].in)
          dlog(cn, in2, "dropped %s into container (item %d/%s/%s)",
               it[in2].name, con[ct].in, it[con[ct].in].name,
               it[con[ct].in].description);
        else if (con[ct].cn)
          dlog(cn, in2, "took %s from container (char %d/%s/%s)", it[in2].name,
               con[ct].cn, ch[con[ct].cn].name, ch[con[ct].cn].description);
        else
          dlog(cn, in2, "dropped %s into container (unspec?)", it[in2].name);
      }
    }
    ...
    if ((in = con[ct].item[pos])) look_item(cn, it + in);
  }
...
int use_item(int cn, int in) {
  ...
  if ((ct = it[in].content)) {
    if (ct < 1 || ct >= MAXCONTAINER) {
      elog("item %s (%d) has illegal container %d. resetting.", it[in].name, in,
           ct);
      it[in].content = 0;
      error = ERR_CONFUSED;
      return 0;
...
int char_swap(int cn) {
  ...
  if ((ch[cn].flags & CF_PLAYER) &&
      (ppd = (struct misc_ppd *)set_data(cn, DRD_MISC_PPD,
                                         sizeof(struct misc_ppd))))
    ppd->swapped = realtime;
