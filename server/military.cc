...
void military_master_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct military_master_data*)set_data(cn, DRD_MILITARYMASTER, sizeof(struct military_master_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (!(ppd = (struct military_ppd*)set_data(co, DRD_MILITARY_PPD, sizeof(struct military_ppd)))) {
        remove_message(cn, msg);
        continue;
      }
    ...
    if (msg->type == NT_TEXT) {
      co = msg->dat3;
      if (!(ch[co].flags & CF_PLAYER) || !(ppd = (struct military_ppd*)set_data(co, DRD_MILITARY_PPD, sizeof(struct military_ppd)))) {
        remove_message(cn, msg);
        continue;
      }
...
void military_advisor_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct military_advisor_data*)set_data(cn, DRD_MILITARYADVISOR, sizeof(struct military_advisor_data));
  if (!dat) return;	// oops...
