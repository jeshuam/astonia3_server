...
static const char *diff_name[5] = {"easy", "normal", "hard", "impossible",
                                   "insane"};
...
void generate_demon_mission(int lvl, struct military_ppd *ppd) {
  ...
  ppd->mis[0].exp =
      1 * pow(cbrt(ppd->military_pts) + 5, 4) / 16;  // normal experience worth
  ...
  ppd->mis[1].exp =
      2 * pow(cbrt(ppd->military_pts) + 5, 4) / 16;  // normal experience worth
  ...
  ppd->mis[2].exp =
      4 * pow(cbrt(ppd->military_pts) + 5, 4) / 16;  // normal experience worth
  ...
  ppd->mis[3].exp =
      10 * pow(cbrt(ppd->military_pts) + 5, 4) / 16;  // normal experience worth
  ...
  ppd->mis[4].exp =
      25 * pow(cbrt(ppd->military_pts) + 5, 4) / 16;  // normal experience worth
}

// additional misison - may overwrite one or more
void generate_sewer_mission(int lvl,
                            struct military_ppd *ppd) {  // 9...39 step 2
  switch (RANDOM(5)) {
    case 0:
      ...
      ppd->mis[0].pts = 1;               // worth one military point
      ppd->mis[0].exp = 1 * pow(cbrt(ppd->military_pts) + 5, 4) /
                        16;  // normal experience worth
      break;
    ...
    case 1:
      ...
      ppd->mis[1].pts = 2;               // worth one military point
      ppd->mis[1].exp = 2 * pow(cbrt(ppd->military_pts) + 5, 4) /
                        16;  // normal experience worth
      break;
    ...
    case 2:
      ...
      ppd->mis[2].pts = 4;                 // worth one military point
      ppd->mis[2].exp = 4 * pow(cbrt(ppd->military_pts) + 5, 4) /
                        16;  // normal experience worth
      break;
    ...
    case 3:
      ...
      ppd->mis[3].pts = 10;                  // worth one military point
      ppd->mis[3].exp = 10 * pow(cbrt(ppd->military_pts) + 5, 4) /
                        16;  // normal experience worth
      break;
    ...
    case 4:
      ...
      ppd->mis[4].pts = 25;                  // worth one military point
      ppd->mis[4].exp = 25 * pow(cbrt(ppd->military_pts) + 5, 4) /
                        16;  // normal experience worth
      break;
  }
}
...
void generate_mission(int cn, struct military_ppd *ppd) {
  ...
  if (rank * rank * rank > ppd->military_pts)
    ppd->military_pts = rank * rank * rank;
...
void offer_missions(int cn, int co, struct military_ppd *ppd) {
  ...
  for (n = 0; n < 5; n++) {
    ...
    switch (ppd->mis[n].type) {
      case 1:
        say(cn,
            "I have an °c4%s°c0 mission for you, %s. It is to slay %d level %d "
            "demons in the Pentagram Quest.",
            diff_name[n], ch[co].name, ppd->mis[n].opt1, ppd->mis[n].opt2);
        break;
      case 2:
        say(cn,
            "I have an °c4%s°c0 mission for you, %s. It is to slay %d level %d "
      ...
      case 3:
        say(cn,
            "I have an °c4%s°c0 mission for you, %s. It is to find %d units of "
            "silver in the Mine.",
            diff_name[n], ch[co].name, ppd->mis[n].opt1);
        break;
    }
  }
}
...
void display_mission(int cn, int co, int nr, struct military_ppd *ppd) {
  switch (ppd->mis[nr].type) {
    case 1:
      say(cn,
          "Your mission is to slay %d level %d demons in the Pentagram Quest.",
          ppd->mis[nr].opt1, ppd->mis[nr].opt2);
      break;
    case 2:
...
void military_master_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct military_master_data *)set_data(
      cn, DRD_MILITARYMASTER, sizeof(struct military_master_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      co = msg->dat1;
      if (char_dist(cn, co) > 10 || !(ch[co].flags & CF_PLAYER) ||
          !char_see_char(cn, co)) {
        remove_message(cn, msg);
        continue;
      }
      if (!(ppd = (struct military_ppd *)set_data(
                co, DRD_MILITARY_PPD, sizeof(struct military_ppd)))) {
        remove_message(cn, msg);
        continue;
      }
      if ((nr = get_char_clan(co))) {
        if (dat->storage_data.clan_pts[nr] > 12000 &&
            dat->last_recom != ch[co].ID) {
          ppd->current_pts += 5;
          dat->storage_data.clan_pts[nr] -= 12000;
          dat->last_recom = ch[co].ID;
          say(cn, "Be greeted, %s. You've been recommended by your clan!",
              ch[co].name);
        }
      }
      if (ppd->recommend != yday + 1) {
        for (n = 0; n < MAXADVISOR; n++) {
          if (ppd->advisor_last[n] == yday + 1) {
            say(cn,
                "Be greeted, %s. You have been recommended by my trusty "
                "advisor %d",
                ch[co].name, n);
          }
        }
      ...
      if (ppd->master_state == 0) {
        if (ppd->took_mission) {
          say(cn,
              "Ah, hello %s. Any luck with your mission? Or would you like to "
              "°c4hear°c0 it again? Or have you °c4failed°c0 to complete it?",
              ch[co].name);
          ppd->master_state = 2;
        } else if (ppd->solved_yday == yday + 1) {
          say(cn, "I don't have another mission for you today, %s.",
              ch[co].name);
          ppd->master_state = 2;
        } else if (get_army_rank_int(co)) {
          say(cn, "Hello, %s. I might have a °c4mission°c0 for you.",
              ch[co].name);
          ppd->master_state = 2;
        } else {
        ...
        say(cn, "Hello again, %s. I might have a °c4mission°c0 for you.",
            ch[co].name);
        ppd->master_state = 2;
      }
      if (ppd->solved_mission) {
        ...
        if (ch[co].prof[P_MERCENARY]) {
          ...
          say(cn,
              "Well done, %s. You've solved your mission! Your pay is %.2fG.",
              ch[co].name, ppd->mis[n].exp / 500.0);

          pts = ppd->mis[n].pts + ppd->mis[n].pts / 2 +
                ppd->mis[n].pts * ch[cn].prof[P_MERCENARY] * 3 / 100 + 1;
        ...
        dlog(cn, 0, "Got %d mil exp, %d normal exp for solving a mission",
             ppd->mis[n].exp, pts);
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (!(ch[co].flags & CF_PLAYER) ||
          !(ppd = (struct military_ppd *)set_data(
                co, DRD_MILITARY_PPD, sizeof(struct military_ppd)))) {
        remove_message(cn, msg);
        continue;
      }
      ...
      switch (res) {
        ...
        case 10:
          if (ppd->took_mission) {
            say(cn,
                "You already have a mission. Would you like to °c4hear°c0 it "
                "again?");
            break;
          }
          ...
          if (!get_army_rank_int(co)) {
            say(cn,
                "But you don't even belong to the army, %s. Talk to Seymour "
                "about enrollment.",
                ch[co].name);
            break;
          }
        ...
        case 11:
          if (ppd->took_mission) {
            say(cn,
                "You already have a mission, %s. Would you like to °c4hear°c0 "
                "it again?",
                ch[co].name);
            break;
          }
          ...
          if (ppd->mission_yday != yday + 1) {
            say(cn, "I haven't offered you that kind of mission today, %s.",
                ch[co].name);
            break;
          }
        ...
        case 12:
          ...
          if (ppd->current_pts < ppd->mis[1].pts) {
            say(cn, "I have not offered you that kind of mission, %s.",
                ch[co].name);
            break;
          }
          if (ppd->mission_yday != yday + 1) {
        ...
        case 16:
          ...
          say(cn,
              "So, you failed? Well, %s, I'll remove that mission from your "
              "record. Would you like to get another °c4mission°c0?",
              get_army_rank_string(co));
          ppd->took_mission = 0;
          break;
        case 17:
          if (!ppd->took_mission) {
            say(cn, "But you do not have a °c4mission°c0 yet, %s.",
                get_army_rank_string(co));
          }
        ...
        case 18:
          ...
          say(cn, "You have %d pts and you have gained %d exp.",
              ppd->military_pts, ppd->normal_exp);
          for (n = 1; n < 32; n++) {
          ...
          for (n = 0; n < 5; n++) {
            say(cn,
                "I have given %d %s quests, %d of these have been solved "
                "(%.2f%%) for a total of %d exp (%.2f exp per quest)",
                dat->storage_data.quests_given[n], diff_name[n],
                dat->storage_data.quests_solved[n],
                100.0 / dat->storage_data.quests_given[n] *
                    dat->storage_data.quests_solved[n],
                dat->storage_data.exp_given[n],
                (double)dat->storage_data.exp_given[n] /
                    dat->storage_data.quests_solved[n]);
          }
          break;
        case 19:
  ...
  switch (dat->storage_state) {
    case 0:
      if (create_storage(dat->storage_ID, "Master Data", &dat->storage_data,
                         sizeof(dat->storage_data)))
        dat->storage_state++;
      break;
    case 1:
      if (check_create_storage()) dat->storage_state++;
      break;
    case 2:
      if (read_storage(dat->storage_ID, dat->storage_version))
        dat->storage_state++;
      break;
    case 3:
      res = check_read_storage(&dat->storage_version, &tmp, &size);
    ...
    case 4:
      break;  // no op
    case 5:
      if (update_storage(dat->storage_ID, dat->storage_version,
                         &dat->storage_data, sizeof(dat->storage_data)))
        dat->storage_state++;
      break;
    case 6:
      if (check_update_storage()) {
        dat->storage_state++;
        dat->storage_version++;
      }
      break;
...
static void adv_introduction(int cn, int co,
  ...
  switch (dat->storage_ID % 4) {
    case 0:
      ptr =
          "I could do you a °c4favor°c0, %s, I could mention your name to the "
          "military governor of Aston. I'm sure that'd help you get that "
          "promotion early!";
      break;
    case 1:
      ptr =
          "Say, %s, would you like to speed up your way up the rank ladder? I "
          "could speak to the military governor of Aston if you want me to do "
          "you that °c4favor°c0.";
      break;
    case 2:
      ptr =
          "Not getting promoted as fast as you want, %s? I could do you the "
          "°c4favor°c0 of talking to the military governor of Aston about you.";
      break;
    default:
      ptr = "Need a °c4favor°c0, %s?";
      break;
  }
...
static void adv_favor_desc(int cn, int co, struct military_advisor_data *dat) {
  ...
  switch (dat->storage_ID % 2) {
    case 0:
      ptr =
          "My favors come in five sizes, °c4small°c0, °c4medium°c0, °c4big°c0, "
          "°c4huge°c0 and °c4vast°c0.";
      break;
    default:
      ptr =
          "I could do you a °c4small°c0, °c4medium°c0, °c4big°c0, °c4huge°c0 "
          "or °c4vast°c0 favor, %s.";
      break;
  }
...
void military_advisor_driver(int cn, int ret, int lastact) {
  ...
  struct msg *msg, *next;
  void *tmp;
  static const char *fav_name[5] = {"small", "normal", "big", "huge", "vast"};

  dat = (struct military_advisor_data *)set_data(
      cn, DRD_MILITARYADVISOR, sizeof(struct military_advisor_data));
  if (!dat) return;  // oops...

  if (dat->storage_ID < 27)
    idx = dat->storage_ID - 7;
  else
    idx = dat->storage_ID - 31 + 3;
      ...
      switch (res) {
        ...
        case 3:
          if (ppd->advisor_last[idx] == yday + 1)
            quiet_say(
                cn,
                "Mentioning your name twice a day won't accomplish much, %s.",
                ch[co].name);
          else
            adv_favor_desc(cn, co, dat);
          break;
        case 4:
          if (ppd->advisor_last[idx] == yday + 1)
            quiet_say(
            ...
            cost = advisor_price(ch[co].level) *
                   1;  // max(500,calc_cost(100*100,dat->storage_data+0,30));
            quiet_say(cn,
                      "You can get a small favor for the humble fee of %dG, "
                      "%dS, %s. Say °c4pay°c0 if you want it.",
                      cost / 100, cost % 100, ch[co].name);
        ...
        case 5:
          if (ppd->advisor_last[idx] == yday + 1)
            ...
            cost = advisor_price(ch[co].level) *
                   3;  // max(1500,calc_cost(200*100,dat->storage_data+1,20));
            quiet_say(cn,
                      "You can get a medium favor for the humble fee of %dG, "
                      "%dS, %s. Say °c4pay°c0 if you want it.",
                      cost / 100, cost % 100, ch[co].name);
            ppd->advisor_cost = cost;
            ppd->advisor_state = 2;
        ...
        case 6:
          if (ppd->advisor_last[idx] == yday + 1)
            ...
            cost = advisor_price(ch[co].level) *
                   10;  // max(4000,calc_cost(400*100,dat->storage_data+2,15));
            quiet_say(cn,
                      "You can get a big favor for the humble fee of %dG, %dS, "
                      "%s. Say °c4pay°c0 if you want it.",
                      cost / 100, cost % 100, ch[co].name);
            ppd->advisor_cost = cost;
            ppd->advisor_state = 2;
        ...
        case 7:
          if (ppd->advisor_last[idx] == yday + 1)
            ...
            cost = advisor_price(ch[co].level) *
                   20;  // max(10000,calc_cost(800*100,dat->storage_data+3,10));
            quiet_say(cn,
                      "You can get a huge favor for the humble fee of %dG, "
                      "%dS, %s. Say °c4pay°c0 if you want it.",
                      cost / 100, cost % 100, ch[co].name);
            ppd->advisor_cost = cost;
            ppd->advisor_state = 2;
        ...
        case 8:
          if (ppd->advisor_last[idx] == yday + 1)
            ...
            cost = advisor_price(ch[co].level) *
                   35;  // max(25000,calc_cost(1600*100,dat->storage_data+4,5));
            quiet_say(cn,
                      "You can get a vast favor for the humble fee of %dG, "
                      "%dS, %s. Say °c4pay°c0 if you want it.",
                      cost / 100, cost % 100, ch[co].name);
            ppd->advisor_cost = cost;
            ppd->advisor_state = 2;
          ...
          break;
        case 9:
          if (ppd->current_advisor != dat->storage_ID ||
              ppd->advisor_state != 2) {
          ...
          add_cost(ppd->advisor_cost,
                   dat->storage_data + ppd->advisor_storage_nr);
          if (dat->storage_state == 4) dat->storage_state++;
          ...
          quiet_say(
              cn,
              "Alright, I'll mention your name to the military governor, %s.",
              ch[co].name);
        ...
        case 18:
          ...
          for (n = 0; n < 5; n++) {
            quiet_say(
                cn, "I have earned %.2fG on %d %s favors (%.2fG per favor)",
                dat->storage_data[n].earned / 100.0, dat->storage_data[n].sold,
                fav_name[n], dat->storage_data[n].earned / 100.0 /
  ...
  switch (dat->storage_state) {
    case 0:
      if (create_storage(dat->storage_ID, "Advisor Data", dat->storage_data,
                         sizeof(dat->storage_data)))
        dat->storage_state++;
      break;
    case 1:
    ...
    case 3:
      ...
      if (res == 1 && size == sizeof(dat->storage_data)) {
        ...
        xfree(tmp);
        dat->storage_state++;
        // charlog(cn,"read storage:
        // earned=%lldG/%lldG/%lldG/%lldG/%lldG",dat->storage_data[0].earned/100,dat->storage_data[1].earned/100,dat->storage_data[2].earned/100,dat->storage_data[3].earned/100,dat->storage_data[4].earned/100);
      } else if (res) {
        elog("check storage said %d (%d)", res, size);
        dat->storage_state++;
      }
...
void immortal_dead(int cn, int co) { charlog(cn, "I DIED! IMMORTAL!"); }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
  switch (nr) {
    case CDR_MILITARY_MASTER:
      ...
      return 1;
    case CDR_MILITARY_ADVISOR:
      military_advisor_driver(cn, ret, lastact);
      return 1;
