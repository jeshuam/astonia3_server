...
int creeper_tab[191] = {
    13,  15,  16,  18,  19,  20,  22,  23,  25,  26,  28,  29,  30,  31,  33,
    34,  36,  37,  39,  40,  41,  42,  44,  45,  46,  48,  49,  50,  51,  53,
    54,  55,  57,  58,  59,  60,  61,  62,  64,  65,  66,  68,  69,  70,  71,
    72,  73,  75,  76,  77,  79,  80,  81,  82,  83,  84,  86,  87,  88,  89,
    90,  91,  92,  94,  95,  96,  97,  99,  100, 101, 102, 103, 104, 105, 107,
    108, 109, 110, 111, 112, 113, 115, 116, 117, 118, 120, 121, 122, 123, 124,
    125, 126, 127, 129, 130, 131, 132, 133, 134, 135, 137, 138, 139, 140, 141,
    142, 143, 145, 146, 147, 148, 149, 150, 151, 152, 154, 155, 156, 157, 158,
    160, 161, 162, 163, 164, 165, 166, 168, 169, 170, 171, 172, 173, 174, 175,
    177, 178, 179, 180, 181, 182, 183, 184, 186, 187, 188, 189, 190, 191, 192,
    193, 195, 196, 197, 198, 199, 200, 201, 202, 204, 205, 206, 207, 208, 210,
    211, 212, 213, 214, 215, 216, 217, 219, 220, 221, 222, 223, 224, 225, 226,
    228, 229, 230, 231, 232, 233, 234, 235, 237, 238, 239};

void tunneldoor(int in, int cn) {
  int x, y, in2, m, b1, b2, c, b1cnt = 0, b2cnt = 0, co, xoff, yoff, used, n,
                               value;  //,want;
  ...
  ppd = (struct tunnel_ppd *)set_data(cn, DRD_TUNNEL_PPD,
                                      sizeof(struct tunnel_ppd));
  if (!ppd) return;  // oops...
  ...
  if (it[in].drdata[0] == 2 || it[in].drdata[0] == 3) {
    if (teleport_char_driver(cn, 250, 250)) {
      if (ppd->used[ppd->clevel] < 20) {
        ...
        if (it[in].drdata[0] == 2) {
          ...
          give_exp(cn, value);
          dlog(cn, 0, "got %d exp for solving long tunnel section %d (%d)",
               value, ppd->clevel, ppd->used[ppd->clevel]);
        }
        ...
        if (it[in].drdata[0] == 3) {
          ...
          dlog(cn, 0,
               "got %d military rank for solving long tunnel section %d (%d)",
               value, ppd->clevel, ppd->used[ppd->clevel]);
        }

      } else
        log_char(cn, LOG_SYSTEM, 0,
                 "You cannot get any more experience for this level.");
      ppd->clevel = 10;
    }
    return;
    ...
    if (ch[cn].level < 20)
      ppd->clevel = 10;
    else if (ch[cn].level <= 100)
      ppd->clevel = ch[cn].level - 10;
    else {
    ...
    for (yoff = 1; yoff < 255; yoff += 127) {
      ...
      for (x = 1 + xoff; x < 32 + xoff && !used; x++) {
        for (y = 1 + yoff; y < 128 + yoff && !used; y++) {
          m = x + y * MAXMAP;
          if ((co = map[m].ch) && (ch[co].flags & CF_PLAYER) && co != cn)
            used = 1;
        }
      }
      if (!used) break;
  ...
  if (used) {
    log_char(cn, LOG_SYSTEM, 0,
             "All tunnels are busy. Please try again later.");
    return;
  }
    ...
    for (y = 1 + yoff; y < 128 + yoff; y++) {
      ...
      if ((in2 = map[m].it)) {
        if (it[in2].ID == IID_TUNNELDOOR1) {  // block marker 1
          ...
          it[in2].sprite = 0;
          if (c == 0)
            build_fighter(it[in2].x, it[in2].y, creeper_tab[ppd->clevel - 10],
                          ppd->clevel);
          ...
          build_fighter(it[in2].x, it[in2].y, creeper_tab[ppd->clevel - 10],
                        ppd->clevel);
        } else if (it[in2].driver == IDR_TUNNELDOOR &&
                   (it[in2].drdata[0] == 2 || it[in2].drdata[0] == 3)) {
          sprintf(it[in2].name, "Column %d, used %d times", ppd->clevel,
                  ppd->used[ppd->clevel]);
  ...
  if (!it[in].drdata[0])
    ...
    ch[cn].hp = min(ch[cn].value[0][V_HP] * POWERSCALE,
                    ch[cn].hp + ch[cn].value[1][V_HP] * POWERSCALE / 2);
...
int ch_driver(int nr, int cn, int ret, int lastact) {
  switch (nr) {
    ...
    // case CDR_MISSIONFIGHT: mission_fighter_driver(cn,ret,lastact); return
    // 1;
