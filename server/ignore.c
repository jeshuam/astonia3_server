...
int ignoring(int cn, int ID)
{
  ...
  if (!(ppd = (struct ignore_ppd*)set_data(cn, DRD_IGNORE_PPD, sizeof(struct ignore_ppd)))) return 0;	// OOPS
...
void ignore(int cn, int ID)
{
  ...
  if (!(ppd = (struct ignore_ppd*)set_data(cn, DRD_IGNORE_PPD, sizeof(struct ignore_ppd)))) return;	// OOPS
