...
void list_punishment(int rID, struct punishment *pun, int cID, int date,
                     int ID) {
  char name[80];
  struct tm *tm;
  ...
  tell_chat(0, rID, 1,
            "P%d: Level: %d, Exp: %d, Karma: %d, Creator: %s, Date: "
            "%02d/%02d/%04d %02d:%02d:%02d, Reason: %s",
            ID, pun->level, pun->exp, pun->karma, name, tm->tm_mon + 1,
            tm->tm_mday, tm->tm_year + 1900, tm->tm_hour, tm->tm_min,
            tm->tm_sec, pun->reason);
}

// pID is punishing co
int punish(int pID, struct character *co, int level, char *reason, int *plock,
           int *pkick) {
  ...
  return add_note(co->ID, 1, pID, (unsigned char *)&pm, sizeof(pm));
}
