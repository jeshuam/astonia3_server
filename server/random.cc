...
int build_shrine_sub(int x, int y, int level, int type, const char *name, int sprite)
{
  int in;
...
void randommaster(int cn, int ret, int lastact)
{
  ...
  dat = (struct master_data*)set_data(cn, DRD_RANDOMMASTER, sizeof(struct master_data));
  if (!dat) return;	// oops...
...
void randomshrine(int in, int cn)
{
  ...
  ppd = (struct shrine_ppd*)set_data(cn, DRD_RANDOMSHRINE_PPD, sizeof(struct shrine_ppd));
  if (!ppd) return;
