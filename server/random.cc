...
struct qa qa[] = {
    ...
    {{"wizard", NULL},
     "The Wizard of Yendor was fascinated by the Labyrinth created by Ishtar "
     "and spent his life building an imitation of it. Before his death, he "
     "transfered all his magical energies into various °c4shrines°c0 he hid in "
     "his dungeons. With the Wizard no longer around, I take care of these "
     "°c4dungeons°c0.",
     0},
    {{"shrines", NULL},
     "Unfortunately, the Wizard had a peculiar sense of humor, and some of his "
     "shrines do strange things to those who use them. Do look at them before "
     "thou usest them, and decide carefully.",
     0},
    {{"yendor", NULL},
     "No one really knows where the name 'Yendor' stems from. But rumor has it "
     "that the Wizard's name used to be 'Rodney', before he changed it to 'The "
     "Wizard of Yendor'.",
     0},
    {{"dungeons", NULL},
     "The Wizard of Yendor created 99 different dungeons, but the first nine "
     "have collapsed. The remaining dungeons are of the levels 10 to 99. Thou "
     "canst ask me for °c4help°c0 if thou wishest to enter one of them.",
     0},
    {{"help", NULL}, NULL, 2},
    {{"list", NULL}, NULL, 3},
    {{"reset", NULL}, NULL, 4}};
...
int build_shrine_sub(int x, int y, int level, int type, const char *name,
                     int sprite) {
  int in;
  ...
  if ((in = map[m].it)) {
    if (it[in].flags & IF_PLAYERBODY) {
      ...
      if (!drop_item(in, 250, 245) && !drop_item(in, 250, 240) &&
        ...
        dlog(0, in, "RD: Could not teleport body %s (%d). Destroyed.",
             it[in].name, in);
        destroy_item(in);
      } else
...
void build_shrine(int x, int y, int level) {
  ...
  switch (level) {
    ...
    case 20:
      ...
      break;
    case 21:
      build_shrine_sub(x, y, level, 41, "Shrine of Kindness", 59396);
      break;
    case 22:
    ...
    case 54:
      ...
      break;
    case 55:
      build_shrine_sub(x, y, level, 42, "Shrine of Kindness", 59396);
      break;
    case 56:
      break;
    case 57:
...
int create_maze(int base, int level, int show) {
  ...
  while (maxi > 0 && panic--) {
    ...
    if (cell[m].t && !cell[m].l && cell[m + xsize].t && !cell[m + xsize].l)
      cell[m].special = 7;
    else if (!cell[m].t && cell[m].l && !cell[m + 1].t && cell[m + 1].l)
      cell[m].special = 8;
    else
      continue;
...
void create_dungeon(int cn, int co, int level, struct master_data *dat) {
  ...
  for (n = 0; n < 16; n++) {
    if (dat->owner[n] == ch[co].ID) {
      say(cn,
          "You have created a dungeon already, you may not create another one "
          "before the first one has collapsed. You can use °c4destroy %d°c0 to "
          "collapse it now.",
          n + 1);
      return;
    }
  ...
  if (best < DUNGEONTIME) {
    say(cn, "Sorry, all dungeons are busy. Please try again in %.2f minutes",
  ...
  say(cn, "This dungeon will collapse in %.2f minutes.",
...
void enter_dungeon(int cn, int co, int target, struct master_data *dat) {
  ...
  if (abs(dat->ownerlevel[target] - ch[co].level) > 3) {
    say(cn, "Sorry, this dungeon was created for levels %d to %d.",
        dat->ownerlevel[target] - 3, dat->ownerlevel[target] + 3);
    return;
  }
  ...
  for (n = 0; n < 16; n++) {
    if (dat->level[n]) {
      say(cn, "Dungeon %d: Level %d (%d to %d), remaining time: %.2f minutes.",
          n + 1, dat->level[n], dat->ownerlevel[n] - 3, dat->ownerlevel[n] + 3,
    ...
    if (ch[co].flags & CF_PLAYER) {
      if (((ch[co].x - 2) / 61) + ((ch[co].y - 2) / 61) * 4 == nr) {
        log_char(co, LOG_SYSTEM, 0,
                 "This dungeon will collapse in %.2f minutes.",
                 left / (TICKS * 60.0));
      }
    }
  }
...
void randommaster(int cn, int ret, int lastact) {
  ...
  dat = (struct master_data *)set_data(cn, DRD_RANDOMMASTER,
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      say(cn,
          "Hello %s! Welcome to the dungeons of the °c4Wizard°c0 of "
          "°c4Yendor°c0!",
          ch[co].name);
    ...
    if (msg->type == NT_TEXT) {
      ...
      switch (analyse_text_driver(cn, msg->dat1, (char *)msg->dat2, co)) {
        case 2:
          say(cn,
              "Use: 'create <nr>' to create a dungeon of level <nr>, 'enter "
              "<nr>' to enter dungeon <nr>, 'list' to get a listing of all "
              "dungeons or 'destroy <nr>' to destroy the dungeon <nr> you have "
              "created.");
          break;
        case 3:
          list_dungeon(cn, dat);
      ...
      for (haystack = (char *)msg->dat2; *haystack && *haystack != ':';
           haystack++)
        ;
      ...
      if ((ptr = strcasestr(haystack, "destroy"))) {
        ...
        if (target > 0 && target < 17) {
          ...
          if (dat->owner[target] == ch[co].ID) {
            ...
            say(cn,
                "Sorry, you did not create that dungeon, hence you may not "
                "destroy it.");
        } else
          say(cn, "Sorry, that number is out of bounds.");
      }
    }
...
void shrine_indecisiveness(int in, int cn, int nr, int level,
...
void shrine_bribes(int in, int cn, int nr, int level, struct shrine_ppd *ppd) {
  ...
  if (val < need) {
    log_char(cn, LOG_SYSTEM, 0,
             "You feel a hand reach into your pocket and touch your purse. A "
             "second later, it is removed with a sneer.");
    return;
  }
  log_char(cn, LOG_SYSTEM, 0,
           "You feel a hand reach into your pocket and touch your purse. "
           "Shocked, you reach for your purse and find it %sempty.",
           want < val ? "almost " : "");
  ...
  dlog(cn, 0, "Used shrine of bribes (%d G, %d exp (%d,%d), lvl %d)", val / 100,
       val / 4, level, level_value(level), level);
...
int can_receive_mod(int in, int *pslot, int v) {
  ...
  if (it[in].ID != IID_GENERIC_SPECIAL && it[in].ID != IID_HARDKILL &&
      it[in].ID != 0)
    return 0;
  ...
  for (n = 0; n < MAXMOD; n++) {
    ...
    switch (it[in].mod_index[n]) {
      ...
      case V_LIGHT:
        break;
      default:
        if (it[in].mod_value[n] > 0 && it[in].mod_index[n] >= 0) cnt++;
    }
  }
  if (cnt > 2) return 0;
...
void shrine_welding(int in, int cn, int nr, int level, struct shrine_ppd *ppd) {
  ...
  if (ch[cn].level + ch[cn].level / 4 + 2 < level) {
    log_char(cn, LOG_SYSTEM, 0,
             "You are not powerful enough to use this shrine.");
    return;
  }
  ...
  if (!cnt) {
    log_char(cn, LOG_SYSTEM, 0,
             "You feel a cold hand touch your equipment. After it has touched "
             "all your items, it leaves with a laugh of contempt.");
    return;
  }
  ...
  for (n = 0; n < 12; n++) {
    for (m = 0; m < MAXMOD; m++) {
      if ((tmp = ch[cn].item[n]) && (in2 = can_give_mod(tmp, m)) && !cnt--) {
        slot2 = m;
        break;
      }
    }
  ...
  for (n = cnt = 0; n < 12; n++) {
    if ((tmp = ch[cn].item[n]) && tmp != in2 &&
        can_receive_mod(tmp, &slot1, it[in2].mod_index[slot2]))
      cnt++;
  }
  if (!cnt) {
    log_char(cn, LOG_SYSTEM, 0,
             "You feel a cold hand touch your equipment. After it has touched "
             "all your items, it leaves with a laugh of regret.");
    return;
  }
  // xlog("cnt=%d",cnt);
  cnt = RANDOM(cnt);
  ...
  for (n = 0; n < 12; n++) {
    if ((tmp = ch[cn].item[n]) && tmp != in2 &&
        (in1 = can_receive_mod(tmp, &slot1, it[in2].mod_index[slot2])) &&
        !cnt--)
      break;
  }

  dlog(cn, in1,
       "Used shrine of welding (lvl %d): Welding %s (%d) with %s (%d) (%s) (1)",
       level, it[in1].name, in1, it[in2].name, in2,
       skill[it[in2].mod_index[slot2]].name);
  dlog(cn, in2,
       "Used shrine of welding (lvl %d): Welding %s (%d) with %s (%d) (%s) (2)",
       level, it[in1].name, in1, it[in2].name, in2,
       skill[it[in2].mod_index[slot2]].name);

  log_char(cn, LOG_SYSTEM, 0,
           "You feel a burning hand touch your %s and your %s.", it[in1].name,
           it[in2].name);
  ...
  if (!strstr(it[in1].description, "Christmas"))
    snprintf(it[in1].description, sizeof(it[in1].description) - 1,
             "%s of Welding.", it[in1].name);
  ...
  if (!strstr(it[in2].description, "Christmas"))
    snprintf(it[in2].description, sizeof(it[in2].description) - 1,
             "%s of Unwelding.", it[in2].name);
...
void shrine_edge(int in, int cn, int nr, int level, struct shrine_ppd *ppd) {
  ...
  dlog(cn, 0,
       "Used shrine of living on the edge (%d saves, %d exp (%d,%d) (lvl %d))",
       ch[cn].saves, bonus, level, level_value(level), level);

  give_exp_bonus(cn, bonus);
  ch[cn].saves = 0;

  log_char(cn, LOG_SYSTEM, 0,
           "A booming voice declares: 'Living on the edge has its merits - and "
           "its dangers!'");
...
void shrine_kindness(int in, int cn, int nr, int level,
                     struct shrine_ppd *ppd) {
  ...
  log_char(cn, LOG_SYSTEM, 0,
           "A tender voice whispers: 'Mayest thou find other ways to amuse "
           "thyself. Thou art not a killer henceforth.'");
  shrine_set(nr, ppd);
  sendquestlog(cn, ch[cn].player);
}

void shrine_vitality(int in, int cn, int nr, int level,
                     struct shrine_ppd *ppd) {
  int cost, n, v, s, cnt;
  ...
  if (s)
    cnt = min(5, 100 - ch[cn].value[1][v]);
  else
    cnt = min(5, 115 - ch[cn].value[1][v]);
  ...
  dlog(cn, 0, "Used shrine of vitality (%d %s, %d exp, lvl %d)", cnt,
       skill[v].name, cost, level);
...
void shrine_continuity(int in, int cn, int nr, int level,
                       struct shrine_ppd *ppd) {
  int cost;
  ...
  if (level < ppd->continuity) {
    if (level == 99) {
      ...
      log_char(cn, LOG_SYSTEM, 0,
               "A steady voice says: 'Thou hast visited me already.'");
    return;
  }
  if (level > ppd->continuity) {
    log_char(
  ...
  dlog(cn, 0, "Used shrine of continuity (lvl %d, %d exp (%d,%d))", level, cost,
...
void shrine_braveness(int in, int cn, int nr, int level,
                      struct shrine_ppd *ppd) {
  int cost;
  ...
  dlog(cn, 0, "Used shrine of braveness (%d exp, %d gold (%d,%d), lvl %d)",
       cost, cost / 10, level, level_value(level), level);

  log_char(cn, LOG_SYSTEM, 0,
           "A triumphant voice says: 'Thou art brave indeed!'");
...
void shrine_security(int in, int cn, int nr, int level,
                     struct shrine_ppd *ppd) {
  if (ch[cn].saves > 5) {
    log_char(cn, LOG_SYSTEM, 0,
             "A scared voice whispers: 'Thou art secure already.'");
    return;
  }
  if (ch[cn].flags & CF_HARDCORE) {
    log_char(cn, LOG_SYSTEM, 0,
             "A scared voice whispers: 'Thou wilt never be secure.'");
    return;
  }
  log_char(cn, LOG_SYSTEM, 0,
           "A scared voice whispers: 'Thou shalt be secure.'");

  ch[cn].saves++;
  log_char(cn, LOG_SYSTEM, 0, "Thou hast %s save%s.", save_number(ch[cn].saves),
           ch[cn].saves == 1 ? "" : "s");
...
void shrine_jobless(int in, int cn, int nr, int level, struct shrine_ppd *ppd) {
  ...
  if (n == P_MAX) {
    log_char(cn, LOG_SYSTEM, 0,
             "A bored voice says: 'Thou art jobless already.'");
    return;
  }
...
void randomshrine(int in, int cn) {
  ...
  ppd = (struct shrine_ppd *)set_data(cn, DRD_RANDOMSHRINE_PPD,
                                      sizeof(struct shrine_ppd));
  if (!ppd) return;

  for (n = 0; n < INVENTORYSIZE; n++)
    if ((in2 = ch[cn].item[n]) && it[in2].ID == IID_AREA14_SHRINEKEY &&
        it[in2].drdata[0] == it[in].drdata[1])
      break;
  if (n == INVENTORYSIZE) {
    if (!(in2 = ch[cn].citem) || it[in2].ID != IID_AREA14_SHRINEKEY ||
        it[in2].drdata[0] != it[in].drdata[1]) {
      log_char(cn, LOG_SYSTEM, 0,
               "Nothing happens. You seem to need some kind of magical item to "
               "invoke the powers of the shrine.");
      return;
    }
  }
  ...
  switch (it[in].drdata[0]) {
    ...
    default:
      elog("item %s at %d,%d: unknown type %d", it[in].name, it[in].x, it[in].y,
           it[in].drdata[0]);
  }
}
...
void trapdoor(int in, int cn) {
  ...
  if (cn) {                                              // open or block
    if (ch[cn].x == it[in].x && ch[cn].y == it[in].y) {  // stepped on it
      ...
      log_char(cn, LOG_SYSTEM, 0,
               "A trapdoor opens under your feet, but you manage to jump back "
               "in time.");
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "You'd need something like a hard stick to lock the door.");
...
void gastrap(int in, int cn) {
  ...
  if (cn) {
    if (!it[in].drdata[1]) {
      ...
      return;
  } else if (!it[in].drdata[1])
    return;

  n = it[in].x + it[in].y * MAXMAP;
  if (map[n].fsprite >= 15291 && map[n].fsprite <= 15326)
    m = n;
  else if (map[n + 1].fsprite >= 15291 && map[n + 1].fsprite <= 15326)
    m = n + 1;
  else if (map[n - 1].fsprite >= 15291 && map[n - 1].fsprite <= 15326)
    m = n - 1;
  else if (map[n + MAXMAP].fsprite >= 15291 && map[n + MAXMAP].fsprite <= 15326)
    m = n + MAXMAP;
  else if (map[n - MAXMAP].fsprite >= 15291 && map[n - MAXMAP].fsprite <= 15326)
    m = n - MAXMAP;
  else {
  ...
  if (ch[cn].flags & CF_PLAYER)
    hurt(cn, POWERSCALE * it[in].drdata[0], 0, 1, 50, 33);
}
