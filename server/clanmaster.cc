 ...
 * added char_see_char call to text commands of clan clerk (avoids shouting
 *commands accidentally reaching the wrong clerk)
 *
...
struct qa qa[] = {
    ...
    {{"clan", NULL},
     "If you wish to found a clan, tell me the name you want that clan to "
     "have, and hand me a Clan Jewel. If you wish to tell me the name, use: "
     "'name: <clan name>', that is, to name your clan 'Black Rose', use: "
     "'name: Black Rose'. Be aware that the game will use the phrase 'The "
     "<clan name> clan', ie. 'The Black Rose Clan', so avoid 'The' and 'Clan' "
     "in the name.",
     0},
    {{"jewels", NULL}, NULL, 2},
    {{"repeat", NULL}, NULL, 3},
    {{"raid", NULL},
     "I will enter the clan you name, kill any guards I see and try to steal a "
     "clan jewel. If I succeed I will transfer that jewel to your clan vault. "
     "I can only attack a clan if you are at war with that clan. If you want "
     "me to attack clan 2, say 'attack 2'.",
     0},
    {{"scout", NULL},
     "On a scouting mission, I will just take a peek into the clan you name "
     "and give you a report about its guards. Say 'sneak 2' if you want me to "
     "scout clan number 2.",
     0},
    {{"info", NULL}, NULL, 4}};
...
void clanmaster_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct clanmaster_driver_data *)set_data(
      cn, DRD_CLANMASTERDRIVER, sizeof(struct clanmaster_driver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (!get_char_club(co) && !get_char_clan(co))
        quiet_say(cn, "Hello %s! Would you like to found a °c4clan°c0?",
                  ch[co].name);
    ...
    if (msg->type == NT_TEXT) {
      ...
      if ((msg->dat1 == 1 || msg->dat1 == 2) &&
          (co = msg->dat3) != cn) {  // talk, and not our talk
        if ((ptr = strcasestr((char *)msg->dat2, "name:")) &&
            (fnd = (struct clan_found_data *)set_data(
                 co, DRD_CLANFOUND, sizeof(struct clan_found_data)))) {
          if (!(ch[co].flags & CF_PAID)) {
            quiet_say(cn,
                      "I'm sorry, %s, but only paying players may found clans.",
                      ch[co].name);
            ...
            fnd->state = 1;
            quiet_say(cn,
                      "Your clan, %s, will be named '%s'. Try again if that is "
                      "not what you want. Or hand me a Clan Jewel to proceed. "
                      "You can buy them at Jeremy's",
                      ch[co].name, fnd->name);
          } else
            quiet_say(cn,
                      "You are already a member of a clan or club. You cannot "
                      "found a new one.");
        }
        if ((ptr = strcasestr((char *)msg->dat2, "accept:"))) {
          if (!get_char_clan(co) || ch[co].clan_rank < 2) {
            ...
            quiet_say(cn, "To join %s's clan %s, say: 'join: %s'", dat->join,
                      dat->accept, dat->join);
          }
        }
        if ((ptr = strcasestr((char *)msg->dat2, "join:"))) {
          if (get_char_clan(co) || get_char_club(co)) {
            ...
            if (strcasecmp(dat->accept, ch[co].name)) {
              ...
              quiet_say(cn, "%s, you are now a member of %s's clan.",
                        ch[co].name, dat->join);
              dat->accept[0] = 0;
              dat->accept_clan = 0;
              dat->join[0] = 0;
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "leave!"))) {
          if (!get_char_clan(co)) {
            ...
            remove_member(co, co);
            quiet_say(cn, "You are no longer a member of any clan, %s",
                      ch[co].name);
          }
        }
        if ((ptr = strcasestr((char *)msg->dat2, "rank:"))) {
          if (!get_char_clan(co) || ch[co].clan_rank < 4) {
            ...
            for (cc = getfirst_char(); cc; cc = getnext_char(cc)) {
              if (!strcasecmp(tmp, ch[cc].name) && (ch[cc].flags & CF_PLAYER))
                break;
            }
            if (cc) {
              if (!(ch[cc].flags & CF_PAID) && rank > 1) {
                quiet_say(cn,
                          "%s is not a paying player, you cannot set the rank "
                          "higher than 1.",
                          ch[cc].name);
              } else if (get_char_clan(cc) == get_char_clan(co)) {
                ch[cc].clan_rank = rank;
                add_clanlog(ch[cc].clan, clan_serial(ch[cc].clan), ch[cc].ID,
                            30, "%s rank was set to %d by %s", ch[cc].name,
                            rank, ch[co].name);
                quiet_say(cn, "Set %s's rank to %d.", ch[cc].name, rank);
              } else
                quiet_say(cn,
                          "You cannot change the rank of those not belonging "
                          "to your clan.");
            } else {
              int uID;
              ...
              if (uID == -1) {
                ...
                task_set_clan_rank(uID, ch[co].ID, get_char_clan(co), rank,
                                   ch[co].name);
          ...
          if (!get_char_clan(co) || ch[co].clan_rank < 4) {
            ...
            if (cc) {
              ...
              if (uID == -1) {
                ...
                task_fire_from_clan(uID, ch[co].ID, get_char_clan(co),
                                    ch[co].name);
      ...
      if ((in = ch[cn].citem)) {  // we still have it
        if (it[in].ID == IID_CLANJEWEL &&
          ...
          if (fnd->state == 1) {
            ...
            if (!res) {
              quiet_say(cn,
                        "So be it. There will be a new clan, named '%s', and "
                        "you, %s, shall be its new master. Good luck, young "
                        "master!",
                        fnd->name, ch[co].name);
              fnd->state = 0;
            ...
            quiet_say(
                cn, "You must name your clan first. Say: 'name: <clan-name>'.");
          }
        }
        // try to give it back
        dat->give_try++;
...
void clanclerk_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct clanclerk_driver_data *)set_data(
      cn, DRD_CLANCLERKDRIVER, sizeof(struct clanclerk_driver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      switch (res) {
        case 2:
          say(cn, "Our clan has %d jewels.", cnt_jewels(dat->clan));
          break;
      }

      if ((msg->dat1 == 1 || msg->dat1 == 2) && (co = msg->dat3) != cn &&
          char_see_char(cn, co)) {  // talk, and not our talk
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "withdraw"))) {
          ...
          if (get_clan_money(dat->clan) < nr) {
            say(cn, "Your clan does not have that much money, %s.",
                ch[co].name);
            remove_message(cn, msg);
            continue;
          }
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "buy"))) {
          ...
          if ((tmp1 = get_clan_dungeon_cost(nr, level)) >
              (tmp2 = get_clan_money(dat->clan))) {
            say(cn,
                "This order amounts to %dg while your treasury only holds %dg.",
                tmp1, tmp2);
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "use"))) {
          ...
          if (!res) {
            ...
            if (res == -1) {
              say(cn,
                  "You can only use 0 to 10 guards of each kind, 0 to 25 "
                  "teleport traps, 0 to 1 fake walls and 0 to 2 locked doors.");
            } else {
              say(cn,
                  "That configuration would cost %d points, but you may only "
                  "spend 400 points.",
                  res);
            }
          }
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "relation"))) {
          ...
          if (level > CS_NEUTRAL && !get_clan_raid(dat->clan)) {
            say(cn,
                "Your clan cannot go to war or feud unless you turn °c4raiding "
                "on°c0.");
          ...
          if (level > CS_NEUTRAL && !get_clan_raid(nr)) {
            say(cn,
                "Your clan cannot go to war or feud unless your opponent has "
                "raiding on.");
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "raiding god on")) &&
            (ch[co].flags & CF_GOD)) {
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "raiding god off")) &&
            (ch[co].flags & CF_GOD)) {
...
void clanspawn_driver(int in, int cn) {
  ...
  if (!cn) {  // timer calls
    ...
    if (!*(unsigned int *)(it[in].drdata + 4)) {
      *(unsigned int *)(it[in].drdata + 4) =
          ((realtime + RANDOM(60 * 60 * freq / 2) + 60 * 60 * freq / 4) /
           1800) *
          1800;
    ...
    if (realtime >= *(unsigned int *)(it[in].drdata + 4)) {
      ...
      *(unsigned int *)(it[in].drdata + 4) =
          ((realtime + RANDOM(60 * 60 * freq) + 60 * 60 * freq / 2) / 1800) *
          1800;
    }
    return;
  }
  ...
  if (it[in].drdata[0] < ch[cn].level) {
    log_char(
        cn, LOG_SYSTEM, 0,
        "Thou mayest not use this clan spawner for thy level is too great.");
    return;
  }

  if ((map[it[in].x + it[in].y * MAXMAP + 1].ch &&
       map[it[in].x + it[in].y * MAXMAP + 1].ch != cn) ||
      (map[it[in].x + it[in].y * MAXMAP - 1].ch &&
       map[it[in].x + it[in].y * MAXMAP - 1].ch != cn) ||
      (map[it[in].x + it[in].y * MAXMAP + MAXMAP].ch &&
       map[it[in].x + it[in].y * MAXMAP + MAXMAP].ch != cn) ||
      (map[it[in].x + it[in].y * MAXMAP - MAXMAP].ch &&
       map[it[in].x + it[in].y * MAXMAP - MAXMAP].ch != cn) ||
      (map[it[in].x + it[in].y * MAXMAP + 1 + MAXMAP].ch &&
       map[it[in].x + it[in].y * MAXMAP + 1 + MAXMAP].ch != cn) ||
      (map[it[in].x + it[in].y * MAXMAP - 1 + MAXMAP].ch &&
       map[it[in].x + it[in].y * MAXMAP - 1 + MAXMAP].ch != cn) ||
      (map[it[in].x + it[in].y * MAXMAP + 1 - MAXMAP].ch &&
       map[it[in].x + it[in].y * MAXMAP + 1 - MAXMAP].ch != cn) ||
    ...
    log_char(
        cn, LOG_SYSTEM, 0,
        "Thou mayest not use this clan spawner while others can touch it.");
    return;
  }
  ...
  if (!it[in].drdata[2]) {
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "%02d:%02d to go, about one jewel every %d hours.", diff / 60,
             diff % 60, freq);
    return;
  }
  ...
  if (cname && cnr) {
    sprintf(buf, "0000000000°c15Clan: %s won a Jewel for %s from level %d!",
            ch[cn].name, cname, it[in].max_level);
    server_chat(5, buf);

    sprintf(buf, "%02d:X:%02d:%10u:%s", cnr, it[in].max_level, ch[cn].ID,
            ch[cn].name);
    server_chat(1028, buf);
  } else {
    sprintf(buf,
            "0000000000°c15Clan: %s has won the spawn at level %d! Not being "
            "in a clan it didn't do %s much good, though.",
            ch[cn].name, it[in].max_level, himname(cn));
    server_chat(5, buf);
  }
...
void clanjewel_driver(int in, int cn) {
  ...
  if (!*(unsigned int *)(it[in].drdata + 0)) {
    *(unsigned int *)(it[in].drdata + 0) =
        realtime;  // remember current time if we havent already done so
  }
  if (realtime >
      *(unsigned int *)(it[in].drdata + 0) + 60 * 60) {  // time's up?
    ...
    call_item(IDR_CLANJEWEL, in, 0, ticker + TICKS * 30);  // please call again
}
