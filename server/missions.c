...
static const char *misstypetext[] = {
  "NULL",
  "Kill"
};
...
int build_fighter(int x, int y, int diff, int keyID, const char *keyname, const char *name, const char *temp, const char *desc, int fID, int sprite, int hasspecialitem, long long extra_flags)
{
  int cn, n, val, in;
  //char buf[80];
...
void mission_giver_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct mission_giver_data*)set_data(cn, DRD_MISSIONGIVEDRIVER, sizeof(struct mission_giver_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      ppd = (struct mission_ppd*)set_data(co, DRD_MISSION_PPD, sizeof(struct mission_ppd));
...
void missionchest_driver(int in, int cn)
{
  ...
  ppd = (struct mission_ppd*)set_data(cn, DRD_MISSION_PPD, sizeof(struct mission_ppd));
  if (!ppd) return;
