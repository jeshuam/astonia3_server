...
#define LADDER_REUSE_TIME \
  (60 * 60 * 24)  // in seconds - uses time(), not the ticker

struct saltmine_ppd {
  char version;      // increase version number to reset ppd
  char useitemflag;  // will be deleted by first monk that is willing to take
                     // the ladder (temporary, but i placed it in the ppd)
  ...
  int salt;  // number of succesful salt deliveries not received a reward for
             // yet
};
...
struct saltmine_ppd *get_saltmine_ppd(int cn) {
  ...
  dat = (struct saltmine_ppd *)set_data(cn, DRD_SALTMINE_PPD,
                                        sizeof(struct saltmine_ppd));
  if (!dat) return NULL;
...
void create_worker(int follow_cn, struct saltmine_ppd *ppd) {
  ...
  snprintf(ch[cn].name, sizeof(ch[cn].name) - 1, "%s's Monk",
           ch[follow_cn].name);
  ...
  // set initital values
  if ((workerdat = (struct saltmine_worker_data *)set_data(
           cn, DRD_SALTMINE_WORKER, sizeof(struct saltmine_worker_data)))) {
...
int monksout(int cn, struct saltmine_ppd *ppd) {
  ...
  if ((co = getfirst_char())) do {
      ...
      if (!(workerdat = (struct saltmine_worker_data *)set_data(
                co, DRD_SALTMINE_WORKER, sizeof(struct saltmine_worker_data))))
        continue;
...
void monk_gatama_driver(int cn, int ret, int lastact) {
  static struct talk_data talkbuf;  // we only have one, so there is no need to
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      // remove cv
      if (talkdat->cv_co) {
        if (!ch[talkdat->cv_co].flags ||
            ch[talkdat->cv_co].serial != talkdat->cv_serial ||
            char_dist(cn, talkdat->cv_co) > 7 ||
            !char_see_char(cn, talkdat->cv_co)) {
          talkdat->cv_co = 0;
        }
      }
      ...
      switch (ppd->gatamastate) {
        ...
        case 10:
          say(cn,
              "Welcome %s. The Monastery of Kir Laka needs thine help. We live "
              "from the salt we get from the saltmine thou can find to the "
              "west. But now many golems appeared in the mine and we had to "
              "run for our lives, and all the salt we had already mined is "
              "lost there.",
              ch[co].name);
          didsay = 1;
          ppd->gatamastate++;
        ...
        case 11:
          say(cn,
              "Thou might want to know more °c4details°c0 of what thee can do "
              "for the monastery. After this, thou might °c4begin°c0 helping "
              "us, or not.");
          didsay = 1;
          ppd->gatamastate++;
          break;
        ...
        case 20:
          say(cn,
              "Thou will have to lead group of monks safely to certain ladders "
              "in the saltmine. Once reached, thou \"use\" the ladder, and one "
              "of the monks will get the salt out of that place. Every ladder "
              "can be used only once every 12 astonian days, and every monk "
              "can only carry one bag of salt.");
          didsay = 1;
          ppd->gatamastate++;
          break;
        case 21:
          say(cn,
              "The monks will follow thee, and help thee against the golems. "
              "Once left, thou canst return to the monastery any time you "
              "like, the monks will then deposit their salt, if they have any, "
              "and will rest then. Thou can give some commands to the monks by "
              "simply speaking to them. Those commands are:");
        ...
        case 31:
          say(cn,
              "Come here, Monks. The mighty %s offers %s help. I want to see "
              "and hear quick feet now!",
        ...
        case 32:
          say(cn,
              "Lead them wise and carfully, %s. Mayest thou all return safe.",
              ch[co].name);
          didsay = 1;
          ppd->gatamastate++;
        case 33:
        ...
        case 40:
          say(cn,
              "Let us wait until all thine Monks are resting. Either in the "
              "rest room, or in peace.");
          didsay = 1;
          ppd->gatamastate++;
        case 41:
          talkdat->cv_co = 0;
        ...
        case 50:
          say(cn,
              "Thanks thee %s for thine help. Thou canst use the saltbag in "
              "the store room, to take thee thine reward. I trust thee, that "
              "thou wilt take the right amount.",
              ch[co].name);
          ppd->gatamastate++;
        case 51:
          talkdat->cv_co = 0;
  ...
  if (talkdat->lasttalk + TICKS * 30 < ticker || noturn) {
    if (secure_move_driver(cn, ch[cn].tmpx, ch[cn].tmpy, DX_LEFT, ret, lastact))
      return;
    if (!noturn && (ticker / TICKS) % 5 == 0)
      whisper(cn, "Auaaauuuuuuummmmmmmmmmm");
  }
...
void monk_worker_driver(int cn, int ret, int lastact) {
  ...
  // kill monk
  if ((!ch[workerdat->follow_cn].flags) ||
      (ch[workerdat->follow_cn].serial != workerdat->follow_serial)) {
    remove_destroy_char(cn);
    return;
  }

  if (char_dist(cn, workerdat->follow_cn) > 90) {
    log_char(workerdat->follow_cn, LOG_SYSTEM, 0,
             "°c3One of thine monks vanished!°c0");
    remove_destroy_char(cn);
    return;
  }
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (co == workerdat->follow_cn) {
        if (strcasestr(str, "COME")) {
          ...
          log_area(ch[cn].x, ch[cn].y, LOG_SYSTEM, 0, 10, "%s %s his head.",
                   ch[cn].name, workerdat->hassalt ? "nods" : "shakes");
        }
      }
    }
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (co == workerdat->follow_cn && !(ch[cn].flags & CF_INVISIBLE)) {
        if (ch[co].x < 216) {
          ...
          workerdat->useitem = map[door_itemx + door_itemy * MAXMAP].it;
      }
    }

    if (msg->type == NT_NPC && msg->dat1 == NTID_SALTMINE_USEITEM) {
      if (msg->dat2 == workerdat->follow_cn && !workerdat->hassalt &&
          !workerdat->useitem && ppd->useitemflag) {
  ...
  if (workerdat->leftmonastery &&
      workerdat->leftmonastery + 10 * TICKS < ticker) {
    if (in_monastery(ch[cn].x, ch[cn].y)) {
      if (workerdat->hassalt && !workerdat->useitem)
        workerdat->useitem = map[saltbag_itemx + saltbag_itemy * MAXMAP].it;
      if (!workerdat->hassalt && !workerdat->useitem)
        workerdat->useitem = map[door_itemx + door_itemy * MAXMAP].it;
    }
  } else {
    if (!workerdat->leftmonastery && !in_monastery(ch[cn].x, ch[cn].y))
      workerdat->leftmonastery = ticker;
  }

  if (workerdat->hassalt && ch[cn].speed_mode != SM_STEALTH)
    ch[cn].speed_mode = SM_STEALTH;
  if (!workerdat->hassalt && ch[cn].speed_mode != SM_NORMAL)
    ch[cn].speed_mode = SM_NORMAL;
  if (workerdat->hassalt && ch[cn].hp < ch[cn].value[1][V_HP] * POWERSCALE &&
      ch[cn].speed_mode != SM_NORMAL)
    ch[cn].speed_mode = SM_NORMAL;
  ...
  if ((ch[cn].flags & CF_INVISIBLE)) {
    ...
    } else if (move_driver(cn, ch[cn].tmpx, ch[cn].tmpy, 0))
      return;
  ...
  // turn to owner
  dir = offset2dx(ch[cn].x, ch[cn].y, ch[workerdat->follow_cn].x,
                  ch[workerdat->follow_cn].y);
  if (ch[cn].dir != dir) turn(cn, dir);

  do_idle(cn, TICKS / 8);  // they need to be fast, otherwise they act a bit
                           // stupid (easy solution. swap driver cause some
                           // deadlocks)
}
...
void saltmine_item(int in, int cn) {
  ...
  if (!cn) {
    ...
    if (drdata[0] == 1) {
      ...
      if (laddernumber < MAXLADDER)
        drdata[1] = laddernumber++;
      else
        xlog("Too many ladders in %s %d!!!", __FILE__, __LINE__);
    }

    // saltbag
    if (drdata[0] == 2) {
      if (saltbag_itemx != it[in].x || saltbag_itemy != it[in].y) {
        xlog("wrong coordinates of saltbag item in %s %d!!!", __FILE__,
             __LINE__);
      }
    ...
    // door
    if (drdata[0] == 3) {
      if (door_itemx != it[in].x || door_itemy != it[in].y) {
        xlog("wrong coordinates of door item in %s %d!!!", __FILE__, __LINE__);
      }
      door_itemx = it[in].x;
      door_itemy = it[in].y;
    }
    ...
    if (drdata[0] == 1) {
      ...
      if (ch[cn].flags & CF_PLAYER) {
        ...
        if (ppd->laddertime[it[in].drdata[1]] + LADDER_REUSE_TIME >
            time(NULL)) {
          log_char(cn, LOG_SYSTEM, 0,
                   "Thou already got all the Salt out of this, so thou have to "
                   "wait until it is refilled again.");
          return;
        }
      ...
      if (ch[cn].driver == CDR_MONK_WORKER) {
        // get data
        if (!(workerdat = (struct saltmine_worker_data *)set_data(
                  cn, DRD_SALTMINE_WORKER,
                  sizeof(struct saltmine_worker_data))))
          return;
        if (!(ppd = get_saltmine_ppd(workerdat->follow_cn))) return;
        ...
        if (ppd->laddertime[it[in].drdata[1]] + LADDER_REUSE_TIME >
            time(NULL)) {
          log_area(ch[cn].x, ch[cn].y, LOG_SYSTEM, 0, 10, "%s shakes his head.",
                   ch[cn].name);
          workerdat->useitem = 0;
          return;
        }
    ...
    if (drdata[0] == 2) {
      if (ch[cn].driver == CDR_MONK_WORKER) {
        ...
        if (workerdat->hassalt) {
          ...
          workerdat->hassalt = 0;
          workerdat->useitem = 0;
          log_area(ch[cn].x, ch[cn].y, LOG_SYSTEM, 0, 10,
                   "%s drops his bag of salt.", ch[cn].name);
        }
      ...
      if (ch[cn].flags & CF_PLAYER) {
        ...
        // if (!ppd->trusttime) { log_char(cn,LOG_SYSTEM,0,"Nothing happens.");
        // return; }
        if (!ppd->salt) {
          log_char(cn, LOG_SYSTEM, 0,
                   "Thou feelst thou should bring salt to the monastery, "
                   "before rewarding thinself.");
          return;
        }
        ...
        *(unsigned int *)(it[in2].drdata) = ppd->salt * 1000;
        ...
        log_char(cn, LOG_SYSTEM, 0,
                 "Thou took %d units of salt, feeling thou have earned it.",
                 *(unsigned int *)(it[in2].drdata));
      }
    }
    ...
    if (drdata[0] == 3) {
      if (ch[cn].driver == CDR_MONK_WORKER)
        ...
        log_char(cn, LOG_SYSTEM, 0, "Thou canst not enter there.");
    }
  }
}
