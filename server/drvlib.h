...
#define CDR_TWOROBBER \
  68  // two cities: robber (simple baddy with special death)
...
#define IDR_TELEPORT \
  10  // teleport to drdata[0,1],drdata[2,3] area drdata[4,5], area=0 means no
      // change
...
#define IDR_RANDCHEST \
  34  // random money in chest-like items (respawn time: 1 hour)
...
#define IDR_LAB3_SPECIAL \
  192  // lab3: some simple special items (like the entry door, etc)
...
// >=1000 && <2000 are used for spells which need to get a timer restarted when
// character is loaded from database
...
#define IDR_DONTSAVE(a)                                                  \
  ((a) == IDR_CURSE || (a) == IDR_MELTINGKEY || (a) == IDR_BACKTOFIRE || \
   (a) == IDR_PALACEBOMB || (a) == IDR_PALACECAP)
#define IDR_ONECARRY(a) \
  ((a) == IDR_CLANJEWEL || (a) == IDR_PALACEBOMB || (a) == IDR_PALACECAP)
