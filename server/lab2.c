...
void lab2_herald_driver(int cn, int ret, int lastact)
{
  ...
  // get data
  dat = (struct lab2_herald_driver_data*)set_data(cn, DRD_LAB2_HERALD, sizeof(*dat));
  if (!dat) return; // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if ((ch[co].flags & CF_PLAYER) && (ppd = (struct lab_ppd*)set_data(co, DRD_LAB_PPD, sizeof(*ppd)))) {
...
void lab2_deamon_create(int x, int y, int co)
{
  ...
  if ((cn = getfirst_char())) do {
      if (ch[cn].flags && ch[cn].driver == CDR_LAB2DEAMON) {
        if (!(dat = (struct lab2_deamon_driver_data*)set_data(cn, DRD_LAB2_DEAMON, sizeof(*dat)))) return;       // oops
  ...
  // get data
  dat = (struct lab2_deamon_driver_data*)set_data(cn, DRD_LAB2_DEAMON, sizeof(*dat));
  if (!dat) return; // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CREATE) {
      ...
      case  1:        dat->observing = 1;
        dat->talkstep = 10;
        if (dat->co) player_dat = (struct lab2_player_data*)set_data(dat->co, DRD_LAB2_PLAYER, sizeof(*player_dat)); else player_dat = NULL;
        if (!player_dat) break;
...
int get_player_grave_value(int in, int cn, struct lab2_grave_data *grave_dat)
{
  ...
  player_dat = (struct lab2_player_data*)set_data(cn, DRD_LAB2_PLAYER, sizeof_player_data);
...
void lab2_undead_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct lab2_undead_driver_data*)set_data(cn, DRD_LAB2_UNDEAD, sizeof(*dat));
  if (!dat) return; // oops...
...
void lab2_undead_died_driver(int cn, int co)
{
  ...
  undead_dat = (struct lab2_undead_driver_data*)set_data(cn, DRD_LAB2_UNDEAD, sizeof(*undead_dat));
  ...
  player_dat = (struct lab2_player_data*)set_data(co, DRD_LAB2_PLAYER, sizeof_player_data);
...
void lab2_grave(int in, int cn)
{
  ...
  if (cn) {
    ...
    if ((cdat = (struct lab2_undead_driver_data*)set_data(co, DRD_LAB2_UNDEAD, sizeof(*cdat)))) {
      cdat->graveit = in;
