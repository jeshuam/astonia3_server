  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if (ch[co].flags & CF_PLAYER) {
        ...
        // get lab ppd
        pd = (struct lab4_player_data*)set_data(co, DRD_LAB4_PLAYER, sizeof(struct lab4_player_data));
...
void lab4_gnalb_driver(int cn, int ret, int lastact)
{
  ...
  // get data
  dat = (struct lab4_gnalb_driver_data*)set_data(cn, DRD_LAB4_GNALB, sizeof(struct lab4_gnalb_driver_data));
  if (!dat) return; // oops...
