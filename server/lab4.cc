...
void set_seyan_state(struct lab4_player_data *pd) {
  if ((pd->seyan4got & (1 << 0)) && (pd->seyan4got & (1 << 1)))
    pd->seyan4state = 30;
  else if ((pd->seyan4got & (1 << 0)))
    pd->seyan4state = 10;
  else if ((pd->seyan4got & (1 << 1)))
    pd->seyan4state = 20;
  else
    pd->seyan4state = 0;
}
...
void lab4_seyan_driver(int cn, int ret, int lastact) {
  ...
  static struct lab4_seyan_data datbuf;  // we only have one, so there is no
                                         // need to use the memory system (?)
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if (ch[co].flags & CF_PLAYER) {
        // get lab ppd
        pd = (struct lab4_player_data *)set_data(
            co, DRD_LAB4_PLAYER, sizeof(struct lab4_player_data));
        if (!pd) {
          remove_message(cn, msg);
          continue;
        }
    ...
    if (msg->type == NT_CHAR) {
      ...
      // get lab ppd
      pd = (struct lab4_player_data *)set_data(co, DRD_LAB4_PLAYER,
                                               sizeof(struct lab4_player_data));
      if (!pd) {
        remove_message(cn, msg);
        continue;
      }
      ...
      switch (pd->seyan4state) {
        // INTRO
        case 0:
          say(cn, "Hello %s. This is thy first mission in the Labyrinth.",
              ch[co].name);
          didsay = 1;
          pd->seyan4state++;
          break;
        case 1:
          say(cn,
              "Listen, %s. To the east is an entrance to the Gnalbs winter "
              "residence.",
              ch[co].name);
          didsay = 1;
          pd->seyan4state++;
        ...
        case 2:
          say(cn,
              "The Gnalbs are peaceful creatures, but their King and their "
              "Mage and the Guards are not.");
          didsay = 1;
          pd->seyan4state++;
          break;
        case 3:
          say(cn,
              "Bring me the King's Crown, and the Mage's Szepter to prove thou "
              "art worthy to enter the next Gate.");
          didsay = 1;
          pd->seyan4state++;
          break;
        case 4:
        ...
        case 10:
          say(cn,
              "Thou broughtst me the Kings Crown. Now, %s, seek for the Mage's "
              "Szepter.",
              ch[co].name);
          didsay = 1;
          pd->seyan4state++;
        ...
        case 20:
          say(cn,
              "Thou broughtst me the Mages Szepter. Now, %s, seek for the "
              "King's Crown.",
              ch[co].name);
          didsay = 1;
          pd->seyan4state++;
        ...
        case 30:
          say(cn,
              "%s, thou broughtst me the King's Crown and the Mage's Szepter.",
              ch[co].name);
          didsay = 1;
          pd->seyan4state++;
          break;
...
void lab4_gnalb_driver_init(int cn, struct lab4_gnalb_driver_data *dat) {
  ...
  if (dat->type == 1) {
    ...
    for (p = 1; p < max_gnalb_path; p++) {
      if ((dist = map_dist(ch[cn].x, ch[cn].y, gnalb_path[p].x,
                           gnalb_path[p].y)) < mindist) {
        mindist = dist;
        dat->path = p;
      }
    }
...
void lab4_gnalb_driver(int cn, int ret, int lastact) {
  ...
  // get data
  dat = (struct lab4_gnalb_driver_data *)set_data(
      cn, DRD_LAB4_GNALB, sizeof(struct lab4_gnalb_driver_data));
  if (!dat) return;  // oops...
  ...
  if (dat->type == 1 && dat->path) {
    ...
    if (swap_move_driver(cn, gnalb_path[dat->path].x, gnalb_path[dat->path].y,
                         1))
      return;
    if (map_dist(ch[cn].x, ch[cn].y, gnalb_path[dat->path].x,
                 gnalb_path[dat->path].y) < 4) {
      int p;

      do
        p = RANDOM(4);
      while (gnalb_path[dat->path].next[p] == 0 ||
             (gnalb_path[dat->path].next[1] != 0 &&
              gnalb_path[dat->path].next[p] == dat->lastpath));
