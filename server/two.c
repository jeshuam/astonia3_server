...
void guard_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct twoguard_data*)set_data(cn, DRD_TWOGUARDDRIVER, sizeof(struct twoguard_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (!dat->busy &&
          ...
          (ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd))) &&
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        if (!ppd) break;
        ppd->guard_intro = 0;
        break;
      case 3:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        if (!ppd) break;
        if (ppd->legal_status == LS_FINE) {
          if (ch[co].gold >= ppd->legal_fine) {
            ...
            bank = (struct bank_ppd*)set_data(co, DRD_BANK_PPD, sizeof(struct bank_ppd));
            if (bank) {
              int need;
        ...
        break;
      case 12:	ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        if (ppd && (ch[co].flags & CF_GOD)) {
          ppd->citizen_status = CS_ENEMY;
        }
        break;
      case 5:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        if (ppd && (ch[co].flags & CF_GOD)) {
        ...
        break;
      case 6:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        if (ppd && (ch[co].flags & CF_GOD)) {
          ppd->citizen_status = CS_CITIZEN;
        }
        break;
      case 7:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        if (ppd && (ch[co].flags & CF_GOD)) {
    ...
    if (msg->type == NT_GOTHIT) {
      ...
      if ((ch[co].flags & CF_PLAYER) && (ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd)))) {
    ...
    if (msg->type == NT_SEEHIT) {
      cc = msg->dat1;
      co = msg->dat2;
      if (co != cn && ch[co].group == ch[cn].group && (ch[cc].flags & CF_PLAYER) && (ppd = (struct twocity_ppd*)set_data(cc, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd))) && char_see_char(cn, cc)) {
        if (ppd->legal_status != LS_DEAD && realtime - ppd->last_attack > 10) {
          ppd->legal_status = LS_FINE;
          ppd->legal_fine += 7500;
          if (ppd->citizen_status == CS_GUEST) {
    ...
    if (msg->type == NT_NPC) {
      ...
      if (msg->dat1 == NTID_TWOCITY_PICK) {
        co = msg->dat2;
        if ((ch[co].flags & CF_PLAYER) && char_see_char(cn, co) && (ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd))) && realtime - ppd->last_attack > 60) {
...
void barkeeper(int cn, int ret, int lastact)
{
  ...
  dat = (struct barkeeper_data*)set_data(cn, DRD_BARKEEPERDRIVER, sizeof(struct barkeeper_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        ...
        break;
      case 13:	ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
...
void servant(int cn, int ret, int lastact)
{
  ...
  dat = (struct servant_data*)set_data(cn, DRD_SERVANTDRIVER, sizeof(struct servant_data));
  if (!dat) return;	// oops...
...
void burndown(int in, int cn)
{
  ...
  ppd = (struct twocity_ppd*)set_data(cn, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
...
void thiefguard(int cn, int ret, int lastact)
{
  ...
  dat = (struct thiefguard_data*)set_data(cn, DRD_THIEFGUARDDRIVER, sizeof(struct thiefguard_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        ...
        break;
      case 15:	ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
...
void thiefmaster(int cn, int ret, int lastact)
{
  ...
  dat = (struct thiefguard_data*)set_data(cn, DRD_THIEFMASTERDRIVER, sizeof(struct thiefguard_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
        ...
        break;
      case 16:	ppd = (struct twocity_ppd*)set_data(co, DRD_TWOCITY_PPD, sizeof(struct twocity_ppd));
...
void sanwyn(int cn, int ret, int lastact)
{
  ...
  dat = (struct sanwyn_driver_data*)set_data(cn, DRD_SANWYNDRIVER, sizeof(struct sanwyn_driver_data));
  if (!dat) return;	// oops...
...
void bookcase(int in, int cn)
{
  ...
  struct twocity_ppd *ppd;
  static const char *standard = {"After reading the title you put the book back."};
  const char *name = "Lady Manners' Guide to Decent Behaviour", *text = standard;
  char buf[120];
...
void skelly(int cn, int ret, int lastact)
{
  ...
  dat = (struct skelly_driver_data*)set_data(cn, DRD_SKELLYDRIVER, sizeof(struct skelly_driver_data));
  if (!dat) return;	// oops...
...
void alchemist(int cn, int ret, int lastact)
{
  ...
  dat = (struct alchemist_driver_data*)set_data(cn, DRD_ALCHEMISTDRIVER, sizeof(struct alchemist_driver_data));
  if (!dat) return;	// oops...
