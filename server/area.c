...
int register_kill_in_section(int cn, int co)
{
  ...
  if (!(dat = (struct section_data*)set_data(cn, DRD_SECTION_DATA, sizeof(struct section_data)))) return 0;	// OOPS
...
void walk_section_msg(int cn)
{
  ...
  if (!(dat = (struct section_data*)set_data(cn, DRD_SECTION_DATA, sizeof(struct section_data)))) return;	// OOPS
  ...
  if (s != dat->current_section) {
    ...
    if (s) {
      ...
      log_char(cn, LOG_SYSTEM, 0, "Now leaving %s.", section[dat->current_section].name);
    }
