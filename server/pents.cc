...
static void solve_pents(int cc) {
  ...
  for (cn = getfirst_char(); cn; cn = getnext_char(cn)) {
    ...
    if (areaID == 25 && ch[cn].x > 107)
      continue;  // no messages for players in RWW

    nppd = (struct pent_nppd *)set_data(cn, DRD_PENT_NPPD,
                                        sizeof(struct pent_nppd));
    if (!nppd) continue;
    ...
    log_char(
        cn, LOG_SYSTEM, 0,
        "%s solved the pentagram quest (tm). You got %d experience points!",
        ch[cc].name, exp);
    if (tpower >= 0)
      log_char(cn, LOG_SYSTEM, 0, "Training area power setting now at %.2f%%.",
               100.0 / 32000 * tpower);
    else
      log_char(
          cn, LOG_SYSTEM, 0,
          "Training area power setting down to 0.00%%, %.2f%% underpowered.",
          -100.0 / 32000 * tpower);

    log_char(cn, LOG_SYSTEM, 0, "#30  - Solved -");
    if (get_char_clan(cc)) {
      log_char(cn, LOG_SYSTEM, 0, "This solve goes to the %s clan!",
               get_char_clan_name(cc));
        ...
        if (cnt > 0) {
          ...
          if (exp) {
            log_char(cn, LOG_SYSTEM, 0,
                     "Your clan's jewels reflected %d exp of the solve to you.",
                     exp);
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "The current record is %d pentagrammas in one run, held by %s. "
             "You have %d pentagrammas so far.",
             pent_record, pent_record_name, nppd->pent_cnt);
  }
  lastsolve = ticker;
}
...
static void add_pent(int cn, int in, int didsolve) {
  ...
  struct pent_nppd *nppd;
  static const char *colortext[4] = {"none", "red", "green", "blue"};
  ...
  nppd =
      (struct pent_nppd *)set_data(cn, DRD_PENT_NPPD, sizeof(struct pent_nppd));
  if (!nppd) return;
  ...
  log_char(
      cn, LOG_SYSTEM, 0,
      "You got a %s Pentagram, value %d. %d of %d Pentagrammas are active.",
  ...
  if (n == 5) {
    if (nppd->status == 0) {
      ...
      if (n < 4) {
        memmove(nppd->pent_it + n + 1, nppd->pent_it + n,
                (4 - n) * sizeof(int));
        memmove(nppd->pent_color + n + 1, nppd->pent_color + n,
                (4 - n) * sizeof(int));
        memmove(nppd->pent_value + n + 1, nppd->pent_value + n,
                (4 - n) * sizeof(int));
        memmove(nppd->pent_worth + n + 1, nppd->pent_worth + n,
                (4 - n) * sizeof(int));
      }
      if (n < 5) {
        nppd->pent_it[n] = in;
      ...
      if (same == 5) {
        nppd->status = 1;
        log_char(cn, LOG_SYSTEM, 0,
                 "You got five Pentagrammas of the same color!");
      }
    } else {
  ...
  if (nppd->pent_value[5]) {
    log_char(cn, LOG_SYSTEM, 0,
             "#3%dCombo: Pentagram value of %d, color of %s, %d exp.",
             nppd->pent_color[5], nppd->pent_value[5],
             colortext[nppd->pent_color[5]], nppd->pent_worth[5]);
    worth += nppd->pent_worth[5];
  } else
    log_char(cn, LOG_SYSTEM, 0, "#3");

  for (n = 0; n < 5; n++) {
    if (!nppd->pent_value[n]) break;

    log_char(cn, LOG_SYSTEM, 0,
             "#%d%dPentagram value of %d, color of %s, %d exp.", n + 4,
             nppd->pent_color[n], nppd->pent_value[n],
             colortext[nppd->pent_color[n]], nppd->pent_worth[n]);
    worth += nppd->pent_worth[n];
  }
  log_char(cn, LOG_SYSTEM, 0, "#90Bonus: %d, total: %d", nppd->bonus,
           worth + nppd->bonus);
  ...
  if (nppd->pent_cnt > pent_record) {
    if (pent_record_ID != ch[cn].ID) {
      log_char(cn, LOG_SYSTEM, 0,
               "You broke %s's record. New record is now %d pents activated in "
               "one run.",
               pent_record_name, nppd->pent_cnt);
    } else {
      if (nppd->pent_cnt % 25 == 0) {
        log_char(cn, LOG_SYSTEM, 0,
                 "You increased your own record to %d pents in one run.",
                 nppd->pent_cnt);
      }
    }
...
static void set_pent_solve_cnt(void) {
  ...
  if (areaID == 25) {
    ...
    for (cnt = 0, cn = getfirst_char(); cn; cn = getnext_char(cn)) {
      if ((ch[cn].flags & CF_PLAYER) && ch[cn].x >= 129 && ch[cn].x <= 238 &&
          ch[cn].y < 188)
        cnt++;
    }
  } else
...
void pent_driver(int in, int cn) {
  ...
  if (cn) {         // player activated pent
    ...
    if (status &&
        (status != solve_ser ||
         areastatus !=
             areaser[level])) {  // pent is active, but quest has been solved
      status = *(unsigned char *)(it[in].drdata + 1) =
          *(unsigned char *)(it[in].drdata + 4) = 0;  // mark pent non-active
      it[in].sprite -= color;
      remove_item_light(in);
      ...
      if (!RANDOM(15))
        cnt = 1;  // spawn one on normal timer call
      else
        return;
    } else
      return;
...
void pentboss_door_driver(int in, int cn) {
  ...
  if (ticker - lastsolve > TICKS * 30 && (dx > 0 || dy > 0)) {
    log_char(cn, LOG_SYSTEM, 0,
             "The door won't open. It seems it is only accessible directly "
             "after a solve.");
    return;
  }
        ...
        if (demoncnt > prob && !ch[cn].item[30]) {
          ...
          if (p < 100) {
            ...
            if (ch[cn].level <= 38) {
              switch (n) {
                case 0:
                  in = create_item("demon_sleeves1");
                  break;
                case 1:
                  in = create_item("demon_leggings1");
                  break;
                case 2:
                  in = create_item("demon_helmet1");
                  break;
                case 3:
                  in = create_item("demon_armor1");
                  break;
                case 4:
                  in = create_item("demon_bracelet1");
                  break;
                case 5:
                  in = create_item("demon_hat1");
                  break;
                case 6:
                  in = create_item("demon_trousers1");
                  break;
                case 7:
                  in = create_item("demon_vest1");
                  break;
                case 8:
                  in = create_item("demon_cape1");
                  break;
                case 9:
                  in = create_item("demon_cape1");
                  break;
                case 10:
                  in = create_item("demon_boots1");
                  break;
                case 11:
                  in = create_item("demon_boots1");
                  break;
              ...
              switch (n) {
                case 0:
                  in = create_item("demon_sleeves2");
                  break;
                case 1:
                  in = create_item("demon_leggings2");
                  break;
                case 2:
                  in = create_item("demon_helmet2");
                  break;
                case 3:
                  in = create_item("demon_armor2");
                  break;
                case 4:
                  in = create_item("demon_bracelet2");
                  break;
                case 5:
                  in = create_item("demon_hat2");
                  break;
                case 6:
                  in = create_item("demon_trousers2");
                  break;
                case 7:
                  in = create_item("demon_vest2");
                  break;
                case 8:
                  in = create_item("demon_cape2");
                  break;
                case 9:
                  in = create_item("demon_cape2");
                  break;
                case 10:
                  in = create_item("demon_boots2");
                  break;
                case 11:
                  in = create_item("demon_boots2");
                  break;
              ...
              switch (n) {
                case 0:
                  in = create_item("demon_sleeves3");
                  break;
                case 1:
                  in = create_item("demon_leggings3");
                  break;
                case 2:
                  in = create_item("demon_helmet3");
                  break;
                case 3:
                  in = create_item("demon_armor3");
                  break;
                case 4:
                  in = create_item("demon_bracelet3");
                  break;
                case 5:
                  in = create_item("demon_hat3");
                  break;
                case 6:
                  in = create_item("demon_trousers3");
                  break;
                case 7:
                  in = create_item("demon_vest3");
                  break;
                case 8:
                  in = create_item("demon_cape3");
                  break;
                case 9:
                  in = create_item("demon_cape3");
                  break;
                case 10:
                  in = create_item("demon_boots3");
                  break;
                case 11:
                  in = create_item("demon_boots3");
                  break;
            ...
            in = create_item("bronzechip");
          else if (p < 990)
            in = create_item("silverchip");
          else
            in = create_item("goldchip");
          ...
          if (in) {
            ...
            demoncnt = 0;
            // xlog("created %s/%s
            // (%X)",it[in].name,it[in].description,it[in].ID);
          }
        }
        break;
    }
...
void penter_dead(int cn, int co) {
  ...
  if ((ch[cn].cls < 258 || ch[cn].cls > 305) &&
      (ch[cn].cls < 404 || ch[cn].cls > 411))
    return;
  ...
  if (co) {
    if (tpower >= 0)
      log_char(co, LOG_SYSTEM, 0,
               "Training area power setting down to %.2f%%, loss of %.2f%%.",
               100.0 / 32000 * tpower, 100.0 / 32000 * str);
    else
      log_char(co, LOG_SYSTEM, 0,
               "Training area power setting down to 0.00%%, %.2f%% "
               "underpowered, loss of %.2f%%.",
               -100.0 / 32000 * tpower, 100.0 / 32000 * str);
  }
}
...
void tester(int cn, int ret, int lastact) {
  ...
  dat = (struct tester_data *)set_data(cn, DRD_TESTERDRIVER,
                                       sizeof(struct tester_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (!dat->killchar && ch[co].driver == CDR_PENTER &&
          char_see_char(cn, co))
        dat->killchar = co;
    }
    ...
    if (msg->type == NT_ITEM) {
      in = msg->dat1;
      if (!dat->pickitem && it[in].driver == IDR_POTION &&
          char_see_item(cn, in))
        dat->pickitem = in;
      if (it[in].driver == IDR_PENT && !it[in].drdata[1] &&
          char_see_item(cn, in)) {
        if (!dat->useitem)
          dat->useitem = in;
        else {
