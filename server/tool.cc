...
int can_help(int cn, int co) {
  ...
  if (co < 1 || co >= MAXCHARS) {
    elog("can_help(): called with illegal character co=%d (cn=%s,%d)", co,
         ch[cn].name, cn);
    return 0;
  }
  ...
  if ((ch[co].flags & CF_PLAYER) && (ch[co].flags & CF_PK) &&
      !is_hate_empty(co))
    return 0;
  ...
  if ((mf1 & MF_ARENA) && (mf2 & MF_ARENA) &&
      pathfinder(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y, 0, arena_check_target,
                 0) != -1)
    return 1;  // both in same
  if ((mf1 & MF_ARENA) || (mf2 & MF_ARENA))
    return 0;  // one in arena or both in different arenas
...
int can_attack(int cn, int co) {
  ...
  if (co < 1 || co >= MAXCHARS) {
    elog("can_attack(): called with illegal character co=%d (cn=%s,%d)", co,
         ch[cn].name, cn);
    {
      char *ptr = NULL;
      *ptr = 0;
  ...
  if ((ch[cn].flags & (CF_PLAYER | CF_PLAYERLIKE)) &&
      (ch[co].flags & CF_NOPLRATT))
    return 0;
  if ((ch[co].flags & (CF_PLAYER | CF_PLAYERLIKE)) &&
      (ch[cn].flags & CF_NOPLRATT))
    return 0;
  ...
  if ((mf1 & MF_ARENA) && (mf2 & MF_ARENA) &&
    ...
    if ((mf1 & MF_CLAN) && (mf2 & MF_CLAN) && (c1 = get_char_clan(cn)) &&
        (c2 = get_char_clan(co)) && (c1 == c2 || clan_alliance(c1, c2)))
      return 0;
    return 1;
  }
  ...
  if ((c1 = get_char_clan(cn)) && (c2 = get_char_clan(co)) && areaID != 1) {
    if ((mf1 & MF_CLAN) && (mf2 & MF_CLAN) && clan_can_attack_inside(c1, c2))
      return 1;
    if (clan_can_attack_outside(c1, c2) &&
        abs(ch[cn].level - ch[co].level) <= 3)
      return 1;
  }

  // one of them is 'player-like', dont allow attacks between him and
  // players/player-likes
  if ((ch[cn].flags & (CF_PLAYER | CF_PLAYERLIKE)) &&
      (ch[co].flags & CF_PLAYERLIKE))
    return 0;
  if ((ch[co].flags & (CF_PLAYER | CF_PLAYERLIKE)) &&
      (ch[cn].flags & CF_PLAYERLIKE))
    return 0;
  ...
  if ((ch[cn].flags & CF_PLAYER) && (ch[co].flags & CF_PLAYER)) {
    ...
    if (!(ppd =
              (struct pk_ppd *)set_data(cn, DRD_PK_PPD, sizeof(struct pk_ppd))))
      return 0;  // OOPS
  ...
  // ... clans members may not attack each other if they're of the same clan or
  // of an alliance...
...
int check_hate(int cn, int co) {
  ...
  if (!(ch[cn].flags & CF_PLAYER) || !(ch[cn].flags & CF_PK) ||
      !(ch[co].flags & CF_PLAYER) || !(ch[co].flags & CF_PK))
    return 0;
...
void list_hate(int cn) {
  ...
  for (n = 0; n < MAXHATE; n++) {
    if (ppd->hate[n]) {
      ...
      if (res >= 0) {
        log_char(cn, LOG_SYSTEM, 0, "Hate: %s", name);
        flag = 1;
      }
    }
  }
...
int leave_pk(int cn) {
  ...
  if (!(ppd =
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "Could not attach data block, please contact game admin and tell "
             "them you found bug #1774");
    return 0;  // OOPS
  }
  ...
  if (ppd->last_kill + 60 * 60 * 24 * 28 > realtime) {
    log_char(
        cn, LOG_SYSTEM, 0,
        "You have killed %.2f days ago, you need to wait %.2f more days.",
        (realtime - ppd->last_kill) / (60 * 60 * 24.0),
        (ppd->last_kill + 60 * 60 * 24 * 28 - realtime) / (60 * 60 * 24.0));
...
int show_pk_info(int cn, int co, char *buf) {
  ...
  if (!(ppd = (struct pk_ppd *)set_data(co, DRD_PK_PPD, sizeof(struct pk_ppd))))
    return 0;  // OOPS

  return sprintf(buf,
                 "%s is a player killer. %s killed %d players and died %d "
                 "times through the hands of other players. ",
                 ch[co].name, Hename(co), ppd->kills, ppd->deaths);
}
...
int check_requirements(int cn, int in) {
  ...
  for (m = 0; m < MAXMOD; m++) {
    ...
    if (v1 <= -V_MAX || v1 >= V_MAX) {
      elog(
          "can_wear(): item %s (%d) has illegal modifier (index %d out of "
          "bounds). Removing modifier.",
          it[in].name, in, v1);
      it[in].mod_value[m] = 0;
      it[in].mod_index[m] = 0;
      continue;
...
int can_wear(int cn, int in, int pos) {
  ...
  if (inl < 0 || inl >= MAXITEM) {
    elog(
        "can_wear(): illegal item %d in WN_LHAND of character %s (%d) found. "
        "Removing.",
        in, ch[cn].name, cn);
  ...
  if (inr < 0 || inr >= MAXITEM) {
    elog(
        "can_wear(): illegal item %d in WN_RHAND of character %s (%d) found. "
        "Removing.",
        in, ch[cn].name, cn);
  ...
  switch (pos) {
    ...
    case WN_LHAND:
      if (inr && (it[inr].flags & IF_WNTWOHANDED))
        return 0;  // left hand is blocked by two-handed weapon
    ...
    case WN_RHAND:
      ...
      } else if (!(it[in].flags & IF_WNRHAND))
        return 0;  // no, but can it be worn on right hand?
      break;
  }
...
int can_carry(int cn, int in, int quiet) {
  ...
  if (IDR_ONECARRY(it[in].driver)) {
    ...
    if (n < INVENTORYSIZE ||
        ((in2 = ch[cn].citem) && it[in2].driver == it[in].driver)) {
      if (!quiet)
        log_char(cn, LOG_SYSTEM, 0, "You can only carry one %s at a time!",
                 it[in].name);
      return 0;
    }
  }
...
double get_spell_average(int cn) {
  return (ch[cn].value[0][V_BLESS] + ch[cn].value[0][V_HEAL] +
          ch[cn].value[0][V_FREEZE] + ch[cn].value[0][V_MAGICSHIELD] +
          ch[cn].value[0][V_FLASH] + ch[cn].value[0][V_FIREBALL] +
          ch[cn].value[0][V_PULSE]) /
         8.0;
}
...
int tactics2immune(int val) { return val * 0.125; }

int tactics2spell(int val) { return val * 0.125; }
...
int get_parry_skill(int cn) {
  ...
  if (ch[cn].value[1][V_PARRY])
    val = get_fight_skill(cn) + ch[cn].value[0][V_PARRY] * 2 +
          tactics2melee(ch[cn].value[0][V_TACTICS]) +
          (ch[cn].rage / 5 / POWERSCALE);
  else if (ch[cn].flags & CF_EDEMON)
    val = get_fight_skill(cn) * 3.5;
  else if (ch[cn].value[1][V_MAGICSHIELD])
    val = get_fight_skill(cn) + ch[cn].value[0][V_MAGICSHIELD] * 2;
  else
    val = get_fight_skill(cn) + get_spell_average(cn) * 2;
...
int get_attack_skill(int cn) {
  ...
  if (ch[cn].value[1][V_ATTACK])
    val = get_fight_skill(cn) + ch[cn].value[0][V_ATTACK] * 2 +
          tactics2melee(ch[cn].value[0][V_TACTICS]) +
          (ch[cn].rage / 5 / POWERSCALE);
  else if (ch[cn].flags & CF_EDEMON)
    val = get_fight_skill(cn) * 3.5;
    ...
    // say(cn,"attack_skill values str=%d old=%d new=%d
    // weight=%.2f",val,val1,val2,new_system);
  }
...
int get_surround_attack_skill(int cn) {
  ...
  if (ch[cn].value[1][V_SURROUND])
    val = get_fight_skill(cn) + ch[cn].value[0][V_SURROUND] * 2 +
          tactics2melee(ch[cn].value[0][V_TACTICS]) +
...
// X exp yield level Y
int exp2level(int val) { return max(1, (int)(sqrt(sqrt(val)))); }

// to reach level X you need Y exp
int level2exp(int level) { return pow(level, 4); }

// to reach level X you need Y exp
int level_value(int level) { return pow(level + 1, 4) - pow(level, 4); }
...
int check_levelup(int cn) {
  ...
  while (exp2level(experience) > ch[cn].level) {
    ...
    log_char(cn, LOG_SYSTEM, 0, "Thou gained a level! Thou art level %d now.",
             ch[cn].level);
    ...
    if (ch[cn].flags & CF_HARDCORE)
      ...
      log_char(cn, LOG_SYSTEM, 0,
               "Thy persistence has pleased Ishtar. He will save thee when "
               "thou art in need.");
      log_char(cn, LOG_SYSTEM, 0, "Thou hast %s saves now.",
               save_number(ch[cn].saves));
    }

    if (ch[cn].level >= 20 && !ch[cn].value[1][V_PROFESSION]) {
      ch[cn].value[1][V_PROFESSION] = 1;
      log_char(cn, LOG_SYSTEM, 0,
               "Thou mayest now choose to learn a profession.");
    }

    if (ch[cn].level % 10 == 0) {
      sprintf(buf, "0000000000°c10Grats: %s is level %d now!", ch[cn].name,
              ch[cn].level);
      server_chat(6, buf);
    }
...
void give_exp_bonus(int cn, int val) { give_exp(cn, val); }

void give_money(int cn, int val, const char *reason) {
  ch[cn].gold += val;
  ch[cn].flags |= CF_ITEMS;
  if (val % 100)
    log_char(cn, LOG_SYSTEM, 0, "You received %dG %dS.", val / 100, val % 100);
  else
    log_char(cn, LOG_SYSTEM, 0, "You received %dG.", val / 100);
...
void remove_spell(int cn, int in, int pos, int cserial, int iserial) {
  ...
  if (it[in].driver == IDR_FREEZE &&
      ch[cn].duration) {  // adjust duration and step to remove effect of freeze
...
int may_add_spell(int cn, int driver) {
  ...
  for (n = 12; n < 30; n++) {
    if ((in = ch[cn].item[n])) {
      if (it[in].driver ==
          driver) {  // another spell with the same driver is already active
        if (driver == IDR_BLESS &&
            *(unsigned *)(it[in].drdata) - ticker < TICKS * 30)
          return n;
        return 0;
      }
    } else
...
int add_same_spell(int cn, int driver) {
  ...
  for (n = 12; n < 30; n++) {
    if ((in = ch[cn].item[n])) {
      if (it[in].driver == driver)
        return n;  // such a spell is already active, return its number
    } else
      fre = n;
  }
  if (fre == 0) return 0;
...
int look_item(int cn, struct item *in) {
  ...
  if (in->ID == IID_HARDKILL)
    log_char(cn, LOG_SYSTEM, 0, "This is a level %d holy weapon.",
             in->drdata[37]);
  ...
  for (n = 0; n < MAXMOD; n++) {
    if ((v = in->mod_value[n]) && (s = in->mod_index[n]) > -1) {
      ...
      if (in->driver == IDR_DECAYITEM) {
        log_char(cn, LOG_SYSTEM, 0, "%s +%d (active: %+d)", skill[s].name, v,
                 in->drdata[2]);
      } else {
        if (ch[cn].flags & CF_GOD) {
          if (s == V_ARMOR)
            log_char(cn, LOG_SYSTEM, 0, "%s %+.2f °c1(%d: %d+%d)",
                     skill[s].name, v / 20.0, n, s, v);
          else
            log_char(cn, LOG_SYSTEM, 0, "%s %+d °c1(%d: %d+%d)", skill[s].name,
                     v, n, s, v);
        } else {
          if (s == V_ARMOR)
            log_char(cn, LOG_SYSTEM, 0, "%s %+.2f", skill[s].name, v / 20.0);
          else
            log_char(cn, LOG_SYSTEM, 0, "%s %+d", skill[s].name, v);
        }
      }
    }
  }
  ...
  for (n = 0; n < MAXMOD; n++) {
    if ((v = in->mod_value[n]) && (s = in->mod_index[n]) < 0) {
      ...
      log_char(cn, LOG_SYSTEM, 0, "%s %d (you have %d)", skill[-s].name, v,
               ch[cn].value[1][-s]);
    }
  }
  ...
  if (in->min_level)
    log_char(cn, LOG_SYSTEM, 0, "Minimum Level: %d", in->min_level);
  if (in->max_level)
    log_char(cn, LOG_SYSTEM, 0, "Maximum Level: %d", in->max_level);

  if (in->needs_class & 1)
    log_char(cn, LOG_SYSTEM, 0, "Only usable by a Warrior.");
  if (in->needs_class & 2)
    log_char(cn, LOG_SYSTEM, 0, "Only usable by a Mage.");
  if (in->needs_class & 4)
    log_char(cn, LOG_SYSTEM, 0, "Only usable by a Seyan'Du.");
  if (in->needs_class & 8)
    log_char(cn, LOG_SYSTEM, 0, "Only usable by an Arch.");

  if (in->flags & IF_BONDTAKE)
    log_char(cn, LOG_SYSTEM, 0,
             "This item is bonded to %s. Only %s can take it.",
             in->ownerID == ch[cn].ID ? "you" : "somebody else",
             in->ownerID == ch[cn].ID ? "you" : "he");
  if (in->flags & IF_BONDWEAR)
    log_char(cn, LOG_SYSTEM, 0,
             "This item is bonded to %s. Only %s can wear it.",
             in->ownerID == ch[cn].ID ? "you" : "somebody else",
             in->ownerID == ch[cn].ID ? "you" : "he");

  if (in->flags & IF_QUEST)
    log_char(cn, LOG_SYSTEM, 0,
             "This is a quest item. You cannot drop it or give it away.");
  if (in->flags & IF_NOENHANCE)
    log_char(cn, LOG_SYSTEM, 0,
             "This item resists magic, so you cannot enhance it using orbs, "
             "metals or shrines.");
  if (in->flags & IF_BEYONDMAXMOD)
    log_char(cn, LOG_SYSTEM, 0,
  ...
  if (in->driver == IDR_DECAYITEM) {
    log_char(cn, LOG_SYSTEM, 0,
             "Duration: %d:%02d:%02d of %d:%02d:%02d active time used up.",
             ((*(unsigned short *)(in->drdata + 3)) * 2) / 60 / 60,
             (((*(unsigned short *)(in->drdata + 3)) * 2) / 60) % 60,
             ((*(unsigned short *)(in->drdata + 3)) * 2) % 60,
             ((*(unsigned short *)(in->drdata + 5)) * 2) / 60 / 60,
             (((*(unsigned short *)(in->drdata + 5)) * 2) / 60) % 60,
             ((*(unsigned short *)(in->drdata + 5)) * 2) % 60);
  }
  /*if (IDR_ISSPELL(in->driver)) {
    log_char(cn,LOG_SYSTEM,0,"Duration: %.2f minutes.",(*(unsigned
  long*)(in->drdata)-ticker)/TICKS/60.0);
  }*/

  if ((in->sprite >= 59200 && in->sprite < 59299) ||
      in->sprite == 59474) {  // gold item
    log_char(cn, LOG_SYSTEM, 0, "The item has been gilded.");
  }
  if ((in->sprite >= 59299 && in->sprite <= 59390) ||
      in->sprite == 59473) {  // silver item
...
int look_char(int cn, int co) {
  ...
  if (ch[cn].flags & CF_PLAYER) {
    shrine = (struct shrine_ppd *)set_data(co, DRD_RANDOMSHRINE_PPD,
                                           sizeof(struct shrine_ppd));
    if (shrine && shrine->used[DEATH_SHRINE_INDEX] & DEATH_SHRINE_BIT) {
      log_char(cn, LOG_SYSTEM, 0, "#1%s%s the Brave (%d):", get_title(co),
               ch[co].name, ch[co].level);
    } else {
      log_char(cn, LOG_SYSTEM, 0, "#1%s%s (%d):", get_title(co), ch[co].name,
               ch[co].level);
    }
    ...
    if (ch[co].flags & CF_PLAYER) {
      if (ch[co].flags & CF_HARDCORE) {
        len +=
            sprintf(buf + len, "%s is a hardcore character and died %d times. ",
                    Hename(co), ch[co].deaths);

      } else {
        len +=
            sprintf(buf + len,
                    "%s has %d saves, was saved %d times and died %d times. ",
                    Hename(co), ch[co].saves, ch[co].got_saved, ch[co].deaths);
      }
      if ((cnt = count_solved_labs(co)))
        len += sprintf(buf + len, "%s solved %d parts of the labyrinth. ",
                       ch[co].name, cnt);

      if (check_first_kill(co, 403))
        len += sprintf(buf + len, "%s has mastered Hell. ", Hename(co));
               ...
               check_first_kill(co, 401) || check_first_kill(co, 402))
        len += sprintf(buf + len, "%s has ventured into Hell. ", Hename(co));
               ...
               check_first_kill(co, 394))
        len += sprintf(buf + len, "%s got %s nose burned in Hell. ", Hename(co),
                       hisname(co));
    }
    ...
    if (ch[co].flags & (CF_PLAYER | CF_PLAYERLIKE)) {
      if (!(ppd = (struct rank_ppd *)set_data(co, DRD_RANK_PPD,
                                              sizeof(struct rank_ppd))))
        return 0;  // OOPS

      if (ppd->army_rank) {
        len += sprintf(buf + len, "%s is a %s in the Imperial Army. ",
                       ch[co].name, rankname[ppd->army_rank]);
      }
...
int set_army_rank(int cn, int rank) {
  ...
  if (!(ppd = (struct rank_ppd *)set_data(cn, DRD_RANK_PPD,
                                          sizeof(struct rank_ppd))))
    return 0;  // OOPS
...
int immunity_reduction(int caster, int subject, int skill_strength) {
  ...
  if (ch[subject].value[1][V_TACTICS])
    immun += tactics2immune(ch[subject].value[0][V_TACTICS] + 14);
  ...
  // say(caster,"skill=%d, immun=%d, diff=%d,
  // stren=%.2f",skill_strength+skill_bonus,immun,diff,stren*STRIKE_DAMAGE/1000.0);
...
int immunity_reduction_no_bonus(int caster, int subject, int skill_strength) {
  ...
  // say(caster,"skill=%d, immun=%d, diff=%d,
  // stren=%f",skill_strength,immun,diff,stren/1000.0);
...
int freeze_value(int cn, int co) {
  ...
  // if (ch[co].value[1][V_TACTICS])
  // str2+=tactics2immune(ch[co].value[0][V_TACTICS]+14)*11;

  // str1=-(200+spellpower(cn,V_FREEZE)*11-ch[co].value[0][V_IMMUNITY]*10);
  // if (ch[co].value[1][V_TACTICS]) str1+=ch[co].value[0][V_TACTICS];

  // str=str1*old_system+str2*new_system;
  // say(cn,"freeze values str=%d old=%d new=%d
  // weight=%.2f",str,str1,str2,new_system);

  str = -(200 + spellpower(cn, V_FREEZE) * 11 -
          (ch[co].value[0][V_IMMUNITY] * 11));
  if (ch[co].value[1][V_TACTICS])
    str += tactics2immune(ch[co].value[0][V_TACTICS] + 14) * 11;

  if ((ch[cn].flags & CF_IDEMON) &&
      ch[co].value[0][V_COLD] > ch[cn].value[1][V_DEMON]) {
...
int warcry_value(int cn, int co, int pwr) {
  ...
  // if (ch[co].value[1][V_TACTICS])
  // str2+=tactics2immune(ch[co].value[0][V_TACTICS]+14)*6;

  // str=str1*old_system+str2*new_system;
  // say(cn,"warcry values str=%d old=%d new=%d
  // weight=%.2f",str,str1,str2,new_system);

  str = -(100 + pwr * 6 - ch[co].value[0][V_IMMUNITY] * 6);
  if (ch[co].value[1][V_TACTICS])
    str += tactics2immune(ch[co].value[0][V_TACTICS] + 14) * 6;
...
void set_item_requirements(int in) { set_item_requirements_sub(in, 120); }

void set_legacy_item_lvl(int in, int cn) {
  if (it[in].flags &
      (IF_WNHEAD | IF_WNNECK | IF_WNBODY | IF_WNARMS | IF_WNBELT | IF_WNLEGS |
       IF_WNFEET | IF_WNLHAND | IF_WNRHAND | IF_WNCLOAK | IF_WNLRING |
       IF_WNRRING | IF_WNTWOHANDED))
...
// maxchance: 10000 (normal), 1000 (no junk), 250 (better), 50 (good), 10 (real
// good), 1 (absolutely cool)
int create_special_item(int strength, int base, int potionprob, int maxchance) {
  ...
  switch (type) {
    case 0:
      sprintf(buf, "armor%dq3", base);
      break;
    case 1:
      sprintf(buf, "helmet%dq3", base);
      break;
    case 2:
      sprintf(buf, "sleeves%dq3", base);
      break;
    case 3:
      sprintf(buf, "leggings%dq3", base);
      break;
    case 4:
      sprintf(buf, "sword%dq3", base);
      break;
    case 5:
      sprintf(buf, "twohanded%dq3", base);
      break;
    case 6:
      sprintf(buf, "dagger%dq3", base);
      break;
    case 7:
      sprintf(buf, "staff%dq3", base);
      break;
    case 8:
    ...
    case 10:
      ...
      break;
    case 11:
      sprintf(buf, "brown_hat");
      break;
    case 12:
      sprintf(buf, "blue_cape");
      break;
    case 13:
      sprintf(buf, "brown_cape");
      break;
    case 14:
      sprintf(buf, "belt");
      break;
  ...
  if (strength == 1)
    ...
    strength = lowhi_random(2);
  else if (strength == 3)
    strength = lowhi_random(3);
  else if (strength == 4)
    strength = lowhi_random(4);
  else if (strength == 5)
    strength = lowhi_random(5);
  else if (strength == 6)
    strength = lowhi_random(6);
  else if (strength == 7)
    strength = lowhi_random(7);
  else
    strength = strength - 7 + lowhi_random(7);
  ...
  for (nr = tot = 0; nr < ARRAYSIZE(special_item); nr++) {
    if (special_item[nr].needflag && (it[in].flags & IF_WEAPON) &&
        !(it[in].flags & special_item[nr].needflag))
      continue;
  ...
  len = snprintf(buf, sizeof(it[0].description) - 1, "%s of %s%s%s.",
                 it[in].name, special_item[nr].the ? "the " : "", str_desc,
...
int create_spell_timer(int cn, int in, int pos) {
  switch (it[in].driver) {
    case IDR_BLESS:
      create_show_effect(EF_BLESS, cn, *(signed long *)(it[in].drdata + 4),
                         *(signed long *)(it[in].drdata), 0,
                         it[in].mod_value[0]);
      set_timer(*(unsigned long *)(it[in].drdata), remove_spell, cn, in, pos,
                ch[cn].serial, it[in].serial);
      break;
    case IDR_WARCRY:
      create_show_effect(EF_WARCRY, cn, *(signed long *)(it[in].drdata + 4),
                         *(signed long *)(it[in].drdata), 0, 0);
      set_timer(*(unsigned long *)(it[in].drdata), remove_spell, cn, in, pos,
                ch[cn].serial, it[in].serial);
      break;
    case IDR_FREEZE:
      create_show_effect(EF_FREEZE, cn, *(signed long *)(it[in].drdata + 4),
                         *(signed long *)(it[in].drdata), 0, 0);
      set_timer(*(unsigned long *)(it[in].drdata), remove_spell, cn, in, pos,
                ch[cn].serial, it[in].serial);
      break;
    ...
    case IDR_MANA:
      ...
      break;
    case IDR_POTION_SP:
      create_show_effect(EF_POTION, cn, *(signed long *)(it[in].drdata + 4),
                         *(signed long *)(it[in].drdata), 0,
                         it[in].mod_value[0]);
      set_timer(*(unsigned long *)(it[in].drdata), remove_spell, cn, in, pos,
                ch[cn].serial, it[in].serial);
    ...
    case IDR_CURSE:
      create_show_effect(EF_CURSE, cn, *(signed long *)(it[in].drdata + 4),
                         *(signed long *)(it[in].drdata), 0,
                         -it[in].mod_value[0]);
      set_timer(*(unsigned long *)(it[in].drdata), remove_spell, cn, in, pos,
                ch[cn].serial, it[in].serial);
    ...
    case IDR_POISON3:
      set_timer(*(unsigned long *)(it[in].drdata), remove_spell, cn, in, pos,
                ch[cn].serial, it[in].serial);
      set_timer(ticker + TICKS, poison_callback, cn, in, pos, ch[cn].serial,
                it[in].serial);
      break;
    case IDR_NONOMAGIC:
    case IDR_FIRERING:
...
void look_values_bg(int cnID, int coID) {
  ...
  sprintf(buf, "%s, level %d, class %s%s%s", ch[co].name, ch[co].level,
  ...
  if (ch[co].paid_till & 1)
    sprintf(buf, "Paying player: %s (%02d:%02d:%02d hours left)",
            (ch[co].flags & CF_PAID) ? "yes" : "no",
            (ch[co].paid_till - time_now) / (60 * 60),
            ((ch[co].paid_till - time_now) / 60) % 60,
            (ch[co].paid_till - time_now) % 60);
  else
    sprintf(buf, "Paying player: %s (%d days left)",
            (ch[co].flags & CF_PAID) ? "yes" : "no",
            (ch[co].paid_till - time_now) / (60 * 60 * 24));
  tell_chat(0, cnID, 1, "%s", buf);

  sprintf(buf, "PK: %s, Hardcore: %s", (ch[co].flags & CF_PK) ? "yes" : "no",
          (ch[co].flags & CF_HARDCORE) ? "yes" : "no");
  tell_chat(0, cnID, 1, "%s", buf);
  ...
  if (ppd) {
    sprintf(buf, "Gold in hand: %.2fG, gold in bank: %.2fG",
            ch[co].gold / 100.0, ppd->imperial_gold / 100.0);
    tell_chat(0, cnID, 1, "%s", buf);
  }
  sprintf(buf, "Mirror: %d, actual mirror: %d. Area %d, %s", ch[co].mirror,
          areaM, areaID, get_section_name(ch[co].x, ch[co].y));
  tell_chat(0, cnID, 1, "%s", buf);

  for (n = 0; n < V_MAX; n += 3) {
    sprintf(buf, "%s: %d/%d \010%s: %d/%d \020%s: %d/%d", skill[n].name,
            ch[co].value[1][n], ch[co].value[0][n],
  ...
  tell_chat(0, cnID, 1,
            "Offensive Value: %d, Defensive Value: %d, WV: %d, AV: %d",
            get_attack_skill(co), get_parry_skill(co),
            ch[co].value[0][V_WEAPON], ch[co].value[0][V_ARMOR] / 20);
}
...
void lollipop_bg(int cnID, int coID) {
  ...
  if (give_char_item(co, in)) {
    log_char(co, LOG_SYSTEM, 0,
             "A lollipop dropped out of thin air into your hand.");
    tell_chat(0, cnID, 1, "Gave lollipop to %s.", ch[co].name);
  } else
    tell_chat(0, cnID, 1, "Could not give lollipop to %s.", ch[co].name);
}
...
void shutdown_warn(void) {
  ...
  if (min != shutdown_last) {
    if (min > 0)
      sprintf(buf,
              "°c3The server will go down in %d minute%s. Expected downtime: "
              "%d minutes.",
              min, min > 1 ? "s" : "", shutdown_down);
    else
      sprintf(buf,
              "°c3The server will go down NOW. Expected downtime: %d minutes.",
              shutdown_down);
...
int calc_cost(int hint, struct cost_data *dat, int wanted) {
  ...
  avg_date = (avg_date + (dat->created - 24 * 60 * 60 * wanted) * (20 - num)) /
             20;  // theoretical creation time is 10 days ago

  days = (realtime - avg_date) /
         (60 * 60 * 24.0 * 10 /
          wanted);  // we want to have wanted missions in 10 days
  if (days < 0.01) days = 0.01;
  if (days > 100) days = 100;

  // xlog("hint=%d, amount=%d, days=%.2f, num=%d, wanted=%d,
  // diff=%.2fd",hint,avg_amount,days,num,wanted,(realtime-avg_date)/(60*60*24.0));
...
void add_cost(int cost, struct cost_data *dat) {
  memmove(dat->amount + 1, dat->amount,
          sizeof(dat->amount) - sizeof(dat->amount[0]));
...
int end_cost(int cn) {
  if (ch[cn].prof[P_ATHLETE])
    return END_COST - (ch[cn].prof[P_ATHLETE] * END_COST / 45);
  else
    return END_COST;
}
...
void give_military_pts(int cn, int co, int pts, int exps) {
  ...
  if (!(ppd = (struct military_ppd *)set_data(co, DRD_MILITARY_PPD,
  ...
  while (rank < 25 && rank > get_army_rank_int(co)) {
    ...
    say(cn, "You've been promoted to %s. Congratulations, %s!",
        get_army_rank_string(co), ch[co].name);
...
void give_military_pts_no_npc(int co, int pts, int exps) {
  ...
  if (rank < 25 && rank > get_army_rank_int(co)) {
    set_army_rank(co, rank);
    log_char(co, LOG_SYSTEM, 0, "You've been promoted to %s!",
             get_army_rank_string(co));
    if (get_army_rank_int(co) > 9) {
      sprintf(buf, "0000000000°c10Grats: %s is a %s now!", ch[co].name,
              get_army_rank_string(co));
...
int give_char_item(int cn, int in) {
  ...
  if (ch[cn].flags & CF_PLAYER)
    dlog(cn, in, "was given %s from NPC", it[in].name);
  return 1;
}
...
int create_orb(void) {
  ...
  switch (r) {
    ...
    case 31:
      ...
      break;
    default:
      elog("unknown %d in create_orb()", r);
      what = V_HP;
      break;
  }
...
void tabunga(int cn, int co, char *ptr) {
  if ((ch[co].flags & CF_GOD) && strcasestr(ptr, "tabunga") &&
      char_dist(cn, co) < 3) {
    say(cn, "%s (%d):", ch[cn].name, ch[cn].level);
    say(cn, "HP:        %3d/%3d (%d)", ch[cn].value[1][V_HP],
        ch[cn].value[0][V_HP], ch[cn].hp / POWERSCALE);
    say(cn, "Endurance: %3d/%3d (%d)", ch[cn].value[1][V_ENDURANCE],
        ch[cn].value[0][V_ENDURANCE], ch[cn].endurance / POWERSCALE);
    say(cn, "Mana:      %3d/%3d (%d)", ch[cn].value[1][V_MANA],
        ch[cn].value[0][V_MANA], ch[cn].mana / POWERSCALE);
    say(cn, "Wisdom:    %3d/%3d", ch[cn].value[1][V_WIS],
        ch[cn].value[0][V_WIS]);
    say(cn, "Intuition: %3d/%3d", ch[cn].value[1][V_INT],
        ch[cn].value[0][V_INT]);
    say(cn, "Agility:   %3d/%3d", ch[cn].value[1][V_AGI],
        ch[cn].value[0][V_AGI]);
    say(cn, "Strength:  %3d/%3d", ch[cn].value[1][V_STR],
        ch[cn].value[0][V_STR]);
    say(cn, "Hand2Hand: %3d/%3d", ch[cn].value[1][V_HAND],
        ch[cn].value[0][V_HAND]);
    say(cn, "Sword:     %3d/%3d", ch[cn].value[1][V_SWORD],
        ch[cn].value[0][V_SWORD]);
    say(cn, "Twohanded: %3d/%3d", ch[cn].value[1][V_TWOHAND],
        ch[cn].value[0][V_TWOHAND]);
    say(cn, "Attack:    %3d/%3d", ch[cn].value[1][V_ATTACK],
        ch[cn].value[0][V_ATTACK]);
    say(cn, "Parry:     %3d/%3d", ch[cn].value[1][V_PARRY],
        ch[cn].value[0][V_PARRY]);
    say(cn, "Tactics:   %3d/%3d", ch[cn].value[1][V_TACTICS],
        ch[cn].value[0][V_TACTICS]);
    say(cn, "Immunity:  %3d/%3d", ch[cn].value[1][V_IMMUNITY],
        ch[cn].value[0][V_IMMUNITY]);
    say(cn, "Bless:     %3d/%3d", ch[cn].value[1][V_BLESS],
        ch[cn].value[0][V_BLESS]);
    say(cn, "M-Shield:  %3d/%3d  (%d)", ch[cn].value[1][V_MAGICSHIELD],
        ch[cn].value[0][V_MAGICSHIELD], ch[cn].lifeshield / POWERSCALE);
    say(cn, "Flash:     %3d/%3d", ch[cn].value[1][V_FLASH],
        ch[cn].value[0][V_FLASH]);
    say(cn, "Freeze:    %3d/%3d", ch[cn].value[1][V_FREEZE],
        ch[cn].value[0][V_FREEZE]);
    say(cn, "Speed:     %3d/%3d", ch[cn].value[1][V_SPEED],
        ch[cn].value[0][V_SPEED]);
    say(cn, "F-Ball:    %3d/%3d", ch[cn].value[1][V_FIREBALL],
        ch[cn].value[0][V_FIREBALL]);
    say(cn, "Percept:   %3d/%3d", ch[cn].value[1][V_PERCEPT],
        ch[cn].value[0][V_PERCEPT]);
    say(cn, "Stealth:   %3d/%3d", ch[cn].value[1][V_STEALTH],
        ch[cn].value[0][V_STEALTH]);
    say(cn, "Warcry:    %3d/%3d", ch[cn].value[1][V_WARCRY],
        ch[cn].value[0][V_WARCRY]);
    ...
    say(cn, "Offensive Value: %d, WV: %d", get_attack_skill(cn),
        ch[cn].value[0][V_WEAPON]);
    say(cn, "Defensive Value: %d, AV: %d", get_parry_skill(cn),
        ch[cn].value[0][V_ARMOR] / 20);
    say(cn, "Mod Immunity: %d",
        ch[cn].value[0][V_IMMUNITY] +
            tactics2immune(ch[cn].value[0][V_TACTICS] + 14));
    say(cn, "War Immunity: %d",
        ch[cn].value[0][V_IMMUNITY] + ch[cn].value[0][V_TACTICS] / 10);
    say(cn, "Flash Power: %d",
        ch[cn].value[0][V_FLASH] + tactics2spell(ch[cn].value[0][V_TACTICS]));
    say(cn, "x=%d, y=%d, speedmode=%d", ch[cn].restx, ch[cn].resty,
        ch[cn].speed_mode);
    say(cn, "undead=%s, alive=%s", (ch[cn].flags & CF_UNDEAD) ? "yes" : "no",
        (ch[cn].flags & CF_ALIVE) ? "yes" : "no");
  }
}
...
void buggy_items(int cn) {
  ...
  ppd =
      (struct depot_ppd *)set_data(cn, DRD_DEPOT_PPD, sizeof(struct depot_ppd));
...
void do_whostaff(int cnID) {
  ...
  for (co = getfirst_char(); co; co = getnext_char(co)) {
    ...
    tell_chat(0, cnID, 1, "%s [%s]%s", ch[co].name, ch[co].staff_code,
...
void player_use_potion(int cn) {
  ...
  ppd = (struct lostcon_ppd *)set_data(cn, DRD_LOSTCON_PPD,
  ...
  if (ch[cn].hp < ch[cn].value[1][V_HP] * (int)(POWERSCALE * 0.50)) {
    if (!ppd->nolife || !ppd->nocombo) {
      for (n = 30; n < INVENTORYSIZE; n++) {
        if ((in = ch[cn].item[n]) && it[in].driver == IDR_POTION &&
            it[in].drdata[1]) {
...
void player_use_recall(int cn) {
  ...
  if (ch[cn].hp < ch[cn].value[1][V_HP] * (int)(POWERSCALE * 0.40) &&
      !ppd->norecall) {
    for (n = 30; n < INVENTORYSIZE; n++) {
      if ((in = ch[cn].item[n]) && it[in].driver == IDR_RECALL &&
          it[in].drdata[0] >= ch[cn].level) {
        use_item(cn, in);
        return;
      }
    }
...
int check_can_wear_item(int cn) {
  ...
  for (n = 0; n < 12; n++) {
    if ((in = ch[cn].item[n]) && !can_wear(cn, in, n) &&
...
void sanitize_item(struct item *in, int cn) {
  ...
  for (n = 0; n < MAXMOD; n++) {
    ...
    switch (in->mod_index[n]) {
      case V_ARMOR:
        armor = in->mod_value[n];
        continue;
      case V_WEAPON:
        weapon = in->mod_value[n];
        continue;
      case V_LIGHT:
        continue;
      case V_SPEED:
        dlog(cn, 0, "%s: change raw speed to speed skill", in->name);
        in->mod_index[n] = V_SPEEDSKILL;
        break;
      case V_EMPTY:
        dlog(cn, 0, "%s: remove +empty", in->name);
        in->mod_index[n] = in->mod_value[n] = 0;
        continue;
      case -V_ARMORSKILL:
        armor_req = in->mod_value[n];
        req_idx = n;
        continue;
      case -V_DAGGER:
      case -V_SWORD:
  ...
  if (cnt > 3) {
    in->min_level = 250;
    strcpy(in->description,
           "This item is locked (4/5stat). Please contact game@astonia.com.");
  ...
  if (armor) {
    if (in->flags & IF_WNARMS) {
      ...
      elog("sanitize_item: cannot deal with item %s of character %s", in->name,
           ch[cn].name);
      return;
    }
    if (req != armor_req) {
      dlog(cn, 0, "%s: armor: ex=%f %d, req=%d, req change %d to %d", in->name,
           ex, (int)(ex + 0.5), req, armor_req, req);
      if (req_idx != -1) {
        ...
        if (n == MAXMOD) {
          elog("sanitize_item: cannot deal with item %s of character %s",
               in->name, ch[cn].name);
          return;
        }
      }
  ...
  if (weapon) {
    if (in->flags & IF_TWOHAND)
      req = (int)((weapon - 3) / 10.0 + 0.5);
    else
      req = (int)(weapon / 10.0 + 0.5);
    if (req > 1) req = req * 10 - 10;

    if (in->flags & IF_DAGGER)
      req_type = V_DAGGER;
    else if (in->flags & IF_SWORD)
      req_type = V_SWORD;
    else if (in->flags & IF_STAFF)
      req_type = V_STAFF;
    else if (in->flags & IF_TWOHAND)
      req_type = V_TWOHAND;
    else {
      elog("sanitize_item: cannot deal with item %s of character %s", in->name,
           ch[cn].name);
    ...
    if (req != weapon_req) {
      dlog(cn, 0, "%s: weapon: req=%d, req change %d to %d", in->name, req,
           weapon_req, req);
      if (req_idx != -1) {
        in->mod_value[req_idx] = req;
      } else {
