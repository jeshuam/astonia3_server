...
void clanlog_help(int cn) {
  log_char(cn, LOG_SYSTEM, 0,
           "Usage: /clanlog [-p player] [-c clan] [-x prio] [-s starttime] [-e "
           "endtime]");
  log_char(cn, LOG_SYSTEM, 0,
           "-p <player name>: restrict output to entries about that player");
  log_char(cn, LOG_SYSTEM, 0,
           "-c <clan number>: restrict output to entries about that clan");
  log_char(cn, LOG_SYSTEM, 0,
           "-x <prio number>: restrict output to entries with prio or higher "
           "(1=highest prio, 100=lowest prio)");
  log_char(cn, LOG_SYSTEM, 0,
           "-s <hours>: restrict output to entries not more then hours old");
  log_char(cn, LOG_SYSTEM, 0,
           "-e <hours>: restrict output to entries at least hours old");
  log_char(cn, LOG_SYSTEM, 0,
           "-i: show clan internal log (same as -x 50 -c <own clan>)");
  log_char(cn, LOG_SYSTEM, 0,
           "Example: /clanlog -p Ishtar -c 4 -x 5 -s 48 -e 24");
  log_char(cn, LOG_SYSTEM, 0,
           "If no times are specified, /clanlog will use the last 24 hours. If "
           "no priority is given, it will be set to 20.");
  log_char(cn, LOG_SYSTEM, 0,
           "If you specify a priority higher than 20, output will be "
           "restricted to your clan.");
}
...
int cmd_clanlog(int cn, char *ptr) {
  ...
  if (prio > 20) {
    if (!ch[cn].clan) {
      log_char(cn, LOG_SYSTEM, 0,
               "Only clan members may set a priority greater than 20.");
      return 1;
    }
    if (clan != ch[cn].clan) {
      clan = ch[cn].clan;
