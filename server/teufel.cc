...
void teufelgambler_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct gamble_data*)set_data(cn, DRD_TEUFELGAMBLE, sizeof(struct gamble_data));
  if (!dat) return;	// oops...
...
void teufelquest_driver(int cn, int ret, int lastact)
{
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (!(ch[co].flags & CF_PLAYER)) { remove_message(cn, msg); continue; }
      ppd = (struct rat_data*)set_data(co, DRD_TEUFELRAT_PPD, sizeof(struct rat_data));
...
void teufelrat_dead(int cn, int cc)
{
  ...
  ppd = (struct rat_data*)set_data(cc, DRD_TEUFELRAT_PPD, sizeof(struct rat_data));
  if (!ppd) return;
