...
struct qa qa[] = {
    ...
    {{"play", NULL},
     "I only play for bronze chips. You can °c4bet one°c0 or °c4bet two°c0 or "
     "°c4bet five°c0 of them. Then you'll roll three dice and depending on the "
     "°c4results°c0 you'll win the most fantastic stuff possible!",
     0},
    {{"results", NULL},
     "The dice are twenty-sided and the numbers are added up. If you roll 3 to "
     "20 you win and if you roll 43 to 60 you win, too. Want to hear about the "
     "°c4prizes°c0?",
     0},
    {{"prizes", NULL},
     "A 3 gets you a Cape of the Warrior. A 60 a Cape of the Mage. +7 if you "
     "bet one chip, +14 if you bet two, +21 if you bet all five. Want to hear "
     "°c4 more prizes°c0?",
     0},
    {{"more", "prizes", NULL},
     "With a 4 or a 59 you'll win 100,000 gold (when betting 5 chips, 20,000 "
     "for 1 chip, 40,000 for 2 chips). And there are many, many more prizes...",
     0},

    {{"play2", NULL},
     "I only play for silver chips. You can °c4bet one°c0 or °c4bet two°c0 or "
     "°c4bet five°c0 of them. Then you'll roll three dice and depending on the "
     "°c4results2°c0 you'll win the most fantastic stuff possible!",
     0},
    {{"results2", NULL},
     "The dice are twenty-sided and the numbers are added up. If you roll 3 to "
     "20 you win and if you roll 43 to 60 you win, too. Want to hear about the "
     ...
     0},
    {{"prizes2", NULL},
     "A 3 gets you boots of the Warrior. A 60 boots of the Mage. +8 if you bet "
     "one chip, +15 if you bet two, +22 if you bet all five. Want to hear °c4 "
     "more prizes2°c0?",
     0},
    {{"more", "prizes2", NULL},
     "With a 4 or a 59 you'll win 150,000 gold (when betting 5 chips, 30,000 "
     "for 1 chip, 60,000 for 2 chips). And there are many, many more prizes...",
     0},

    {{"play3", NULL},
     "I only play for gold chips. You can °c4bet one°c0 or °c4bet two°c0 or "
     "°c4bet five°c0 of them. Then you'll roll three dice and depending on the "
     "°c4results3°c0 you'll win the most fantastic stuff possible!",
     0},
    {{"results3", NULL},
     "The dice are twenty-sided and the numbers are added up. If you roll 3 to "
     "20 you win and if you roll 43 to 60 you win, too. Want to hear about the "
     ...
     0},
    {{"prizes3", NULL},
     "A 3 gets you a helmet of the Warrior. A 60 a hat of the Mage. +9 if you "
     "bet one chip, +16 if you bet two, +23 if you bet all five. Want to hear "
     "°c4 more prizes3°c0?",
     0},
    {{"more", "prizes3", NULL},
     "With a 4 or a 59 you'll win 200,000 gold (when betting 5 chips, 40,000 "
     "for 1 chip, 80,000 for 2 chips). And there are many, many more prizes...",
     0},
    ...
    {{"repeat", NULL},
     "Hello, %s! We have a slight rat problem in the caverns to the north. "
     "There's a nice °c4reward°c0 for killing some rats.",
     0},
    {{"reward", NULL},
     "Yeah. You go kill some rats. The more and bigger the rats you kill, the "
     "more points you get in my book. The more points you have, the better the "
     "rewards you get. You know, °c4experience°c0, °c4military°c0 knowledge or "
     "just plain °c4money°c0 if that's what you want.",
     0},
    {{"experience", NULL},
     "Exactly. Experience. The fire-is-hot-so-don't-touch-it kind of "
     "experience. °c4Give experience°c0 will exchange your points for "
     "experience.",
     0},
    {{"military", NULL},
     "That's right. Everything your drill sergeant told you and you can't "
     "remember. °c4Give military°c0 will exchange your points for military "
     "knowledge.",
     0},
    {{"money", NULL},
     "You know, them greenbacks. Oh, wait. Wrong dimension. Money... Ah, "
     "right. Round, flat and shiny... Coins! That's it. °c4Give money°c0 will "
     "exchange your points for greenbacks. Err, gold coins.",
     0},
...
void teufeldoor(int in, int cn) {
  ...
  if (ch[cn].sprite != 27 && ch[cn].sprite != 157 && ch[cn].sprite != 39) {
    log_char(cn, LOG_SYSTEM, 0,
             "A demon looks through the view-hole in the door and shouts: \"No "
             "humans allowed!\"");
    return;
  }
  ...
  if (it[in].drdata[0] == 3 && ch[cn].sprite != 39) {
    log_char(cn, LOG_SYSTEM, 0,
             "A demon looks through the view-hole in the door and shouts: "
             "\"Only nobles allowed!\"");
    return;
  }
...
void teufelarena(int in, int cn) {
  ...
  switch (nr * 10 + r) {
    ...
    case 17:
      ...
      break;
    default:
      elog("error in teufelarena.c #455");
      x = 250;
      y = 250;
      break;
  }
  ...
  for (n = 0; n < 12; n++) {
    ...
    if (it[in2].sprite >= 53001 && it[in2].sprite <= 53006)
      continue;  // edemon suit
    if (nr > 1 && it[in2].sprite >= 53031 && it[in2].sprite <= 53036)
      continue;  // fdemon suit
    if (nr > 2 && it[in2].sprite >= 53025 && it[in2].sprite <= 53030)
      continue;  // udemon suit
    if (count_enhancements(in2) > 1) {
      log_char(cn, LOG_SYSTEM, 0,
               "You cannot enter while wearing your %s. It has more than one "
               "enhancement.",
               it[in2].name);
      return;
    }
    if (it[in2].flags & (IF_QUEST | IF_BONDTAKE | IF_BONDWEAR)) {
      log_char(cn, LOG_SYSTEM, 0,
               "You cannot enter while wearing your %s. It is a quest or a "
               "bound item.",
               it[in2].name);
      return;
    }
...
5   58    0.0750%   tactical offense,
magical offense (boots)
6   57    0.1250%   tactical defense,
magical defense (armor, vest)
7   56    0.1875%   einsatz x gold chips
8   55    0.2625%   5,000, 10,000, 25,000 gold
9   54    0.3500%   sword, flash
(leggings, trousers)
10    53    0.4500%   two-handed, fire
(leggings, trousers)
...
void give_reward(int cn, int nr, int bet) {
  ...
  switch (nr) {
    ...
    case 10:
      if (bet == 5)
        ptr = "demon_twohanded3";
      else if (bet == 2)
        ptr = "demon_twohanded2";
      else
        ptr = "demon_twohanded1";
      cnt = 1;
      break;
    case 53:
      if (bet == 5)
  ...
  for (n = 0; n < cnt; n++) {
    ...
    if (!give_char_item(cn, in)) {
      log_char(cn, LOG_SYSTEM, 0,
               "Seeing that you have no room to store the %s, the Gamber "
               "quickly pockets it again. Too bad...",
               it[in].name);
      destroy_item(in);
    } else {
...
void give_reward2(int cn, int nr, int bet) {
  ...
  switch (nr) {
    ...
    case 10:
      if (bet == 5)
        ptr = "demon_twohanded3b";
      else if (bet == 2)
        ptr = "demon_twohanded2b";
      else
        ptr = "demon_twohanded1b";
      cnt = 1;
      break;
    case 53:
      if (bet == 5)
...
void give_reward3(int cn, int nr, int bet) {
  ...
  switch (nr) {
    ...
    case 10:
      if (bet == 5)
        ptr = "demon_twohanded3c";
      else if (bet == 2)
        ptr = "demon_twohanded2c";
      else
        ptr = "demon_twohanded1c";
      cnt = 1;
      break;
    case 53:
      if (bet == 5)
...
void teufelgambler_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct gamble_data *)set_data(cn, DRD_TEUFELGAMBLE,
                                       sizeof(struct gamble_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (dat->nr == 1) {
        if (!is_demon(co)) {
          say(cn,
              "Oh. A human. Well, no matter I guess. Wanna °c4play°c0 with me, "
              "kid?");
        } else {
          say(cn,
              "Hello there, %s! Make your bet! Win big! Come on, °c4play°c0 "
              "with me!",
              ch[co].name);
        }
      } else if (dat->nr == 2) {
        if (!is_demon(co)) {
          say(cn,
              "Oh. A human. Well, no matter I guess. Wanna °c4play2°c0 with "
              "me, kid?");
        } else {
          say(cn,
              "Hello there, %s! Make your bet! Win big! Come on, °c4play2°c0 "
              "with me!",
              ch[co].name);
        }
      } else if (dat->nr == 3) {
        if (!is_demon(co)) {
          say(cn,
              "Oh. A human. Well, no matter I guess. Wanna °c4play3°c0 with "
              "me, kid?");
        } else {
          say(cn,
              "Hello there, %s! Make your bet! Win big! Come on, °c4play3°c0 "
              "with me!",
              ch[co].name);
        }
      }
    ...
    if (msg->type == NT_TEXT) {
      ...
      if ((n = analyse_text_driver(cn, msg->dat1, (char *)msg->dat2,
                                   msg->dat3))) {
        talkdir = offset2dx(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y);
        if (n > 1) {
          if (n == 2)
          ...
          for (n = 30; n < INVENTORYSIZE; n++) {
            if (dat->nr == 1 && (in = ch[co].item[n]) &&
                it[in].ID == IID_BRONZECHIP &&
                (have = *(unsigned int *)(it[in].drdata + 0)) >= cnt) {
              if (cnt == have) {
                destroy_item(in);
                ch[co].item[n] = 0;
              } else {
            ...
            if (dat->nr == 2 && (in = ch[co].item[n]) &&
                it[in].ID == IID_SILVERCHIP &&
                (have = *(unsigned int *)(it[in].drdata + 0)) >= cnt) {
              if (cnt == have) {
                destroy_item(in);
                ch[co].item[n] = 0;
            ...
            if (dat->nr == 3 && (in = ch[co].item[n]) &&
                it[in].ID == IID_GOLDCHIP &&
                (have = *(unsigned int *)(it[in].drdata + 0)) >= cnt) {
              if (cnt == have) {
                destroy_item(in);
                ch[co].item[n] = 0;
          ...
          if (n < INVENTORYSIZE) {
            ...
            t = a + b + c;
            if (t > 20 && t < 43)
              say(cn, "Ha! You rolled %d, %d and %d. You lost!", a, b, c);
            else {
              say(cn, "Oh. You rolled %d, %d and %d. You win!", a, b, c);
              if (dat->nr == 1)
                give_reward(co, t, cnt);
              else if (dat->nr == 2)
                give_reward2(co, t, cnt);
              else if (dat->nr == 3)
                give_reward3(co, t, cnt);
              else
                say(cn, "Bug #4227v");
            }
          } else
            say(cn,
                "No chips, no game (make sure you only have one stack of "
                "chips).");
        }
      }

      if ((ch[co].flags & CF_GOD) &&
          (ptr = strstr((char *)msg->dat2, "reward: ")) &&
          (n = atoi(ptr + 8))) {
        if (dat->nr == 1)
          give_reward(co, n, 5);
        else if (dat->nr == 2)
...
void special_rat_reward(int cn, int co, struct rat_data *ppd) {
  ...
  if (ppd->score >= 100000) {
    pts = 100000;
    in = create_special_item(20, 90, 1, 50);
  } else if (ppd->score >= 50000) {
    pts = 50000;
    in = create_special_item(20, 90, 1, 250);
  } else if (ppd->score >= 25000) {
    pts = 25000;
    in = create_special_item(20, 90, 1, 1000);
  } else if (ppd->score >= 10000) {
    pts = 10000;
    in = create_special_item(20, 90, 1, 10000);
  } else if (ppd->score >= 5000) {
    pts = 5000;
    in = create_item("healing_potion3");
  } else if (ppd->score >= 2500) {
    pts = 2500;
    in = create_item("healing_potion2");
  } else if (ppd->score >= 1000) {
    pts = 1000;
    in = create_item("healing_potion1");
  }

  if (pts && in) {
    say(cn, "Here's a little extra for scoring %d points in one go: %s!", pts,
        it[in].name);
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (!is_demon(co)) {
        ...
        say(cn,
            "Hello, %s! We have a slight rat problem in the caverns to the "
            "north. There's a nice °c4reward°c0 for killing some rats.",
            ch[co].name);
      }
    ...
    if (msg->type == NT_TEXT) {
      ...
      ppd = (struct rat_data *)set_data(co, DRD_TEUFELRAT_PPD,
                                        sizeof(struct rat_data));
      if (!ppd) {
        remove_message(cn, msg);
        continue;
      }
      ...
      if ((n = analyse_text_driver(cn, msg->dat1, (char *)msg->dat2,
        ...
        switch (n) {
          case 5:
            ...
            tmp = ppd->score / 20 * ch[cn].level;  // 12% of a level for 1000
                                                   // enemies killed (at max
                                                   // level)
            say(cn,
                "Experience it is. You killed %d rats for a total score of %d.",
                ppd->kills, ppd->score);
            give_exp(co, tmp);
          ...
          case 6:
            tmp = ppd->score /
                  1250;  // 50 pts for 625 enemies killed (at max level)
            say(cn,
                "Military knowledge it is. You killed %d rats for a total "
                "score of %d.",
                ppd->kills, ppd->score);
          ...
          case 7:
            tmp = ppd->score *
                  12;  // aprox 12 gold per enemy killed (at max level)
            say(cn, "Money it is. You killed %d rats for a total score of %d.",
                ppd->kills, ppd->score);
...
void teufelrat_dead(int cn, int cc) {
  ...
  ppd = (struct rat_data *)set_data(cc, DRD_TEUFELRAT_PPD,
                                    sizeof(struct rat_data));
  if (!ppd) return;
  ...
  if (!cn) {  // time call
    ...
    if (nr == 1) {
      if (wave == 0)
        ...
        lvl = 108;  // enormous 107
    } else if (nr == 2) {
      if (wave == 0)
        lvl = 109;  // baby 108
      else if (wave < 100)
        lvl = 113;  // big baby 112
      else if (wave < 200)
        lvl = 117;  // kid 116
      else if (wave < 400)
        lvl = 121;  // big kid 120
        ...
        lvl = 129;  // big teeny 128
      else if (wave < 3200)
        lvl = 133;  // big 132
      else if (wave < 6400)
        lvl = 137;  // very big 136
      else if (wave < 12800)
        lvl = 141;  // huge 140
      else
        lvl = 145;  // enormous 144
      // xlog("2lvl=%d",lvl);
    } else {
      if (wave == 0)
        lvl = 45;  // baby 44
    ...
    sprintf(it[in].description,
            "An Ice Rat nest[%d]. You could destroy it...[%d,%d]", nr, wave,
            lvl);
    ...
    for (n = 0; n < 5; n++) {
      ...
      if (nr == 1) {
        if (wave < 100)
          co = create_char("rat80", 0);  // baby 69
        else if (wave < 200)
          co = create_char("rat80b", 0);  // big baby 73
        else if (wave < 400)
          co = create_char("rat81", 0);  // kid 77
        else if (wave < 800)
          co = create_char("rat81b", 0);  // big kid 82
        else if (wave < 1600)
          co = create_char("rat82", 0);  // teeny 86
        else if (wave < 3200)
          co = create_char("rat82b", 0);  // big teeny 91
        else if (wave < 6400)
          co = create_char("rat83", 0);  // big 95
        else if (wave < 12800)
          co = create_char("rat83b", 0);  // very big 99
        else if (wave < 25600)
          co = create_char("rat84", 0);  // huge 102
        else
          co = create_char("rat84b", 0);  // enormous 107
      } else if (nr == 2) {
        if (wave < 100)
          co = create_char("rat90", 0);  // baby
        else if (wave < 200)
          co = create_char("rat90b", 0);  // big baby
        else if (wave < 400)
          co = create_char("rat91", 0);  // kid
        else if (wave < 800)
          co = create_char("rat91b", 0);  // big kid
        else if (wave < 1600)
          co = create_char("rat92", 0);  // teeny
        else if (wave < 3200)
          co = create_char("rat92b", 0);  // big teeny
        else if (wave < 6400)
          co = create_char("rat93", 0);  // big
        else if (wave < 12800)
          co = create_char("rat93b", 0);  // very big
        else if (wave < 25600)
          co = create_char("rat94", 0);  // huge
        else
          co = create_char("rat94b", 0);  // enormous
      } else {
        if (wave < 100)
          co = create_char("rat70", 0);  // baby
        else if (wave < 200)
          co = create_char("rat70b", 0);  // big baby
        else if (wave < 400)
          co = create_char("rat71", 0);  // kid
        else if (wave < 800)
          co = create_char("rat71b", 0);  // big kid
        else if (wave < 1600)
          co = create_char("rat72", 0);  // teeny
        else if (wave < 3200)
          co = create_char("rat72b", 0);  // big teeny
        else if (wave < 6400)
          co = create_char("rat73", 0);  // big
        else if (wave < 12800)
          co = create_char("rat73b", 0);  // very big
        else if (wave < 25600)
          co = create_char("rat74", 0);  // huge
        else
          co = create_char("rat74b", 0);  // enormous
      }
      if (!co) break;
      ...
      if (item_drop_char(in, co)) {
        ...
        switch (RANDOM(20)) {
          case 0:
            ch[co].value[1][V_ATTACK] += RANDOM(10) + 7;
            name = " *A";
            desc = " Increased Attack.";
            break;
          case 1:
            ch[co].value[1][V_PARRY] += RANDOM(10) + 7;
            name = " *P";
            desc = " Increased Parry.";
            break;
          case 2:
            ch[co].value[1][V_FREEZE] += RANDOM(10) + 7;
            name = " *R";
            desc = " Increased Freeze.";
            break;
          case 3:
            ch[co].value[1][V_FLASH] += RANDOM(10) + 7;
            name = " *F";
            desc = " Increased Flash.";
            break;
          case 4:
            ch[co].value[1][V_IMMUNITY] += RANDOM(10) + 7;
            name = " *I";
            desc = " Increased Immunity.";
            break;
          default:
            name = "";
            desc = "";
        ...
        if (ch[co].value[0][V_BLESS])
          bless_someone(co, ch[co].value[0][V_BLESS], BLESSDURATION);
        ...
        *(unsigned int *)(it[in].drdata + n * 4 + 20) = ch[co].serial;
        if (!nowaveincrease && wave < 50000)
          (*(unsigned short *)(it[in].drdata + 0)) += 10;  // increase wave
        break;
      } else {
        destroy_char(co);
        break;
    ...
    for (n = 0; n < 5; n++) {
      ...
      if (co && (ch[co].flags & CF_USED) && ch[co].serial == ser) {
        log_char(cn, LOG_SYSTEM, 0,
                 "You need a moment of peace to destroy the nest. There is "
                 "still a guard left, distracting you.");
        return;
      }
    }
...
int ch_driver(int nr, int cn, int ret, int lastact) {
  switch (nr) {
    case CDR_TEUFELDEMON:
      teufeldemon_driver(cn, ret, lastact);
      return 1;
    case CDR_TEUFELGAMBLER:
      teufelgambler_driver(cn, ret, lastact);
      return 1;
    case CDR_TEUFELQUEST:
      teufelquest_driver(cn, ret, lastact);
      return 1;
    case CDR_TEUFELRAT:
      teufelrat_driver(cn, ret, lastact);
      return 1;
...
int it_driver(int nr, int in, int cn) {
  switch (nr) {
    case IDR_TEUFELDOOR:
      teufeldoor(in, cn);
      return 1;
    case IDR_TEUFELARENA:
      teufelarena(in, cn);
      return 1;
    case IDR_TEUFELRATNEST:
      teufelratnest(in, cn);
      return 1;
    case IDR_TEUFELARENAEXIT:
      teufelarenaexit(in, cn);
      return 1;
