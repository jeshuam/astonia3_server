...
const char *daemonreal[4] = {"xxrealxx", "Fao Thals", "Breth Ona", "Ch Dae Tyr"};
...
void ritual_create_char(const char *name, int x, int y, int dir, int attackstart)
{
  int cn;
  ...
  dat = (struct lab5_daemon_data*)set_data(cn, DRD_LAB5_DAEMON, sizeof(struct lab5_daemon_data));
  if (dat) {
    dat->dir = dir;
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if (ch[co].flags & CF_PLAYER) {
        ...
        // get lab ppd
        pd = (struct lab5_player_data*)set_data(co, DRD_LAB5_PLAYER, sizeof(struct lab5_player_data));
...
void lab5_item(int in, int cn)
{
  ...
  if (!cn) {
    ...
    if (drdata[0] == 5) {
      ...
      // get pd
      pd = (struct lab5_player_data*)set_data(cn, DRD_LAB5_PLAYER, sizeof(struct lab5_player_data));
      if (!pd) return;
