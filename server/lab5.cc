...
struct lab5_player_data {
  ...
  unsigned char magegot;  // (the door will set this values, so the mage can say
                          // something - not implemented, talking is complex and
                          // i was tired)
  unsigned char magestate;  // the talk state of the mage

  unsigned char ritualdaemon;  // the daemon the ritual is for
  unsigned char ritualstate;  // the ritualstate (0=none at all, 1=touched name,
                              // 2=touched realname)
};
...
const char *daemonname[4] = {"xxnamexx", "Asfaloth", "Beronath", "Cyradeth"};
const char *daemonreal[4] = {"xxrealxx", "Fao Thals", "Breth Ona",
                             "Ch Dae Tyr"};
int namecoordx[4] = {85, 90, 85, 80};  // need initialisation for dll reload (it
                                       // took me a while till i found that
                                       // "bug")
int namecoordy[4] = {33, 28, 23, 28};  // need initialisation for dll reload

#define MAXDOOR 4
int daemondoorx[MAXDOOR] = {119, 119, 119,
                            119};  // need initialisation for dll reload
int daemondoory[MAXDOOR] = {108, 95, 82,
                            69};  // need initialisation for dll reload
...
void ritual_hurt(int cn, struct lab5_player_data *pd, int x, int y) {
  ...
  log_char(cn, LOG_SYSTEM, 0, "°c3The Ritual Of %s ended.°c0",
           daemonname[pd->ritualdaemon]);
  pd->ritualdaemon = 0;
  pd->ritualstate = 0;
...
void ritual_create_char(const char *name, int x, int y, int dir,
                        int attackstart) {
  int cn;
  ...
  dat = (struct lab5_daemon_data *)set_data(cn, DRD_LAB5_DAEMON,
                                            sizeof(struct lab5_daemon_data));
  if (dat) {
    dat->dir = dir;
...
int ritual_start(int cn, int daemon) {
  int i, x = 0, y, sx, sy, ex, ey, break2, mn, doorx, doory,
         co;  // x=0 to avoid compiler warning!
  ...
  for (y = sy; y <= ey; y++) {
    for (x = sx; x <= ex; x++) {
      mn = x + y * MAXMAP;
      if ((co = map[mn].ch) && !(ch[co].flags & CF_PLAYER))
  ...
  if (daemon == 1) {
    ...
    ritual_create_char("lab5_three_servant_mage", doorx + 6, doory - 4, DX_DOWN,
                       7);
    ritual_create_char("lab5_three_servant_mage", doorx + 6, doory + 4, DX_UP,
                       7);
...
void set_seyan_state(struct lab5_player_data *pd) {
  if ((pd->seyangot & (1 << 0)) && (pd->seyangot & (1 << 1)) &&
      (pd->seyangot & (1 << 2)))
    pd->seyanstate = 20;  // done
  else if (pd->seyangot)
    pd->seyanstate = 10;  // some
  else
    pd->seyanstate = 0;  // start
}

void lab5_seyan_driver(int cn, int ret, int lastact) {
  struct lab5_player_data *pd;
  static struct lab5_talk_data datbuf;  // we only have one, so there is no need
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if (ch[co].flags & CF_PLAYER) {
        // get lab ppd
        pd = (struct lab5_player_data *)set_data(
            co, DRD_LAB5_PLAYER, sizeof(struct lab5_player_data));
        if (!pd) {
          remove_message(cn, msg);
          continue;
        }
        ...
        if (it[ch[cn].citem].ID == IID_LAB5_HEAD1) {
          ...
          set_seyan_state(pd);
          if (dat->cv_co &&
              (dat->cv_co != co || ch[dat->cv_co].serial != dat->cv_serial)) {
            say(cn, "%s, please be patient while I'm talking to others.",
                ch[co].name);
          }
        }
    ...
    if (msg->type == NT_CHAR) {
      ...
      // remove cv
      if (dat->cv_co) {
        if (!ch[dat->cv_co].flags || ch[dat->cv_co].serial != dat->cv_serial ||
            char_dist(cn, dat->cv_co) > 10 || !char_see_char(cn, dat->cv_co)) {
          dat->cv_co = 0;
        }
      }
      ...
      // get lab ppd
      pd = (struct lab5_player_data *)set_data(co, DRD_LAB5_PLAYER,
                                               sizeof(struct lab5_player_data));
      if (!pd) {
        remove_message(cn, msg);
        continue;
      }
      ...
      switch (pd->seyanstate) {
        ...
        case 0:
          say(cn,
              "Hello %s. I am here to introduce thee to the quest that has to "
              "be done here.",
              ch[co].name);
          didsay = 1;
          pd->seyanstate++;
        ...
        case 1:
          say(cn,
              "There are three Demons controlling this Labyrinth. Your mission "
              "is extremely simple: Destroy them. To prove their death, bring "
              "me their heads. Then thou art worthy to enter the next Gate.");
          didsay = 1;
          pd->seyanstate++;
          break;
        case 2:
          say(cn,
              "But I have to tell thee, that thou shouldst not carry any "
              "healing or mana potions, nor a combo potion with thee when "
              "entering here. If thou hast some, please deposit them in thine "
              "depot at the Gatekeeper's.");
          didsay = 1;
          pd->seyanstate++;
          break;
        case 3:
        ...
        case 4:
          say(cn,
              "Ah, and %s, thou mightst find a friend of mine here. Listen "
        ...
        // received something
        case 10:
          if (pd->seyangot == 1 || pd->seyangot == 2 || pd->seyangot == 4)
            say(cn, "Very well done, %s.", ch[co].name);
          if (pd->seyangot == 3 || pd->seyangot == 5 || pd->seyangot == 6)
            say(cn, "I'm impressed, %s.", ch[co].name);
          didsay = 1;
          pd->seyanstate++;
          break;
        case 11:
        ...
        case 20:
          say(cn,
              "%s, thou broughtst me the three Demon's heads and proved thine "
              "worth.",
              ch[co].name);
          didsay = 1;
          pd->seyanstate++;
        ...
        case 21:
          say(cn,
              "Now I will open a magic gate for thee. Use it, and thou wilt be "
              "able to travel to the next part of the Labyrinth.");
          didsay = 1;
          pd->seyanstate++;
          break;
        case 22:
...
void lab5_mage_driver(int cn, int ret, int lastact) {
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (dat->cv_co) {
        if (!ch[dat->cv_co].flags || ch[dat->cv_co].serial != dat->cv_serial ||
            char_dist(cn, dat->cv_co) > 7 || !char_see_char(cn, dat->cv_co)) {
          dat->cv_co = 0;
        }
      }
      ...
      switch (pd->magestate) {
        ...
        case 0:
          say(cn,
              "Hello %s. My name is Mathor, and I am the friend Laros surely "
              "mentioned.",
              ch[co].name);
          didsay = 1;
          pd->magestate++;
        ...
        case 1:
          say(cn,
              "It is the entrance room to the Master °c4Demons°c0 what thou "
              "see here. Those stone plates show their names. But thou have to "
              "find their real names written on similar plates somewhere "
              "behind those doors here.");
          didsay = 1;
          pd->magestate++;
          break;
        case 2:
          say(cn,
              ...
              "°c4force°c0 him to summon thee into his place, and fight him "
              "there. Thou might ask me for more details, if thou art "
              "interested.");
          didsay = 1;
          pd->magestate++;
          break;
        ...
        case 10:
          say(cn,
              "Well %s. To force a Master °c4Demon°c0 to summon thee into his "
              "place thou have to perform a certain °c4ritual°c0 first. But be "
              "very careful, %s. If thou makest only one mistake it might kill "
              "thee. The powers that are working here are strong.",
        ...
        case 20:
          say(cn,
              "Well %s, unfortunetaly those Master Demons can't be hurt by "
              "normal weapons. So make sure thou art properly equipped with a "
              "sacred stone weapon when fighting the Masters.",
              ch[co].name);
          didsay = 1;
          pd->magestate++;
          break;
        case 21:
          say(cn,
              "I have heard that those weapon might be found somewhere in the "
              "section behind the south western door of this room.");
          didsay = 1;
          pd->magestate++;
          break;
        case 22:
        ...
        case 30:
          say(cn,
              "Oh %s, it's a ritual of mighty powers thou art asking for. So "
              "listen carefully.",
              ch[co].name);
          didsay = 1;
          pd->magestate++;
        ...
        case 32:
          say(cn,
              "Second, thou hast to touch the correct stone plate of the "
              "Demons real name. Those thou hast to find.");
          didsay = 1;
          pd->magestate++;
          break;
        case 33:
          say(cn,
              "Third, thou hast to enter the inner square from the opposite "
              "entrance.");
          didsay = 1;
          pd->magestate++;
          break;
        case 34:
          say(cn,
              "Then place thineself in the center, and shout the real name of "
              "the Master.");
          didsay = 1;
          pd->magestate++;
          break;
        ...
        case 36:
          say(cn,
              "Ah, and thou couldst do it in any order, but may I suggest "
              "doing them from the east to the west.");
          didsay = 1;
          pd->magestate++;
          break;
        case 37:
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (strcasestr(str, "REPEAT")) {
        ...
        if (strcasestr(str, "SET 1")) {
          pd->ritualdaemon = 1;
          pd->ritualstate = 3;
          say(cn, "set %d %d (%s)", pd->ritualdaemon, pd->ritualstate,
              daemonreal[pd->ritualdaemon]);
      ...
      // inside square
      if (pd->ritualstate && ch[co].x > namecoordx[3] + 2 &&
          ch[co].x < namecoordx[1] - 2 && ch[co].y > namecoordy[2] + 2 &&
          ch[co].y < namecoordy[0] - 2 && strcasestr(str, ":")) {
        called = 0;

        if (strcasestr(str, "shouts:")) {
          if (strcasestr(str, daemonreal[1]))
            called = 1;
          else if (strcasestr(str, daemonreal[2]))
            called = 2;
          else if (strcasestr(str, daemonreal[3]))
            called = 3;
        } else if ((ch[co].flags & CF_GOD) && strcasestr(str, "SET"))
          called = pd->ritualdaemon;

        say(cn, "%d %d %d", pd->ritualdaemon, called, pd->ritualstate);

        if (pd->ritualstate == 3 && pd->ritualdaemon == called &&
            ch[co].x == namecoordx[2] && ch[co].y == namecoordy[1]) {
          if (ritual_start(co, pd->ritualdaemon)) {
            sound_area(ch[cn].x, ch[cn].y, 41);
            log_char(co, LOG_SYSTEM, 0, "°c3The Ritual of %s is fulfilled.°c0",
                     daemonreal[pd->ritualdaemon]);
            pd->ritualstate = 0;
          } else {
            log_char(
                co, LOG_SYSTEM, 0,
                "°c3Thou have to call again, but wait a while to do so!°c0");
            ch[co].endurance = ch[co].value[0][V_ENDURANCE] * POWERSCALE;
          }
        } else
          ritual_hurt(co, pd, namecoordx[pd->ritualdaemon],
                      namecoordy[pd->ritualdaemon]);
      }
    }
...
void lab5_daemon_driver(int cn, int ret, int lastact) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (dat->type == 1) {  // master
        if (ch[co].flags & CF_PLAYER && char_see_char(cn, co)) {
          if ((in = ch[co].item[WN_RHAND]) == 0 || it[in].ID != IID_LAB5_WEAPON)
            imm = 1;
          else if (imm == -1)
            imm = 0;
        }
      ...
      if (dat->type == 2) {  // gunned
        if (ch[co].flags & CF_PLAYER && ch[co].y < namecoordy[0] + 25 &&
            char_see_char(cn, co))
          fight_driver_add_enemy(cn, co, 1, 1);  // dat->aggressive=1;
      }
    }
  ...
  if (secure_move_driver(cn, ch[cn].tmpx, ch[cn].tmpy, dat->dir, ret, lastact))
    return;
...
void count_chestboxes(void) {
  ...
  if (numchestboxes > 255) {
    xlog("too many chestboxes in lab5.");
  }
...
int check_chestbox(int cn, int in) {
  ...
  dat = (unsigned char *)set_data(cn, DRD_LAB5_CHESTBOX, sizeofchestboxes);
  if (!dat) return 1;
...
void lab5_item(int in, int cn) {
  ...
  if (!cn) {
    ...
    if (drdata[0] == 2) {
      ...
      create_fireball(0, it[in].x + dx, it[in].y + dy, it[in].x + dx + dx,
                      it[in].y + dy + dy, 50);

      if (drdata[1] == 0) {
        drdata[1] = 1;
        call_item(it[in].driver, in, 0,
                  ticker + (((it[in].x + it[in].y) % 17) + 1) * TICKS);
      } else
    ...
    if (drdata[0] == 13) {
      ...
      create_ball(0, it[in].x + dx, it[in].y + dy, it[in].x + dx + dx,
                  it[in].y + dy + dy, 40);

      if (drdata[1] == 0) {
        drdata[1] = 1;
        call_item(it[in].driver, in, 0,
                  ticker + (((it[in].x + it[in].y) % 10) + 1) * TICKS);
      } else if (drdata[2] == 4) {
    ...
    if (drdata[0] == 5) {
      // get pd
      pd = (struct lab5_player_data *)set_data(cn, DRD_LAB5_PLAYER,
                                               sizeof(struct lab5_player_data));
      if (!pd) return;
      ...
      if (pd->ritualstate == 0) {
        ...
        log_char(cn, LOG_SYSTEM, 0,
                 "Thou canst read the symbols now. They form the words:");
        log_char(cn, LOG_SYSTEM, 0, "°c3The Ritual of %s started.°c0",
                 daemonname[pd->ritualdaemon]);
      } else
        ritual_hurt(cn, pd, it[in].x, it[in].y);
    }
    ...
    if (drdata[0] == 6) {
      ...
      if (pd->ritualstate == 1 && pd->ritualdaemon == drdata[1]) {
        ...
        log_char(cn, LOG_SYSTEM, 0,
                 "Thou canst read the symbols now. They form the words:");
        log_char(cn, LOG_SYSTEM, 0,
                 "°c3The ritual of %s is the Ritual of %s.°c0",
                 daemonname[pd->ritualdaemon], daemonreal[pd->ritualdaemon]);
      } else
        ritual_hurt(cn, pd, it[in].x, it[in].y);
    }
    ...
    if (drdata[0] == 7) {
      ...
      if (pd->ritualstate == 2 && pd->ritualdaemon == drdata[1]) {
        ...
        log_char(
            cn, LOG_SYSTEM, 0,
            "Mathor tells you: \"The ritual continues. Well done so far, %s.\"",
            ch[cn].name);
        // log_char(cn,LOG_SYSTEM,0,"°c3The Ritual of %s
        // continues.°c0",daemonname[pd->ritualdaemon]);
      } else {
        if (drdata[1] == 2)
          log_char(
              cn, LOG_SYSTEM, 0,
              "Mathor tells you: \"Sorry. But a strange power forced me.\"");
        ritual_hurt(cn, pd, namecoordx[hurttrans[(int)drdata[1]]],
                    namecoordy[hurttrans[(int)drdata[1]]]);
      }
    }
    ...
    if (drdata[0] == 11) {
      if (ch[cn].x < it[in].x && has_potion(cn)) {
        log_char(cn, LOG_SYSTEM, 0,
                 "°c3Thou canst not enter carrying a mana, healing or combo "
                 "potion!°c0");
        return;
      }

      if (ch[cn].x < it[in].x)
        teleport_char_driver(cn, it[in].x - 9, it[in].y - 7);
      else
        teleport_char_driver(cn, it[in].x + 9, it[in].y + 7);
    }
  }
