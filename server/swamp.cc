...
void swamparm(int in, int cn) {
  ...
  if (it[in].drdata[0])
    flag = 1;
  else if (map[it[in].x + 1 + it[in].y * MAXMAP].ch)
    flag = 2;
  else if (map[it[in].x + 2 + it[in].y * MAXMAP].ch)
    flag = 2;
  else if (map[it[in].x - 1 + it[in].y * MAXMAP].ch)
    flag = 2;
  else if (map[it[in].x - 2 + it[in].y * MAXMAP].ch)
    flag = 2;
  else if (map[it[in].x + 1 + it[in].y * MAXMAP + MAXMAP].ch)
    flag = 3;
  else if (map[it[in].x + 2 + it[in].y * MAXMAP + MAXMAP].ch)
    flag = 3;
  else if (map[it[in].x - 1 + it[in].y * MAXMAP + MAXMAP].ch)
    flag = 3;
  else if (map[it[in].x - 2 + it[in].y * MAXMAP + MAXMAP].ch)
  ...
  if (flag) {
    ...
    if (it[in].drdata[0] == 12) {
      if ((co = map[it[in].x + 1 + it[in].y * MAXMAP].ch))
        hurt(co, 10 * POWERSCALE, 0, 1, 50, 90);
      if ((co = map[it[in].x + 2 + it[in].y * MAXMAP].ch))
        hurt(co, 10 * POWERSCALE, 0, 1, 50, 90);
      if ((co = map[it[in].x - 1 + it[in].y * MAXMAP].ch))
        hurt(co, 10 * POWERSCALE, 0, 1, 50, 90);
      if ((co = map[it[in].x - 2 + it[in].y * MAXMAP].ch))
  ...
  for (y = yf; y <= yt; y += 8) {
    for (x = xf; x <= xt; x += 8) {
      for (co = getfirst_char_sector(x, y); co; co = ch[co].sec_next) {
        if (abs(ch[co].x - xc) <= dist && abs(ch[co].y - yc) <= dist &&
...
void swampspawn(int in, int cn) {
  ...
  static const char *name[] = {"swamp25n", "swamp27n", "swamp29n", "swamp31n"};
  ...
  if (!it[in].drdata[2] && co && (ch[co].flags) && ch[co].serial == ser) {
    call_item(it[in].driver, in, 0, ticker + TICKS);
    return;
  }
  ...
  if (it[in].drdata[2] || player_close(it[in].x, it[in].y, 4)) {
    ...
    if (map[m].gsprite >= 59405 && map[m].gsprite <= 59413)
      stop = 6;
    else if (map[m].gsprite >= 59414 && map[m].gsprite <= 59422)
      stop = 5;
    else if (map[m].gsprite >= 59423 && map[m].gsprite <= 59431)
      stop = 3;
    else
      stop = 7;
...
void clara_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct clara_driver_data *)set_data(cn, DRD_CLARADRIVER,
                                             sizeof(struct clara_driver_data));
  if (!dat) return;  // oops...
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (ppd) {
        switch (ppd->clara_state) {
          case 0:
            say(cn,
                "Greetings, %s! I am %s, First Sergeant of the Seyan'Du and "
                "commander of this outpost.",
                ch[co].name, ch[cn].name);
            ppd->clara_state++;
            didsay = 1;
            break;
          case 1:
            if (ppd->kelly_state >= 15)
              ppd->clara_state++;  // fall through...
            else
              break;
          case 2:
            say(cn,
                "I assume thou hast been sent from Aston, %s, to report on our "
                "status. The road through the swamp is no longer secure and we "
                "have been under attack from beasts emerging from the swamp.",
          ...
          case 3:
            say(cn,
                "Under the current circumstances, I do not recommend sending "
                "reinforcements to secure the road. We cannot afford to bind "
                "our forces here. Now go back to Aston and deliver this "
                "report.");
            ppd->clara_state++;
            didsay = 1;
            break;
          case 4:
            say(cn,
                "Afterwards come back here, I have more work for thee. That "
                "will be all, %s. Dismissed!",
                get_army_rank_string(co));
            ppd->clara_state++;
            didsay = 1;
            break;
          case 5:
            if (ppd->kelly_state >= 18)
              ppd->clara_state++;  // fall through...
            else
              break;  // waiting for player to reach kelly and come back
          case 6:
            say(cn,
                "I have a difficult mission for thee, %s. The main reason we "
                "had to retreat to this camp was one huge swamp beast. It "
                "seemed to be immune to our attacks.",
                ch[co].name);
            questlog_open(co, 21);
            ppd->clara_state++;
            didsay = 1;
          ...
          case 7:
            say(cn,
                "I want thee to find a way to slay it. I have heard rumors "
                "about a man who used to live with the swamp beasts north-east "
                "of this camp. Mayhap he knows a way to injure this beast.");
            ppd->clara_state++;
            didsay = 1;
            break;
          case 8:
            say(cn, "Dismissed, %s. And good luck. Thou wilt need it.",
                get_army_rank_string(co));
            ppd->clara_state++;
            didsay = 1;
            break;
          case 9:
            if ((in = ch[co].item[WN_RHAND]) && it[in].ID == IID_HARDKILL) {
              if (questlog_count(co, 21) == 0)
                give_military_pts(cn, co, 4, EXP_AREA15_HARDKILL);
              ppd->clara_state++;
            } else
              break;
          case 10:
            if ((in = ch[co].item[WN_RHAND]) && it[in].ID == IID_HARDKILL &&
                it[in].drdata[37] < 36)
              say(cn,
                  "So that is how one can kill them. Thou wilt need to find "
                  "all three stone circles and perform the ritual in each one, "
                  "then, %s.",
                  ch[co].name);
            else
              say(cn, "So that is how one can kill them.");
            ppd->clara_state++;
            didsay = 1;
            break;
          case 11:
          ...
          case 12:
            say(cn,
                "Now that thou knowest how to kill that beast, please go and "
                "do it.");
            ppd->clara_state++;
            didsay = 1;
            break;
          ...
          case 15:
            say(cn,
                "The swamp will be safer now, but more dangers await thee on "
                "thy travels. May Ishtar be with thee, %s.",
                ch[co].name);
            ppd->clara_state++;
            didsay = 1;
            break;
...
void monster_dead(int cn, int co) {
  ...
  if ((ch[cn].flags & CF_HARDKILL) &&
      (ppd = (struct area3_ppd *)set_data(co, DRD_AREA3_PPD,
                                          sizeof(struct area3_ppd))) &&
      ppd->clara_state >= 12 && ppd->clara_state <= 13) {
    ppd->clara_state = 14;
    log_char(co, LOG_SYSTEM, 0, "Well done. Clara will be proud of thee!");
  }

  if (ch[co].x >= 142 && ch[co].y >= 83 && ch[co].x <= 153 && ch[co].y <= 92)
    bit = 1;
  if (ch[co].x >= 34 && ch[co].y >= 150 && ch[co].x <= 44 && ch[co].y <= 160)
    bit = 2;
  if (ch[co].x >= 183 && ch[co].y >= 154 && ch[co].x <= 192 && ch[co].y <= 162)
    bit = 4;
