...
int char_see_char_nolos(int cn, int co) {
  ...
  if ((ch[cn].flags & (CF_GOD | CF_INFRARED)) == (CF_GOD | CF_INFRARED))
    return 1;
  ...
  if (ch[cn].prof[P_DARK] >= 30 && (ch[co].flags & CF_ALIVE))
    light = max(32, light);
  if (ch[cn].prof[P_LIGHT] >= 30 && (ch[co].flags & CF_UNDEAD))
    light = max(32, light);

  if (!light && (abs(ch[cn].x - ch[co].x) > 1 || abs(ch[cn].y - ch[co].y) > 1))
    return 0;

  if (dist < 3 && (ch[co].speed_mode != SM_STEALTH || !ch[co].prof[P_THIEF] ||
                   !(ch[co].flags & CF_THIEFMODE)))
    return 1;
  ...
  if (ch[co].speed_mode == SM_STEALTH)
    stealth = ch[co].value[0][V_STEALTH];
  else
    stealth = 0;

  // 0...64        4..400
  if (stealth)
    vco = light + stealth + dist;
  else
    vco = 0;

  vcn = ch[cn].value[0][V_PERCEPT] + 16 + 49;

  // if ((ch[co].flags&CF_PLAYER) && (ch[co].flags&CF_THIEFMODE)) say(cn,"cn=%s,
  // dist=%d, light=%d (%d), stealth=%d, vcn=%d,
  // vco=%d",ch[cn].name,dist,light,check_lightm(m),stealth,vcn,vco);
...
int char_see_char(int cn, int co) {
  ...
  // xlog("char_see_char (%d): %s (%d) %s
  // (%d)",ticker,ch[cn].name,cn,ch[co].name,co);
...
int char_see_item(int cn, int in) {
  ...
  if ((it[in].flags & IF_FRONTWALL) &&
      ((map[it[in].x + 1 + it[in].y * MAXMAP].flags &
        (MF_SIGHTBLOCK | MF_TSIGHTBLOCK)) ||
       !los_can_see(cn, ch[cn].x, ch[cn].y, it[in].x + 1, it[in].y, DIST)) &&
      ((map[it[in].x + it[in].y * MAXMAP + MAXMAP].flags &
        (MF_SIGHTBLOCK | MF_TSIGHTBLOCK)) ||
       !los_can_see(cn, ch[cn].x, ch[cn].y, it[in].x, it[in].y + 1, DIST)))
    return 0;
