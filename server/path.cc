...
int init_path(void)
{
  nmap = (struct node**)xcalloc(MAXMAP * MAXMAP * sizeof(struct node *), IM_BASE);
  if (!nmap) return 0;
  xlog("Allocated nmap: %.2fM (%lu*%d)", sizeof(struct node *)*MAXMAP * MAXMAP / 1024.0 / 1024.0, sizeof(struct node *), MAXMAP * MAXMAP);
  mem_usage += sizeof(struct node *)*MAXMAP * MAXMAP;

  nodes = (struct node*)xcalloc(sizeof(struct node) * MAXNODE, IM_BASE);
  if (!nodes) return 0;
  xlog("Allocated nodes: %.2fM (%lu*%d)", sizeof(struct node)*MAXNODE / 1024.0 / 1024.0, sizeof(struct node), MAXNODE);
