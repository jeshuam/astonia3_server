...
static int cost(int fx, int fy) { return xcost(fx, fy, tx1, ty1); }
...
int rect_check_target(int m) {
  ...
  if (x < path_rect_fx || x > path_rect_tx || y < path_rect_fy ||
      y > path_rect_ty)
    return 0;
...
int add_node(int x, int y, int dir, int ccost) {
  ...
  tcost =
      ccost + gcost;  // total cost is current cost (ccost) plus guessed cost

  if ((tmp = nmap[m]) !=
      NULL) {  // is there already a node on this map position?
    if (tmp->tcost <= tcost)
      return 0;  // other node is better or equal, no need to try this one

    if (!tmp->visited) {  // remove the old (worse) node from the list if we
                          // haven't visited it yet
      prev = tmp->prev;   // if we visited it already, it won't be on the list
                          // anyway
      next = tmp->next;
...
//#define dr_check_target(m)  (!((*(unsigned char*)(&map[m].flags))&((unsigned
//char)(MF_MOVEBLOCK|MF_TMOVEBLOCK))))

static void add_suc(struct node *node) {
  if (!node->dir) {
    if (dr_check_target(node->x + node->y * MAXMAP + 1))
      add_node(node->x + 1, node->y, DX_RIGHT, node->cost + 2);
    if (dr_check_target(node->x + node->y * MAXMAP - 1))
      add_node(node->x - 1, node->y, DX_LEFT, node->cost + 2);
    if (dr_check_target(node->x + node->y * MAXMAP + MAXMAP))
      add_node(node->x, node->y + 1, DX_DOWN, node->cost + 2);
    if (dr_check_target(node->x + node->y * MAXMAP - MAXMAP))
      add_node(node->x, node->y - 1, DX_UP, node->cost + 2);
    ...
    if (dr_check_target(node->x + node->y * MAXMAP + 1))
      add_node(node->x + 1, node->y, node->dir, node->cost + 2);
    if (dr_check_target(node->x + node->y * MAXMAP - 1))
      add_node(node->x - 1, node->y, node->dir, node->cost + 2);
    if (dr_check_target(node->x + node->y * MAXMAP + MAXMAP))
      add_node(node->x, node->y + 1, node->dir, node->cost + 2);
    if (dr_check_target(node->x + node->y * MAXMAP - MAXMAP))
      add_node(node->x, node->y - 1, node->dir, node->cost + 2);
...
int pathfinder(int fx, int fy, int tx, int ty, int mindist,
  ...
  ty1 = ty;
  if (check_target)
    dr_check_target = check_target;
  else
    dr_check_target = normal_check_target;

  // shortcut: if the target isn't reachable, there's not much sense in
  // trying...
...
int pathbestdist(void) {
  if (best)
    return abs(best->x - tx1) + abs(best->y - ty1);
  else
    return 0;
}
...
int pathnodes(void) { return maxnode; }

int init_path(void) {
  nmap =
      (struct node **)xcalloc(MAXMAP * MAXMAP * sizeof(struct node *), IM_BASE);
  if (!nmap) return 0;
  xlog("Allocated nmap: %.2fM (%lu*%d)",
       sizeof(struct node *) * MAXMAP * MAXMAP / 1024.0 / 1024.0,
       sizeof(struct node *), MAXMAP * MAXMAP);
  mem_usage += sizeof(struct node *) * MAXMAP * MAXMAP;

  nodes = (struct node *)xcalloc(sizeof(struct node) * MAXNODE, IM_BASE);
  if (!nodes) return 0;
  xlog("Allocated nodes: %.2fM (%lu*%d)",
       sizeof(struct node) * MAXNODE / 1024.0 / 1024.0, sizeof(struct node),
       MAXNODE);
