...
static int connected = 0, sock = -1, ilen = 0, state = 0;
...
static int connect_chat(void) {
  ...
  addr.sin_addr.s_addr =
      *(unsigned long *)(*he->h_addr_list);  // htonl((195<<24)+(50<<16)+(130<<8)+3);
...
int init_chat(void) {
  inbuf = (char *)xmalloc(INBUFSIZE, IM_BASE);
...
static void rec_chat(unsigned short channel, const char *text) {
  ...
  if (channel == 0) {
    ...
    for (n = 1; n < MAXCHARS; n++) {
      ...
      if (channel == 12 && cnr != get_char_clan(n) &&
          !clan_alliance(cnr, get_char_clan(n)))
        continue;
    ...
    for (n = 1; n < MAXCHARS; n++) {
      ...
      if (ret && coID) {  // server sends messages with ID 0 and does not need a
                          // received note
...
void join_chat(int cn, int nr) {
  ...
  if (ch[cn].channel & bit) {
    log_char(cn, LOG_SYSTEM, 0, "You have already joined channel %d (%s).", nr,
             cname[nr].name);
    return;
  }

  if (nr == 31 && !(ch[cn].flags & (CF_STAFF | CF_GOD))) {
    log_char(cn, LOG_SYSTEM, 0, "Permission denied to join channel %d (%s).",
             nr, cname[nr].name);
    return;
  }
  ...
  log_char(cn, LOG_SYSTEM, 0, "You have joined channel %d (%s).", nr,
           cname[nr].name);
}
...
void leave_chat(int cn, int nr) {
  ...
  if (!(ch[cn].channel & bit)) {
    log_char(cn, LOG_SYSTEM, 0, "You have already left channel %d (%s).", nr,
             cname[nr].name);
    return;
  }
  ...
  log_char(cn, LOG_SYSTEM, 0, "You have left channel %d (%s).", nr,
           cname[nr].name);
}
...
void list_chat(int cn) {
  ...
  for (n = 0; n < 33; n++) {
    if (cname[n].name)
      log_char(cn, LOG_SYSTEM, 0, "%2d: %-10.10s - %s", n, cname[n].name,
               cname[n].desc);
  }
}
...
static int write_chat(int cn, int channel, const char *text) {
  ...
  if ((ch[cn].flags & CF_PLAYER) && (channel == 0 || channel == 32) &&
      !(ch[cn].flags & CF_GOD)) {
    log_char(cn, LOG_SYSTEM, 0, "Access denied.");
    return 1;
  }

  if ((ch[cn].flags & CF_PLAYER) && channel == 31 &&
  ...
  if ((ch[cn].flags & CF_PLAYER) && channel) {
    ...
    if (!(ch[cn].channel & bit)) {
      log_char(cn, LOG_SYSTEM, 0,
               "You must join a channel before you can use it.");
      return 1;
    }
  }
  if ((ch[cn].flags & CF_PLAYER) && (channel == 7 || channel == 12) &&
      get_char_clan(cn) == 0) {
  ...
  if (ch[cn].flags & CF_PLAYER)
    dlog(cn, 0, "chat(%s): \"%s\"", cname[channel].name, text);

  if (ch[cn].flags & CF_STAFF) {
    for (n = 0; n < 75 && ch[cn].name[n]; n++)
      name[n] = toupper(ch[cn].name[n]);
    name[n] = 0;
  } else
    strcpy(name, ch[cn].name);

  if (ch[cn].flags & (CF_STAFF | CF_GOD))
    xID = 0;
  else
    xID = ch[cn].ID;
  ...
  if (channel == 0)
    sprintf(buf, "°c%d%s", col, text);  // announce
  else if (channel == 7 || channel == 12)
    sprintf(buf, "%010u:%02u:°c%d%s: °c17%s°c18 (%d) says: \"%s\"", xID,
            get_char_clan(cn), col, cname[channel].name, name, ch[cn].mirror,
            text);  // clan internal
  else if (channel == 13)
    sprintf(buf, "%010u:%02u:°c%d%s: °c17%s°c18 (%d) says: \"%s\"", xID,
            get_char_club(cn), col, cname[channel].name, name, ch[cn].mirror,
            text);  // club internal
  else if (channel == 8) {
    sprintf(buf, "%010u:%02u:°c%d%s: °c17%s°c18%s%s%s (%d) says: \"%s\"", xID,
            areaID, col, cname[channel].name, name,
            (ch[cn].flags & CF_STAFF) ? " [" : "",
            (ch[cn].flags & CF_STAFF) ? ch[cn].staff_code : "",
            (ch[cn].flags & CF_STAFF) ? "]" : "", ch[cn].mirror,
            text);  // area internal
  } else if (channel == 9) {
    sprintf(buf, "%010u:%02u:%02u:°c%d%s: °c17%s°c18%s%s%s (%d) says: \"%s\"",
            xID, areaID, areaM, col, cname[channel].name, name,
            (ch[cn].flags & CF_STAFF) ? " [" : "",
            (ch[cn].flags & CF_STAFF) ? ch[cn].staff_code : "",
            (ch[cn].flags & CF_STAFF) ? "]" : "", ch[cn].mirror,
            text);  // mirror internal
  } else if (channel == 4) {
    sprintf(buf, "%010u°c%d%s: °c17%s°c18%s%s%s (%s) says: \"%s\"", xID, col,
            cname[channel].name, name, (ch[cn].flags & CF_STAFF) ? " [" : "",
            (ch[cn].flags & CF_STAFF) ? ch[cn].staff_code : "",
            (ch[cn].flags & CF_STAFF) ? "]" : "", "OW",
            text);  // normal
  } else {
    sprintf(buf, "%010u°c%d%s: °c17%s°c18%s%s%s (%d) says: \"%s\"", xID, col,
            cname[channel].name, name, (ch[cn].flags & CF_STAFF) ? " [" : "",
            (ch[cn].flags & CF_STAFF) ? ch[cn].staff_code : "",
            (ch[cn].flags & CF_STAFF) ? "]" : "", ch[cn].mirror,
            text);  // normal
...
int tell_chat(int cnID, int coID, int staffmode, const char *format, ...) {
  ...
  if (staffmode)
    ret = send_chat(1030, buf);
  else
    ret = send_chat(1024, buf);
