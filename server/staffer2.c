...
void staffer_animation_book(int in, int cn)
{
  ...
  if (!(ppd = (struct staffer_ppd*)set_data(cn, DRD_STAFFER_PPD, sizeof(struct staffer_ppd)))) return;
...
void count_brannington_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct count_brannington_data*)set_data(cn, DRD_COUNTBRANDRIVER, sizeof(struct count_brannington_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if ((in = ch[cn].citem)) {	// we still have it
        ...
        ppd = (struct staffer_ppd*)set_data(co, DRD_STAFFER_PPD, sizeof(struct staffer_ppd));
        ppd2 = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
...
void brenneth_brannington_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct brenneth_brannington_data*)set_data(cn, DRD_BRENNETHBRANDRIVER, sizeof(struct brenneth_brannington_data));
  if (!dat) return;	// oops...
...
void spirit_brannington_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct spirit_brannington_data*)set_data(cn, DRD_SPIRITBRANDRIVER, sizeof(struct spirit_brannington_data));
  if (!dat) return;	// oops...
...
void forest_brannington_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct forest_brannington_data*)set_data(cn, DRD_FORESTBRANDRIVER, sizeof(struct forest_brannington_data));
  if (!dat) return;	// oops...
...
void countessa_brannington_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct countessa_brannington_data*)set_data(cn, DRD_COUNTESSABRANDRIVER, sizeof(struct countessa_brannington_data));
  if (!dat) return;	// oops...
...
void daughter_brannington_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct daughter_brannington_data*)set_data(cn, DRD_DAUGHTERBRANDRIVER, sizeof(struct daughter_brannington_data));
  if (!dat) return;	// oops...
...
void guard_brannington_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct spirit_brannington_data*)set_data(cn, DRD_GUARDBRANDRIVER, sizeof(struct spirit_brannington_data));
  if (!dat) return;	// oops...
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (ppd) {
        ...
        case 5:         quiet_say(cn, "Your mission, %s is to find out who is up there, and if they are friendly or hostile.", get_army_rank_string(co));
          ...
          questlog_open(co, 64);
          break;
        case 6:		appd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
...
void broklin_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct broklin_data*)set_data(cn, DRD_BROKLINDRIVER, sizeof(struct broklin_data));
  if (!dat) return;	// oops...
...
void grinnich_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct grinnich_data*)set_data(cn, DRD_GRINNICHDRIVER, sizeof(struct grinnich_data));
  if (!dat) return;	// oops...
...
void shanra_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct shanra_data*)set_data(cn, DRD_SHANRADRIVER, sizeof(struct shanra_data));
  if (!dat) return;	// oops...
