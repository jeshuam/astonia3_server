...
int set_item_map(int in, int x, int y) {
  ...
  if (!it[in].flags) {
    elog("set_item(): trying to drop unused item %s (%d) at %d,%d", it[in].name,
         in, x, y);
    return 0;
  }
...
int remove_item_map(int in) {
  ...
  if (it[in].x < 1 || it[in].x >= MAXMAP - 1 || it[in].y < 1 ||
      it[in].y >= MAXMAP - 1) {
    elog(
        "remove_item(): Item %s (%d) thinks it is on pos %d,%d, but that pos "
        "is out of bounds.",
  ...
  if (map[m].it != in) {
    elog(
        "remove_item(): Item %s (%d) thinks it is on pos %d,%d, but it is not.",
        it[in].name, in, it[in].x, it[in].y);
    it[in].x = it[in].y = 0;
    return 1;  // we return success anyway. we did remove it, sort of...
  }
  ...
  if (it[in].flags & IF_MOVEBLOCK) {
    if (!(map[m].flags & MF_TMOVEBLOCK))
      elog(
          "remove_item(): Item %s (%d) has IF_MOVEBLOCK set but the map on "
          "it's position %d,%d has no MF_TMOVEBLOCK",
          it[in].name, in, it[in].x, it[in].y);
    else
  ...
  if (it[in].flags & IF_SIGHTBLOCK) {
    if (!(map[m].flags & MF_TSIGHTBLOCK))
      elog(
          "remove_item(): Item %s (%d) has IF_SIGHTBLOCK set but the map on "
          "it's position %d,%d has no MF_TSIGHTBLOCK",
          it[in].name, in, it[in].x, it[in].y);
    else
...
int remove_item_char(int in) {
  ...
  if (cn < 1 || cn >= MAXCHARS) {
    elog(
        "remove_item_char(): Item %s (%d) thinks it is carried by character "
        "%d, but that number is out of bounds.",
        it[in].name, in, cn);
    it[in].carried = 0;
  ...
  if (ch[cn].citem == in) {
    ...
    if (n == INVENTORYSIZE) {
      elog(
          "remove_item(): Item %s (%d) thinks it is carried by character %s "
          "(%d), but it is not.",
          it[in].name, in, ch[cn].name, cn);
      // we're going to return success anyway: the character does NOT carry the
      // item anymore
    } else
      ch[cn].item[n] = 0;
    ch[cn].flags |= CF_ITEMS;
...
int replace_item_char(int old, int new_item) {
  int cn, n;
  ...
  if (new_item < 1 || new_item >= MAXITEM) {
    elog("replace_item(): illegal new item number %d", new_item);
    return 0;
  }
  ...
  if (cn < 1 || cn >= MAXCHARS) {
    elog(
        "replace_item(): Item %s (%d) thinks it is carried by character %d, "
        "but that number is out of bounds.",
        it[old].name, old, cn);
  ...
  if (ch[cn].citem == old) {
    ch[cn].citem = new_item;
    ch[cn].flags |= CF_ITEMS;
    it[old].carried = 0;
    it[new_item].carried = cn;
    return 1;
  } else {
    for (n = 0; n < INVENTORYSIZE; n++)
      if (ch[cn].item[n] == old) break;
    if (n == INVENTORYSIZE) {
      elog(
          "replace_item(): Item %s (%d) thinks it is carried by character %s "
          "(%d), but it is not.",
          it[old].name, old, ch[cn].name, cn);
      btrace("replace_item_char");
      return 0;
    }

    ch[cn].item[n] = new_item;
    ch[cn].flags |= CF_ITEMS;
    it[old].carried = 0;
    it[new_item].carried = cn;
...
int remove_char(int cn) {
  ...
  if (ch[cn].x < 1 || ch[cn].x >= MAXMAP - 1 || ch[cn].y < 1 ||
      ch[cn].y >= MAXMAP - 1) {
    elog(
        "remove_char(): Char %s (%d) thinks it is on pos %d,%d, but that pos "
        "is out of bounds.",
    ...
    if (map[m].ch != cn) {
      elog(
          "remove_char(): Char %s (%d) thinks it is on pos %d,%d, but it is "
          "not.",
          ch[cn].name, cn, ch[cn].x, ch[cn].y);
      ch[cn].x = ch[cn].y = 0;
      ...
      if (!(map[m].flags & MF_TMOVEBLOCK))
        elog(
            "remove_char(): Char %s (%d) was on the map but the map on it's "
            "position %d,%d has no MF_TMOVEBLOCK",
            ch[cn].name, cn, ch[cn].x, ch[cn].y);
      else
        map[m].flags &= ~MF_TMOVEBLOCK;
  ...
  if (ch[cn].tox < 1 || ch[cn].tox >= MAXMAP - 1 || ch[cn].toy < 1 ||
      ch[cn].toy >= MAXMAP - 1) {
    if (ch[cn].tox)
      elog(
          "remove_char(): Char %s (%d) thinks it is going to pos %d,%d, but "
          "that pos is out of bounds.",
    ...
    if (ch[cn].tox) {
      if (!(map[m].flags & MF_TMOVEBLOCK))
        elog(
            "remove_char(): Char %s (%d) was moving to position %d,%d that has "
            "no MF_TMOVEBLOCK",
            ch[cn].name, cn, ch[cn].tox, ch[cn].toy);
      else
        map[m].flags &= ~MF_TMOVEBLOCK;
...
int drop_char(int cn, int x, int y, int nosteptrap) {
  ...
  if (x < MAXMAP - 1 && y < MAXMAP - 1 &&
      set_char(cn, x + 1, y + 1, nosteptrap))
    return 1;
  if (x > 1 && y < MAXMAP - 1 && set_char(cn, x - 1, y + 1, nosteptrap))
    return 1;
  if (x < MAXMAP - 1 && y > 1 && set_char(cn, x + 1, y - 1, nosteptrap))
    return 1;
...
int drop_char_extended(int cn, int x, int y, int maxdist) {
  ...
  for (dx = 1; dx < maxdist; dx++) {
    // direct neighbors
    if (x + dx < MAXMAP - 1 &&
        pathfinder(x, y, x + dx, y, 0, path_ignore_char, 20) != -1 &&
        set_char(cn, x + dx, y, 0))
      return 1;
    if (y + dx < MAXMAP - 1 &&
        pathfinder(x, y, x, y + dx, 0, path_ignore_char, 20) != -1 &&
        set_char(cn, x, y + dx, 0))
      return 1;
    if (x - dx > 1 &&
        pathfinder(x, y, x - dx, y, 0, path_ignore_char, 20) != -1 &&
        set_char(cn, x - dx, y, 0))
    ...
    for (dy = 1; dy < maxdist; dy++) {
      // diagonal neighbors
      if (x + dx < MAXMAP - 1 && y + dy < MAXMAP - 1 &&
          pathfinder(x, y, x + dx, y + dy, 0, path_ignore_char, 20) != -1 &&
          set_char(cn, x + dx, y + dy, 0))
        return 1;
      if (x - dx > 1 && y + dy < MAXMAP - 1 &&
          pathfinder(x, y, x - dx, y + dy, 0, path_ignore_char, 20) != -1 &&
          set_char(cn, x - dx, y + dy, 0))
        return 1;
      if (x + dx < MAXMAP - 1 && y - dy > 1 &&
          pathfinder(x, y, x + dx, y - dy, 0, path_ignore_char, 20) != -1 &&
          set_char(cn, x + dx, y - dy, 0))
        return 1;
      if (x - dx > 1 && y - dy > 1 &&
...
int drop_item_extended(int in, int x, int y, int maxdist) {
  ...
  for (dx = 1; dx < maxdist; dx++) {
    ...
    if (x + dx < MAXMAP - 1 &&
        pathfinder(x, y, x + dx, y, 0, path_ignore_char, 20) != -1 &&
        set_item_map(in, x + dx, y))
      return 1;

    // xlog("trying %d,%d",0,dx);
    if (y + dx < MAXMAP - 1 &&
        pathfinder(x, y, x, y + dx, 0, path_ignore_char, 20) != -1 &&
        set_item_map(in, x, y + dx))
      return 1;
    ...
    for (dy = 1; dy <= dx; dy++) {
      ...
      if (x + dy < MAXMAP - 1 && y + dx < MAXMAP - 1 &&
          pathfinder(x, y, x + dy, y + dx, 0, path_ignore_char, 20) != -1 &&
          set_item_map(in, x + dy, y + dx))
        return 1;
      // xlog("trying %d,%d",-dy,dx);
      if (x - dy > 1 && y + dx < MAXMAP - 1 &&
          pathfinder(x, y, x - dy, y + dx, 0, path_ignore_char, 20) != -1 &&
          set_item_map(in, x - dy, y + dx))
        return 1;
      // xlog("trying %d,%d",dy,-dx);
      if (x + dy < MAXMAP - 1 && y - dx > 1 &&
          pathfinder(x, y, x + dy, y - dx, 0, path_ignore_char, 20) != -1 &&
          set_item_map(in, x + dy, y - dx))
        return 1;
      // xlog("trying %d,%d",-dy,-dx);
...
int item_drop_char(int in, int cn) {
  ...
  if (x < MAXMAP - 1 && y < MAXMAP - 1 && set_char(cn, x + 1, y + 1, 0))
    return 1;
  ...
  if (x < MAXMAP - 2 && y < MAXMAP - 1 && set_char(cn, x + 2, y + 1, 0))
    return 1;
  if (x < MAXMAP - 1 && y < MAXMAP - 2 && set_char(cn, x + 1, y + 2, 0))
    return 1;
  if (x < MAXMAP - 2 && y < MAXMAP - 2 && set_char(cn, x + 2, y + 2, 0))
    return 1;
  if (x < MAXMAP - 2 && y < MAXMAP - 2 && set_char(cn, x + 2, y + 2, 0))
    return 1;
...
void check_map(void) {
  ...
  for (cn = 1; cn < MAXCHARS; cn++) {
    if (ch[cn].flags) {
      elog("Unlinked character %d, name=%s, pos=%d,%d (%llX)", cn, ch[cn].name,
           ch[cn].x, ch[cn].y, ch[cn].flags);
    }
  }
  for (in = 1; in < MAXITEM; in++) {
    if (it[in].flags && !(it[in].flags & IF_VOID)) {
      elog("Unlinked item %d, name=%s, pos=%d,%d, carried=%d, container=%d", in,
           it[in].name, it[in].x, it[in].y, it[in].carried, it[in].contained);
    }
  }
