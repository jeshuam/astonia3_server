...
int swearing(int cn, const char *text) {
  ...
  ppd =
      (struct swear_ppd *)set_data(cn, DRD_SWEAR_PPD, sizeof(struct swear_ppd));
  if (!ppd) return 0;  // oops...

  if (ppd->banned_till > realtime) {
    log_char(cn, LOG_SYSTEM, 0, "°c3Chat is blocked for %.2f minutes.",
             (ppd->banned_till - realtime) / 60.0);
    return 1;
  }
  ...
  if (realtime - ppd->lasttalk[1] < 1) {  // 0.3s per line
    log_char(
        cn, LOG_SYSTEM, 0,
        "°c3Chat has been blocked for 30 seconds for excessive usage (1).");
    ppd->bad = realtime;
    return 1;
  }
  if (realtime - ppd->lasttalk[4] < 10) {  // 2s per line
    log_char(
        cn, LOG_SYSTEM, 0,
        "°c3Chat has been blocked for 30 seconds for excessive usage (2).");
    ppd->bad = realtime;
    return 1;
  }
  if (realtime - ppd->lasttalk[9] < 30) {  // 3s per line
    log_char(
        cn, LOG_SYSTEM, 0,
        "°c3Chat has been blocked for 30 seconds for excessive usage (3).");
    ppd->bad = realtime;
    return 1;
  }

  if (strcasestr(text, "fuck") || strcasestr(text, "cunt") ||
      strcasestr(text, "faggot")) {
    log_char(cn, LOG_SYSTEM, 0,
             "°c3Swearing is illegal in this game. While only a few words are "
             "blocked by the system, you will get punished and eventually "
             "banned if you swear using non-blocked words.");
  ...
  if (strlen(text) > 3 && all_upper(text)) {
    log_char(cn, LOG_SYSTEM, 0,
             "°c3Using capitalized letters only is impolite. Trying to get "
             "around the block by using mostly caps will get you punished and "
             "eventually banned.");
    log_char(cn, LOG_SYSTEM, 0, "°c3Chat has been blocked for 30 seconds.");
    ppd->bad = realtime;
    return 1;
  ...
  if (strlen(text) > 20) {
    for (n = flag = 0; n < 10; n++) {
      if (!strncmp(ppd->last_sentence[n], text, 78) &&
          realtime - ppd->last_time[n] < 30) {
        if (ppd->last_cnt[n] > 2 || realtime - ppd->last_time[n] < 4) {
          log_char(cn, LOG_SYSTEM, 0,
                   "°c3Repeating the same sentence is impolite. Repeating "
                   "variants of the same sentence will get you punished and "
                   "eventually banned.");
          log_char(cn, LOG_SYSTEM, 0,
                   "°c3Chat has been blocked for 30 seconds.");
          ppd->bad = realtime;
          return 1;
        }
        ppd->last_cnt[n]++;
    ...
    if (!flag) {
      ...
      strncpy(ppd->last_sentence[ppd->last_pos], text, 78);
      ppd->last_sentence[ppd->last_pos][78] = 0;
      ppd->last_time[ppd->last_pos] = realtime;
  ...
  for (n = 9; n > 0; n--) ppd->lasttalk[n] = ppd->lasttalk[n - 1];
...
void shutup_bg(int cnID, int coID, int minutes) {
  ...
  ppd =
      (struct swear_ppd *)set_data(co, DRD_SWEAR_PPD, sizeof(struct swear_ppd));
  if (!ppd) return;  // oops...

  ppd->banned_till = realtime + (minutes * 60);

  if (minutes > 0)
    log_char(co, LOG_SYSTEM, 0, "°c3Your ability to talk has been disabled.");
  else
    log_char(co, LOG_SYSTEM, 0, "°c3Your ability to talk has been enabled.");
