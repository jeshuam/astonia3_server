...
static void respawn_callback(int tmp, int tmpx, int tmpy, int tmpa, int dum) {
  ...
  if (char_driver(ch[cn].driver, CDT_RESPAWN, cn, 0, 0) == 1 &&
      set_char(cn, tmpx, tmpy, 0)) {
    update_char(cn);
    ...
    if (ch[cn].lifeshield < 0) {
      elog("respawn_callback(): lifeshield=%d (%X) for char %d (%s). fixed.",
           ch[cn].lifeshield, ch[cn].lifeshield, cn, ch[cn].name);
      ch[cn].lifeshield = 0;
    }
...
void log_player_death(int cn, const char *reason, int itemflag) {
  ...
  dlog(cn, 0,
       "Death: Lag Control=%s, Settings: Maxlag=%d, Bless=%s, Fire=%s, "
       "Flash=%s, Health Pot=%s, Mana Pot=%s, Combo Pot=%s, Recall=%s, Magic "
       "Shield=%s, Warcry=%s",
       ch[cn].driver == CDR_LOSTCON ? "Yes" : "No", ppd->maxlag,
       ppd->nobless ? "off" : "on", ppd->nofireball ? "off" : "on",
       ppd->noflash ? "off" : "on", ppd->nolife ? "off" : "on",
       ppd->nomana ? "off" : "on", ppd->nocombo ? "off" : "on",
       ppd->norecall ? "off" : "on", ppd->noshield ? "off" : "on",
       ppd->nowarcry ? "off" : "on");
  ...
  if (itemflag) {
    for (n = 0; n < INVENTORYSIZE; n++) {
      ...
      if (n < 30)
        dlog(cn, in, "item in grave (worn)");
      else
        dlog(cn, in, "item in grave (inventory)");
    }
...
struct firstkill_ppd  // persistent player data
    {
  unsigned int
      kill[32];  // bitfield, containing ones for all kills already done
};
...
static void give_first_kill(int cn, int co) {
  ...
  if (ch[co].cls < 1 || ch[co].cls > 1023)
    return;  // victim has no class set, so no first kill...

  if (!(ppd = (struct firstkill_ppd *)set_data(cn, DRD_FIRSTKILL_PPD,
                                               sizeof(struct firstkill_ppd))))
    return;  // OOPS
  ...
  if (ch[co].flags & CF_HASNAME) {
    log_char(cn, LOG_SYSTEM, 0,
             "You just killed %s for the first time. Congratulations!",
             ch[co].name);
             ...
             (ch[co].cls >= 336 &&
              ch[co].cls <= 365) ||  // bones, skelly, zombie, wizard
             (ch[co].cls >= 388 && ch[co].cls <= 403)) {  // hell pents demons
    log_char(cn, LOG_SYSTEM, 0,
             "You just killed your first level %d %s. Congratulations!",
             ch[co].level, ch[co].name);
  } else if ((ch[co].cls >= 258 && ch[co].cls <= 305) ||
             (ch[co].cls >= 404 && ch[co].cls <= 411)) {
    if (get_army_rank_int(cn)) {
      log_char(cn, LOG_SYSTEM, 0,
               "You just killed your first level %d %s! The Governor will be "
               "proud of you.",
               ch[co].level, ch[co].name);
      give_military_pts_no_npc(cn, min(ch[co].level / 3, 10),
                               kill_score(co, cn) * 15);
    } else
      log_char(cn, LOG_SYSTEM, 0, "You just killed your first level %d %s!",
               ch[co].level, ch[co].name);
  } else {
    log_char(cn, LOG_SYSTEM, 0,
             "You just killed your first %s. Congratulations!", ch[co].name);
  }
}
...
void check_military_solve(int cn, int co) {
  ...
  if (!(ppd = (struct military_ppd *)set_data(cn, DRD_MILITARY_PPD,
  ...
  if (ppd->took_mission && !ppd->solved_mission) {
    ...
    switch (ppd->mis[nr].type) {
      case 1:
        if ((ch[co].cls >= 52 && ch[co].cls <= 84) ||
            (ch[co].cls >= 107 && ch[co].cls <= 170) ||
            (ch[co].cls >= 388 && ch[co].cls <= 403)) {  // its a pent demon
          if (ch[co].level == ppd->mis[nr].opt2) {
            ...
            if (ppd->mis[nr].opt1) {
              if (ppd->mis[nr].opt1 < 10 ||
                ...
                log_char(cn, LOG_SYSTEM, 0, "°c1Mission kill, %d to go.",
                         ppd->mis[nr].opt1);
              }
            } else {
              log_char(cn, LOG_SYSTEM, 0,
                       "You solved your mission. Talk to the governor to claim "
                       "your reward.");
        ...
        break;
      case 2:
        if (ch[co].cls >= 85 && ch[co].cls <= 100) {  // its a sewer ratling
          if (ch[co].level == ppd->mis[nr].opt2) {
            ppd->mis[nr].opt1--;
            if (ppd->mis[nr].opt1) {
              if (ppd->mis[nr].opt1 < 10 ||
...
int kill_char(int cn, int co) {
  ...
  if (ch[cn].flags & CF_RESPAWN) {
    ...
    set_timer(ticker + ch[cn].respawn, respawn_callback, ch[cn].tmp,
              ch[cn].tmpx, ch[cn].tmpy, ch[cn].tmpa, 0);
  }
  ...
  if (ch[co].flags & CF_PLAYER)
    ch[cn].act2 = 1;  // remember if PK death (to avoid races due to log-outs)
  else
    ch[cn].act2 = 0;
...
static int transfer_to_restarea(int cn) {
  ...
  if (ch[cn].resta != areaID) {  // respawn point is on different area server
    // if the target server is there, send player there. if it isn't, let him
    // remain here
  ...
  if (ch[cn].resta != areaID ||
      !drop_char(cn, ch[cn].restx, ch[cn].resty,
                 0)) {  // yuck! no space to drop char!
...
int death_loss(int total_exp) { return total_exp / 25; }
...
int die_char(int cn, int co, int ispk) {
  ...
  if (in) {
    if (!drop_grave(in, x, y, (ch[cn].flags & CF_PLAYER) != 0)) {
      ...
      if (ch[cn].flags & CF_PLAYER) {
        dlog(cn, 0, "drop body succeeded at %d,%d for item %d", it[in].x,
             it[in].y, in);
      }
      if (!(it[in].flags & IF_TAKE)) {
        if (ch[cn].flags & CF_ITEMDEATH)
          set_expire(in, NPC_BODY_DECAY_TIME);
        else if (ch[cn].flags & CF_PLAYER)
          set_expire_body(in, PLAYER_BODY_DECAY_TIME);
        else {
          if (areaID != 32)
            set_expire_body(in, NPC_BODY_DECAY_TIME);
          else
            set_expire_body(in, NPC_BODY_DECAY_TIME_AREA32);
        }
      } else
  ...
  if (ch[cn].flags & CF_PLAYER) {
    if (ispk) {
      log_char(cn, LOG_SYSTEM, 0,
               "Thou died by the hands of a player. Thou did not lose any "
               "experience points.");
      ...
      if (loss)
        log_char(cn, LOG_SYSTEM, 0,
                 "Thou died and lost some experience points.");
      else
        log_char(cn, LOG_SYSTEM, 0,
                 "Thou died, but since thou art still a Newbie, thou did not "
                 "lose any experience points.");
...
static int god_save_char(int cn) {
  ...
  log_char(cn, LOG_SYSTEM, 0, "Thou hast %s saves left.",
...
static int shutdown_save_char(int cn) {
  ...
  if (loss)
    log_char(cn, LOG_SYSTEM, 0,
             "Thou died and lost some experience points. Because of the coming "
             "shutdown, you did not lose your items.");
  else
    log_char(cn, LOG_SYSTEM, 0,
             "Thou died, but since thou art still a Newbie, thou did not lose "
             "any experience points. Because of the coming shutdown, you did "
             "not lose your items. Consider yourself lucky.");
...
static int area_save_char(int cn) {
  ...
  if (loss)
    log_char(cn, LOG_SYSTEM, 0,
             "Thou died and lost some experience points. Because of the warped "
             "ways of the area, you did not lose your items.");
  else
    log_char(cn, LOG_SYSTEM, 0,
             "Thou died, but since thou art still a Newbie, thou did not lose "
             "any experience points. Because of the warped ways of the area, "
             "you did not lose your items. Consider yourself lucky.");
...
static int arena_save_char(int cn) {
  log_char(cn, LOG_SYSTEM, 0,
           "Yuck. If you hadn't been in an arena... You'd be dead by now. Ave "
...
int allow_body(int cn, char *name) {
  ...
  if (coID == 0) { /*log_char(cn,LOG_SYSTEM,0,"Please repeat.");*/
    return 0;
  }
  if (coID == -1) {
...
int hurt(int cn, int dam, int cc, int armordiv, int armorper, int shieldper) {
  ...
  if (dam < 0) {
    elog("got negative damage %d to %s (%d) from %s (%d). Set to 0.", dam,
         ch[cn].name, cn, ch[cc].name, cc);
    dam = 0;
  }

  if (dam && showattack)
    say(cn, "hurt by %s, dam=%.2f, armor=%.2f armorper=%d shieldper=%d",
        ch[cc].name, dam / 1000.0, ch[cn].value[0][V_ARMOR] / 20.0 / armordiv,
        armorper, shieldper);

  str = dam * armorper / 100;
  if (str > ch[cn].value[0][V_ARMOR] * POWERSCALE / 20 / armordiv)
    dam -= ch[cn].value[0][V_ARMOR] * POWERSCALE / 20 / armordiv;
  else
    dam -= str;
  if (dam < 0) dam = 0;
  ...
  if (ch[cn].flags & CF_HARDKILL) {
    ...
    if (!cc || !(in = ch[cc].item[WN_RHAND]) || it[in].ID != IID_HARDKILL ||
        it[in].drdata[37] < ch[cn].level)
      dam = 0;
  }
  ...
  if (dam) {  // check for magic shield and death if there was any damage
    ...
    if (ch[cn].lifeshield && shieldper > 0) {
      if (ch[cn].value[1][V_MAGICSHIELD]) {
        ...
        if (n == 4)
          create_show_effect(EF_MAGICSHIELD, cn, ticker, ticker + 3, 16, 0);
      }

      str = dam * shieldper / 100;

      if (ch[cn].lifeshield >= str)
        ch[cn].lifeshield -= str;
      else {
    ...
    if (areaID == 34 && (ch[cn].flags & CF_PLAYER) && cc &&
        (ch[cc].flags & CF_PLAYER) &&
        (map[ch[cn].x + ch[cn].y * MAXMAP].flags & MF_ARENA))
      teufel_damage(cn, cc, teufel_life + teufel_shield);

    if (dam >= POWERSCALE && (ch[cn].flags & CF_PLAYER)) {
      if (ch[cn].flags & CF_MALE)
        sound_area(ch[cn].x, ch[cn].y, 9);  // ouch
      else
        sound_area(ch[cn].x, ch[cn].y, 32);
    }
    ...
    if (ch[cn].hp < (POWERSCALE / 2)) {
      if (ch[cn].flags & CF_PLAYER) {
        log_char(cn, LOG_SYSTEM, 0, "Killed with %.2f damage by a lvl %d %s.",
                 dam / 1000.0, cc ? ch[cc].level : 0,
                 cc ? ch[cc].name : "unknown causes");
        if (ch[cn].flags & CF_MALE)
          sound_area(ch[cn].x, ch[cn].y, 4);
        else
          sound_area(ch[cn].x, ch[cn].y, 33);
      }
      if (ch[cn].flags & CF_NODEATH) {
        ch[cn].hp = 1;
      } else {
        if (areaID == 34 && (ch[cn].flags & CF_PLAYER) &&
            (map[ch[cn].x + ch[cn].y * MAXMAP].flags & MF_ARENA))
          teufel_death(cn, cc);  // teufelheim PK special
        else if ((areaID == 20 || areaID == 35) &&
                 (ch[cn].flags & CF_PLAYER)) {  // LQ area special
          struct misc_ppd *ppd;
          ...
          log_char(cn, LOG_SYSTEM, 0,
                   "You lose. You may enter again after a 5 minute penalty.");
          ...
          log_char(cn, LOG_SYSTEM, 0,
                   "Fortunately, death is not real here. But neither is "
                   "experience.");
          ch[cn].hp = 5 * POWERSCALE;
        } else if (ch[cn].flags & (CF_PLAYER | CF_PLAYERLIKE) &&
                   (map[ch[cn].x + ch[cn].y * MAXMAP].flags &
                    MF_ARENA)) {  // arena death, no loss
          arena_save_char(cn);
        } else if (cc && (ch[cn].flags & CF_PLAYER) &&
                   (ch[cc].flags & CF_PLAYER)) {  // PK death, no saves
          kill_char(cn, cc);
          add_pk_kill(cc, cn);
          add_pk_death(cn);
        } else {  // normal death, check for saves
          if ((ch[cn].flags & CF_PLAYER) && ch[cn].saves > 0)
            god_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && shutdown_at &&
                   realtime - shutdown_at < 30)
            shutdown_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 25 && ch[cn].x > 107)
            area_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 22 &&
                   ch[cn].x >= 119 && ch[cn].y >= 63 && ch[cn].x <= 133 &&
                   ch[cn].y <= 114)
            area_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 12 && ch[cn].x < 96)
            area_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 31 &&
                   ch[cn].x < 66 && ch[cn].y > 126)
            area_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 32)
            area_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 33)
            area_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 11 &&
                   ch[cn].x >= 138 && ch[cn].x <= 147 && ch[cn].y >= 49 &&
                   ch[cn].y <= 58)
            area_save_char(cn);
          else if ((ch[cn].flags & CF_PLAYER) && areaID == 36 &&
                   get_section(ch[cn].x, ch[cn].y) >= 149 &&
                   get_section(ch[cn].x, ch[cn].y) <= 152)
            area_save_char(cn);
          else {
            kill_char(cn, cc);
          }
...
int kill_score_level(int cnlev, int cclev) {
  ...
  switch (diff) {
    case -4:
      return ceil(val * 0.95);
    case -3:
      return ceil(val * 0.90);
    case -2:
      return ceil(val * 0.85);
    case -1:
      return ceil(val * 0.80);
    case 0:
      return ceil(val * 0.75);
    case 1:
      return ceil(val * 0.70);
    case 2:
      return ceil(val * 0.65);
    case 3:
      return ceil(val * 0.60);
    case 4:
      return ceil(val * 0.40);
    case 5:
      return ceil(val * 0.20);
    default:
      return 0;
  }
}
...
void delayed_hurt_callback(int cn, int serial, int dam, int armorper,
                           int shieldper) {
...
int delayed_hurt(int delay, int cn, int dam, int armorper, int shieldper) {
  return set_timer(ticker + delay, delayed_hurt_callback, cn, ch[cn].serial,
                   dam, armorper, shieldper);
}
