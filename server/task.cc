...
int set_clan_rank(struct set_clan_rank_data *set, struct character *tmp,
  ...
  if (set->clan < CLUBOFFSET) {
    if (tmp->clan != set->clan) {
      tell_chat(0, set->master_ID, 1,
                "%s is not a member of your clan, you cannot set the rank.",
                tmp->name);
      return 0;
    }
    if (!(tmp->flags & CF_PAID) && set->rank > 1) {
      tell_chat(
          0, set->master_ID, 1,
          "%s is not a paying player, you cannot set the rank higher than 1.",
          tmp->name);
      return 0;
    }
    tmp->clan_rank = set->rank;

    add_clanlog(set->clan, clan_serial(set->clan), tmp->ID, 30,
                "%s rank was set to %d by %s", tmp->name, set->rank,
                set->master_name);

    tell_chat(0, set->master_ID, 1, "Set %s's rank to %d.", tmp->name,
              set->rank);
  } else {
    if (tmp->clan != set->clan) {
      tell_chat(0, set->master_ID, 1,
                "%s is not a member of your club, you cannot set the rank.",
                tmp->name);
      return 0;
    }
    if (!(tmp->flags & CF_PAID) && set->rank > 0) {
      tell_chat(
          0, set->master_ID, 1,
          "%s is not a paying player, you cannot set the rank higher than 0.",
          tmp->name);
      return 0;
    }
    if (tmp->clan_rank == 2) {
      tell_chat(0, set->master_ID, 1,
                "%s is the club's founder, can't change rank.", tmp->name);
      return 0;
    }
    tmp->clan_rank = set->rank;
...
int fire_from_clan(struct fire_from_clan_data *set, struct character *tmp,
  ...
  if (set->clan < CLUBOFFSET) {
    if (tmp->clan != set->clan) {
      tell_chat(0, set->master_ID, 1,
                "%s is not a member of your clan, you cannot fire him/her.",
                tmp->name);
      return 0;
    }
    tmp->clan = 0;
    ...
    add_clanlog(set->clan, clan_serial(set->clan), set->target_ID, 15,
                "%s was fired by %s", tmp->name, set->master_name);

    tell_chat(0, set->master_ID, 1, "Fired %s.", tmp->name);
  } else {
    if (tmp->clan != set->clan) {
      tell_chat(0, set->master_ID, 1,
                "%s is not a member of your club, you cannot fire him/her.",
                tmp->name);
      return 0;
    }
    if (tmp->clan_rank > 1) {
      tell_chat(0, set->master_ID, 1,
                "You cannot fire %s, he is the founder of the club.",
                tmp->name);
      return 0;
    }
    tmp->clan = 0;
...
void punish_player(struct punish_player_data *set, struct character *tmp,
                   int *plock, int *pkick) {
  if (punish(set->master_ID, tmp, set->level, set->reason, plock, pkick)) {
    tell_chat(0, set->master_ID, 1,
              "Punished %s with a level %d punishment for %s", tmp->name,
              set->level, set->reason);
    if (set->level)
      tell_chat(0, set->target_ID, 1,
                "°c03You have just been punished for %s. You have lost "
                "experience and karma. Your karma is now down to %d. If your "
                "karma reaches %d, you will be banned from this game.",
                set->reason, tmp->karma, (tmp->flags & CF_PAID) ? -12 : -5);
    else
      tell_chat(0, set->target_ID, 1,
                "°c03You have been warned for %s. You will not be warned "
                "again. Next time you will lose experience and karma.",
                set->reason);
  }
}

// unpunish needs a rewrite: it is using the database while the server is
// locked!!
void unpunish_player(struct unpunish_player_data *set, struct character *tmp,
                     int *plock, int *pkick) {
...
int set_flags(struct set_flags *set, struct character *tmp, int *plock,
              int *pkick) {
  if (plock) *plock = 0;
  if (pkick) *pkick = 0;
  ...
  tell_chat(0, set->master_ID, 1, "Set flag on %s to %s.", tmp->name,
            (tmp->flags & set->flags) ? "on" : "off");

  return 1;
}

int set_task(struct task_data *set,
             int (*proc)(void *, struct character *, int *, int *)) {
  MYSQL_RES *result;
  MYSQL_ROW row;
  unsigned long *len;
  int cn, lock, kick;
  ...
  if (cn) {  // yes: set data and delete task
    ...
    if (lock > 0 && ch[cn].player) {
      kick_player(ch[cn].player,
                  "You have been locked as a result of your punishment.");
    ...
    if (lock > 0) {
      ...
      if (mysql_query_con(&mysql, buf)) {
        elog("Failed to update account ID=%d: Error: %s (%d)", set->uID,
             mysql_error(&mysql), mysql_errno(&mysql));
      }
#ifdef CHARINFO
      sprintf(buf, "update charinfo set locked='Y' where ID=%d", set->uID);
      if (mysql_query_con(&mysql, buf)) {
        elog("Failed to update charinfo ID=%d: Error: %s (%d)", set->uID,
             mysql_error(&mysql), mysql_errno(&mysql));
      }
#endif
    }
  ...
  if (mysql_query_con(&mysql, buf)) {
    elog("Failed to select account ID=%d: Error: %s (%d)", set->uID,
         mysql_error(&mysql), mysql_errno(&mysql));
    return 0;
  }
  ...
  if (proc(set, &tmp, &lock, &kick)) {
    xlog("set task: lock=%d, kick=%d for ID=%d, task=%d", lock, kick, set->uID,
         set->tasknr);

    mysql_real_escape_string(&mysql, data, (char *)&tmp,
                             sizeof(struct character));

    if (lock > 0)
      sprintf(buf,
              "update chars set chr='%s', "
              "locked='Y',karma=%d,clan=%d,clan_rank=%d,experience=%d where "
              "ID=%d",
              data, tmp.karma, tmp.clan, tmp.clan_rank, tmp.exp, set->uID);
    else if (lock < 0)
      sprintf(buf,
              "update chars set "
              "chr='%s',karma=%d,clan=%d,clan_rank=%d,experience=%d,locked='N' "
              "where ID=%d",
              data, tmp.karma, tmp.clan, tmp.clan_rank, tmp.exp, set->uID);
    else
      sprintf(buf,
              "update chars set "
              "chr='%s',karma=%d,clan=%d,clan_rank=%d,experience=%d where "
              "ID=%d",
              data, tmp.karma, tmp.clan, tmp.clan_rank, tmp.exp, set->uID);
    ...
    if (lock > 0)
      sprintf(buf,
              ...
              "locked='Y',karma=%d,clan=%d,clan_rank=%d,experience=%d where "
              "ID=%d",
              tmp.karma, tmp.clan, tmp.clan_rank, tmp.exp, set->uID);
    else if (lock < 0)
      sprintf(buf,
              "update charinfo set "
              "karma=%d,clan=%d,clan_rank=%d,experience=%d,locked='N' where "
              "ID=%d",
              tmp.karma, tmp.clan, tmp.clan_rank, tmp.exp, set->uID);
    else
      sprintf(buf,
              "update charinfo set karma=%d,clan=%d,clan_rank=%d,experience=%d "
              "where ID=%d",
              tmp.karma, tmp.clan, tmp.clan_rank, tmp.exp, set->uID);
  ...
  switch (task) {
    ...
    case 1:
      del = set_task(
          ...
          (int (*)(void *, struct character *, int *, int *))set_clan_rank);
      break;
    case 2:
      del = set_task(
          (struct task_data *)(taskdata),
          (int (*)(void *, struct character *, int *, int *))fire_from_clan);
      break;
    case 3:
      del = set_task(
          (struct task_data *)(taskdata),
          (int (*)(void *, struct character *, int *, int *))punish_player);
      break;
    case 4:
      del = set_task(
          (struct task_data *)(taskdata),
          (int (*)(void *, struct character *, int *, int *))unpunish_player);
      break;
    case 5:
      del = set_task(
          (struct task_data *)(taskdata),
          (int (*)(void *, struct character *, int *, int *))set_flags);
      break;
...
int task_set_clan_rank(int target_ID, int master_ID, int clan, int rank,
                       char *master_name) {
...
int task_fire_from_clan(int target_ID, int master_ID, int clan,
...
int task_punish_player(int target_ID, int master_ID, int level,
                       const char *reason) {
