...
int quit = 0,        // SIGINT: leave program
    ...
    server_net = 0,  // server is supposed to use this network (if multiply
                     // interfaces are present)
    server_idle,     // idle percentage of this server
    shutdown_at = 0, shutdown_down = 0, nologin = 0, serverID = 0, isxmas = 0;

volatile long long
    sent_bytes_raw =
        0,           // bytes sent including approximation of tcp/ip overhead
...
void sig_showprof(int dummy) { showprof = 1; }
...
void showmem(void) {
  ...
  xlog("Used characters: %5d/%5d (%4.1f%%)", used_chars, MAXCHARS,
       100.0 / MAXCHARS * used_chars);
  xlog("Used items     : %5d/%5d (%4.1f%%)", used_items, MAXITEM,
       100.0 / MAXITEM * used_items);
  xlog("Used effects   : %5d/%5d (%4.1f%%)", used_effects, MAXEFFECT,
       100.0 / MAXEFFECT * used_effects);
  xlog("Used containers: %5d/%5d (%4.1f%%)", used_containers, MAXCONTAINER,
       100.0 / MAXCONTAINER * used_containers);
  ...
  xlog("malloc/mmap    : %.3fM in %d blocks", mi.hblkhd / 1024.0 / 1024.0,
       mi.hblks);
  xlog("malloc/sbrk    : %.3fM", mi.arena / 1024.0 / 1024.0);
  xlog("malloc/total   : %.3fM", (mi.arena + mi.hblkhd) / 1024.0 / 1024.0);
  xlog("malloc/unused  : %.3fM in %d blocks", (mi.fordblks) / 1024.0 / 1024.0,
       mi.ordblks);
  xlog("brk has grown  : %.3fM",
       ((long)(sbrk(0)) - (long)(end_of_data_ptr)) / (1024.0 * 1024.0));
  xlog("------------------------------------");
  list_mem();
}
...
int main(int argc, char *args[]) {
  ...
  if (argc > 1) {
    while (1) {
      ...
      switch (c) {
        ...
        case 'h':
          fprintf(stderr,
                  "Usage: %s [-a <areaID>] [-m <mirror>] [-n <use this class A "
                  "net>] [-d] [-c]\n\n-d Demonize\n-c Disable concurrent "
                  "database access\n\n",
                  args[0]);
          exit(0);
        case 'c':
  ...
  printf(
      "serverID=%d, areaID=%d, areaM=%d, maxchars=%d, maxitem=%d, "
      "maxeffect=%d\n\n",
  ...
  while (!quit) {
    sprintf(args[0], "./server -a %d -m %d -i %d # %d on %d%% load (busy)",
            areaID, areaM, serverID, online, (10000 - server_idle) / 100);
    lock_server();
    ...
    sprintf(args[0], "./server -a %d -m %d -i %d # %d on %d%% load (idle)",
            areaID, areaM, serverID, online, (10000 - server_idle) / 100);
...
static const char *profname[100] = {
    "*ALL*",  // 0
    "IDLE",  // 1
    "sector_hear",  // 2
    "tick_chat",  // 3
...
void prof_stop(int task, unsigned long long cycle) {
  ...
  if (td < 0 || td > 1000000000) {
    xlog("prof overflow %lld (%llu,%llu) %s %d", td, rdtsc(), cycle,
         profname[task], task);
    return;
  }
...
void show_prof(void) {
  ...
  for (n = 0; n < maxprof; n++) {
    ...
    printf("%llu calls, %llu cycles, %.2f%%\n", prof[n].calls, prof[n].cycles,
           100.0 / total * prof[n].cycles);
  }
