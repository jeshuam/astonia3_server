...
int plr_attack_driver(int cn, int co)
{
  ...
  dat = (struct plr_attack_driver_data*)set_data(cn, DRD_PLRATTACK, sizeof(struct plr_attack_driver_data));
...
void tutorial(int cn, int nr, struct tutorial_ppd *ppd)
{
  ...
  if (ch[cn].level < 3 && (ppd2 = (struct area1_ppd*)set_data(cn, DRD_AREA1_PPD, sizeof(struct area1_ppd))) && !ppd2->lydia_state && ppd->lydia_cnt < 10 && realtime - ppd->lydia_last > 60) {
    const char *dirtext1, *dirtext2;
    int dir;
    static const char *dir1[9] = {"unknown", "south-east", "south", "south-west", "west", "north-west", "north", "north-east", "east"};
    static const char *dir2[9] = {"unknown", "down and right", "down", "down and left", "left", "up and left", "up", "up and right", "right"};
  ...
  // how to find quest
  if (ch[cn].level < 4 && (ppd2 = (struct area1_ppd*)set_data(cn, DRD_AREA1_PPD, sizeof(struct area1_ppd))) && ppd2->lydia_state == 4 && realtime - ppd2->lydia_seen_timer > 60 && get_section(ch[cn].x, ch[cn].y) != 45 && !has_item(cn, IID_AREA1_WOODPOTION) && ppd->thief_cnt < 10 && realtime - ppd->thief_last > 60) {
    const char *dirtext1, *dirtext2;
    int dir;
    static const char *dir1[9] = {"unknown", "south-east", "south", "south-west", "west", "north-west", "north", "north-east", "east"};
    static const char *dir2[9] = {"unknown", "down and right", "down", "down and left", "left", "up and left", "up", "up and right", "right"};
...
void player_driver(int cn, int ret, int last_action)
{
  ...
  ppd = (struct tutorial_ppd*)set_data(cn, DRD_TUTORIAL_PPD, sizeof(struct tutorial_ppd));
  if (!ppd) return;
  ...
  lppd = (struct lostcon_ppd*)set_data(cn, DRD_LOSTCON_PPD, sizeof(struct lostcon_ppd));
