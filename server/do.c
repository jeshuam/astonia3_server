...
int do_give(int cn, int dir)
{
  ...
  if ((ch[cn].flags & CF_PLAYER) && (ch[co].flags & CF_PLAYER) && (ppd = (struct misc_ppd*)set_data(co, DRD_MISC_PPD, sizeof(struct misc_ppd))) && realtime - ppd->swapped < 20) {
...
int char_swap(int cn)
{
  ...
  if ((ch[cn].flags & CF_PLAYER) && (ppd = (struct misc_ppd*)set_data(cn, DRD_MISC_PPD, sizeof(struct misc_ppd)))) ppd->swapped = realtime;
