...
#define ITEM_DECAY_TIME \
  (5 * 60 * TICKS)  // time an item which was placed on the ground takes before
                    // it gets destroyed
#define PLAYER_BODY_DECAY_TIME \
  (30 * 60 * TICKS)  // time a player body takes before getting destroyed
#define NPC_BODY_DECAY_TIME \
  (2 * 60 * TICKS)  // time a NPC body takes before getting destroyed
#define NPC_BODY_DECAY_TIME_AREA32 \
  (15 * 60 * TICKS)  // time a NPC body takes before getting destroyed (area 32)
#define RESPAWN_TIME \
  (1 * 60 * TICKS)  // respawn time of NPCs - can be overriden by giving a
                    // respawn=xxx statement in the character definition
#define LAGOUT_TIME \
  (5 * 60 * TICKS)  // time it takes to lag out (time difference between loss of
                    // connection and removal of character from server)
#define REGEN_TIME \
  (4 * TICKS)  // time character has to be idle to start regenerating
...
#define POWERSCALE \
  1000  // ch.hp = ch.value[0][V_HP] * POWERSCALE, same for endurance and mana
...
// extern int server_net;  // server is supposed to use this network (if
// multiply interfaces are present)
...
extern volatile long long
    sent_bytes_raw;  // bytes sent including approximation of tcp/ip overhead
extern volatile long long sent_bytes;  // bytes sent
extern volatile long long
    rec_bytes_raw;  // bytes received including approximation of tcp/ip overhead
...
#define MF_DOOR \
  MF_FLAGCAST((1ull << 6))  // set if door-like, i.e. might open when used
...
#define MF_NEUTRAL \
  MF_FLAGCAST((1ull << 13))  // NPCs will not attack peaceful characters
#define MF_FIRETHRU \
  MF_FLAGCAST((1ull << 14))  // fireball fly through this MF_MOVEBLOCK
#define MF_SLOWDEATH \
  MF_FLAGCAST((1ull << 15))  // player characters on this field take damage
#define MF_NOLIGHT \
  MF_FLAGCAST(     \
      (1ull << 16))  // don't allow light from any character or take-able item
#define MF_NOMAGIC \
  MF_FLAGCAST((1ull << 17))  // don't allow any magic or stat bonuses
#define MF_UNDERWATER MF_FLAGCAST((1ull << 18))  // map tile is under water
#define MF_NOREGEN \
  MF_FLAGCAST((1ull << 19))  // no regeneration on this map tile
...
struct map {
  ...
  short light;  // strength of light (objects only, daylight is computed
                // independendly)
...
#define IF_WNTWOHANDED \
  (1ull << 17)  // two-handed weapon, fills both WNLHAND & WNRHAND
...
#define IF_STEPACTION \
  (1ull << 28)                 // call driver if character steps on this item
...
#define IF_FRONTWALL \
  (1ull << 31)  // item is part of wall but only visible if seen from in front
...
#define IF_LABITEM \
  (1ull << 39)  // labyrinth item - not saved during area changes
#define IF_VOID \
  (1ull << 40)  // item is in void, .x, .y, .carried, .contained reserved and
                // possibly used by item driver
#define IF_NOENHANCE \
  (1ull              \
   << 41)  // item may not be enhanced through orbs/silver/gold/shrines etc.
#define IF_BEYONDBOUNDS (1ull << 42)  // item may have more than 3 +20 mods
#define IF_BEYONDMAXMOD \
  (1ull << 43)  // item modifiers are added to character after max mod check

#define IF_WEAPON \
  (IF_AXE | IF_DAGGER | IF_HAND | IF_STAFF | IF_SWORD | IF_TWOHAND)
#define IF_WEAR                                                            \
  (IF_WNHEAD | IF_WNNECK | IF_WNBODY | IF_WNARMS | IF_WNBELT | IF_WNLEGS | \
   IF_WNFEET | IF_WNLHAND | IF_WNRHAND | IF_WNCLOAK | IF_WNLRING | IF_WNRRING)
...
struct item {
  ...
  unsigned char needs_class;  // bitmapped: 1=must be warrior, 2=must be mage,
                              // 4=must be seyan, 8=must be arch
  ...
  signed short mod_index[MAXMOD];  // index into v_???, negative values are
                                   // requirements, positive ones modifiers
  ...
  unsigned short
      content;  // ID of container which holds the contents of this item
...
#define CF_KICKED \
  (1ull << 7)  // player got kicked, may not login again for a certain time
...
#define CF_DEAD \
  (1ull << 11)  // character is dying (death animation is running, doesnt take
                // long)
...
#define CF_WARRIOR \
  (1ull << 16)  // is warrior - or seyan, if warrior and mage are set
...
#define CF_HASNAME \
  (1ull << 21)  // NPC has a proper name (ie Shiva), not a group name (Orc)
#define CF_QUESTITEM \
  (1ull << 22)  // this character can receive quest items through give
#define CF_INFRARED \
  (1ull << 23)              // this character has infrared vision (NPC version)
#define CF_PK (1ull << 24)  // player killer (players only)
#define CF_ITEMDEATH (1ull << 25)  // character turns into an item on death
#define CF_NODEATH \
  (1ull << 26)  // character does not actually die (death is handle by driver)
...
#define CF_PLAYERLIKE \
  (1ull << 32)  // treat like a player when it comes to can_attack
...
#define CF_HARDKILL \
  (1ull << 39)  // 'hard to kill' - needs special weapon to get killed
...
#define CF_RESERVED4 \
  (1ull << 43)  // legacy flag (was: character won the game (ie killed islena))
#define CF_THIEFMODE (1ull << 44)  // is a thief and turned on thief mode
#define CF_NOTELL (1ull << 45)     // does not wish to receive tells
#define CF_INFRAVISION \
  (1ull << 46)  // this character has infrared vision (player version)
#define CF_NOMAGIC \
  (1ull << 47)  // character is on no-magic tile and no-magic is active
...
#define CF_HARDCORE \
  (1ull << 53)  // hardcore character (ie. no saves, harder penalty on death,
                // more exp for kills)
#define CF_NONOTIFY \
  (1ull << 54)  // this char does not send out NT_CHAR messages
...
struct character {
  ...
  unsigned int item[INVENTORYSIZE];  // 0-11 worn, 12-29 spells,
                                     // 30-(INVENTORYSIZE-1) inventory
  ...
  unsigned short tox,
      toy;            // target coordinates, where the char will be next turn
  ...
  unsigned int
      group;  // group the character belongs to. NPCs must not have group 0.
  unsigned int serial;      // per-run, per-server unique ID
  ...
  unsigned int cls;         // class the NPC belongs to (for First-Kill-Bonus)
  ...
  char staff_code[4];  // re-used old clan value: staffer code for easy
                       // recognition
