...
void professor_driver_parse(int cn, struct professor_driver_data *dat) {
  ...
  for (ptr = nextnv(ch[cn].arg, name, value); ptr;
    ...
    if (!strcmp(name, "dir"))
      ...
      dat->quest = atoi(value);
    else if (!strcmp(name, "option"))
      dat->quest_option = atoi(value);
    else if (!strcmp(name, "cost"))
      dat->improve_cost = atoi(value);
    else
...
int learn_prof(int cn, int co, int nr) {
  ...
  if (nr == P_LIGHT && ch[co].prof[P_DARK]) {
    say(cn,
        "Thou hast learned Master of Dark already, thou mayest not learn "
        "Master of Light.");
    return 0;
  }
  if (nr == P_DARK && ch[co].prof[P_LIGHT]) {
    say(cn,
        "Thou hast learned Master of Light already, thou mayest not learn "
        "Master of Dark.");
    return 0;
  }
  ...
  if (cnt < prof[nr].base) {
    say(cn,
        "Thou have not the required profession points. Thou needst %d, but "
        "thou hast only %d.",
        prof[nr].base, cnt);
    return 0;
  }
...
int improve_prof(int cn, int co, int nr) {
  ...
  if (!ch[co].prof[nr]) {
    say(cn,
        "But thou knowest not the ways of the %s. Thou must learn them first.",
        prof[nr].name);
    return 0;
  }
  if (ch[co].prof[nr] >= prof[nr].max) {
    say(cn, "Thou hast reached mastery in the art of the %s already.",
        prof[nr].name);
    return 0;
  }
  cnt = free_prof_points(co);
  ...
  if (!(ch[co].flags & CF_PAID) && ch[co].prof[nr] + step >= 20) {
    say(cn, "Only paying players may raise this profession higher than %d.",
        ch[co].prof[nr]);
    return 0;
  }

  if (cnt < step) {
    say(cn, "Thou needst have at least %d unused profession points.",
        prof[nr].step);
    return 0;
  }
  ch[co].prof[nr] += step;
  ...
  say(cn, "Thy profession %s was improved to %d.", prof[nr].name,
      ch[co].prof[nr]);
  return 1;
}
...
void professor_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct professor_driver_data *)set_data(
      cn, DRD_PROFDRIVER, sizeof(struct professor_driver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      say(cn,
          "Hello %s! I am a professor at Aston University, and I °c4teach°c0 "
          "°c4%s°c0.",
          ch[co].name, prof[dat->nr].name);
      ...
      switch (ret) {
        case 2:
          switch (dat->quest) {
            case 0:
              say(cn,
                  "If thou wishest to learn the art of the %s, thou must pay "
                  "%d gold coins and %d profession points. Say °c4learn°c0 if "
                  "this is thy wish. Thou canst also °c4improve°c0 thy "
                  "knowledge of this art for the fee of %d gold coins and %d "
                  "profession points.",
                  prof[dat->nr].name, dat->quest_option, prof[dat->nr].base,
                  dat->improve_cost * prof[dat->nr].step, prof[dat->nr].step);
              break;
            default:
        ...
        case 3:
          switch (dat->nr) {
            case P_ATHLETE:
              say(cn,
                  "The art of the athlete are fast, precise movements. Skilled "
                  "athletes make better use of their endurance and move faster "
                  "than untrained humans.");
              break;
            case P_ALCHEMIST:
              say(cn,
                  "The alchemist can create better potions, calling on the "
                  "powers of the moons and the seasons at any time.");
              break;
            case P_MINER:
              say(cn,
                  "A skilled miner will make better use of every vein of "
                  "precious metal he finds. He will also not exhaust as fast "
                  "as an unskilled miner.");
              break;
            case P_ASSASSIN:
              say(cn,
                  "The assassin is especially skilled at attacking an enemy "
                  "from the side or behind, and he can backstab an unware "
                  "opponent from behind.");
              break;
            case P_THIEF:
              say(cn,
                  "A skilled thief can remain unseen even when next to another "
                  "person. But when he uses this skill of stealth he cannot do "
                  "anything but wait or walk, and the effort of remaining "
                  "unseen will drain his endurance.");
              break;
            case P_LIGHT:
              say(cn,
                  "A master of light will receive a bonus to his basic "
                  "abilities during the day. If he masters this skill he will "
                  "also be able to see all undead creatures in the dark.");
              break;
            case P_DARK:
              say(cn,
                  "A master of dark will receive a bonus to his basic "
                  "abilities during the night. If he masters this skill he "
                  "will also be able to see all living creatures in the dark.");
              break;
            case P_TRADER:
              say(cn,
                  "A skilled trader will get better prices when dealing with "
                  "merchants.");
              break;
            case P_MERCENARY:
              say(cn,
                  "Those skilled in the art of the mercenary will advance in "
                  "military rank faster. They will also collect pay for their "
                  "missions.");
              break;
            case P_CLAN:
              say(cn,
                  "A clan master has received special training in the art of "
                  "clan warfare. He will be at an advantage in any fight in "
                  "the clan catacombs.");
              break;
            case P_HERBALIST:
              say(cn,
                  "A herbalist knows the art of making plants ripe faster. Any "
                  "flower, berry or mushroom he picks will grow back in less "
                  "time.");
              break;
          ...
          switch (dat->quest) {
            case 0:
              if (ch[co].gold < dat->quest_option * 100) {
                say(cn, "But thou cannot afford my fee of %dG.",
                    dat->quest_option);
                break;
              }
        ...
        case 5:
          if (ch[co].gold < dat->improve_cost * prof[dat->nr].step * 100) {
            say(cn, "But thou cannot afford my fee of %dG.",
                dat->improve_cost * prof[dat->nr].step);
            break;
          }
