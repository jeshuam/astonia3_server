...
int init_sector(void)
{
  ...
  for (n = 0; n < 8; n++) {
    sector[n] = (int*)xcalloc(MAXMAP * MAXMAP / ((1 << n) * (1 << n)) * sizeof(int), IM_BASE);
    xlog("Allocated sector %d: %.2fM (%lu*%d)", n, MAXMAP * MAXMAP / ((1 << n) * (1 << n))*sizeof(int) / 1024.0 / 1024.0, sizeof(int), MAXMAP * MAXMAP / ((1 << n) * (1 << n)));
    mem_usage += MAXMAP * MAXMAP / ((1 << n) * (1 << n)) * sizeof(int);
  }

  char_sector = (int*)xcalloc(MAXMAP * MAXMAP / 64 * sizeof(int), IM_BASE);
  xlog("Allocated char sector: %.2fM (%lu*%d)", MAXMAP * MAXMAP / 64 * sizeof(int) / 1024.0 / 1024.0, sizeof(int), MAXMAP * MAXMAP / 64);
  mem_usage += MAXMAP * MAXMAP / 64 * sizeof(int);

  sound_sector = (int*)xcalloc(MAXMAP * MAXMAP * sizeof(int), IM_BASE);
  xlog("Allocated sound sector: %.2fM (%lu*%d)", MAXMAP * MAXMAP * sizeof(int) / 1024.0 / 1024.0, sizeof(int), MAXMAP * MAXMAP);
  mem_usage += MAXMAP * MAXMAP * sizeof(int);

  shout_sector = (int*)xcalloc(MAXMAP * MAXMAP * sizeof(int), IM_BASE);
  xlog("Allocated shout sector: %.2fM (%lu*%d)", MAXMAP * MAXMAP * sizeof(int) / 1024.0 / 1024.0, sizeof(int), MAXMAP * MAXMAP);
  mem_usage += MAXMAP * MAXMAP * sizeof(int);
...
int fill_sound_sector(int xs, int ys, int nr)
{
  ...
  pos = (struct pos*)xmalloc(sizeof(struct pos) * MAXMAP * MAXMAP * 4, IM_TEMP);
...
void init_sound_sector(void)
{
  ...
  xlog("Allocated sector2: %.2fM (%lu*%d)", sec2_cnt * sizeof(struct sector2) / 1024.0 / 1024.0, sizeof(struct sector2), sec2_cnt);
  ...
  for (n = 0; n < MAXDOOR; n++) {
    ...
    sec2[nr].door_cnt++;
    sec2[nr].door = (struct door2*)xrealloc(sec2[nr].door, sec2[nr].door_cnt * sizeof(struct door2), IM_BASE);
