...
int init_sector(void) {
  ...
  for (n = 0; n < 8; n++) {
    sector[n] = (int *)xcalloc(
        MAXMAP * MAXMAP / ((1 << n) * (1 << n)) * sizeof(int), IM_BASE);
    xlog(
        "Allocated sector %d: %.2fM (%lu*%d)", n,
        MAXMAP * MAXMAP / ((1 << n) * (1 << n)) * sizeof(int) / 1024.0 / 1024.0,
        sizeof(int), MAXMAP * MAXMAP / ((1 << n) * (1 << n)));
    mem_usage += MAXMAP * MAXMAP / ((1 << n) * (1 << n)) * sizeof(int);
  }

  char_sector = (int *)xcalloc(MAXMAP * MAXMAP / 64 * sizeof(int), IM_BASE);
  xlog("Allocated char sector: %.2fM (%lu*%d)",
       MAXMAP * MAXMAP / 64 * sizeof(int) / 1024.0 / 1024.0, sizeof(int),
       MAXMAP * MAXMAP / 64);
  mem_usage += MAXMAP * MAXMAP / 64 * sizeof(int);

  sound_sector = (int *)xcalloc(MAXMAP * MAXMAP * sizeof(int), IM_BASE);
  xlog("Allocated sound sector: %.2fM (%lu*%d)",
       MAXMAP * MAXMAP * sizeof(int) / 1024.0 / 1024.0, sizeof(int),
       MAXMAP * MAXMAP);
  mem_usage += MAXMAP * MAXMAP * sizeof(int);

  shout_sector = (int *)xcalloc(MAXMAP * MAXMAP * sizeof(int), IM_BASE);
  xlog("Allocated shout sector: %.2fM (%lu*%d)",
       MAXMAP * MAXMAP * sizeof(int) / 1024.0 / 1024.0, sizeof(int),
       MAXMAP * MAXMAP);
  mem_usage += MAXMAP * MAXMAP * sizeof(int);
...
static void add_door(int x, int y, int nr) {
  ...
  if (!door[n].nr1)
    ...
    elog("add_door: three sectors reaching door?? (%d,%d,%d)", nr, door[n].nr1,
         door[n].nr2);
}
...
int fill_sound_sector(int xs, int ys, int nr) {
  ...
  pos =
      (struct pos *)xmalloc(sizeof(struct pos) * MAXMAP * MAXMAP * 4, IM_TEMP);
...
void init_sound_sector(void) {
  ...
  door = (struct door *)xcalloc(sizeof(struct door) * MAXDOOR, IM_TEMP);
  ...
  sec2_cnt = nr;
  sec2 = (struct sector2 *)xcalloc(sizeof(struct sector2) * sec2_cnt, IM_BASE);
  xlog("Allocated sector2: %.2fM (%lu*%d)",
       sec2_cnt * sizeof(struct sector2) / 1024.0 / 1024.0,
       sizeof(struct sector2), sec2_cnt);
  mem_usage += sec2_cnt * sizeof(struct sector2);

  for (n = 0; n < MAXDOOR; n++) {
    if (door[n].nr1 == 0 && door[n].nr2 == 0) continue;

    // xlog("door %d: %d,%d connecting %d with
    // %d",n,door[n].x,door[n].y,door[n].nr1,door[n].nr2);

    nr = door[n].nr1;
    to = door[n].nr2;
    sec2[nr].door_cnt++;
    sec2[nr].door = (struct door2 *)xrealloc(
        sec2[nr].door, sec2[nr].door_cnt * sizeof(struct door2), IM_BASE);
...
int sector_follow_door(int s1, int s2, int *stack, int s) {
  ...
  for (n = 0; n < sec2[s1].door_cnt; n++) {
    if (map[sec2[s1].door[n].x + sec2[s1].door[n].y * MAXMAP].flags &
        MF_TSOUNDBLOCK)
      continue;
