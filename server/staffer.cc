...
void smugglecom_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct smugglecom_data*)set_data(cn, DRD_SMUGGLECOMDRIVER, sizeof(struct smugglecom_data));
  if (!dat) return;	// oops...
...
void rouven_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct rouven_data*)set_data(cn, DRD_ROUVENDRIVER, sizeof(struct rouven_data));
  if (!dat) return;	// oops...
