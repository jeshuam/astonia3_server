...
void staffer_fireball_machine(int in, int cn) {
  ...
  fn = create_fireball(0, it[in].x + dxs, it[in].y + dys, it[in].x + dx,
                       it[in].y + dy, power);
...
void vault_skull(int in, int cn) {
  ...
  ppd = (struct staffer_ppd *)set_data(cn, DRD_STAFFER_PPD,
  ...
  if (ppd->rouven_state >= 0 && ppd->rouven_state <= 5) {
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "You feel an evil aura coming from the skulls. A magical barrier "
             "prevents anyone from getting close and you decide to report back "
             "to Rouven.");
    questlog_done(cn, 62);
  }
}
...
void vault_shelf(int in, int cn) {
  ...
  if (it[in].drdata[1] == 2 && (in2 = create_item("vault_ritual"))) {
    ...
    log_char(cn, LOG_SYSTEM, 0, "After a lengthy search you find %s.",
             it[in2].name);
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "After a lengthy search you find nothing of interest");
}
...
void smugglecom_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct smugglecom_data *)set_data(cn, DRD_SMUGGLECOMDRIVER,
                                           sizeof(struct smugglecom_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (ppd) {
        switch (ppd->smugglecom_state) {
          ...
          case 1:
            quiet_say(cn,
                      "I want you to find a book for me called 'the contraband "
                      "book', which contains the names of four of the "
                      "smuggler's most precious items.");
            ppd->smugglecom_state++;
            didsay = 1;
            break;
          case 2:  // quiet_say(cn,"Also, I will reward you for every piece of
                   // contraband you bring me. ");
          ...
          case 5:
            ...
            quiet_say(cn,
                      "It lists four important items which I want you to "
                      "retrieve: the Rainbow Pearls, the Crimson Ring, the "
                      "Leopard Cape, and the Emerald Necklace. Find them, and "
                      "bring them to me.");
            questlog_open(co, 36);
            ppd->smugglecom_state++;
            didsay = 1;
          ...
          case 7:
            quiet_say(cn,
                      "Thank you, you are of great help in hurting the "
            ...
            quiet_say(cn,
                      "Now, as a final task, I want you to kill the smuggler's "
                      "leader. Good luck!");
            questlog_open(co, 37);
            ppd->smugglecom_state++;
            didsay = 1;
          ...
          case 9:
            quiet_say(
                cn,
                "Thank you for helping us, %s, you have been of great value.",
                ch[co].name);
            ppd->smugglecom_state++;
            didsay = 1;
            questlog_done(co, 37);
      ...
      if (ch[co].flags & CF_PLAYER) {
        ...
        switch ((didsay = analyse_text_driver(cn, msg->dat1, (char *)msg->dat2,
          ...
          case 2:
            ...
            if (ppd && ppd->smugglecom_state >= 5 &&
                ppd->smugglecom_state <= 6) {
              dat->last_talk = 0;
              ppd->smugglecom_state = 5;
            }
            if (ppd && ppd->smugglecom_state >= 7 &&
                ppd->smugglecom_state <= 8) {
              dat->last_talk = 0;
              ppd->smugglecom_state = 7;
            }
            break;
          case 3:
            if (ch[co].flags & CF_GOD)
              ppd->smugglecom_bits = ppd->smugglecom_state = 0;
            break;
        }
        if (didsay) {
          talkdir = offset2dx(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y);
    ...
    if (msg->type == NT_GIVE) {
      ...
      if ((in = ch[cn].citem)) {  // we still have it
        ...
        if (it[in].ID == IID_STAFF_SMUGGLEBOOK && ppd &&
          ...
          ppd->smugglecom_state = 5;
        } else if (it[in].ID == IID_STAFF_SMUGGLEPEARLS && ppd &&
                   !(ppd->smugglecom_bits & SMUGGLEBIT_PEARLS) &&
                   (ch[co].flags & CF_PLAYER)) {
          quiet_say(cn, "Thank you for bringing back the %s, %s.", it[in].name,
                    ch[co].name);

          val = questlog_scale(questlog_count(co, 36), 1000);
          dlog(cn, 0,
               "Received %d exp for doing quest Contraband I for the %d. time "
               "(nominal value %d exp)",
               val, questlog_count(co, 36) + 1, 1000);
          give_exp(co, min(val, level_value(ch[co].level) / 4));

          ppd->smugglecom_bits |= SMUGGLEBIT_PEARLS;
        } else if (it[in].ID == IID_STAFF_SMUGGLERING && ppd &&
          ...
          val = questlog_scale(questlog_count(co, 36), 1000);
          dlog(cn, 0,
               "Received %d exp for doing quest Contraband II for the %d. time "
               "(nominal value %d exp)",
               val, questlog_count(co, 36) + 1, 1000);
          give_exp(co, min(val, level_value(ch[co].level) / 4));

          ppd->smugglecom_bits |= SMUGGLEBIT_RING;
        } else if (it[in].ID == IID_STAFF_SMUGGLECAPE && ppd &&
          ...
          val = questlog_scale(questlog_count(co, 36), 1000);
          dlog(cn, 0,
               "Received %d exp for doing quest Contraband III for the %d. "
               "time (nominal value %d exp)",
               val, questlog_count(co, 36) + 1, 1000);
          give_exp(co, min(val, level_value(ch[co].level) / 4));

          ppd->smugglecom_bits |= SMUGGLEBIT_CAPE;
        } else if (it[in].ID == IID_STAFF_SMUGGLENECKLACE && ppd &&
          ...
          val = questlog_scale(questlog_count(co, 36), 1000);
          dlog(cn, 0,
               "Received %d exp for doing quest Contraband IV for the %d. time "
               "(nominal value %d exp)",
               val, questlog_count(co, 36) + 1, 1000);
          give_exp(co, min(val, level_value(ch[co].level) / 4));
...
void rouven_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct rouven_data *)set_data(cn, DRD_ROUVENDRIVER,
                                       sizeof(struct rouven_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (ppd) {
        switch (ppd->rouven_state) {
          case 0:
            if (ppd->carlos2_state == 0) {
              if (!mem_check_driver(cn, co, 0)) {
                quiet_say(cn, "Hullo %s. Please talk to Carlos first.",
                          ch[co].name);
            ...
            quiet_say(cn,
                      "Hail, %s. Carlos sent you for a ritual? Did he mention "
                      "the place is cursed? Well, I have two quests of my own "
                      "for you.",
                      ch[co].name);
            ppd->rouven_state++;
            didsay = 1;
          ...
          case 1:
            quiet_say(cn,
                      "First, I beg you to try to locate the source of the "
                      "curse that has befallen the Imperial Vault.");
            ppd->rouven_state++;
            didsay = 1;
            break;
          case 2:
            quiet_say(cn,
                      "It was in the armory where the guards first started "
                      "acting strangely and attacking everyone.");
            ppd->rouven_state++;
            didsay = 1;
            break;
          case 3:
            quiet_say(cn, "The way there is through the left door.");
            ppd->rouven_state++;
            didsay = 1;
            break;
          case 4:
            quiet_say(cn, "Good luck.");
            ppd->rouven_state++;
            didsay = 1;
            break;
          case 5:
          ...
          case 6:
            quiet_say(cn,
                      "You say there's demons and a pile of strange skulls? "
                      "They must have burrowed in from the underground. We'll "
          ...
          case 7:
            quiet_say(cn,
                      "Now I ask you to retrieve the chronicles of Seyan I. He "
                      "kept a journal detailing many of his plans. Including "
                      "those for the Aston Empire.");
            ppd->rouven_state++;
            didsay = 1;
            break;
          case 8:
            quiet_say(cn, "It was stored in the archives to the right.");
            ppd->rouven_state++;
            didsay = 1;
            break;
          case 9:
          ...
          case 10:
            quiet_say(cn,
                      "Thank you %s. It will be most useful in our rebuilding "
                      "efforts.",
                      ch[co].name);
            ppd->rouven_state++;
            didsay = 1;
          ...
          case 11:
            quiet_say(cn,
                      "Now we get to the magical ritual. It was kept within "
                      "the treasury behind me in the emperor's personal "
                      "vault.");
            ppd->rouven_state++;
            didsay = 1;
            break;
          case 12:
            ...
            if (!has_item(co, IID_MAX_VAULTKEY) &&
                (in2 = create_item("vault_key1"))) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (ch[co].flags & CF_PLAYER) {
        ...
        switch ((didsay = analyse_text_driver(cn, msg->dat1, (char *)msg->dat2,
                                              co))) {
          case 2:
            if (ppd && ppd->rouven_state <= 5) {
              dat->last_talk = 0;
              ppd->rouven_state = 0;
            }
            if (ppd && ppd->rouven_state >= 6 && ppd->rouven_state <= 9) {
              dat->last_talk = 0;
              ppd->rouven_state = 6;
            }
            if (ppd && ppd->rouven_state >= 10 && ppd->rouven_state <= 13) {
              dat->last_talk = 0;
              ppd->rouven_state = 10;
            }
            break;
    ...
    if (msg->type == NT_GIVE) {
      ...
      if ((in = ch[cn].citem)) {  // we still have it
        ...
        if (it[in].ID == IID_MAX_CHRONICLES && ppd && ppd->rouven_state >= 6 &&
            ppd->rouven_state <= 9 && (ch[co].flags & CF_PLAYER)) {
          quiet_say(cn, "Thank you for the book, %s.", ch[co].name);
          questlog_done(co, 63);
          destroy_item_byID(co, IID_MAX_CHRONICLES);
          ppd->rouven_state = 10;
        } else {
          if (it[in].ID == IID_MAX_RITUAL)
            quiet_say(cn, "Please take the ritual to Carlos.");
          else
            quiet_say(cn,
                      "Thou hast better use for this than I do. Well, if there "
                      "is use for it at all.");
