...
static const char *memname[] = {
    "none",      "IM_BASE",   "IM_TEMP",  "IM_CHARARGS", "IM_TALK",
    "IM_DRDATA", "IM_DRHDR",  "IM_QUERY", "IM_DATABASE", "IM_NOTIFY",
    "IM_TIMER",  "IM_PLAYER", "IM_STORE", "IM_STORAGE",  "IM_ZLIB",
    "IM_MYSQL",  "unknown1",  "unknown2", "unknown3",    "unknown4"};
...
#else
  mem = (struct mem_block *)smalloc(size + sizeof(struct mem_block), ID);
#endif
  if (!mem) return NULL;
...
void *xrealloc(void *ptr, int size, int ID) {
  ...
  mem = (struct mem_block *)((char *)(ptr) - sizeof(struct mem_block));
...
#else
  mem = (struct mem_block *)srealloc(mem, size + sizeof(struct mem_block), ID);
#endif
  if (!mem) return NULL;
...
void xfree(void *ptr) {
  ...
  if (memcmp(mem->magic, "ABCDEFGHIJKLMNOPQRSTUVWXYZ012345", 32)) {
    ...
    elog("%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", mem->content[0],
         mem->content[1], mem->content[2], mem->content[3], mem->content[4],
         mem->content[5], mem->content[6], mem->content[7], mem->content[8],
         mem->content[9], mem->content[10], mem->content[11], mem->content[12],
         mem->content[13], mem->content[14], mem->content[15]);
  }
  if (memcmp(mem->content + mem->size, "abcdefghijklmnopqrstuvwxyz678901",
             32)) {
...
void list_mem(void) {
  ...
  for (mem = mb; mem; mem = mem->next) {
    ...
    if (memcmp(mem->magic, "ABCDEFGHIJKLMNOPQRSTUVWXYZ012345", 32)) {
      ...
      elog("%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", mem->content[0],
           mem->content[1], mem->content[2], mem->content[3], mem->content[4],
           mem->content[5], mem->content[6], mem->content[7], mem->content[8],
           mem->content[9], mem->content[10], mem->content[11],
           mem->content[12], mem->content[13], mem->content[14],
           mem->content[15]);
    }
    if (memcmp(mem->content + mem->size, "abcdefghijklmnopqrstuvwxyz678901",
               32)) {
      elog("memory fail: bottom, type=%d, size=%d, p=%p", mem->ID, mem->size,
           mem);
      elog("%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", mem->content[0],
           mem->content[1], mem->content[2], mem->content[3], mem->content[4],
           mem->content[5], mem->content[6], mem->content[7], mem->content[8],
  ...
  for (n = 0; n < 100; n++) {
    if (get_region_size(n)) {
      xlog(
          "%-10.10s: %4d blks, %8.2fK (size %7.3fM/%3.0fM %5.1f%%, free: "
          "%8.2fK, %4d blks)",
          ...
          get_region_freesize(n) / 1024.0, get_region_free(n));
      rsize += get_region_size(n);
      //_list_free(n);
    }
  }
  xlog("total: %d blocks, total size: %.2fM (%.2fM, r=%.2fM/%.2fM)", tcnt,
       tsize / (1024.0 * 1024.0), msize / (1024.0 * 1024.0),
       rsize / 1024.0 / 1024.0, mmem_usage / 1024.0 / 1024.0);
  pthread_mutex_unlock(&alloc_mutex);
}
...
int init_smalloc(void) {
  ...
  for (n = 0, addr = (unsigned char *)0x50000000; n < 32; n++) {
    reg_addr[n].start = addr;
...
int smalloc_addr2nr(unsigned char *ap) {
  ...
  for (n = 0; n < 32; n++) {
    if (reg_addr[n].start < ap && reg_addr[n].start + reg_addr[n].size > ap)
      return n;
  }
...
void *size_region(int nr, int diff) {
  ...
  start =
      reg_addr[nr].start;  //(void*)(0x50000000+(nr*24*1024*1024)); !!!!!!!!!!

  change = region[nr].usedsize - region[nr].havesize;
  if (change >= 0)
    change = ((change + 4095) / 4096) * 4096;
  else
    change = (change / 4096) * 4096;
  ...
  if (change > 0 &&
      mmap(start + region[nr].havesize, change, PROT_READ | PROT_WRITE,
           MAP_ANON | MAP_PRIVATE, 0, region[nr].havesize) == (void *)(-1))
    return 0;
...
void sfree_nolock(void *ap, int dorelease) {
  ...
  nr = smalloc_addr2nr(
      (unsigned char *)
          ap);  //(((int)ap)-0x50000000)/(24*1024*1024); !!!!!!!!!!!!!!!
...
struct header *morecore(int nu, int nr) {
  ...
  if (!(cp = (char *)size_region(nr, nu * sizeof(struct header)))) return NULL;
...
void *srealloc(void *ptr, int nbytes, int nr) {
  ...
  size = min(nbytes, ((struct header *)ptr - 1)->size * sizeof(struct header) -
...
void minfo(void)
{
        ...
        for (nr=0; nr<3; nr++) {
                scompact(nr);
                //printf("region %d: size=%.2fM/%.2fM (%d/%d
