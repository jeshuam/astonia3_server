    ...
    switch (ppd->mis[nr].type) {
      case 3:
        if (amount < ppd->mis[nr].opt1) {
          ...
          log_char(
              cn, LOG_SYSTEM, 0,
              "You fulfilled part of your mission, you still need %d silver.",
              ppd->mis[nr].opt1);
        } else {
          log_char(cn, LOG_SYSTEM, 0,
                   "You solved your mission. Talk to the governor to claim you "
                   "reward.");
  ...
  if (it[in].drdata[3] < 9) {
    ...
    if (it[in].drdata[3] == 8) {
      ...
      call_item(it[in].driver, in, 0,
                ticker + TICKS * 60 * 5 + RANDOM(TICKS * 60 * 25));
      ...
      if (amount && in2) {
        ...
        *(unsigned int *)(it[in2].drdata + 1) = amount;
        sprintf(it[in2].description, "%d units of %s.",
                *(unsigned int *)(it[in2].drdata + 1), it[in2].name);
        ...
        log_char(cn, LOG_SYSTEM, 0, "You found %d units of %s.", amount,
                 it[in2].name);
...
int enhance_item_sprite(int in) {
  ...
  if (it[in].sprite >= 10120 && it[in].sprite <= 10159)
    spr = it[in].sprite - 10120 + 59300;
  if (it[in].sprite >= 10190 && it[in].sprite <= 10199)
    spr = it[in].sprite - 10190 + 59340;
  if (it[in].sprite >= 10220 && it[in].sprite <= 10229)
    spr = it[in].sprite - 10220 + 59350;
  if (it[in].sprite >= 10250 && it[in].sprite <= 10259)
    spr = it[in].sprite - 10250 + 59360;
  if (it[in].sprite >= 10280 && it[in].sprite <= 10289)
    spr = it[in].sprite - 10280 + 59370;
  if (it[in].sprite >= 59300 && it[in].sprite <= 59399)
    spr = it[in].sprite - 59300 + 59200;
  ...
  for (m = 0; m < MAXMOD; m++) {
    switch (it[in].mod_index[m]) {
      ...
      case V_LIGHT:
        break;
      default:
        if (it[in].mod_index[m] >= 0) maxmod = max(maxmod, it[in].mod_value[m]);
        break;
    }
  }
  return maxmod;
...
static int enhance_item_price(int in) { return get_maxmod(in) * 100 + 100; }
...
static int enhance_cannot_use(int in, int cn) {
  ...
  for (m = 0; m < MAXMOD; m++) {
    if (it[in].mod_value[m]) {
      switch (it[in].mod_index[m]) {
        case -V_ARMORSKILL:
          if (ch[cn].value[1][V_ARMORSKILL] < it[in].mod_value[m] + 10)
            return 1;
          else
            break;
        case -V_DAGGER:
          if (ch[cn].value[1][V_DAGGER] < it[in].mod_value[m] + 10)
            return 1;
          else
            break;
        case -V_STAFF:
          if (ch[cn].value[1][V_STAFF] < it[in].mod_value[m] + 10)
            return 1;
          else
            break;
        case -V_SWORD:
          if (ch[cn].value[1][V_SWORD] < it[in].mod_value[m] + 10)
            return 1;
          else
            break;
        case -V_TWOHAND:
          if (ch[cn].value[1][V_TWOHAND] < it[in].mod_value[m] + 10)
            return 1;
          else
            break;
      }
...
int enhance_item(int in, int cn) {
  ...
  if ((spr >= 59200 && spr < 59299) || spr == 59474) {  // silver item
    if (it[in].drdata[0] != 2) {                        // needs gold
      log_char(cn, LOG_SYSTEM, 0,
               "This item has already been enhanced once. For further "
               "enhancements, you need gold.");
      return 1;
    }
  } else {                        // normal item
  ...
  if (need > *(unsigned int *)(it[in].drdata + 1)) {
    log_char(
        cn, LOG_SYSTEM, 0,
        "You do not have enough %s to enhance this item. You need %d units.",
        it[in].name, need);
    return 1;
  }

  if (enhance_cannot_use(in2, cn) &&
      (*(unsigned int *)(it[in].drdata + 8) != in2 ||
       realtime - *(unsigned int *)(it[in].drdata + 12) > 15)) {
    log_char(cn, LOG_SYSTEM, 0,
             "Enhancing this item would make it unusable for you. Click again "
  ...
  *(unsigned int *)(it[in].drdata + 1) -= need;
  if (*(unsigned int *)(it[in].drdata + 1) < 1) {
    if (ch[cn].flags & CF_PLAYER)
      dlog(cn, in, "dropped because it was used to enhance item");
    remove_item_char(in);
    destroy_item(in);
  } else {
    it[in].value -= price;  // it[in].value*(*(unsigned
                            // int*)(it[in].drdata+1))/(need+(*(unsigned
                            // int*)(it[in].drdata+1)));
    ...
    if (it[in2].mod_value[m]) {
      switch (it[in2].mod_index[m]) {
        ...
        case -V_TWOHAND:
          ...
          break;
        case V_ARMOR:
          it[in2].mod_value[m] += armorbonus(in2);
          break;
        case V_WEAPON:
          it[in2].mod_value[m] += 10;
          break;
        default:
          if (it[in2].mod_index[m] >= 0 && it[in2].mod_value[m] < 20)
            it[in2].mod_value[m]++;
          break;
      }
    }
  ...
  look_item(cn, it + in2);
  log_char(cn, LOG_SYSTEM, 0, "You used %d units to enhance your %s.", need,
           it[in2].name);
...
void collect_item(int in, int cn) {
  ...
  if (!(in2 = ch[cn].citem)) {
    ...
    if (!in2) {
      log_char(cn, LOG_SYSTEM, 0,
               "You need to use another item with this one.");
      return;
    }
    ...
    *(unsigned int *)(it[in2].drdata + 1) = b;
    ...
    if (ch[cn].flags & CF_PLAYER)
      dlog(cn, in2, "took from splitting metal (%d %d)", a, b);
    ch[cn].citem = in2;
    it[in2].carried = cn;
    ch[cn].flags |= CF_ITEMS;
  ...
  if (it[in2].driver != it[in].driver ||
      it[in2].drdata[0] != it[in].drdata[0]) {
    if (!enhance_item(in, cn))
  ...
  *(unsigned int *)(it[in].drdata + 1) += *(unsigned int *)(it[in2].drdata + 1);
  ...
  sprintf(it[in].description, "%d units of %s.",
          *(unsigned int *)(it[in].drdata + 1), it[in].name);
  log_char(cn, LOG_SYSTEM, 0, "%d units of %s.",
           *(unsigned int *)(it[in].drdata + 1), it[in].name);
...
void minedoor(int in, int cn) {
    ...
    for (in2 = 1; in2 < MAXITEM; in2++) {
      ...
      if (!it[in2].drdata[1]) {   // source door
        if (it[in2].drdata[3]) {  // usable source door
          ...
          sdoor[nr] = in2;
          // xlog("source %d is at
          // %d,%d",it[in2].drdata[0],it[in2].x,it[in2].y);
        }
      } else {  // target door
  ...
  if (!cn) {  // timer call
    ...
    if (!(map[it[in].x + it[in].y * MAXMAP + 1].flags &
        ...
        !(map[it[in].x + it[in].y * MAXMAP - 1].flags &
          (MF_SIGHTBLOCK | MF_TSIGHTBLOCK)) ||
        !(map[it[in].x + it[in].y * MAXMAP + MAXMAP].flags &
          (MF_SIGHTBLOCK | MF_TSIGHTBLOCK)) ||
        !(map[it[in].x + it[in].y * MAXMAP - MAXMAP].flags &
          (MF_SIGHTBLOCK | MF_TSIGHTBLOCK)) ||
        !(map[it[in].x + it[in].y * MAXMAP + 1 + MAXMAP].flags &
          (MF_SIGHTBLOCK | MF_TSIGHTBLOCK)) ||
        !(map[it[in].x + it[in].y * MAXMAP + 1 - MAXMAP].flags &
          (MF_SIGHTBLOCK | MF_TSIGHTBLOCK)) ||
        !(map[it[in].x + it[in].y * MAXMAP - 1 + MAXMAP].flags &
    ...
    if ((in2 = sdoor[nr])) {  // there is a source door already, close it
      if (!(map[it[in2].x + it[in2].y * MAXMAP + 1].flags &
            (MF_MOVEBLOCK | MF_TMOVEBLOCK)) ||
          !(map[it[in2].x + it[in2].y * MAXMAP - 1].flags &
            (MF_MOVEBLOCK | MF_TMOVEBLOCK)) ||
          !(map[it[in2].x + it[in2].y * MAXMAP + MAXMAP].flags &
            (MF_MOVEBLOCK | MF_TMOVEBLOCK)) ||
          !(map[it[in2].x + it[in2].y * MAXMAP - MAXMAP].flags &
            (MF_MOVEBLOCK | MF_TMOVEBLOCK)) ||
          !(map[it[in2].x + it[in2].y * MAXMAP + 1 + MAXMAP].flags &
            (MF_MOVEBLOCK | MF_TMOVEBLOCK)) ||
          !(map[it[in2].x + it[in2].y * MAXMAP + 1 - MAXMAP].flags &
            (MF_MOVEBLOCK | MF_TMOVEBLOCK)) ||
          !(map[it[in2].x + it[in2].y * MAXMAP - 1 + MAXMAP].flags &
  ...
  if (!ret) {
    log_char(cn, LOG_SYSTEM, 0,
             "The mine has collapsed behind this door. Some friendly spirit "
             "grabs you and teleports you somewhere else.");
    if (areaID != 31)
      teleport_char_driver(cn, 230, 240);
    else
      teleport_char_driver(cn, 211, 231);
  }
}
...
void keyholder_door(int in, int cn) {
  ...
  if (flag) {
    log_char(cn, LOG_SYSTEM, 0,
             "You hear fighting noises from behind the door. It won't open "
             "while the fight lasts.");
    return;
  }
  if (!(in2 = ch[cn].citem) || it[in2].driver != IDR_ENHANCE ||
      it[in2].drdata[0] != 2 || *(unsigned int *)(it[in2].drdata + 1) != 2000) {
    log_char(cn, LOG_SYSTEM, 0,
             "You'll need to use 2000 gold units as a key to open the door.");
    return;
  }
...
void keyhold_fight_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct gate_fight_driver_data *)set_data(
      cn, DRD_GATE_FIGHT, sizeof(struct gate_fight_driver_data));
  if (!dat) return;  // oops...
...
void minegateway(int in, int cn) {
  ...
  if (!has_item(cn, IID_MINEGATEWAY)) {
    log_char(cn, LOG_SYSTEM, 0,
             "The door won't open. You notice an inscription: \"This door "
             "leads to the Dwarven town Grimroot. Only those who have proven "
             "their abilities as miners and fighters may enter.");
    return;
  }
