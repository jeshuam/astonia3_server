...
int attack_driver(int cn, int co) {
  ...
  if (dir == -1) {
    ...
    if (dir != -1) {
      ...
      if (pathbestdist() < dist)
        return walk_or_use_driver(cn, dir);
      else if (!(ch[cn].flags & CF_PLAYER))
        return do_idle(cn, TICKS / 4);
      else
        return 0;
    }
  } else
...
int distance_driver(int cn, int co, int distance) {
  ...
  if (ch[co].tox)
    dir = pathfinder(ch[cn].x, ch[cn].y, ch[co].tox, ch[co].toy, distance, NULL,
                     0);
  if (dir == -1)
    dir = pathfinder(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y, distance, NULL, 0);
...
int give_driver(int cn, int co) {
  ...
  if ((it[in].flags & IF_QUEST) && !(ch[co].flags & (CF_QUESTITEM | CF_GOD)) &&
      !(ch[cn].flags & (CF_QUESTITEM | CF_GOD))) {
    error = ERR_QUESTITEM;
    return 0;
  }
...
int use_driver(int cn, int in, int spec) {
  ...
  if (dx == 0 && dy == 1 && !(it[in].flags & IF_FRONTWALL))
    return do_use(cn, DX_DOWN, spec);
  if (dx == 0 && dy == -1) return do_use(cn, DX_UP, spec);
  if (dx == 1 && dy == 0 && !(it[in].flags & IF_FRONTWALL))
    return do_use(cn, DX_RIGHT, spec);
  if (dx == -1 && dy == 0) return do_use(cn, DX_LEFT, spec);

  if (it[in].flags & IF_FRONTWALL) {
    dir = -1;
    if (!(map[it[in].x + it[in].y * MAXMAP + 1].flags &
          (MF_MOVEBLOCK | MF_TMOVEBLOCK)))
      dir = pathfinder(ch[cn].x, ch[cn].y, it[in].x + 1, it[in].y, 0, NULL, 0);
    if (dir == -1 &&
        !(map[it[in].x + it[in].y * MAXMAP + MAXMAP].flags &
          (MF_MOVEBLOCK | MF_TMOVEBLOCK)))
      dir = pathfinder(ch[cn].x, ch[cn].y, it[in].x, it[in].y + 1, 0, NULL, 0);
  } else
    dir = pathfinder(ch[cn].x, ch[cn].y, it[in].x, it[in].y, 1, NULL, 0);
...
int scan_item_driver(int cn) {
  ...
  dat = (struct scan_item_data *)set_data(cn, DRD_SCANITEM,
                                          sizeof(struct scan_item_data));
  ...
  if (ch[cn].x != dat->x) {  // scan left/right
    ...
    if (ch[cn].x > dat->x)
      xs = min(MAXMAP - 2, dat->x + SCANDIST);
    else
      xe = max(1, dat->x - SCANDIST);
  ...
  if (ch[cn].y != dat->y) {  // scan up/down
    ...
    if (ch[cn].y > dat->y)
      ys = min(MAXMAP - 2, dat->y + SCANDIST);
    else
      ye = max(1, dat->y - SCANDIST);
...
int mem_add_driver(int cn, int co, int nr) {
  ...
  dat = (struct char_mem_data *)set_data(cn, DRD_CHARMEM + nr,
                                         sizeof(struct char_mem_data));
  if (!dat) return 0;  // oops...

  if (ch[co].ID)
    xID = ch[co].ID | 0x80000000;
  else
    xID = ch[co].serial & 0x7fffffff;
  ...
  if (dat->cnt == dat->max) {
    dat->max += 8;
    dat = (struct char_mem_data *)set_data(
        cn, DRD_CHARMEM + nr,
        sizeof(struct char_mem_data) + sizeof(unsigned int) * dat->max);
    if (!dat) return 0;
  }
...
int remove_item(int in) {
  ...
  if (it[in].carried && it[in].contained) {
    elog("remove_item: item %s (%d) carried and contained set", it[in].name,
         in);
  }
  if (it[in].x) {
...
static void call_item_callback(int driver, int in, int cn, int serial,
                               int dum2) {
  if (!it[in].flags) return;  // item was deleted, dont call driver
  if (it[in].serial != serial)
    return;  // different item by now, dont call driver
...
struct fight_driver_data
{
        ...
        int start_dist;   // distance from respawn point at which
to start attacking
        int stop_dist;    // distance from respawn point at which to
stop attacking
        int char_dist;    // distance from character we start attacking

        int home_x,home_y;  // position to compare start_dist and
start_dist with
...
int fight_driver_flee(int cn) {
  ...
  int dir, n, dist, co, bdir = 0, bscore = -99999, dx, dy, dt, score,
                        mindist = 99;
  struct fight_driver_data *dat;

  if (!(dat = (struct fight_driver_data *)set_data(
            cn, DRD_FIGHTDRIVER, sizeof(struct fight_driver_data))))
    return 0;
  ...
  if (mindist < 10 &&
      (ch[cn].endurance > 4 * POWERSCALE || ch[cn].speed_mode == SM_FAST)) {
...
static int fight_driver_fireball_value(int cn, int co) {
  ...
  val =
      MED_PRIO + ch[cn].value[0][V_FIREBALL];  //+ch[cn].value[0][V_FIREBALL]/2;
...
static int fight_driver_regen_value(int cn, int co,
                                    struct fight_driver_data *dat) {
  int val, diff;
  ...
  val = max(ch[cn].value[0][V_FIREBALL],
            max(ch[cn].value[0][V_FLASH],
                max(ch[cn].value[0][V_FREEZE], ch[cn].value[0][V_ATTACK]))) *
        2;
...
static int fight_driver_attack_value(int cn, int co) {
  ...
  if (ch[cn].value[1][V_ATTACK])
    val = MED_PRIO + get_attack_skill(cn) / 3.5 + 10;
  else
    val = LOW_PRIO + get_attack_skill(cn) / 3;
...
static int fight_driver_distance3_value(int cn, int co) {
  ...
  if (ch[cn].value[1][V_ATTACK])
    val = LOW_PRIO + ch[cn].value[0][V_FREEZE];
  else
    val = MED_PRIO + ch[cn].value[0][V_FREEZE];
...
static int fight_driver_distance3_flash_value(int cn, int co) {
  ...
  if (ch[cn].value[1][V_ATTACK])
    val = LOW_PRIO + ch[cn].value[0][V_FLASH];
  else
    val = MED_PRIO + ch[cn].value[0][V_FLASH];
...
static int fight_driver_distance7_value(int cn, int co) {
  ...
  if (ch[cn].value[1][V_ATTACK])
    val = LOW_PRIO + ch[cn].value[0][V_FIREBALL];
  else
    val = MED_PRIO + ch[cn].value[0][V_FIREBALL];
...
static int check_pulse_field(int cn, int x, int y) {
  ...
  if (ch[co].mana > POWERSCALE * 4 &&
      (ch[co].value[1][V_MAGICSHIELD] || ch[co].value[1][V_HEAL]))
    return 0;
  // dont fire if enemy is casting heal or ms
  if (ch[co].action == AC_HEAL_SELF || ch[co].action == AC_MAGICSHIELD)
    return 0;

  dam = pulse_damage(
      cn, co, min(spellpower(cn, V_PULSE), ch[cn].mana * 8 / POWERSCALE));

  // say(cn,"%.2f vs
  // %.2f",(double)dam/POWERSCALE,(double)(ch[co].hp+ch[co].lifeshield)/POWERSCALE);

  has = ch[co].hp + ch[co].lifeshield;
  total = ch[co].value[0][V_HP] * POWERSCALE +
          ch[co].value[0][V_MAGICSHIELD] * POWERSCALE + 1;
...
int fight_driver_pulse_value(int cn) {
  ...
  // say(cn,"pulse gain: %.2f, cost:
  // %.2f",(double)dam/POWERSCALE,(double)min(ch[cn].value[0][V_PULSE]*POWERSCALE/8,ch[cn].mana*8)/POWERSCALE);

  if (dam * 0.95 >
      min(ch[cn].value[0][V_PULSE] * POWERSCALE / 8, ch[cn].mana * 8))
    val = HIGH_PRIO + ch[cn].value[0][V_PULSE];
  else
    val = 0;
...
static int fight_driver_attackback_value(int cn, int co) {
  ...
  if (map[x + y * MAXMAP].flags & (MF_MOVEBLOCK | MF_TMOVEBLOCK))
    return 0;  // spot behind him is blocked, no sense in trying
  ...
  if (ch[cn].x == x && ch[cn].y == y)
    return 0;  // dont try if i'm in front of him

  if (ch[co].action == AC_IDLE && ticker - ch[co].regen_ticker > TICKS / 2)
    return HIGH_PRIO;  // target back if he's just standing around

  if (!map[x + y * MAXMAP].ch)
    return 0;  // dont try if no one is in front of him

  x = ch[co].x + ty;
  y = ch[co].y + tx;
  if (x < 1 || x >= MAXMAP || y < 1 || y >= MAXMAP) return 0;

  if ((cc = map[x + y * MAXMAP].ch) && cc != cn && ch[cc].group == ch[cn].group)
    return 0;  // dont try if someone is on his side already (he will go)
...
void drv_stealth(int cn) { ch[cn].speed_mode = SM_STEALTH; }

void drv_fast(int cn) { ch[cn].speed_mode = SM_FAST; }
...
// attack specific enemy. enemy should be in the fight driver enemy array for
// fireball to work
static int fight_driver_attack_enemy(int cn, int co, int nomove, int nobless,
                                     int noheal, int noflash, int nofireball,
                                     int noball, int noshield, int nowarcry,
                                     int nofreeze, int nopulse) {
  // static char
  // *typename[]={"freeze","fireball","ball","flash","warcry","atttack","moveright","moveleft","moveup","movedown","regenerate","distance3","distance7","bless","earthrain","earthmud","heal","ms","pulse","attackback","flee","firering","max"};
  ...
  if (!nofreeze && ch[cn].value[0][V_FREEZE] > 1 &&
      freeze_value(cn, co) < -10 && ch[cn].mana >= FREEZECOST && tdist < 4 &&
  ...
  if (!noheal && ch[cn].value[0][V_HEAL] > 1 && ch[cn].mana >= POWERSCALE * 2 &&
  ...
  if (!noshield && ch[cn].value[0][V_MAGICSHIELD] > 1 &&
  ...
  if ((ch[cn].flags & CF_EDEMON) && ch[cn].value[1][V_DEMON] == 30 &&
      ch[cn].hp >= ch[cn].value[0][V_HP] *POWERSCALE / 2 &&
  ...
  if (!nobless && ch[cn].value[0][V_BLESS] > 1 && ch[cn].mana >= BLESSCOST &&
  ...
  if (!nofireball && ch[cn].value[0][V_FIREBALL] > 1 &&
      fireball_damage(cn, co, ch[cn].value[0][V_FIREBALL]) >= POWERSCALE &&
      ch[cn].mana >= FIREBALLCOST) {
    if (ishit_fireball(cn, ch[cn].x, ch[cn].y, ch[co].x, ch[co].y,
                       fight_driver_fireball_enemy_check)) {
      ...
      for (n = 1; n < 5; n++) {
        ...
        if (!dirdead[DX_RIGHT] && (map[(ch[cn].x + n) + (ch[cn].y)].flags &
                                   (MF_MOVEBLOCK | MF_TMOVEBLOCK)))
          dirdead[DX_RIGHT] = 1;

        if (!dirdead[DX_LEFT] && ch[cn].x - n >= MAXMAP) dirdead[DX_LEFT] = 1;
        if (!dirdead[DX_LEFT] && (map[(ch[cn].x - n) + (ch[cn].y)].flags &
                                  (MF_MOVEBLOCK | MF_TMOVEBLOCK)))
          dirdead[DX_LEFT] = 1;

        if (!dirdead[DX_DOWN] && ch[cn].y + n >= MAXMAP) dirdead[DX_DOWN] = 1;
        if (!dirdead[DX_DOWN] && (map[(ch[cn].x) + (ch[cn].y + n)].flags &
                                  (MF_MOVEBLOCK | MF_TMOVEBLOCK)))
          dirdead[DX_DOWN] = 1;

        if (!dirdead[DX_UP] && ch[cn].y - n >= MAXMAP) dirdead[DX_UP] = 1;
        if (!dirdead[DX_UP] && (map[(ch[cn].x) + (ch[cn].y - n)].flags &
                                (MF_MOVEBLOCK | MF_TMOVEBLOCK)))
          dirdead[DX_UP] = 1;

        if (!dirdead[DX_RIGHT] &&
            ishit_fireball(cn, ch[cn].x + n, ch[cn].y, ch[co].x, ch[co].y,
                           fight_driver_fireball_enemy_check)) {
          task[maxtask].task = moveright;
          maxvalue += (task[maxtask].value =
                           fight_driver_fireball_value(cn, co) / n + 1);
          maxtask++;
          break;
        }
        if (!dirdead[DX_LEFT] &&
        ...
        if (!dirdead[DX_DOWN] &&
            ishit_fireball(cn, ch[cn].x, ch[cn].y + n, ch[co].x, ch[co].y,
                           fight_driver_fireball_enemy_check)) {
  ...
  if (!noball && ch[cn].value[0][V_FLASH] > 1 &&
      strike_damage(cn, co, ch[cn].value[0][V_FLASH]) > POWERSCALE &&
      ch[cn].mana >= FLASHCOST && cdist > 10 && cdist < 30) {
    if (calc_steps_ball(cn, ch[cn].x, ch[cn].y, ch[co].x, ch[co].y) >
        tdist * 2 - 5) {
  ...
  if (!noflash && ch[cn].value[0][V_FLASH] > 1 &&
      strike_damage(cn, co, ch[cn].value[0][V_FLASH]) > POWERSCALE &&
      ch[cn].mana >= FLASHCOST && tdist < 4 && may_add_spell(cn, IDR_FLASH)) {
  ...
  if (!nofireball && ch[cn].value[0][V_FIREBALL] > 1 &&
      fireball_damage(cn, co, ch[cn].value[0][V_FIREBALL]) >= POWERSCALE &&
      ch[cn].mana >= FIREBALLCOST && tdist < 2 &&
  ...
  if (!nowarcry && ch[cn].value[0][V_WARCRY] > 1 &&
      ch[cn].endurance > ch[cn].value[0][V_WARCRY] * POWERSCALE / 3 &&
      cdist < 8 &&
      (may_add_spell(co, IDR_WARCRY) ||
       (!ch[cn].value[1][V_MAGICSHIELD] &&
        ch[cn].lifeshield < ch[cn].value[0][V_WARCRY] * POWERSCALE / 4))) {
  ...
  tmp = 0;
  if (!nofreeze && !nomove && ch[cn].mana > POWERSCALE * 3 &&
      ch[cn].value[1][V_FREEZE] && freeze_value(cn, co) < -10 &&
      (may_add_spell(co, IDR_FREEZE) && tdist > 3)) {
    tmp += fight_driver_distance3_value(cn, co);
  }
  if (!noflash && !nomove && ch[cn].mana > POWERSCALE * 3 &&
      ch[cn].value[1][V_FLASH] && !may_add_spell(cn, IDR_FLASH)) {
  ...
  if (!nofireball && !nomove && ch[cn].mana > FIREBALLCOST &&
  ...
  if (!nopulse && ch[cn].value[1][V_PULSE] && ch[cn].mana > POWERSCALE &&
      (tmp = fight_driver_pulse_value(cn))) {
  ...
  if (0 && ch[cn].hp < ch[cn].value[0][V_HP] *POWERSCALE / 2 &&
      (tmp = HIGH_PRIO)) {
  ...
  /*//if (ch[cn].flags&CF_PLAYER) {
        say(cn,"---");
        for (n=0; n<maxtask; n++) {
                say(cn,"%d: value %d, type
  %s",cn,task[n].value,typename[task[n].task]);
        }
  //}*/
  for (n = 0; n < maxtask; n++) {
    // if (ch[cn].flags&CF_PLAYER)
    // say(cn,"doing %s",typename[task[n].task]);
    if (task[n].task == attackback &&
        (n == maxtask - 1 || task[n + 1].task != attack))
      ret = 0;
    else
      switch (task[n].task) {
        case freeze:
  ...
  // emergency solution... ????
  if ((!nomove || cdist == 2) && char_dist(cn, co) < 4 &&
      attack_driver(cn, co)) { /* say(cn,"all failed, doing attack"); */
    return 1;
  }
...
int person_cmp(const void *a, const void *b) {
  ...
  if (!((struct person *)b)->visible && ((struct person *)a)->visible)
    return -1;

  if (((struct person *)b)->hurtme && !((struct person *)a)->hurtme) return 1;
  if (!((struct person *)b)->hurtme && ((struct person *)a)->hurtme) return -1;

  tmpa = abs(((struct person *)a)->lastx - mex) +
         abs(((struct person *)a)->lasty - mey);
  tmpb = abs(((struct person *)b)->lastx - mex) +
         abs(((struct person *)b)->lasty - mey);
...
// add enemy to fight driver data structures. returns 1 if new enemy, 0
// otherwise
// be aware that the driver remembers only up to 10 enemies, so it might still
// be
// an old enemy.
int fight_driver_add_enemy(int cn, int co, int hurtme, int visible) {
  ...
  if (!hurtme && dat->start_dist &&
      fight_driver_dist_from_home(cn, co) > dat->start_dist) {
    // say(cn,"(%d) rejected %s because of home_dist
    // %d",cn,ch[co].name,dat->home_dist);
    return 0;
  }
  ...
  if (!hurtme && dat->char_dist && char_dist(cn, co) > dat->char_dist) {
    // say(cn,"(%d) rejected %s because of char_dist
    // %d",cn,ch[co].name,dat->char_dist);
    return 0;
  }

  if (ch[co].x < 1 || ch[co].x >= MAXMAP || ch[co].y < 1 || ch[co].y >= MAXMAP)
    return 0;

  m = ch[co].x + ch[co].y * MAXMAP;
  if (!hurtme && (map[m].flags & MF_NEUTRAL))
    return 0;  // dont attack if enemy is in neutral zone and didnt hurt us
  ...
  memmove(dat->enemy+1,dat->enemy,sizeof(dat->enemy)-sizeof(dat->enemy[0])*(10-n));
...
int fight_driver_update(int cn) {
  ...
  for (n = 0; n < 10; n++) {
    ...
    // say(cn,"enemy %s at %d
    // %d",ch[co].name,dat->enemy[n].lastx,dat->enemy[n].lasty);

    if (char_see_char(cn, co)) {
      // if enemy is too far away from respawn point, remove him from the list
      if (dat->stop_dist &&
          fight_driver_dist_from_home(cn, co) > dat->stop_dist) {
        dat->enemy[n].cn = dat->enemy[n].ID = 0;
...
int fight_driver_attack_visible(int cn, int nomove) {
  ...
  while (42) {
    ...
    for (n = 0; n < 10; n++) {
      ...
      // if (dat->enemy[n].hurtme) score+=5;  // prefer those who hit me
      // before
    ...
    if ((ch[cn].flags & CF_PLAYER) &&
      ...
      if (fight_driver_attack_enemy(cn, dat->enemy[bn].cn, nomove, ppd->nobless,
                                    ppd->noheal, ppd->noflash, ppd->nofireball,
                                    ppd->noball, ppd->noshield, ppd->nowarcry,
                                    ppd->nofreeze, ppd->nopulse))
        return 1;
    } else {
      if (fight_driver_attack_enemy(cn, dat->enemy[bn].cn, nomove, 0, 0, 0, 0,
                                    0, 0, 0, 0, 0)) {
...
int fight_driver_follow_invisible(int cn) {
  ...
  if (abs(ch[cn].x - dat->enemy[n].lastx) < 2 &&
      abs(ch[cn].y - dat->enemy[n].lasty) < 2) {
    dat->enemy[n].cn = 0;
    return 0;
  }

  dir = pathfinder(ch[cn].x, ch[cn].y, dat->enemy[n].lastx, dat->enemy[n].lasty,
                   0, NULL, 0);
  if (dir == -1)
    dir = pathfinder(ch[cn].x, ch[cn].y, dat->enemy[n].lastx,
                     dat->enemy[n].lasty, 1, NULL, 0);
...
int fight_driver_set_dist(int cn, int start_dist, int char__dist,
                          int stop_dist) {
  struct fight_driver_data *dat;
...
int spell_self_driver(int cn) {
  if (ch[cn].value[0][V_BLESS] && ch[cn].mana >= BLESSCOST &&
      may_add_spell(cn, IDR_BLESS) && do_bless(cn, cn)) {
    return 1;
  }
  if (ch[cn].value[0][V_MAGICSHIELD] * POWERSCALE > ch[cn].lifeshield &&
      ch[cn].mana >= POWERSCALE * 3 && do_magicshield(cn)) {
    return 1;
  }
  if (ch[cn].value[0][V_HEAL] &&
      ch[cn].hp < ch[cn].value[0][V_HP] * POWERSCALE / 2 &&
      ch[cn].mana >= POWERSCALE * 3 && do_heal(cn, cn)) {
    return 1;
  }
...
void standard_message_driver(int cn, struct msg *msg, int agressive,
                             int helper) {
  int co, cc;
  ...
  switch (msg->type) {
    ...
    case NT_GOTHIT:  // did we get hit?
      ...
      if (!char_see_char(cn, co))
        fight_driver_add_enemy(cn, co, 1, 0);
      else
...
int is_room_empty(int xs, int ys, int xe, int ye) {
  ...
  if (xs < 0 || xs >= MAXMAP || ys < 0 || ys >= MAXMAP || xe < 0 ||
      xe >= MAXMAP || ye < 0 || ye >= MAXMAP)
    return 0;

  for (y = ys; y <= ye; y += 8) {
    for (x = xs; x <= xe; x += 8) {
      for (cn = getfirst_char_sector(x, y); cn; cn = ch[cn].sec_next) {
        if ((ch[cn].flags & CF_PLAYER) && ch[cn].x >= xs && ch[cn].x <= xe &&
            ch[cn].y >= ys && ch[cn].y <= ye) {
          return 0;
        }
      }
    }
