...
int scan_item_driver(int cn)
{
  ...
  dat = (struct scan_item_data*)set_data(cn, DRD_SCANITEM, sizeof(struct scan_item_data));
...
int mem_add_driver(int cn, int co, int nr)
{
  ...
  dat = (struct char_mem_data*)set_data(cn, DRD_CHARMEM + nr, sizeof(struct char_mem_data));
  if (!dat) return 0;	// oops...
  ...
  if (dat->cnt == dat->max) {
    dat->max += 8;
    dat = (struct char_mem_data*)set_data(cn, DRD_CHARMEM + nr, sizeof(struct char_mem_data) + sizeof(unsigned int) * dat->max);
    if (!dat) return 0;
  }
...
int fight_driver_flee(int cn)
{
  ...
  if (!(dat = (struct fight_driver_data*)set_data(cn, DRD_FIGHTDRIVER, sizeof(struct fight_driver_data)))) return 0;
...
void fight_driver_note_hit(int cn)
{
  ...
  if (!(dat = (struct fight_driver_data*)set_data(cn, DRD_FIGHTDRIVER, sizeof(struct fight_driver_data)))) return;
...
int fight_driver_attack_visible(int cn, int nomove)
{
  ...
  while (42) {
    ...
    if ((ch[cn].flags & CF_PLAYER) && (ppd = (struct lostcon_ppd*)set_data(cn, DRD_LOSTCON_PPD, sizeof(struct lostcon_ppd)))) {
