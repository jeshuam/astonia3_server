...
int driver(int type, int nr, int obj, long ret, int lastact)
{
  ...
  case CDT_SPECIAL:	return special_driver(nr, obj, (char*)(ret));
  default: 	return 0;
  }
}
...
void cmd_npc(int cn, char *ptr)
{
  ...
  static const char *usage = "/npc <base:str> <level:int> <mode:chr> <respawn:int> [nick1:str] [nick2:str]";
...
void cmd_killthrall(int cn, char *ptr)
{
  ...
  for (n = getfirst_char(); n; n = next) {
    ...
    if ((dat = (struct lq_npc_data*)set_data(n, DRD_LQ_NPC_DATA, sizeof(struct lq_npc_data))) && !strcasecmp(dat->thrallname, name)) {
...
void cmd_npcreply(int cn, char *ptr)
{
  ...
  static const char *usage = "/npcreply <npcID|nick> <nr:int> <trigger:str> <reply:str>";
...
void cmd_npcitem(int cn, char *ptr)
{
  int n, cnt = 0;
  char nick[40];
  static const char *usage = "/npcitem <npcID|nick> <base:str> [keyID:int] [name:str] [description:str]";
  struct lq_item li;
...
void cmd_npcrewarditem(int cn, char *ptr)
{
  int n, cnt = 0;
  char nick[40];
  static const char *usage = "/npcrewarditem <npcID|nick> <base:str> [keyID:int] [name:str] [description:str]";
  struct lq_item li;
...
void cmd_questend(int cn, char *ptr)
{
  ...
  for (co = getfirst_char(); co; co = getnext_char(co)) {
    ...
    pdat = (struct lq_plr_data*)set_data(co, DRD_LQ_PLR_DATA, sizeof(struct lq_plr_data));
    if (!pdat) continue;
...
int spawn_npc(int n, int isthrall, int tx, int ty, char *thrallname)
{
  ...
  dat = (struct lq_npc_data*)set_data(cn, DRD_LQ_NPC_DATA, sizeof(struct lq_npc_data));
  if (!dat) {
...
void cmd_exit(int cn, char *ptr)
{
  ...
  if ((pdat = (struct lq_plr_data*)set_data(cn, DRD_LQ_PLR_DATA, sizeof(struct lq_plr_data)))) { co = pdat->usurp; pdat->usurp = 0; }
  if (co && (dat = (struct lq_npc_data*)set_data(co, DRD_LQ_NPC_DATA, sizeof(struct lq_npc_data)))) dat->usurp = 0;
...
void cmd_usurp(int cn, char *ptr)
{
  ...
  if (bco) {
    cmd_exit(cn, NULL);
    if ((pdat = (struct lq_plr_data*)set_data(cn, DRD_LQ_PLR_DATA, sizeof(struct lq_plr_data)))) pdat->usurp = bco;
    if ((dat = (struct lq_npc_data*)set_data(bco, DRD_LQ_NPC_DATA, sizeof(struct lq_npc_data)))) {
      dat->usurp = cn;
    ...
    for (x = xs; x <= xe; x += 8) {
      for (co = getfirst_char_sector(x, y); co; co = ch[co].sec_next) {
        if (ch[co].driver == CDR_LQNPC && strcasestr(ch[co].name, name)) {
          if ((dat = (struct lq_npc_data*)set_data(co, DRD_LQ_NPC_DATA, sizeof(struct lq_npc_data)))) {
            dat->follow = cn;
            cnt++;
          }
        }
...
void cmd_doorlock(int cn, char *ptr)
{
  int n, cnt = 0, keyID;
  char nick[40];
  static const char *usage = "/doorlock <doornick> <keyID:int> (keyID=0 for unlocked)";
...
int special_driver(int nr, int cn, char *ptr)
{
  ...
  if (*ptr == '#' || *ptr == '/') {
    ...
    if (ch[cn].flags & (CF_GOD | CF_LQMASTER)) {
      if (!(pdat = (struct lq_plr_data*)set_data(cn, DRD_LQ_PLR_DATA, sizeof(struct lq_plr_data)))) return 0;
      ...
      if ((co = pdat->usurp) && ch[co].flags && ch[co].driver == CDR_LQNPC &&
          (dat = (struct lq_npc_data*)set_data(co, DRD_LQ_NPC_DATA, sizeof(struct lq_npc_data))) && dat->usurp == cn) {
...
void lqnpc(int cn, int ret, int lastact)
{
  ...
  if ((co = dat->usurp) && (ch[co].flags & (CF_GOD | CF_LQMASTER)) && (pdat = (struct lq_plr_data*)set_data(co, DRD_LQ_PLR_DATA, sizeof(struct lq_plr_data))) && pdat->usurp == cn) {
...
void lq_entrance(int in, int cn)
{
  ...
  if ((ppd = (struct misc_ppd*)set_data(cn, DRD_MISC_PPD, sizeof(struct misc_ppd))) && realtime - ppd->last_lq_death < 60 * 5) {
