...
int get_lq_item(int cn, char *ptr, struct lq_item *li, const char *usage) {
  char basename[40], modbasename[80], name[sizeof(it[0].name)],
      desc[sizeof(it[0].description)];
  int keyID, base;
...
void cmd_npc(int cn, char *ptr) {
  ...
  static const char *usage =
      "/npc <base:str> <level:int> <mode:chr> <respawn:int> [nick1:str] "
      "[nick2:str]";
  ...
  // log_sys(cn,"basename=\"%s\", level=%d, mode=%c, respawn=%d, nick1=\"%s\",
  // nick2=\"%s\".",basename,level,mode,respawn,nick[0],nick[1]);
  ...
  for (n = 1; n < MAXLQNPC; n++) {
    if (lq_npc[n].basename[0] && lq_npc[n].x == ch[cn].x &&
        lq_npc[n].y == ch[cn].y) {
      log_sys(cn, "°c3 %d %s %s is already at this position", n,
              lq_npc[n].nick[0], lq_npc[n].nick[1]);
      return;
    }
  }
  ...
  n = find_free_npc();
  if (!n) {
    log_sys(cn, "No free NPC slots left.");
    return;
  }
...
void cmd_thrall(int cn, char *ptr) {
  ...
  static const char *usage = "/thrall <nick|ID> <count:int> [thrallname:str]";
  ...
  if (n > 0 && n < MAXLQNPC) {
    if (!lq_npc[n].basename[0]) {
      log_sys(cn, "Template not found");
      return;
    }
  } else {
    for (n = 1; n < MAXLQNPC; n++) {
      if (lq_npc[n].basename[0] && (!strcasecmp(lq_npc[n].nick[0], nick) ||
                                    !strcasecmp(lq_npc[n].nick[1], nick))) {
        break;
      }
    }
    if (n == MAXLQNPC) {
...
void cmd_killthrall(int cn, char *ptr) {
  ...
  static const char *usage = "/killthrall <thrallname:str>";
  ...
  for (n = getfirst_char(); n; n = next) {
    ...
    if ((dat = (struct lq_npc_data *)set_data(n, DRD_LQ_NPC_DATA,
                                              sizeof(struct lq_npc_data))) &&
        !strcasecmp(dat->thrallname, name)) {
...
void cmd_npcname(int cn, char *ptr) {
  ...
  static const char *usage = "/npcname <npcID|nick> <name:str>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set name of %d NPCs", cnt);
}

void cmd_npcgold(int cn, char *ptr) {
  int cnt = 0, n, gold;
  char nick[40];
  static const char *usage = "/npcgold <npcID|nick> <gold:int>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set gold of %d NPCs", cnt);
}

void cmd_npcsprite(int cn, char *ptr) {
  int cnt = 0, n, sprite;
  char nick[40];
  static const char *usage = "/npcgold <npcID|nick> <sprite:int>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set sprite of %d NPCs", cnt);
}

void cmd_npcdesc(int cn, char *ptr) {
  int cnt = 0, n;
  char nick[40], desc[sizeof(ch[0].description)];
  static const char *usage = "/npcdesc <npcID|nick> <description:str>";
  ...
  if (n > 0 && n < MAXLQNPC) {
    if (lq_npc[n].basename[0]) {
      strcpy(lq_npc[n].description, desc);
      cnt++;
    }
  } else {
    for (n = 1; n < MAXLQNPC; n++) {
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set description of %d NPCs", cnt);
}

void cmd_npcgreet(int cn, char *ptr) {
  int cnt = 0, n;
  char nick[40], text[256];
  static const char *usage = "/npcgreet <npcID|nick> <text:str>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set greeting of %d NPCs", cnt);
}

void cmd_npckillmark(int cn, char *ptr) {
  int cnt = 0, n, mark;
  char nick[40];
  static const char *usage = "/npckillmark <npcID|nick> <mark:int>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set killmark of %d NPCs", cnt);
}

void cmd_npchurtmark(int cn, char *ptr) {
  int cnt = 0, n, mark;
  char nick[40];
  static const char *usage = "/npchurtmark <npcID|nick> <mark:int>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set hurtmark of %d NPCs", cnt);
}

void cmd_npcmodlevel(int cn, char *ptr) {
  int cnt = 0, n, mod;
  char nick[40];
  static const char *usage = "/npcmodlevel <npcID|nick|all> <mod:int>";
  ...
  if (n > 0 && n < MAXLQNPC) {
    if (lq_npc[n].basename[0]) {
      ...
      if (lq_npc[n].level < 1) {
        lq_npc[n].level = 1;
        log_sys(cn, "NPC %d (%s %s %s) set to level 1 to avoid negative level.",
                n, lq_npc[n].name, lq_npc[n].nick[0], lq_npc[n].nick[1]);
      }
      if (lq_npc[n].level > 200) {
        lq_npc[n].level = 200;
        log_sys(cn,
                "NPC %d (%s %s %s) set to level 200 to avoid too high levels.",
                n, lq_npc[n].name, lq_npc[n].nick[0], lq_npc[n].nick[1]);
      }
    }
  } else {
    for (n = 1; n < MAXLQNPC; n++) {
      if (lq_npc[n].basename[0] &&
          (!strcasecmp(lq_npc[n].nick[0], nick) ||
           !strcasecmp(lq_npc[n].nick[1], nick) || !strcasecmp("all", nick))) {
        lq_npc[n].level += mod;
        cnt++;
        if (lq_npc[n].level < 1) {
          lq_npc[n].level = 1;
          log_sys(cn,
                  "NPC %d (%s %s %s) set to level 1 to avoid negative level.",
                  n, lq_npc[n].name, lq_npc[n].nick[0], lq_npc[n].nick[1]);
        }
        if (lq_npc[n].level > 200) {
          lq_npc[n].level = 200;
          log_sys(
              cn,
              "NPC %d (%s %s %s) set to level 200 to avoid too high levels.", n,
              lq_npc[n].name, lq_npc[n].nick[0], lq_npc[n].nick[1]);
        }
      }
    }
  }
  ...
  if (!cnt)
    ...
    log_sys(cn, "Changed level of %d NPCs", cnt);
}

void cmd_npcrespawn(int cn, char *ptr) {
  int cnt = 0, n, mod;
  char nick[40];
  static const char *usage = "/npcrespawn <npcID|nick|all> <mod:int>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Changed respawn time of %d NPCs to %d", cnt, mod);
}

void cmd_npcpos(int cn, char *ptr) {
  char nick[40];
  int x, y, n, m;
  static const char *usage = "/npcpos <npcID|nick> [x:int] [y:int]";
  ...
  if (n < 1 || n >= MAXLQNPC || !lq_npc[n].basename[0]) {
    for (n = 0, m = 1; m < MAXLQNPC; m++) {
      if (lq_npc[m].basename[0] && (!strcasecmp(lq_npc[m].nick[0], nick) ||
                                    !strcasecmp(lq_npc[m].nick[1], nick))) {
        if (n) {
  ...
  for (m = 1; m < MAXLQNPC; m++) {
    ...
    if (lq_npc[m].x == x && lq_npc[m].y == y) {
      log_sys(cn, "°c3 %d %s %s is already at this position", m,
              lq_npc[m].nick[0], lq_npc[m].nick[1]);
      return;
    }
  }
  lq_npc[n].x = x;
...
void cmd_npcreply(int cn, char *ptr) {
  ...
  static const char *usage =
      "/npcreply <npcID|nick> <nr:int> <trigger:str> <reply:str>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set trigger/reply of %d NPCs", cnt);
}

void cmd_npcwantitem(int cn, char *ptr) {
  int n, cnt = 0, ID;
  char nick[40];
  static const char *usage = "/npcwantitem <npcID|nick> <ID:int>";
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set wantitem of %d NPCs", cnt);
}
...
void show_npc(int cn, int n) {
  ...
  for (i = 0; i < 5; i++)
    if (lq_npc[n].trigger[i][0])
      log_sys(cn, "Trigger/Reply %d: %s/%s", i, lq_npc[n].trigger[i],
              lq_npc[n].reply[i]);

  if (lq_npc[n].carry_gold)
    log_sys(cn, "Gold: %.2fG", lq_npc[n].carry_gold / 100.0);
  if (lq_npc[n].carry_item.base[0])
    log_sys(cn, "Carry Item: %s ID: %d", lq_npc[n].carry_item.base,
            lq_npc[n].carry_item.keyID);

  if (lq_npc[n].want_keyID) log_sys(cn, "Wants ID: %d", lq_npc[n].want_keyID);
  if (lq_npc[n].reward_item.base[0])
    log_sys(cn, "Reward Item: %s ID: %d", lq_npc[n].reward_item.base,
            lq_npc[n].reward_item.keyID);

  if (lq_npc[n].hurt_markID)
    log_sys(cn, "Hurtmark ID: %s (%d), %d exp",
            lq_data.reward_desc[lq_npc[n].hurt_markID], lq_npc[n].hurt_markID,
            lq_data.reward[lq_npc[n].hurt_markID]);
  if (lq_npc[n].kill_markID)
    log_sys(cn, "Killmark ID: %s (%d), %d exp",
            lq_data.reward_desc[lq_npc[n].kill_markID], lq_npc[n].kill_markID,
            lq_data.reward[lq_npc[n].kill_markID]);
}

void cmd_npcshow(int cn, char *ptr) {
  int n, cnt = 0;
  char nick[40];
  static const char *usage = "/npcshow <npcID|nick>";
...
void cmd_npcitem(int cn, char *ptr) {
  ...
  char nick[40];
  static const char *usage =
      "/npcitem <npcID|nick> <base:str> [keyID:int] [name:str] "
      "[description:str]";
  struct lq_item li;
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set item of %d NPCs", cnt);
}
...
void cmd_npcrewarditem(int cn, char *ptr) {
  ...
  char nick[40];
  static const char *usage =
      "/npcrewarditem <npcID|nick> <base:str> [keyID:int] [name:str] "
      "[description:str]";
  struct lq_item li;
...
void cmd_npclist(int cn, char *ptr) {
  int n, cnt = 0, start;
  char nick[40];
  static const char *usage = "/npclist <nick|start>";
  ...
  for (n = max(1, start); n < MAXLQNPC; n++) {
    ...
    if (nick[0] && strcasecmp(lq_npc[n].nick[0], nick) &&
        strcasecmp(lq_npc[n].nick[1], nick))
      continue;
    log_sys(cn, "NPC %3d: base %s, level %d, nicks: %s %s, pos: %d,%d", n,
            lq_npc[n].basename, lq_npc[n].level, lq_npc[n].nick[0],
            lq_npc[n].nick[1], lq_npc[n].x, lq_npc[n].y);
    cnt++;
    if (cnt > 99) break;
  }
  log_sys(cn, "%d of %d NPCs (%d%%)", cnt, MAXLQNPC - 1,
          100 * cnt / (MAXLQNPC - 1));
}
...
void cmd_npcdel(int cn, char *ptr) {
  int n, cnt = 0;
  char nick[40];
  static const char *usage = "/npcdel <npcID|nick>";
  ...
  if (n > 0 && n < MAXLQNPC) {
    if (lq_npc[n].basename[0]) {
      remove_npc(n);
      bzero(&lq_npc[n], sizeof(lq_npc[n]));
      cnt++;
    }
  } else {
    for (n = 1; n < MAXLQNPC; n++) {
...
void cmd_questend(int cn, char *ptr) {
  ...
  for (co = getfirst_char(); co; co = getnext_char(co)) {
    ...
    pdat = (struct lq_plr_data *)set_data(co, DRD_LQ_PLR_DATA,
                                          sizeof(struct lq_plr_data));
    if (!pdat) continue;
    ...
    // xlog("sum=%d, val=%d, level=%d,
    // res=%d",sum,val,ch[co].level,val*min(100,sum)/100);
...
void cmd_questsave(int cn, char *ptr) {
  ...
  char xpassword[40];
  static const char *usage = "/questsave <name> [password]";
  ...
  if (handle != -1) {
    ...
    handle = open(file, O_RDWR | O_CREAT, 0600);
  if (handle == -1) {
...
void cmd_questdel(int cn, char *ptr) {
  ...
  char name[40], password[40], file[80];
  char xpassword[40];
  static const char *usage = "/questdel <name> [password]";
  ...
  if (unlink(file))
    log_sys(cn, "°c3Delete failed at system level.");
  else
    log_sys(cn, "Deleted quest %s.", name);
}
...
void cmd_questload(int cn, char *ptr) {
  ...
  char name[40], password[40], file[80];
  char xpassword[40];
  static const char *usage = "/questload <name> [password]";
...
void lq_raise(int cn, int level) {
  ...
  for (n = 0; n < V_MAX; n++) {
    ...
    ch[cn].value[1][n] =
        cost2skill(n, cost, (ch[cn].flags & (CF_WARRIOR | CF_MAGE)) ==
                                (CF_WARRIOR | CF_MAGE));
    // charlog(cn,"set %s to %d (%d
    // exp)",skill[n].name,ch[cn].value[1][n],cost);
  }
...
void lq_statboost(int cn) {
  ...
  // weapon skill
  if (ch[cn].value[1][V_DAGGER])
    it[in].mod_index[0] = V_DAGGER;
  else if (ch[cn].value[1][V_STAFF])
    it[in].mod_index[0] = V_STAFF;
  else if (ch[cn].value[1][V_SWORD])
    it[in].mod_index[0] = V_SWORD;
  else if (ch[cn].value[1][V_TWOHAND])
    it[in].mod_index[0] = V_TWOHAND;
  else
    it[in].mod_index[0] = V_HAND;

  it[in].mod_value[0] = ch[cn].level / 5 + 1;

  // warrior stuff
  if (ch[cn].value[1][V_ATTACK]) {  // warrior skills
    it[in].mod_index[1] = V_ATTACK;
    it[in].mod_value[1] = ch[cn].level / 5 + 1;
    it[in].mod_index[2] = V_PARRY;
    it[in].mod_value[2] = ch[cn].level / 5 + 1;
    it[in].mod_index[3] = V_TACTICS;
    it[in].mod_value[3] = ch[cn].level / 5 + 1;
  }
  if (ch[cn].value[1][V_WARCRY]) {
    it[in].mod_index[4] = V_WARCRY;
    it[in].mod_value[4] = ch[cn].level / 5 + 1;
  }
  ...
  if (ch[cn].value[1][V_BLESS]) {
    it[in].mod_index[0] = V_BLESS;
    it[in].mod_value[0] = ch[cn].level / 5 + 1;
    flag = 1;
  }
  if (ch[cn].value[1][V_LIGHT]) {
    it[in].mod_index[1] = V_LIGHT;
    it[in].mod_value[1] = ch[cn].level / 5 + 1;
    flag = 1;
  }
  if (ch[cn].value[1][V_FIREBALL]) {
    it[in].mod_index[2] = V_FIREBALL;
    it[in].mod_value[2] = ch[cn].level / 5 + 1;
    flag = 1;
  }
  if (ch[cn].value[1][V_MAGICSHIELD]) {
    it[in].mod_index[3] = V_MAGICSHIELD;
    it[in].mod_value[3] = ch[cn].level / 5 + 1;
    flag = 1;
  }
  if (ch[cn].value[1][V_FREEZE]) {
    it[in].mod_index[4] = V_FREEZE;
    it[in].mod_value[4] = ch[cn].level / 5 + 1;
    flag = 1;
  }
  if (flag) {
  ...
  // misc
  it[in].mod_index[0] = V_IMMUNITY;
  it[in].mod_value[0] = ch[cn].level / 5 + 1;
  it[in].mod_index[1] = V_WIS;
  it[in].mod_value[1] = ch[cn].level / 3 + 1;
  it[in].mod_index[2] = V_INT;
  it[in].mod_value[2] = ch[cn].level / 3 + 1;
  it[in].mod_index[3] = V_AGI;
  it[in].mod_value[3] = ch[cn].level / 3 + 1;
  it[in].mod_index[4] = V_STR;
  it[in].mod_value[4] = ch[cn].level / 3 + 1;
...
int spawn_npc(int n, int isthrall, int tx, int ty, char *thrallname) {
  ...
  if (lq_npc[n].description[0])
    strcpy(ch[cn].description, lq_npc[n].description);
  ...
  dat = (struct lq_npc_data *)set_data(cn, DRD_LQ_NPC_DATA,
                                       sizeof(struct lq_npc_data));
  if (!dat) {
  ...
  if (lq_npc[n].carry_item.base[0] &&
      (in = create_lq_item(&lq_npc[n].carry_item)) && !give_char_item(cn, in)) {
    destroy_item(in);
  }
...
void cmd_nspawn(int cn, char *ptr) {
  int n, cnt = 0;
  char nick[40];
  static const char *usage = "/nspawn <npcID|nick|all>";
  ...
  if (n > 0 && n < MAXLQNPC) {
    ...
    for (n = 1; n < MAXLQNPC; n++) {
      if (lq_npc[n].basename[0] &&
          (!strcasecmp(nick, "all") || !strcasecmp(lq_npc[n].nick[0], nick) ||
           !strcasecmp(lq_npc[n].nick[1], nick))) {
        if (spawn_npc(n, 0, 0, 0, NULL)) cnt++;
      }
    }
  ...
  if (!cnt)
    ...
    log_sys(cn, "Spawned %d NPCs.", cnt);
}

void cmd_nremove(int cn, char *ptr) {
  int n, cnt = 0;
  char nick[40];
  static const char *usage = "/nremove <npcID|nick|all>";
...
void cmd_nsay(int cn, char *ptr) {
  int n, cnt = 0, co;
  char nick[40], text[256];
  static const char *usage = "/nsay <npcID|nick> <text:str>";
...
void cmd_nimmortal(int cn, char *ptr) {
  ...
  char nick[40];
  static const char *usage = "/nimmortal <npcID|nick> <0|1>";
  ...
  if (n > 0 && n < MAXLQNPC) {
    if (lq_npc[n].basename[0] && (co = get_lq_char(n))) {
      if (onoff)
        ch[co].flags |= (CF_IMMORTAL | CF_NOATTACK);
      else
        ch[co].flags &= ~(CF_IMMORTAL | CF_NOATTACK);
      cnt++;
    }
  ...
  if (!cnt)
    ...
    log_sys(cn, "Set immortal to %s on %d NPCs", onoff ? "ON" : "OFF", cnt);
}

void cmd_nemote(int cn, char *ptr) {
  int n, cnt = 0, co;
  char nick[40], text[256];
  static const char *usage = "/nemote <npcID|nick> <text:str>";
...
void cmd_nattack(int cn, char *ptr) {
  int n, cnt = 0, co, cc;
  char nick[40], plr[40];
  static const char *usage = "/nattack <npcID|nick> <player:str>";
  ...
  n = atoi(nick);
  if (n > 0 && n < MAXLQNPC) {
    if (lq_npc[n].basename[0] && (cc = get_lq_char(n))) {
      fight_driver_add_enemy(cc, co, 0, 1);
      cnt++;
    }
  } else {
    for (n = 1; n < MAXLQNPC; n++) {
  ...
  if (!cnt)
    ...
    log_sys(cn, "%d NPCs attacking.", cnt);
}
...
void cmd_exit(int cn, char *ptr) {
  ...
  if ((pdat = (struct lq_plr_data *)set_data(cn, DRD_LQ_PLR_DATA,
                                             sizeof(struct lq_plr_data)))) {
    co = pdat->usurp;
    pdat->usurp = 0;
  }
  if (co && (dat = (struct lq_npc_data *)set_data(co, DRD_LQ_NPC_DATA,
                                                  sizeof(struct lq_npc_data))))
    dat->usurp = 0;
...
void cmd_usurp(int cn, char *ptr) {
  int co;
  char name[40];
  static const char *usage = "/usurp <name:str>";
  ...
  if (bco) {
    ...
    if ((pdat = (struct lq_plr_data *)set_data(cn, DRD_LQ_PLR_DATA,
                                               sizeof(struct lq_plr_data))))
      pdat->usurp = bco;
    if ((dat = (struct lq_npc_data *)set_data(bco, DRD_LQ_NPC_DATA,
                                              sizeof(struct lq_npc_data)))) {
      dat->usurp = cn;
    ...
    log_sys(cn, "NPC not found.");
}

void cmd_follow(int cn, char *ptr) {
  int co;
  char name[40];
  static const char *usage = "/follow <name:str>";
  ...
  for (y = ys; y <= ye; y += 8) {
    for (x = xs; x <= xe; x += 8) {
      for (co = getfirst_char_sector(x, y); co; co = ch[co].sec_next) {
        if (ch[co].driver == CDR_LQNPC && strcasestr(ch[co].name, name)) {
          if ((dat = (struct lq_npc_data *)set_data(
                   co, DRD_LQ_NPC_DATA, sizeof(struct lq_npc_data)))) {
            dat->follow = cn;
            cnt++;
          }
        }
...
void cmd_stop(int cn, char *ptr) {
  int co;
  char name[40];
  static const char *usage = "/stop <name:str>";
  int x, y, xs, xe, ys, ye, cnt = 0;
  struct lq_npc_data *dat;
  ...
  for (n = 1; n < MAXCHARS; n++) {
    if (ch[n].flags && ch[n].driver == CDR_LQNPC) {
      ...
      if (!teleport_char_driver(n, 240, 240) &&
          ...
          !teleport_char_driver(n, 245, 245))
        log_sys(cn, "°c3Could not remove all players, please try again soon.");
    }
  }
  for (n = 1; n < MAXITEM; n++) {
    if (!it[n].x) continue;
    ...
    if (it[n].flags & IF_PLAYERBODY) {
      remove_item_map(n);
      if (!drop_item(n, 240, 240) && !drop_item(n, 235, 240) &&
          !drop_item(n, 240, 235) && !drop_item(n, 235, 235) &&
          !drop_item(n, 245, 240) && !drop_item(n, 240, 245) &&
          !drop_item(n, 245, 245))
        log_sys(
            cn,
            "°c3Could not remove all player bodies, please try again soon.");
      else
        set_expire(n, PLAYER_BODY_DECAY_TIME);
...
void cmd_questreward(int cn, char *ptr) {
  ...
  static const char *usage = "/questreward <nr:int> <amount:int> <desc:str>";
  ...
  if (amount < 1 || amount > 100) {
    log_sys(cn,
            "°c3Amount is out of bounds. It must be in the range 1..100. "
...
void cmd_questlevel(int cn, char *ptr) {
  int mini, maxi;
  static const char *usage = "/questlevel <min:int> <max:int>";
...
void cmd_questshow(int cn, char *ptr) {
  ...
  for (n = 1; n < MAXLQMARK; n++) {
    if (lq_data.reward[n])
      log_sys(cn, "Reward for mark %s (%d) is %d exp.", lq_data.reward_desc[n],
              n, lq_data.reward[n]);
  }
}
...
void cmd_doorlist(int cn, char *ptr) {
  ...
  for (n = 0; n < MAXLQDOOR; n++) {
    if (lq_door[n].nick[0]) {
      log_sys(cn, "Door %d, Nick: %s, Pos: %d,%d, Key: %d.", n, lq_door[n].nick,
              it[lq_door[n].in].x, it[lq_door[n].in].y, lq_door[n].keyID);
    }
  }
}
...
void update_lqdoor(int nr) {
  ...
  *(unsigned int *)(it[in].drdata + 1) =
      MAKE_ITEMID(DEV_ID_LQ, lq_door[nr].keyID);
}
...
void cmd_doorlock(int cn, char *ptr) {
  ...
  char nick[40];
  static const char *usage =
      "/doorlock <doornick> <keyID:int> (keyID=0 for unlocked)";
  ...
  if (!cnt)
    log_sys(cn, "°c3Door not found.");
  else
    log_sys(cn, "Set key for %d doors.", cnt);
}
...
int special_driver(int nr, int cn, char *ptr) {
  ...
  if (*ptr == '#' || *ptr == '/') {
    ...
    if (ch[cn].flags & (CF_GOD | CF_LQMASTER)) {
      ...
      if ((len = cmdcmp(ptr, "npc", 3))) {
        cmd_npc(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "thrall", 3))) {
        cmd_thrall(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "killthrall", 3))) {
        cmd_killthrall(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "usurp", 3))) {
        cmd_usurp(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "follow", 3))) {
        cmd_follow(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "stop", 3))) {
        cmd_stop(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "exit", 3))) {
        cmd_exit(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcname", 5))) {
        cmd_npcname(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcgold", 5))) {
        cmd_npcgold(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcsprite", 5))) {
        cmd_npcsprite(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcpos", 5))) {
        cmd_npcpos(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcdescription", 5))) {
        cmd_npcdesc(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcgreeting", 5))) {
        cmd_npcgreet(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcreply", 5))) {
        cmd_npcreply(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npclist", 5))) {
        cmd_npclist(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcdelete", 5))) {
        cmd_npcdel(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcwantitem", 5))) {
        cmd_npcwantitem(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcitem", 5))) {
        cmd_npcitem(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcshow", 5))) {
        cmd_npcshow(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npckillmark", 5))) {
        cmd_npckillmark(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npchurtmark", 5))) {
        cmd_npchurtmark(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcrewarditem", 5))) {
        cmd_npcrewarditem(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcmodlevel", 5))) {
        cmd_npcmodlevel(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "npcrespawn", 5))) {
        cmd_npcrespawn(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "doorlist", 6))) {
        cmd_doorlist(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "doorlock", 6))) {
        cmd_doorlock(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questsave", 8))) {
        cmd_questsave(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questdelete", 8))) {
        cmd_questdel(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questend", 8))) {
        cmd_questend(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questload", 8))) {
        cmd_questload(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questshow", 8))) {
        cmd_questshow(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questreward", 8))) {
        cmd_questreward(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questlevel", 8))) {
        cmd_questlevel(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questreset", 10))) {
        cmd_questreset(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "questentrance", 8))) {
        cmd_questentrance(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "queststart", 8))) {
        cmd_queststart(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "nspawn", 5))) {
        cmd_nspawn(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "nremove", 5))) {
        cmd_nremove(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "nsay", 4))) {
        cmd_nsay(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "nimmortal", 4))) {
        cmd_nimmortal(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "nemote", 4))) {
        cmd_nemote(cn, ptr + len);
        return 2;
      }
      if ((len = cmdcmp(ptr, "nattack", 4))) {
        cmd_nattack(cn, ptr + len);
        return 2;
      }
      ...
      if ((co = pdat->usurp) && ch[co].flags && ch[co].driver == CDR_LQNPC &&
          (dat = (struct lq_npc_data *)set_data(co, DRD_LQ_NPC_DATA,
                                                sizeof(struct lq_npc_data))) &&
          dat->usurp == cn) {
        ...
        if ((len = cmdcmp(ptr, "c9", 2)) || (len = cmdcmp(ptr, "mirror", 2))) {
          ...
          sprintf(buf, "%010u:%02u:%02u:°c%d%s: %s says: \"%s\"", 0, areaID,
                  areaM, 11, "LQ", ch[co].name, ptr);
          server_chat(9, buf);
...
void lqnpc(int cn, int ret, int lastact) {
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (dat->greeting[0] && cn != co && (ch[co].flags & CF_PLAYER) &&
          char_dist(cn, co) < 12 && char_see_char(cn, co) &&
          !mem_check_driver(cn, co, 7)) {
    ...
    if (msg->type == NT_GOTHIT) {
      co = msg->dat1;
      if (co && (ch[co].flags & CF_PLAYER) && dat->hurt_markID > 0 &&
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (strcasestr(ptr, "followme") && (ch[co].flags & CF_LQMASTER) &&
          char_dist(cn, co) < 20) {
        dat->follow = co;
      }
      if (strcasestr(ptr, "stopfollow") && (ch[co].flags & CF_LQMASTER) &&
          char_dist(cn, co) < 20) {
        dat->follow = 0;
      }
      ...
      if (!co || !char_see_char(cn, co) || cn == co) {
  ...
  if ((co = dat->usurp) && (ch[co].flags & (CF_GOD | CF_LQMASTER)) &&
      (pdat = (struct lq_plr_data *)set_data(co, DRD_LQ_PLR_DATA,
                                             sizeof(struct lq_plr_data))) &&
      pdat->usurp == cn) {
  ...
  if (!domirror && (co = dat->follow) &&
      (ch[co].flags & (CF_GOD | CF_LQMASTER))) {
  ...
  if (!domirror &&
      secure_move_driver(cn, ch[cn].tmpx, ch[cn].tmpy, dat->dir, ret, lastact))
    return;
...
void lq_ticker(int in, int cn) {
  ...
  if (!init_done) {
    ...
    for (m = in2 = 1; in2 < MAXITEM && m < MAXLQDOOR; in2++) {
      if (it[in2].flags && it[in2].driver == 2 && it[in2].drdata[10]) {
        ...
        lq_door[m].in = in2;
        update_lqdoor(m);
        xlog("found door %d %s at %d,%d", m, lq_door[m].nick, it[in2].x,
             it[in2].y);
        m++;
      }
    }
  ...
  for (n = 1; n < MAXLQNPC; n++)
    if (lq_respawn[n] && ticker >= lq_respawn[n] && spawn_npc(n, 0, 0, 0, NULL))
      lq_respawn[n] = 0;
...
void lqnpc_died(int cn, int co) {
  ...
  if (co && (ch[co].flags & CF_PLAYER) &&
      (dat->kill_markID || dat->hurt_markID)) {
    pdat = (struct lq_plr_data *)set_data(co, DRD_LQ_PLR_DATA,
                                          sizeof(struct lq_plr_data));
    if (!pdat) return;  // oops...

    if (dat->kill_markID > 0 && dat->kill_markID < MAXLQMARK)
      pdat->mark[dat->kill_markID] = 1;
    if (dat->hurt_markID > 0 && dat->hurt_markID < MAXLQMARK)
      pdat->mark[dat->hurt_markID] = 1;
  }
}
...
void lq_entrance(int in, int cn) {
  ...
  if (lq_data.min_level > ch[cn].level || lq_data.max_level < ch[cn].level) {
    log_char(cn, LOG_SYSTEM, 0,
             "This quest is for levels %d to %d, you may not enter.",
             lq_data.min_level, lq_data.max_level);
    return;
  }
  ...
  if ((ppd = (struct misc_ppd *)set_data(cn, DRD_MISC_PPD,
                                         sizeof(struct misc_ppd))) &&
      realtime - ppd->last_lq_death < 60 * 5) {
    log_char(
        cn, LOG_SYSTEM, 0,
        "You may not enter again yet. Your remaining penalty is: %.2f minutes.",
        5.0 - (realtime - ppd->last_lq_death) / 60.0);
    return;
  }
