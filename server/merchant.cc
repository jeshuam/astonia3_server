...
void merchant_driver_parse(int cn, struct merchant_driver_data *dat) {
  ...
  for (ptr = nextnv(ch[cn].arg, name, value); ptr;
    ...
    if (!strcmp(name, "dir"))
      ...
      dat->special = atoi(value);
    else if (!strcmp(name, "pricemulti"))
      dat->pricemulti = atoi(value);
    else
...
void merchant_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct merchant_driver_data *)set_data(
      cn, DRD_MERCHANTDRIVER, sizeof(struct merchant_driver_data));
  if (!dat) return;  // oops...
  ...
  if (!ch[cn].store) {
    if (dat->pricemulti)
      create_store(cn, dat->ignore, dat->pricemulti);
    else
      create_store(cn, dat->ignore, 400);
    if (dat->special) {
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      quiet_say(cn, "Hello %s! If you'd like to trade, say: '°c4%s, trade°c0!",
      ...
      if ((msg->dat1 == 1 || msg->dat1 == 2) &&
        ...
        if (strcasestr((char *)msg->dat2, ch[cn].name) &&
          ...
          /*if (abuser(ch[co].ID)) {
                switch(RANDOM(3)) {
                                                        case 0:
          murmur(cn,"I hate cheaters."); break;
                        case 1:   emote(cn,"clenches his fists and
          stares at %s.",ch[co].name); break;
                        case 2:   murmur(cn,"I wish the cheaters
          would leave me alone."); break;
                }
          }*/
          ch[co].merchant = cn;
        }
  ...
  if (dat->dayx) {  // we have day / night positions
    if (!opening_time(dat->open, dat->close)) {  // we're closed
      ...
      if (secure_move_driver(cn, dat->nightx, dat->nighty, dat->nightdir, ret,
                             lastact))
        return;
    } else {  // we're open
      if (dat->doorx &&
  ...
  if (ticker > dat->last_talk + TICKS * 60 && !RANDOM(25)) {
    switch (RANDOM(11)) {
      ...
      case 8:
        ...
        break;
      case 9:
        if (map[ch[cn].x + ch[cn].y * MAXMAP].flags & MF_INDOORS)
          emote(cn, "stares at the ceiling");
        else
          emote(cn, "stares at the sky");
        break;
      case 10:
