...
struct saltmine_ppd *get_saltmine_ppd(int cn)
{
  ...
  dat = (struct saltmine_ppd*)set_data(cn, DRD_SALTMINE_PPD, sizeof(struct saltmine_ppd));
  if (!dat) return NULL;
...
void create_worker(int follow_cn, struct saltmine_ppd *ppd)
{
  ...
  // set initital values
  if ((workerdat = (struct saltmine_worker_data*)set_data(cn, DRD_SALTMINE_WORKER, sizeof(struct saltmine_worker_data)))) {
...
void remove_worker(int cn)
{
  ...
  // get data
  if (!(workerdat = (struct saltmine_worker_data*)set_data(cn, DRD_SALTMINE_WORKER, sizeof(struct saltmine_worker_data)))) return;
...
int monksout(int cn, struct saltmine_ppd *ppd)
{
  ...
  if ((co = getfirst_char())) do {
      ...
      if (!(workerdat = (struct saltmine_worker_data*)set_data(co, DRD_SALTMINE_WORKER, sizeof(struct saltmine_worker_data)))) continue;
