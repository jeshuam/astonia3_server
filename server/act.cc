 ...
 *
 * Act is the counterpart to Do. When a character wants to do something, he
 *calls the
 * corresponding Do-function. This triggers the execution of the Act-function of
 *the
 * same name some time later (exact time depends on character speed and the
 *action).
 *
 * Act is the part which actually does something. act_walk() moves a character
 *from
...
static int act_idle(int cn) {
  ...
  if (ch[cn].x < 1 || ch[cn].x > MAXMAP - 2 || ch[cn].y < 1 ||
      ch[cn].y > MAXMAP - 2)
    return 0;

  m = ch[cn].x + ch[cn].y * MAXMAP;

  if (ticker > ch[cn].regen_ticker + REGEN_TIME &&
      (!(map[m].flags & MF_NOREGEN) || !(ch[cn].flags & CF_PLAYER))) {
    ...
    if (ch[cn].endurance < ch[cn].value[0][V_ENDURANCE] * POWERSCALE) {
      ...
      ch[cn].endurance = min(ch[cn].value[0][V_ENDURANCE] * POWERSCALE,
                             ch[cn].endurance + ch[cn].act1 * val * 15);
      ch[cn].flags |= CF_SMALLUPDATE;
    }
  ...
  if (!(ch[cn].flags & CF_NONOTIFY))
    notify_area(ch[cn].x, ch[cn].y, NT_CHAR, cn, 0, 0);
...
static int act_walk(int cn) {
  ...
  if (ch[cn].tox < 1 || ch[cn].tox > MAXMAP - 2 || ch[cn].toy < 1 ||
      ch[cn].toy > MAXMAP - 2) {
    elog("act_walk(): character %s (%d) has illegal to-position %d,%d.",
  ...
  if (map[m].ch != cn) {
    elog(
        "act_walk(): character %s (%d) is about to leave position %d,%d, but "
        "is not in map.ch.",
    ...
    if (!(map[m].flags & MF_TMOVEBLOCK)) {
      elog(
          "act_walk(): character %s (%d) is about to leave position %d,%d, but "
          "is MF_TMOVEBLOCK is not set in map.flags.",
          ch[cn].name, cn, ch[cn].x, ch[cn].y);
    }
    map[m].ch = 0;
  ...
  if (map[m].ch) {
    elog(
        "act_walk(): character %s (%d) is about to enter position %d,%d, but "
        "there's already someone in map.ch",
        ch[cn].name, cn, ch[cn].x, ch[cn].y);
  }
  ...
  if ((ch[cn].flags & CF_PLAYER) && (map[m].flags & MF_RESTAREA)) {
    if (ch[cn].resta != areaID ||
        abs(ch[cn].x - ch[cn].restx) + abs(ch[cn].y - ch[cn].resty) > 10) {
  ...
  if ((in = map[m].it)) {
    if (in < 1 || in >= MAXITEM) {
      elog("act_walk(): found illegal item %d at %d,%d. removing.", in,
           ch[cn].x, ch[cn].y);
      map[m].it = 0;
      return 1;
    }
...
static int act_take(int cn) {
  ...
  if (!dx2offset(ch[cn].dir, &x, &y, NULL)) {
    elog("act_take(): dx2offset(%d,...) returned error for character %s (%d)",
         ch[cn].dir, ch[cn].name, cn);
    return 0;
  }
  ...
  if (x < 1 || x > MAXMAP - 2 || y < 1 || y > MAXMAP - 2) {
    elog(
        "act_take(): character %s (%d) tried to take item from illegal "
        "position %d,%d.",
        ch[cn].name, cn, x, y);
    return 0;
  }
  ...
  if (in < 1 || in >= MAXITEM) {
    elog("act_take(): item number %d on map position %d,%d is out of bounds.",
         in, x, y);
    map[m].it = 0;
    return 0;
  }
...
static int act_use(int cn) {
  ...
  if (!dx2offset(ch[cn].dir, &x, &y, NULL)) {
    elog("act_use(): dx2offset(%d,...) returned error for character %s (%d)",
         ch[cn].dir, ch[cn].name, cn);
    return 0;
  }
  ...
  if (x < 1 || x > MAXMAP - 2 || y < 1 || y > MAXMAP - 2) {
    elog(
        "act_use(): character %s (%d) tried to take item from illegal position "
        "%d,%d.",
        ch[cn].name, cn, x, y);
    return 0;
  }
  ...
  if (in < 1 || in >= MAXITEM) {
    elog("act_use(): item number %d on map position %d,%d is out of bounds.",
         in, x, y);
    map[m].it = 0;
    return 0;
  }
...
static int act_drop(int cn) {
  ...
  if (!dx2offset(ch[cn].dir, &x, &y, NULL)) {
    elog("act_drop(): dx2offset(%d,...) returned error for character %s (%d)",
         ch[cn].dir, ch[cn].name, cn);
    return 0;
  }
  ...
  if (x < 1 || x > MAXMAP - 2 || y < 1 || y > MAXMAP - 2) {
    elog(
        "act_drop(): character %s (%d) tried to drop item to illegal position "
        "%d,%d.",
        ch[cn].name, cn, x, y);
    return 0;
  }
  ...
  if (in < 1 || in >= MAXITEM) {
    elog(
        "act_drop(): item number %d in citem of character %s (%d) is out of "
        "bounds.",
        in, ch[cn].name, cn);
    ch[cn].citem = 0;
    return 0;
...
static int sub_attack(int cn, int co, int vcn, int vco, int direct) {
  ...
  if (!(ch[cn].flags & (CF_INFRARED | CF_INFRARED)) &&
      check_lightm(ch[co].x + ch[co].y * MAXMAP) < 2)
    vcn -= 8;
  if (!(ch[co].flags & (CF_INFRARED | CF_INFRARED)) &&
      check_lightm(ch[cn].x + ch[cn].y * MAXMAP) < 2)
    vco -= 8;
  ...
  if ((tmp = die(1, 100)) < chance) {
    ...
    if (direct && ch[cn].prof[P_ASSASSIN] && is_back(co, cn) &&
        ch[co].action == AC_IDLE) {
    ...
    dam = 0;
    if (direct) {
      if (!ch[cn].item[WN_RHAND] || !ch[co].item[WN_RHAND])
        sound_area(ch[cn].x, ch[cn].y, 8);
      else if (!RANDOM(2))
        sound_area(ch[cn].x, ch[cn].y, 34);
      else
        sound_area(ch[cn].x, ch[cn].y, 35);
    }
  }

  if (showattack)
    say(cn, "attack %s, diff=%d (%d %d), chan=%d, percent=%d, dam=%d",  //,
        // vcn=%d
        //(%s),
        // vco=%d
        //(%s)
        ch[co].name, diff, vcn, vco, chance, per, dam);
...
static int sub_surround(int cn, int dx, int dy) {
  ...
  // if (ch[cn].flags&CF_PLAYER) say(cn,"sub attack %s %s
  // (surround)",ch[cn].name,ch[co].name);
...
static int act_attack(int cn) {
  ...
  if (!dx2offset(ch[cn].dir, &x, &y, NULL)) {
    elog("act_attack(): dx2offset(%d,...) returned error for character %s (%d)",
         ch[cn].dir, ch[cn].name, cn);
    return 0;
  }
  ...
  if (x < 1 || x > MAXMAP - 2 || y < 1 || y > MAXMAP - 2) {
    elog(
        "act_attack(): character %s (%d) tried to attack at illegal position "
        "%d,%d.",
        ch[cn].name, cn, x, y);
    return 0;
  }
  ...
  // away.
  if (co != map[m].ch && co != map[m + 1].ch && co != map[m - 1].ch &&
      co != map[m + MAXMAP].ch && co != map[m - MAXMAP].ch &&
      co != map[m + MAXMAP + 1].ch && co != map[m + MAXMAP - 1].ch &&
      co != map[m - MAXMAP + 1].ch && co != map[m - MAXMAP - 1].ch)
    return 0;  // not the one he intended to hit
  ...
  if (!is_facing(co,
                 cn)) {  // attacking someone from the side or back gets a bonus
    vco -= 8;
    if (ch[cn].prof[P_ASSASSIN])
      vcn += ch[cn].prof[P_ASSASSIN];  // assassins get special bonus
  }
  if (is_back(
          co,
          cn)) {  // attacking someone from the back gets an additional bonus
    vco -= 8;
...
static int act_give(int cn) {
  ...
  if (!dx2offset(ch[cn].dir, &x, &y, NULL)) {
    elog("act_give(): dx2offset(%d,...) returned error for character %s (%d)",
         ch[cn].dir, ch[cn].name, cn);
    return 0;
  }
  ...
  if (x < 1 || x > MAXMAP - 2 || y < 1 || y > MAXMAP - 2) {
    elog(
        "act_give(): character %s (%d) tried to attack at illegal position "
        "%d,%d.",
        ch[cn].name, cn, x, y);
    return 0;
  }
  ...
  if (ch[cn].flags & CF_PLAYER) {
    ...
    log_char(cn, LOG_SYSTEM, 0, "You gave a %s (%s) to %s.", it[in].name,
             it[in].description, ch[co].name);
  }
  if (ch[co].flags & CF_PLAYER) {
    dlog(co, in, "was given %s from %s", it[in].name, ch[cn].name);
    log_char(co, LOG_SYSTEM, 0, "You received a %s (%s) from %s.", it[in].name,
             it[in].description, ch[cn].name);
  }
...
static int act_firering(int cn) {
  ...
  *(signed long*)(it[in].drdata + 4) = ticker;
  ...
  if (ch[cn].flags) {  // need to check here since hurt might kill cn
                       // (indirectly, via a char-dead driver)
    if (!(ch[cn].flags & CF_NONOTIFY))
      notify_area(ch[cn].x, ch[cn].y, NT_CHAR, cn, 0, 0);
...
static int act_fireball(int cn) {
  ...
  fn = create_fireball(cn, ch[cn].x, ch[cn].y, ch[cn].act1, ch[cn].act2,
                       spellpower(cn, V_FIREBALL));
...
static int act_earthrain(int cn) {
  ...
  fn = create_earthrain(cn, ch[cn].act1 % MAXMAP, ch[cn].act1 / MAXMAP,
                        ch[cn].act2);
...
static int act_earthmud(int cn) {
  ...
  fn = create_earthmud(cn, ch[cn].act1 % MAXMAP, ch[cn].act1 / MAXMAP,
                       ch[cn].act2);
...
static int act_flash(int cn) {
  ...
  if (ch[cn].value[1][V_DURATION])
    duration += FLASHDURATION * ch[cn].value[0][V_DURATION] / DURATION;
  else if (ch[cn].flags & CF_ARCH)
    duration += FLASHDURATION * ch[cn].level / DURATION / 2;
...
static int act_ball(int cn) {
  ...
  fn = create_ball(cn, ch[cn].x, ch[cn].y, ch[cn].act1, ch[cn].act2,
                   spellpower(cn, V_FLASH));
  if (fn) {
    if (!(ch[cn].flags & CF_NONOTIFY))
      notify_area(ch[cn].x, ch[cn].y, NT_CHAR, cn, 0, 0);
    notify_area(ch[cn].x, ch[cn].y, NT_SPELL, cn, V_FLASH, fn);
...
static int act_ok(int cn) { return 1; }
...
static int act_magicshield(int cn) {
  ...
  ch[cn].lifeshield =
      min(ch[cn].value[0][V_MAGICSHIELD] * POWERSCALE, ch[cn].lifeshield + str);
...
int bless_self(int cn) {
  ...
  if (ch[cn].value[1][V_DURATION])
    duration += BLESSDURATION * ch[cn].value[0][V_DURATION] / DURATION;
  else if (ch[cn].flags & CF_ARCH)
    duration += BLESSDURATION * ch[cn].level / DURATION / 2;
...
static int act_bless(int cn, int flag) {
  ...
  if (cn != co && (ch[cn].flags & CF_PLAYER) && (ch[co].flags & CF_NOBLESS))
    return 0;

  // dont allow bless other in teufelheim arena
  if (areaID == 34 && cn != co &&
      (map[ch[cn].x + ch[cn].y * MAXMAP].flags & MF_ARENA))
    return 0;
...
static void warcry_someone(int cn, int co, int pwr) {
  ...
  if (ch[cn].flags & CF_ARCH)
    endtime =
        ticker + WARCRYDURATION + WARCRYDURATION * ch[cn].level / DURATION / 2;
  else
...
static int act_warcry(int cn) {
  ...
  for (y = ys; y <= ye; y++) {
    for (x = xs; x <= xe; x++) {
      ...
      if (pathfinder(ch[cn].x, ch[cn].y, x, y, 0, warcry_check_target, 50) ==
          -1)
        continue;
  ...
  if (!ch[cn].value[1][V_MAGICSHIELD] && areaID != 33) {
    ch[cn].lifeshield =
        min(get_lifeshield_max(cn) * POWERSCALE,
            ch[cn].lifeshield + ch[cn].value[0][V_WARCRY] * POWERSCALE / 2);
  }
...
static void freeze_someone(int cn, int co) {
  ...
  if (ch[cn].value[1][V_DURATION])
    duration += FREEZEDURATION * ch[cn].value[0][V_DURATION] / DURATION;
  else if (ch[cn].flags & CF_ARCH)
    duration += FREEZEDURATION * ch[cn].level / DURATION / 2;
  ...
  *(signed long*)(it[in].drdata + 4) = ticker;
  ...
  if ((ch[cn].flags & CF_IDEMON) &&
      ch[cn].value[1][V_DEMON] > ch[co].value[0][V_COLD] &&
      ice_curse(co, ch[cn].value[1][V_DEMON] - ch[co].value[0][V_COLD],
                (ch[cn].value[1][V_DEMON] - ch[co].value[0][V_COLD]) * 50)) {
    log_char(
        co, LOG_SYSTEM, 0,
        "You have been frozen by %s. You feel like you'll never thaw again.",
        ch[cn].name);
  }  // else log_char(co,LOG_SYSTEM,0,"You have been frozen by
     // %s.",ch[cn].name);
...
static void pulse_someone(int cn, int co) {
  ...
  // say(cn,"pulse of pwr %d giving str %.2f against %s
  // (%d,%d,%d)",ch[cn].value[0][V_PULSE],(double)str/POWERSCALE,ch[co].name,ch[co].value[0][V_IMMUNITY],ch[co].value[0][V_TACTICS],ch[co].value[0][V_IMMUNITY]+ch[co].value[0][V_TACTICS]/5);
...
void check_container_item(int cn) {
  ...
  if (!dx2offset(ch[cn].dir, &x, &y, NULL)) {
    elog(
        "check_container_item(): dx2offset(%d,...) returned error for "
        "character %s (%d)",
        ch[cn].dir, ch[cn].name, cn);
  ...
  if (x < 1 || x > MAXMAP - 2 || y < 1 || y > MAXMAP - 2) {
    elog(
        "check_container_item(): character %s (%d) tried to use item from "
  ...
  if (in < 1 || in >= MAXITEM) {
    elog(
        "check_container_item(): item number %d on map position %d,%d is out "
        "of bounds.",
        in, x, y);
    map[m].it = 0;
  ...
  if (con[ct].owner && charID(cn) != con[ct].owner &&
      charID(cn) != con[ct].killer &&
      charID(cn) != con[ct].access) {  // access denied
    ch[cn].con_in = 0;
    return;
  }
}
...
static int act(int cn) {
  ...
  if (ch[cn].x < 1 || ch[cn].x > MAXMAP - 2 || ch[cn].y < 1 ||
      ch[cn].y > MAXMAP - 2) {
    elog("act(): character %s (%d) has illegal position %d,%d.", ch[cn].name,
         cn, ch[cn].x, ch[cn].y);
    if (ch[cn].flags & CF_PLAYER)
      kick_char(cn);
    else
  ...
  switch (ch[cn].action) {
    ...
    default:
      elog("act(): unknown action %d for character %s (%d)", ch[cn].action,
           ch[cn].name, cn);
      return 0;
  }
}
...
void demon_protect_check(int cn) {
  ...
  m = ch[cn].x + ch[cn].y * MAXMAP;
  if (!(map[m].flags & MF_INDOORS) && ch[cn].value[0][V_DEMON] &&
      sunlight > 200) {
    ch[cn].value[0][V_DEMON] = 0;
    log_char(cn, 0, LOG_SYSTEM,
             "The full sunlight destroyed your demonic protection.");
  }
      ...
      if (map[m].flags & MF_UNDERWATER) {
        if (!(ch[cn].flags & CF_OXYGEN))
          ...
          if (!(tmp % 6) && !((tmp / TICKS) % 3)) {
            create_map_effect(EF_BUBBLE, ch[cn].x, ch[cn].y, ticker, ticker + 1,
                              0, 45);
            if (!(tmp % 12)) sound_area(ch[cn].x, ch[cn].y, 44 + RANDOM(3));
          }
        }
      } else {
        if ((map[m].gsprite & 0xffff) >= 59706 &&
            (map[m].gsprite & 0xffff) <= 59709)
          hurt(cn, 250, 0, 1, 25, 66);
        else
          hurt(cn, 100, 0, 1, 25, 66);
      }
    }
  }
}
...
void stealth_special(int cn) {
  ...
  if (ch[cn].endurance < POWERSCALE / 2) {
    ...
    log_char(cn, LOG_SYSTEM, 0,
             "Your concentration was broken. You can no longer hide.");
  }
}
...
void regenerate(int cn) {
  ...
  if (diff > 0 &&
    ...
    if (ch[cn].speed_mode != SM_FAST &&
        (ch[cn].speed_mode != SM_STEALTH || !(ch[cn].flags & CF_THIEFMODE))) {
      if (ch[cn].value[1][V_REGENERATE] &&
          ch[cn].endurance < ch[cn].value[0][V_ENDURANCE] * POWERSCALE) {
        ch[cn].endurance = min(ch[cn].value[0][V_ENDURANCE] * POWERSCALE,
                               ch[cn].endurance +
                                   (ch[cn].value[0][V_REGENERATE] +
                                    ch[cn].value[1][V_REGENERATE]) *
                                       diff * 5);
        ch[cn].flags |= CF_SMALLUPDATE;
      }
      if (ch[cn].value[1][V_MAGICSHIELD] && ch[cn].value[1][V_MEDITATE] &&
          ch[cn].lifeshield < ch[cn].value[0][V_MAGICSHIELD] * POWERSCALE &&
          areaID != 33) {
        ch[cn].lifeshield = min(
            ch[cn].value[0][V_MAGICSHIELD] * POWERSCALE,
            ch[cn].lifeshield +
                (ch[cn].value[0][V_MEDITATE] + ch[cn].value[1][V_MEDITATE]) *
                    diff * 4);
        ch[cn].flags |= CF_SMALLUPDATE;
      }
    }
  ...
  if (ch[cn].lifeshield < 0) {
    xlog("character %d (%s) lifeshield=%d (%X). fixed.", cn, ch[cn].name,
         ch[cn].lifeshield, ch[cn].lifeshield);
    ch[cn].lifeshield = 0;
  }
}
...
void payment_check(int cn) {
  if (ch[cn].paid_till && ch[cn].paid_till < time_now) {
    ...
    if (ch[cn].player)
      kick_player(ch[cn].player, "Your payment ran out.");
    else
      kick_char(cn);
  }
}
...
void tick_char(void) {
  ...
  for (n = getfirst_char(); n; n = next) {
    next = getnext_char(n);  // we need to get next here since act might free
                             // (kill) the current char

    if (!ch[n].flags) {
      xlog("tick_char: got unused char %d (%s) from getfirst/getnext (%d)!!", n,
           ch[n].name, next);
      break;
    }
    ...
    if (lastact != AC_IDLE || ch[n].action != AC_IDLE || !lastdur ||
