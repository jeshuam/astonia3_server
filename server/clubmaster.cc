...
struct qa qa[] = {
    ...
    {{"club", NULL},
     "Say 'found: <club name>' to found a club. The first weekly payment of "
     "10000g is due immediately.",
     0},
...
void clubmaster_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct clubmaster_driver_data *)set_data(
      cn, DRD_CLUBMASTERDRIVER, sizeof(struct clubmaster_driver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (!get_char_club(cn) && !get_char_clan(cn))
        quiet_say(cn, "Hello %s! Would you like to found a °c4club°c0?",
                  ch[co].name);
      ...
      if ((msg->dat1 == 1 || msg->dat1 == 2) &&
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "found:"))) {
          if (!(ch[co].flags & CF_PAID)) {
            quiet_say(cn,
                      "I'm sorry, %s, but only paying players may found clubs.",
                      ch[co].name);
            ...
            if (ch[co].gold >= 10000 * 100) {
              ...
              if ((n = create_club(name))) {
                ...
                ch[co].clan_rank = 2;
                quiet_say(cn,
                          "Congratulations, %s, you are now the leader of the "
                          "club %s.",
                          ch[co].name, club[n].name);
                dlog(co, 0, "created club %d %s", n, club[n].name);
              } else
                quiet_say(cn, "Something's wrong with the name.");
            } else
              quiet_say(cn, "You cannot pay the fee of 10,000 gold.");
          } else
            quiet_say(cn,
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "accept:"))) {
          if (!get_char_club(co) || ch[co].clan_rank < 1) {
            ...
            quiet_say(cn, "To join %s's club %s, say: 'join: %s'", dat->join,
                      dat->accept, dat->join);
          }
        }
        if ((ptr = strcasestr((char *)msg->dat2, "join:"))) {
          if (get_char_clan(co) || get_char_club(co)) {
            quiet_say(cn, "You are already a clan or club member, %s.",
                      ch[co].name);
          } else {
            ptr += 5;
            while (isspace(*ptr)) ptr++;
            ...
            if (strcasecmp(dat->accept, ch[co].name)) {
              ...
              ch[co].clan_rank = 0;
              quiet_say(cn, "%s, you are now a member of %s's club.",
                        ch[co].name, dat->join);
              dat->accept[0] = 0;
              dat->accept_clan = 0;
              dat->join[0] = 0;
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "leave!"))) {
          if (!get_char_club(co)) {
            ...
            remove_member(co, co);
            quiet_say(cn, "You are no longer a member of any club, %s",
                      ch[co].name);
          }
        }
          ...
          if (!get_char_club(co) || ch[co].clan_rank < 2) {
            ...
            if (cc) {
              if (!(ch[cc].flags & CF_PAID) && rank > 0) {
                ...
                quiet_say(cn, "%s is the club's founder, cannot change rank.",
                          ch[cc].name);
              ...
              if (uID == -1) {
                ...
                task_set_clan_rank(uID, ch[co].ID,
                                   get_char_club(co) + CLUBOFFSET, rank,
                                   ch[co].name);
            ...
            if (cc) {
              if (get_char_club(cc) == get_char_club(co)) {
                if (ch[cc].clan_rank < 2) {
                  ...
                  quiet_say(cn, "You cannot fire the founder of the club.");
              } else
                quiet_say(cn,
              ...
              if (uID == -1) {
                ...
                task_fire_from_clan(uID, ch[co].ID,
                                    get_char_club(co) + CLUBOFFSET,
                                    ch[co].name);
        ...
        if ((ptr = strcasestr((char *)msg->dat2, "deposit:"))) {
          if (!(n = get_char_club(co))) {
            ...
            if (val > 0 && ch[co].gold >= val) {
              club[n].money += val;
              take_money(co, val);
              quiet_say(cn, "You have deposited %dG, for a total of %dG, %s.",
                        val / 100, club[n].money / 100, ch[co].name);
              dlog(co, 0, "Deposited %dG into club %d, for a new total of %dG",
                   val / 100, n, club[n].money / 100);
              db_update_club(n);
            } else
              quiet_say(cn, "You do not have that much gold, %s.", ch[co].name);
          }
        }
        if ((ptr = strcasestr((char *)msg->dat2, "withdraw:"))) {
          if (!(n = get_char_club(co)) || ch[co].clan_rank < 2) {
            ...
            if (val > 0 && club[n].money >= val) {
              ...
              quiet_say(cn,
                        "You have withdrawn %dG, money left in club %dG, %s.",
                        val / 100, club[n].money / 100, ch[co].name);
              dlog(co, 0, "Withdrew %dG from club %d, for a new total of %dG",
                   val / 100, n, club[n].money / 100);
              db_update_club(n);
            } else
              quiet_say(cn, "The club does not have that much gold, %s.",
                        ch[co].name);
          }
        }
      }
