...
void superior_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct superior_driver_data *)set_data(
      cn, DRD_SUPERIORDRIVER, sizeof(struct superior_driver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      if ((msg->dat1 == 1 || msg->dat1 == 2) &&
          msg->dat3 != cn) {  // talk, and not our talk
        if (strcasestr((char *)msg->dat2, "Nazimah") && dat->nr == 1)
          dat->stun = ticker + TICKS * 60;
        if (strcasestr((char *)msg->dat2, "Argatoth") && dat->nr == 2)
          dat->stun = ticker + TICKS * 60;
        if (strcasestr((char *)msg->dat2, "Lorganoth") && dat->nr == 3)
          dat->stun = ticker + TICKS * 60;
        if (strcasestr((char *)msg->dat2, "Markanoth") && dat->nr == 4)
          dat->stun = ticker + TICKS * 60;
      }
    }
  ...
  if (ch[cn].hp < 10 * POWERSCALE || ch[cn].lifeshield < POWERSCALE * 5)
    dat->mode = M_RUN;
  ...
  if (type == 0) {
    if (!in2 || it[in2].ID != IID_AREA2_ZOMBIESKULL1) {
      log_char(cn, LOG_SYSTEM, 0,
               "You sense that this ancient shrine used to receive gifts. "
               "Strange gifts. You feel a craving for bone.");
      return;
    }
  } else if (type == 1) {
    if (!in2 || it[in2].ID != IID_AREA2_ZOMBIESKULL2) {
      log_char(cn, LOG_SYSTEM, 0,
               "You sense that this ancient shrine used to receive gifts. "
               "Strange gifts. You feel a craving for bone and silver.");
      return;
    }
  } else {
    if (!in2 || it[in2].ID != IID_AREA2_ZOMBIESKULL3) {
      log_char(cn, LOG_SYSTEM, 0,
               "You sense that this ancient shrine used to receive gifts. "
               "Strange gifts. You feel a craving for bone and gold.");
      return;
    }
  }
  ...
  if (type == 0) {
    ...
    switch (r) {
      ...
      case 18:
        ...
        if (ch[cn].flags & CF_WARRIOR)
          add_bonus_spell(cn, IDR_HP, 5, TICKS * 60 * 5);
        else
          add_bonus_spell(cn, IDR_MANA, 5, TICKS * 60 * 5);
        break;
    ...
    switch (r) {
      ...
      case 9:
        ...
        if (ch[cn].flags & CF_WARRIOR)
          add_bonus_spell(cn, IDR_HP, 10, TICKS * 60 * 15);
        else
          add_bonus_spell(cn, IDR_MANA, 10, TICKS * 60 * 15);
        break;
...
void moonie_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct moonie_data *)set_data(cn, DRD_MOONIE_DATA,
                                       sizeof(struct moonie_data));
  if (!dat) return;  // oops...
...
void vampire_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct vampire_driver_data *)set_data(
      cn, DRD_VAMPIREDRIVER, sizeof(struct vampire_driver_data));
  if (!dat) return;  // oops...
...
void vampire2_driver(int cn, int ret, int lastact) {
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GOTHIT) {
      ...
      if ((ch[co].flags) && (in = ch[co].item[WN_RHAND]) &&
          it[in].ID == IID_AREA2_DAGGERRIGHT) {
        say(cn, "Arrrgh!");
        kill_char(cn, co);

        if (ch[co].flags & CF_PLAYER)
          dlog(co, in, "dropped because vampire was killed with it");
        remove_item_char(in);
        destroy_item(in);
      ...
      if ((ch[co].flags) && (in = ch[co].item[WN_RHAND]) &&
          it[in].ID == IID_AREA2_DAGGERWRONG) {
        say(cn, "Hahahaha!");

        if (ch[co].flags & CF_PLAYER)
          dlog(co, in, "dropped because vampire was not killed with it");
        remove_item_char(in);
        destroy_item(in);
        log_char(co, LOG_SYSTEM, 0, "Your dagger was destroyed in the blow.");
        return;
...
void parkshrine_driver(int in, int cn) {
  ...
  switch (it[in].drdata[0]) {
    case 1:
      new_ = !ppd->kelly_found1;
      ppd->kelly_found1 = 1;
      break;
    case 2:
      new_ = !ppd->kelly_found2;
      ppd->kelly_found2 = 1;
      break;
    case 3:
      new_ = !ppd->kelly_found3;
      ppd->kelly_found3 = 1;
      break;
    default:
...
void vampire_dead_driver(int cn, int co) {
  ...
  ppd =
      (struct area3_ppd *)set_data(co, DRD_AREA3_PPD, sizeof(struct area3_ppd));
  if (!ppd) return;
