...
void superior_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct superior_driver_data*)set_data(cn, DRD_SUPERIORDRIVER, sizeof(struct superior_driver_data));
  if (!dat) return;	// oops...
...
void moonie_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct moonie_data*)set_data(cn, DRD_MOONIE_DATA, sizeof(struct moonie_data));
  if (!dat) return;	// oops...
...
void vampire_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct vampire_driver_data*)set_data(cn, DRD_VAMPIREDRIVER, sizeof(struct vampire_driver_data));
  if (!dat) return;	// oops...
...
void parkshrine_driver(int in, int cn)
{
  ...
  case 1:		new_ = !ppd->kelly_found1; ppd->kelly_found1 = 1; break;
  case 2:		new_ = !ppd->kelly_found2; ppd->kelly_found2 = 1; break;
  case 3:		new_ = !ppd->kelly_found3; ppd->kelly_found3 = 1; break;
