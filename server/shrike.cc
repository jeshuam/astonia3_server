...
static int is_fullnight() { return moonlight && dlight < 100; }
...
void rock_driver(int in, int cn) {
  ...
  if (!(in2 = ch[cn].citem)) {
    log_char(
        cn, LOG_SYSTEM, 0,
        "You cannot take the piece of silver. The stone is too heavy to move.");
    return;
  }

  if (it[in2].driver != IDR_FORESTSPADE) {
    log_char(cn, LOG_SYSTEM, 0,
             "You cannot get enough leverage with this to move the stone. "
             "Maybe a long stick with something thin on its end would do the "
             "trick.");
    return;
  }
  ...
  destroy_item(in2);
  log_char(cn, LOG_SYSTEM, 0,
           "You use the spade as a lever to move the stone. You take the piece "
           "of silver. Just as you try to remove the spade it snaps.");
...
void door_driver(int in, int cn) {
  ...
  if (!cn) {
    ...
    if (ch[cn].level < 65) {
      log_char(cn, LOG_SYSTEM, 0,
               "You feel a tingling in your fingers and decide not to touch "
               "the door again until you have grown stronger (requires level "
               "65).");
      return;
    }
    if (!(in2 = ch[cn].citem) ||
        !strstr(it[in2].description, " of the Moon.")) {
      log_char(cn, LOG_SYSTEM, 0,
               "Your fingers tingle as you run them over the metal of the door "
               "and a whispered voice can be heard \"The Moon Blade\" is the "
               "key.");
      return;
    }
...
void pool_driver(int in, int cn) {
  ...
  if (it[in2].driver != IDR_SHRIKEAMULET || it[in2].drdata[0] != 7 ||
      !is_fullnight()) {
    log_char(cn, LOG_SYSTEM, 0, "Your %s is wet now.",
             lower_case(it[in2].name));
    return;
  }
...
void cube_driver(int in, int cn) {
  ...
  if (cn) {  // player using item
    ...
    if ((map[m2].flags & (MF_MOVEBLOCK | MF_TMOVEBLOCK)) || map[m2].it ||
        map[m2].gsprite < 59753 || map[m2].gsprite > 59761) {
...
void shr_werewolf_dead(int cn, int co) {
  ...
  if ((ch[co].flags & CF_PLAYER) &&
    ...
    ppd->shrike_fails++;
    say(cn,
        "I have deserved death. But still... I was hoping for something "
        "better.");
  }
}
