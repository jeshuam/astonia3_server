 ...
 * fixed bug in clan_dungeon_chat(): missing break would remove simple potion
...
static const char *rel_name[] = {"none",    "Alliance", "Peace-Treaty",
                                 "Neutral", "War",      "Feud"};
...
static const char *bonus_name[MAXBONUS] = {
    "Pentagram Quest",  // 0
    "Military Advisor",  // 1
    "Merchant",  // 2
    "unassigned",  // 3
...
int score_to_level(int score) { return score / 100; }
...
void showclan(int cn) {
  ...
  for (n = 1; n < MAXCLAN; n++) {
    ...
    log_char(
        cn, LOG_SYSTEM, 0, "\001%d \004%d jewels \010%s \030Raiding %s (+%d)",
        n, clan[n].treasure.jewels - (clan[n].treasure.debt / 1000),
        clan[n].name, clan[n].dungeon.doraid
                          ? "ON"
                          : (clan[n].dungeon.raidonstart ? "PENDING" : "OFF"),
  ...
  if ((cnr = get_char_clan(cn))) {
    ...
    if (rank < 0 || rank > 4) {
      elog("showclan(): got illegal clan rank (%s,%d,%d)", ch[cn].name, cn,
           rank);
      rank = 0;
    }

    log_char(cn, LOG_SYSTEM, 0,
             "You are a member of the clan '%s' (%d), your rank is %s.",
             clan[cnr].name, cnr, clan[cnr].rankname[rank]);
    if (rank > 0) {
      log_char(cn, LOG_SYSTEM, 0,
               "Your clan has %d jewels, %.3f expenses per week, and a debt of "
               "%.3f jewels. Your clan's treasure holds %dG.",
               clan[cnr].treasure.jewels,
               clan[cnr].treasure.cost_per_week / 1000.0,
               clan[cnr].treasure.debt / 1000.0, clan[cnr].depot.money);

      log_char(cn, LOG_SYSTEM, 0,
               "%d +0 Warriors, %d +2 Warriors, %d +4 Warriors, %d +6 "
               "Warriors, %d +8 Warriors, %d +10 Warriors",
               clan[cnr].dungeon.warrior[1][0], clan[cnr].dungeon.warrior[1][1],
               clan[cnr].dungeon.warrior[1][2], clan[cnr].dungeon.warrior[1][3],
               clan[cnr].dungeon.warrior[1][4],
               clan[cnr].dungeon.warrior[1][5]);
      log_char(cn, LOG_SYSTEM, 0,
               "%d +0 Mages, %d +2 Mages, %d +4 Mages, %d +6 Mages, %d +8 "
               "Mages, %d +10 Mages",
               clan[cnr].dungeon.mage[1][0], clan[cnr].dungeon.mage[1][1],
               clan[cnr].dungeon.mage[1][2], clan[cnr].dungeon.mage[1][3],
               clan[cnr].dungeon.mage[1][4], clan[cnr].dungeon.mage[1][5]);

      log_char(cn, LOG_SYSTEM, 0,
               "%d +0 Seyans, %d +2 Seyans, %d +4 Seyans, %d +6 Seyans, %d +8 "
               "Seyans, %d +10 Seyans",
               clan[cnr].dungeon.seyan[1][0], clan[cnr].dungeon.seyan[1][1],
               clan[cnr].dungeon.seyan[1][2], clan[cnr].dungeon.seyan[1][3],
               clan[cnr].dungeon.seyan[1][4], clan[cnr].dungeon.seyan[1][5]);

      log_char(cn, LOG_SYSTEM, 0, "%d teleports, %d fake walls, %d keys",
               clan[cnr].dungeon.teleport[1], clan[cnr].dungeon.fake[1],
               clan[cnr].dungeon.key[1]);

      for (n = 1; n < 22; n++) {
        if (n < 7)
          cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.warrior[1][n - 1]);
        else if (n < 13)
          cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.mage[1][n - 7]);
        else if (n < 19)
          cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.seyan[1][n - 13]);
        else if (n == 19)
          cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.teleport[1]);
        else if (n == 20)
          cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.fake[1]);
        else if (n == 21)
          cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.key[1]);
      }
      ...
      log_char(
          cn, LOG_SYSTEM, 0,
          "Training score is at %d, guard levels are +%d, next update in %dm",
    ...
    for (n = 0; n < MAXBONUS; n++) {
      if (clan[cnr].bonus[n].level) {
        log_char(cn, LOG_SYSTEM, 0, "Bonus '%s' (%d) is at level %d.",
                 bonus_name[n], n, clan[cnr].bonus[n].level);
      }
    }
    ...
    if (clan[cnr].dungeon.doraid) {
      ...
      if (clan[cnr].dungeon.raidonstart) {
        log_char(cn, LOG_SYSTEM, 0, "Raiding will be on in %.2f minutes.",
                 ((60 * 60 * 48) - (realtime - clan[cnr].dungeon.raidonstart)) /
                     60.0);
      } else {
...
int show_clan_info(int cn, int co, char *buf) {
  ...
  return sprintf(buf, "%s is a member of the clan '%s', %s rank is %s. ",
                 Hename(co), clan[cnr].name, hisname(co),
                 clan[cnr].rankname[rank]);
}
...
void show_clan_relation(int cn, int cnr) {
  ...
  for (n = 1; n < MAXCLAN; n++) {
    ...
    if (crel < 1 || crel > 5) {
      elog(
          "show_clan_relation(): got illegal current_relation %d in clan %d "
          "for clan %d",
          crel, cnr, n);
      crel = 0;
    }
    if (wrel < 1 || wrel > 5) {
      elog(
          "show_clan_relation(): got illegal want_relation %d in clan %d for "
          "clan %d",
          wrel, cnr, n);
      crel = 0;
    }
    ...
    log_char(cn, LOG_SYSTEM, 0, "%d: %s: %s (%s [%02d:%02d] - %s [%02d:%02d])",
             n, clan[n].name, rel_name[crel], rel_name[wrel], (wdiff / 60) / 60,
             (wdiff / 60) % 60, rel_name[orel], (odiff / 60) / 60,
             (odiff / 60) % 60);
  }
}
  ...
  switch (update_state) {
    ...
    case 1:
      ...
      if (res == 1) {
        if (size) {
          if (size != sizeof(clan)) {
            elog("Incompatible Clan data found! (%d vs %lu)", size,
                 sizeof(clan));
            exit(1);
          } else
            memcpy(clan, tmp, size);
          xfree(tmp);
        }
        update_time = ticker;
        update_state++;
...
int found_clan(char *name, int cn, int *pclan) {
  ...
  *pclan = n;
  ...
  add_clanlog(*pclan, clan_serial(*pclan), ch[cn].ID, 1,
              "Clan was founded by %s", ch[cn].name);
...
int add_jewel(int nr, int cn) {
  ...
  add_clanlog(nr, clan_serial(nr), ch[cn].ID, 25, "%s added a jewel",
              ch[cn].name);
...
int cnt_jewels(int nr) { return clan[nr].treasure.jewels; }

int get_clan_bonus(int cnr, int nr) { return clan[cnr].bonus[nr].level; }
...
int set_clan_raid(int cnr, int cn, int onoff) {
  if (onoff && !clan[cnr].dungeon.doraid && !clan[cnr].dungeon.raidonstart) {
    ...
    add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 1, "%s set raiding to ON",
                ch[cn].name);
    clan_changed = 1;
    return 0;
  }
  ...
  if (!onoff && !clan[cnr].dungeon.doraid && clan[cnr].dungeon.raidonstart) {
    ...
    add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 1, "%s canceled raiding",
                ch[cn].name);
    clan_changed = 1;
    return 0;
  }
...
int set_clan_website(int cnr, char *site, int cn) {
  ...
  add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 35, "%s set website %s",
              ch[cn].name, site);
...
int set_clan_message(int cnr, char *site, int cn) {
  ...
  add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 35, "%s set message %s",
              ch[cn].name, site);
...
int set_clan_dungeon_use(int cnr, int type, int number, int cn) {
  ...
  for (n = 1; n < 22; n++) {
    if (n == type)
      cost += get_clan_dungeon_cost(n, number);
    else if (n < 7)
      cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.warrior[1][n - 1]);
    else if (n < 13)
      cost += get_clan_dungeon_cost(n, clan[cnr].dungeon.mage[1][n - 7]);
  ...
  add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 35,
              "%s set dungeon use of type %d to %d", ch[cn].name, type, number);
...
int set_clan_relation(int cnr, int onr, int rel, int cn) {
  ...
  if (clan[cnr].status.want_relation[onr] != rel)
    clan[cnr].status.want_date[onr] = realtime;
  clan[cnr].status.want_relation[onr] = rel;
  clan_changed = 1;

  add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 22,
              "%s set relation to clan %s (%d) to %s (%d)", ch[cn].name,
              get_clan_name(onr), onr, rel_name[rel], rel);
...
int set_clan_rankname(int cnr, int rank, char *name, int cn) {
  ...
  add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 33, "%s set rank name %d to %s",
              ch[cn].name, rank, name);
...
static int update_relations(void) {
  ...
  for (n = 1; n < MAXCLAN; n++) {
    ...
    for (m = 1; m < MAXCLAN; m++) {
      ...
      if (!clan[n].dungeon.doraid) {
        clan[n].status.current_relation[m] =
            min(CS_NEUTRAL, clan[n].status.current_relation[m]);
        clan[n].status.want_relation[m] =
            min(CS_NEUTRAL, clan[n].status.want_relation[m]);
      }
      ...
      if (want1 == want2) {  // both want the same, set it
        clan[n].status.current_relation[m] =
            clan[m].status.current_relation[n] = want1;
        clan_changed = 1;
        add_clanlog(n, clan_serial(n), 0, 10, "%s with %s (%d) started",
                    rel_name[want1], get_clan_name(m), m);
        add_clanlog(m, clan_serial(m), 0, 10, "%s with %s (%d) started",
                    rel_name[want1], get_clan_name(n), n);
        continue;
      }
      ...
      switch (cur) {
        case CS_ALLIANCE:  // we are at alliance, but one of them wants out
          if (want1 > CS_ALLIANCE && diff1 > 60 * 60 * 24) {
            cur++;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "Alliance with %s (%d) ended",
                        get_clan_name(m), m);
            add_clanlog(m, clan_serial(m), 0, 10, "Alliance with %s (%d) ended",
                        get_clan_name(n), n);
            break;
          }
          ...
          break;
        case CS_PEACETREATY:  // we're at treaty, going up to alliance is only
                              // possible if both want
          // it, and thats dealt with above. therefore, one of them wants out
          if (want1 > CS_PEACETREATY && diff1 > 60 * 60 * 24) {
            cur++;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10,
                        "Peace Treaty with %s (%d) ended", get_clan_name(m), m);
            add_clanlog(m, clan_serial(m), 0, 10,
                        "Peace Treaty with %s (%d) ended", get_clan_name(n), n);
            break;
          }
        ...
        case CS_NEUTRAL:  // we're at neutral, lets see
          if (want1 > CS_NEUTRAL &&
              want2 >
                  CS_NEUTRAL) {  // both want at least war, ok without timecheck
            cur++;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "War with %s (%d) started",
                        get_clan_name(m), m);
            add_clanlog(m, clan_serial(m), 0, 10, "War with %s (%d) started",
                        get_clan_name(n), n);
            break;
          }
          if (want1 > CS_NEUTRAL &&
              diff1 > 60 * 60) {  // nr1 wants war, needs 1 hour
            cur++;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "War with %s (%d) started",
                        get_clan_name(m), m);
          ...
          if (want2 > CS_NEUTRAL &&
              diff2 > 60 * 60) {  // nr2 wants war, needs 1 hour
            cur++;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "War with %s (%d) started",
                        get_clan_name(m), m);
          ...
          if (want1 < CS_NEUTRAL &&
              want2 < CS_NEUTRAL) {  // both want a better status, ok at once
            cur--;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10,
                        "Peace Treaty with %s (%d) started", get_clan_name(m),
                        m);
            add_clanlog(m, clan_serial(m), 0, 10,
                        "Peace Treaty with %s (%d) started", get_clan_name(n),
                        n);
            break;
          }
          break;
        case CS_WAR:  // we're at war. changing to feud can only happen if both
                      // want it
          if (want1 < CS_WAR &&
              want2 < CS_WAR) {  // both want a better status, ok at once
            cur--;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "War with %s (%d) ended",
                        get_clan_name(m), m);
            add_clanlog(m, clan_serial(m), 0, 10, "War with %s (%d) ended",
                        get_clan_name(n), n);
            break;
          }
          break;
        case CS_FEUD:  // we have a feud, but at least one of them wants out
          if (want1 < CS_FEUD &&
              want2 < CS_FEUD) {  // both want a better status, ok at once
            cur--;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "Feud with %s (%d) ended",
                        get_clan_name(m), m);
            add_clanlog(m, clan_serial(m), 0, 10, "Feud with %s (%d) ended",
                        get_clan_name(n), n);
            break;
          }
          if (want1 < CS_FEUD &&
              diff1 > 60 * 60 * 24) {  // nr1 wants out, needs 24 hours
            cur--;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "Feud with %s (%d) ended",
                        get_clan_name(m), m);
          ...
          if (want2 < CS_FEUD &&
              diff2 > 60 * 60 * 24) {  // nr2 wants out, needs 24 hours
            cur--;
            clan_changed = 1;
            add_clanlog(n, clan_serial(n), 0, 10, "Feud with %s (%d) ended",
                        get_clan_name(m), m);
      ...
      clan[n].status.current_relation[m] = clan[m].status.current_relation[n] =
          cur;
    }
  }
  return 0;
...
static int update_treasure(void) {
  ...
  for (cnr = 1; cnr < MAXCLAN; cnr++) {
    ...
    if (clan[cnr].treasure.debt >= 2000) {
      ...
      add_clanlog(cnr, clan_serial(cnr), 0, 1,
                  "Clan %s went broke and was deleted", get_clan_name(cnr));
      clan[cnr].name[0] = 0;
...
static int update_training(void) {
  ...
  for (cnr = 1; cnr < MAXCLAN; cnr++) {
    ...
    clan[cnr].dungeon.training_score =
        (int)(clan[cnr].dungeon.training_score * 0.95f);
    xlog("clan %d, new training score %d", cnr,
         clan[cnr].dungeon.training_score);
    clan_changed = 1;
  }
...
void add_member(int cn, int cnr, char *master) {
  ...
  add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 15,
              "%s was added to clan by %s", ch[cn].name, master);
...
void remove_member(int cn, int master_cn) {
  ...
  if (ch[cn].clan < CLUBOFFSET)
    add_clanlog(ch[cn].clan, clan_serial(ch[cn].clan), ch[cn].ID, 15,
                "%s was fired from clan by %s", ch[cn].name,
                ch[master_cn].name);
...
void clan_money_change(int cnr, int diff, int cn) {
  ...
  if (cn && (diff >= 100 || diff < 0)) {
    if (diff > 0)
      add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 28, "%s deposited %dG",
                  ch[cn].name, diff);
    else
      add_clanlog(cnr, clan_serial(cnr), ch[cn].ID, 28, "%s withdrew %dG",
                  ch[cn].name, -diff);
  }
...
void clan_dungeon_chat(const char *ptr) {
  ...
  switch (type) {
    ...
    case 'J':
      ...
      if (cnt_jewels(onr) < 10) {
        return;
      }

      if (cnt > 0) {
        xlog("clan %d: clan %d stole %d jewels (level=%d)", cnr, onr, cnt,
             level);
        ...
        add_clanlog(cnr, clan_serial(cnr), cID, 5,
                    "Clan was raided by %s of %s (%d) for %d jewels", ptr + 19,
                    get_clan_name(onr), onr, cnt);
        add_clanlog(onr, clan_serial(onr), cID, 5,
                    "%s raided clan %s (%d) for %d jewels", ptr + 19,
                    get_clan_name(cnr), cnr, cnt);
      }
      break;
    ...
    case 'X':  // %02d:X:%02d:%10u:%s
      ...
      level = atoi(ptr + 5);
      cID = atoi(ptr + 8);
      xlog("clan %d: %s won a jewel from level % d spawn", cnr, ptr + 19,
           level);

      add_clanlog(cnr, clan_serial(cnr), cID, 5,
                  "%s won a jewel from level %d spawn", ptr + 19, level);
  }
}
...
void show_clan_pots(int cn) {
  int cnr, n;
  static const char *psize[] = {"Small", "Medium", "Big"};
  ...
  for (n = 0; n < 6; n++) {
    log_char(cn, LOG_SYSTEM, 0, "Attack, Parry, Immunity+%d: \016%d", n * 4 + 4,
             clan[cnr].dungeon.alc_pot[0][n]);
  }
  for (n = 0; n < 6; n++) {
    log_char(cn, LOG_SYSTEM, 0, "Flash, Magic Shield, Immunity+%d: \016%d",
             n * 4 + 4, clan[cnr].dungeon.alc_pot[1][n]);
  }
  for (n = 0; n < 3; n++) {
    log_char(cn, LOG_SYSTEM, 0, "%s healing potions: \016%d", psize[n],
             clan[cnr].dungeon.simple_pot[0][n]);
  }
  for (n = 0; n < 3; n++) {
    log_char(cn, LOG_SYSTEM, 0, "%s mana potions: \016%d", psize[n],
             clan[cnr].dungeon.simple_pot[1][n]);
  }
  for (n = 0; n < 3; n++) {
    log_char(cn, LOG_SYSTEM, 0, "%s combo potions: \016%d", psize[n],
             clan[cnr].dungeon.simple_pot[2][n]);
  }
}
...
int add_alc_potion(int nr, int in) {
  ...
  if (it[in].mod_index[0] == V_ATTACK && it[in].mod_index[1] == V_PARRY &&
      it[in].mod_index[2] == V_IMMUNITY) {
    str = min(5, (it[in].mod_value[0] / 4) - 1);
    clan[nr].dungeon.alc_pot[0][str]++;
    clan_changed = 1;
    return 0;
  }
  if (it[in].mod_index[0] == V_FLASH && it[in].mod_index[1] == V_MAGICSHIELD &&
      it[in].mod_index[2] == V_IMMUNITY) {
    str = min(5, (it[in].mod_value[0] / 4) - 1);
...
int add_simple_potion(int nr, int co) {
  ...
  for (n = 30; n < INVENTORYSIZE; n++) {
    if ((in = ch[co].item[n]) && it[in].driver == IDR_POTION) {
      ...
      if (it[in].drdata[1] == 8 && it[in].drdata[2] == 8 &&
          it[in].drdata[3] == 8) {
        flag = 1;
        clan[nr].dungeon.simple_pot[2][0]++;
      }
      if (it[in].drdata[1] == 16 && it[in].drdata[2] == 16 &&
          it[in].drdata[3] == 16) {
        flag = 1;
        clan[nr].dungeon.simple_pot[2][1]++;
      }
      if (it[in].drdata[1] == 32 && it[in].drdata[2] == 32 &&
          it[in].drdata[3] == 32) {
        flag = 1;
...
int get_clan_ready() { return update_done; }

int get_clan_raid(int cnr) { return clan[cnr].dungeon.doraid; }
