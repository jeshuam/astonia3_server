...
void guard_driver(int cn, int ret, int lastact)
{
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      ppd = (struct caligar_ppd*)set_data(co, DRD_CALIGAR_PPD, sizeof(struct caligar_ppd));
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 2:		ppd = (struct caligar_ppd*)set_data(co, DRD_CALIGAR_PPD, sizeof(struct caligar_ppd));
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      case 1: 	quiet_say(cn, "Hello there. I hear you need a key made. Well, for a small fee of 5000 gold I would be more than willing to do it. °c4Yes, Okay°c0 / °c4No, not today°c0");
        ...
        didsay = 1;
        break;
      case 2:         appd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
    ...
    if (msg->type == NT_TEXT) {
      ...
      case 4:		quiet_say(cn, "Okay, come back if you change your mind.");
        break;
      case 5:		appd = (struct arkhata_ppd*)set_data(co, DRD_ARKHATA_PPD, sizeof(struct arkhata_ppd));
...
void caligar_training(int in, int cn)
{
  ...
  ppd = (struct caligar_ppd*)set_data(cn, DRD_CALIGAR_PPD, sizeof(struct caligar_ppd));
  if (!ppd) return;
