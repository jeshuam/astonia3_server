...
static void expire_timer(int in, int serial, int x, int y, int expire_sn) {
  ...
  if (it[in].x != x || it[in].y != y)
    return;  // item is not where it was supposed to be
  ...
  // xlog("%s (%d): max=%d, cur=%d,
  // dam=%d",it[in].name,in,it[in].max_damage,it[in].cur_damage,dam);
...
int set_expire(int in, int duration) {
  ...
  return set_timer(ticker + duration, expire_timer, in, it[in].serial, it[in].x,
                   it[in].y, it[in].expire_sn);
}
...
static void expire_timer_body(int in, int serial, int x, int y, int dummy) {
  ...
  if (container_itemcnt(in) && *(unsigned int*)(it[in].drdata + 8)) {
    set_timer(ticker + TICKS * 5, expire_timer_body, in, it[in].serial,
              it[in].x, it[in].y, 0);
    return;
  }

  if (it[in].flags & IF_PLAYERBODY)
    ilog("expire: destroying item %s (%s) (%d), cnt=%d, pos=%d,%d", it[in].name,
         it[in].description, in, container_itemcnt(in), it[in].x, it[in].y);
  if (remove_item_map(in)) {
    if (it[in].content) destroy_item_container(in);
    free_item(in);
    return;
...
int set_expire_body(int in, int duration) {
  ...
  *(unsigned int*)(it[in].drdata + 8) = duration / (TICKS * 5);
  ...
  return set_timer(ticker + TICKS * 5, expire_timer_body, in, it[in].serial,
                   it[in].x, it[in].y, 0);
}
