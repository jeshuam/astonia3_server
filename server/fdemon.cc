...
void take_soldiers(int cn)
{
  ...
  ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd));
  if (!ppd) return;	// oops...
  ...
  for (n = 0; n < MAXSOLDIER; n++) {
    ...
    if (ppd->soldier[n].type) {
      ...
      dat = (struct farmy_data*)set_data(co, DRD_FARMYDATA, sizeof(struct farmy_data));
      if (dat) {
...
void platoon_exp(int cn, int cm, int amount, int pts, struct farmy_ppd *ppd)
{
  ...
  ppd2 = (struct military_ppd*)set_data(cm, DRD_MILITARY_PPD, sizeof(struct military_ppd));
...
void fdemon_army(int cn, int ret, int lastact)
{
  ...
  dat = (struct farmy_data*)set_data(cn, DRD_FARMYDATA, sizeof(struct farmy_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 && char_see_char(cn, co) && ch[cn].driver != CDR_LOSTCON &&
          (ppd = (struct farmy_ppd*)set_data(co, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && realtime - ppd->boss_timer > 5) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (res == 8 && (ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 && char_see_char(cn, co) &&
          (ppd = (struct farmy_ppd*)set_data(co, DRD_FARMY_PPD, sizeof(struct farmy_ppd)))) {
        switch (ppd->boss_stage) {
        case 0:
        case 1:
        case 2:
...
void fdemon_loader(int in, int cn)
{
  ...
  if (cn) {	// player using item
    ...
    if (it[in].drdata[6] == 1 && (ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 0 && ppd->boss_stage <= 5) {
      log_char(cn, LOG_SYSTEM, 0, "You've solved your mission. Now head back to the commander to claim your reward!");
      ppd->boss_stage = 6;
    }
    if (it[in].drdata[6] == 3 && (ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 7 && ppd->boss_stage <= 8) {
      log_char(cn, LOG_SYSTEM, 0, "You've solved your mission. Now head back to the commander to claim your reward!");
      ppd->boss_stage = 9;
    }
    if (it[in].drdata[6] == 2 && (ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 10 && ppd->boss_stage <= 11) {
      log_char(cn, LOG_SYSTEM, 0, "You've solved your mission. Now head back to the commander to claim your reward!");
      ppd->boss_stage = 12;
    }
    if (it[in].drdata[6] == 4 && (ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 13 && ppd->boss_stage <= 14) {
      ppd->boss_counter |= 1;
    ...
    if (it[in].drdata[6] == 5 && (ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 13 && ppd->boss_stage <= 14) {
      ppd->boss_counter |= 2;
      if ((ppd->boss_counter & 3) == 3) {
        log_char(cn, LOG_SYSTEM, 0, "You've solved your mission. Now head back to the commander to claim your reward!");
        ppd->boss_stage = 15;
    ...
    if (it[in].drdata[6] == 6 && (ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 25 && ppd->boss_stage <= 26) {
      log_char(cn, LOG_SYSTEM, 0, "You've solved your mission. Now head back to the commander to claim your reward!");
      ppd->boss_stage = 27;
    }
    if (it[in].drdata[6] >= 7 && it[in].drdata[6] <= 35 && (ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) && ppd->boss_stage >= 28) {
      int bit;
...
void fdemon_demon(int cn, int ret, int lastact)
{
  ...
  dat = (struct fdemon_data*)set_data(cn, DRD_FDEMONDATA, sizeof(struct fdemon_data));
  if (!dat) return;	// oops...
...
void fdemon_demon_dead(int cn, int co)
{
  ...
  if ((ch[co].flags & CF_PLAYERLIKE) && (dat = (struct farmy_data*)set_data(co, DRD_FARMYDATA, sizeof(struct farmy_data)))) {	// its a soldier
    cc = dat->platoon[MAXSOLDIER];
  } else cc = co;

  if (!(ch[cc].flags & CF_PLAYER)) return;

  if (!(ppd = (struct farmy_ppd*)set_data(cc, DRD_FARMY_PPD, sizeof(struct farmy_ppd)))) return;
...
void fdemon_blood(int in, int cn)
{
  ...
  if ((ppd = (struct farmy_ppd*)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd))) &&
