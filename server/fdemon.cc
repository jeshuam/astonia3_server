...
struct qa qa[] = {
    ...
    {{"oh", "what", "a", "nice", "day", "it", "is", "isn't", "it", NULL},
     NULL,
     1,
     QA_NICEDAY},
    {{"the", "nights", "here", "are", "scary", "aren't", "they", NULL},
     NULL,
     1,
     QA_NICEDAY},
    ...
    {{"shouldn't", "we", "turn", "back", "what", "do", "you", "think", NULL},
     NULL,
     1,
     QA_TURNBACK},
    {{"ha", "i'm", "the", "greatest", "right", NULL}, NULL, 1, QA_GREATEST},
    {{"i'm", "becoming", "a", "great", "soldier", NULL},
     NULL,
     1,
     QA_GREATSOLDIER},
    {{"i'm", "afraid", NULL}, NULL, 1, QA_AFRAID},

    {{"why", "are", "you", "so", "mean", "to", "me", NULL},
     NULL,
     1,
     QA_WHYMEAN},
    {{"and", "you", "small", "like", "a", "ratling", NULL},
     NULL,
     1,
     QA_SMELLRATLING},
    {{"what's", "up", "with", "you", NULL}, NULL, 1, QA_WHATSUP},
    {{"shut", "up", NULL}, NULL, 1, QA_SHUTUP},
    {{"yeah", "i", "hope", "we'll", "do", "something", "soon", NULL},
     NULL,
     1,
     QA_DOSOON},
    {{"stop", "bothering", "me", NULL}, NULL, 1, QA_STOPBOTHER},
    {{"I'm", "bored", "too", "let's", "not", "fight", NULL},
     NULL,
     1,
     QA_NOTFIGHT},
    ...
    {{"one", "day", "you'll", "be", "a", "great", "soldier", NULL},
     NULL,
     1,
     QA_ONEDAY},
...
void update_soldier(int co, int n, struct farmy_ppd *ppd) {
  ...
  if (ppd->soldier[n].type == 1)
    cc = create_char("army1s", 0);
  else
    cc = create_char("army2s", 0);
  if (!cc) {
...
void take_soldiers(int cn) {
  ...
  ppd =
      (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD, sizeof(struct farmy_ppd));
  if (!ppd) return;  // oops...
  ...
  for (n = 0; n < MAXSOLDIER; n++) {
    if (n == 0 && rank > 0 && !ppd->soldier[n].type) {
      ...
      if (ch[cn].flags & CF_MALE)
        assign_profile(
            n, RANDOM(ARRAYSIZE(profile)) / 2 + ARRAYSIZE(profile) / 2, ppd);
      else
        assign_profile(n, RANDOM(ARRAYSIZE(profile)) / 2, ppd);
    }
    if (n == 1 && rank > 4 && !ppd->soldier[n].type) {
      ...
      do {
        if (ch[cn].flags & CF_MALE)
          pro = RANDOM(ARRAYSIZE(profile) / 2);
        else
          pro = RANDOM(ARRAYSIZE(profile) / 2) + ARRAYSIZE(profile) / 2;
        for (m = 0; m < n; m++)
    ...
    if (ppd->soldier[n].type) {
      if (ppd->soldier[n].type == 1)
        co = create_char("army1s", 0);
      else
        co = create_char("army2s", 0);
      if (!co) {
        elog("take_soldiers: could not create char");
        continue;
      }
      ...
      if (profile[ppd->soldier[n].profile].gender == 'M') {
        ch[co].flags |= CF_MALE;
        if (ppd->soldier[n].type == 1)
          ch[co].sprite = profile[ppd->soldier[n].profile].sprite;
        else
          ch[co].sprite = profile[ppd->soldier[n].profile].sprite + 1;
      } else {
      ...
      dat = (struct farmy_data *)set_data(co, DRD_FARMYDATA,
                                          sizeof(struct farmy_data));
      if (dat) {
      ...
      if (dat) {
        for (m = 0; m < MAXSOLDIER; m++) {
          if (ppd->soldier[n].type) {
            ...
            // say(co,"I am %s, slot %d is
            // %s",ch[co].name,m,ch[dat->platoon[m]].name);
          } else
            dat->platoon[m] = 0;
        }
...
void drop_soldiers(int cn) {
  ...
  for (co = getfirst_char(); co; co = next) {
    ...
    if (ch[co].group == ch[cn].group && !(ch[co].flags & CF_PLAYER)) {
      ...
      if (n < MAXSOLDIER) {
        // say(co,"slot=%d, group=%d,
        // (%d/%d)",n,ch[co].group,ch[co].exp-ch[co].exp_used,ppd->soldier[n].exp);
        dat = (struct farmy_data *)set_data(co, DRD_FARMYDATA,
                                            sizeof(struct farmy_data));
        if (dat) {
...
int army_follow_driver(int cn, struct farmy_data *dat, int dist) {
  ...
  if (char_see_char(cn, co)) {
    dat->lx = ch[co].x;
    dat->ly = ch[co].y;
    if (abs(ch[cn].x - dat->lx) + abs(ch[cn].y - dat->ly) <= dist ||
        (dir = pathfinder(ch[cn].x, ch[cn].y, dat->lx, dat->ly, 2, NULL, 0)) ==
            -1)
      return 0;

  } else {
    if (abs(ch[cn].x - dat->lx) + abs(ch[cn].y - dat->ly) == 0 ||
        (dir = pathfinder(ch[cn].x, ch[cn].y, dat->lx, dat->ly, 0, NULL, 0)) ==
            -1)
      return 0;
  }
...
int army_front_driver(int cn, struct farmy_data *dat, int dist) {
  ...
  if (abs(ch[cn].x - x) + abs(ch[cn].y - y) <= dist || !char_see_char(cn, co) ||
      (dir = pathfinder(ch[cn].x, ch[cn].y, x, y, 2, NULL, 0)) == -1) {
    return 0;
  }
...
int army_back_driver(int cn, struct farmy_data *dat) {
  if (ch[cn].x == dat->opt1 && ch[cn].y == dat->opt2 &&
      do_walk(cn, (ch[cn].dir + 3) % 8 + 1))
    return 1;
  else {
    if (ticker - dat->timer > TICKS * 5)
      dat->mission = MIS_FOLLOW;
    else
      return do_idle(cn, TICKS / 2);
  }
  return 0;
}
...
void platoon_exp(int cn, int cm, int amount, int pts, struct farmy_ppd *ppd) {
  ...
  ppd2 = (struct military_ppd *)set_data(cm, DRD_MILITARY_PPD,
  ...
  if (rank < 24 && rank > get_army_rank_int(cm)) {
    ...
    say(cn, "You've been promoted to %s. Congratulations, %s!",
        get_army_rank_string(cm), ch[cm].name);
  }
}
...
void do_emote(int cn, struct farmy_data *dat) {
  ...
  if (dat->emote.lonely > 5000) {
    for (n = 0; n < MAXSOLDIER + 1; n++) {
      co = dat->platoon[n];
      if (!co) continue;
      if ((ch[co].flags & (CF_MALE | CF_FEMALE)) ==
          (ch[cn].flags & (CF_MALE | CF_FEMALE)))
        continue;
...
void got_emote(int cn, int co, int slot, int nr, struct farmy_data *dat) {
  switch (nr) {
    case QA_YES:  // yes etc.
      if (dat->emote.answer_type != AT_YESNO || dat->emote.answer_cn != co ||
          ticker - dat->emote.answer_timer > TICKS * 30) {
    ...
    case QA_THANKS:  // thanks etc
      if (dat->emote.answer_type != AT_THANKS || dat->emote.answer_cn != co ||
          ticker - dat->emote.answer_timer > TICKS * 30) {
    ...
    case QA_AFRAID:
      ...
      break;
    case QA_WHYMEAN:
      if (dat->emote.answer_type != AT_INSULT || dat->emote.answer_cn != co ||
          ticker - dat->emote.answer_timer > TICKS * 30) {
    ...
    case QA_SHUTUP:
      ...
      break;
    case QA_DOSOON:
      if (dat->emote.answer_type != AT_RELAX || dat->emote.answer_cn != co ||
          ticker - dat->emote.answer_timer > TICKS * 30) {
    ...
    case QA_ISTHATSO:
      ...
      break;
    case QA_NOTTHATBAD:
      if (dat->emote.answer_type != AT_ENCOURAGE ||
          dat->emote.answer_cn != co ||
          ticker - dat->emote.answer_timer > TICKS * 30) {
    ...
    case QA_COWARD:
      ...
      break;
    case QA_TOUGHFELLOW:
      if (dat->emote.answer_type != AT_AFFIRM || dat->emote.answer_cn != co ||
          ticker - dat->emote.answer_timer > TICKS * 30) {
...
void fdemon_army(int cn, int ret, int lastact) {
  struct farmy_data *dat;
  struct msg *msg, *next;
  int co, bless = 0, heal = 0, res, friend_;

  dat = (struct farmy_data *)set_data(cn, DRD_FARMYDATA,
                                      sizeof(struct farmy_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if (ch[co].group == ch[cn].group && cn != co) {
        if (ch[cn].value[0][V_BLESS] &&
            ...
            ch[cn].mana >= BLESSCOST && may_add_spell(co, IDR_BLESS) &&
            char_see_char(cn, co))
          bless = co;
        if (ch[cn].value[0][V_HEAL] &&
            ch[co].hp < ch[co].value[1][V_HP] * POWERSCALE / 3 &&
            ch[cn].mana >= POWERSCALE * 4 && char_see_char(cn, co))
          heal = co;
      }
    }
    ...
    if (msg->type == NT_TEXT) {
      ...
      // emotes
      if ((friend_ = find_platoon(co, dat)) == -1) {
        remove_message(cn, msg);
        continue;
      }
      if (res >= 20) got_emote(cn, co, friend_, res, dat);
      ...
      switch (res) {
        ...
        case 7:
          say(cn,
              "cuddly=%d, lonely=%d, angst=%d, fear=%d, bore=%d, boredom=%d, "
              "bigmouth=%d, praise=%d, like=%d/%d/%d %d, replied=%d/%d/%d %d",
              dat->emote.cuddly, dat->emote.lonely, dat->emote.angst,
              dat->emote.fear, dat->emote.bore, dat->emote.boredom,
              dat->emote.bigmouth, dat->emote.praise, dat->emote.likes[0],
              dat->emote.likes[1], dat->emote.likes[2], dat->emote.likes[3],
              dat->emote.talked[0], dat->emote.talked[1], dat->emote.talked[2],
              dat->emote.talked[3]);
          break;
      }
    }
  ...
  if (dat->leader_cn) {
    if (!(ch[dat->leader_cn].flags) ||
        ch[dat->leader_cn].group != ch[cn].group) {  // we lost our master
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 &&
          ...
          (ppd = (struct farmy_ppd *)set_data(co, DRD_FARMY_PPD,
                                              sizeof(struct farmy_ppd))) &&
        ...
        switch (ppd->boss_stage) {
          case 0:
            if (get_army_rank_int(co) < 2) {
              ...
              say(cn,
                  "Welcome, %s, to our underground headquarters. I am the "
                  "commander of the underground army. We are trying to stop "
                  "the demon's progress here, before they invade Aston again.",
                  ch[co].name);
              ppd->boss_stage++;
            }
          ...
          case 1:
            say(cn,
                "Unfortunately, I have a lack of good leaders. But you can "
                "°c4take°c0 some men to explore the underground and solve your "
                "missions. Just be sure to °c4drop°c0 them off here again "
                "before you leave.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 2:
            say(cn,
                "These soldiers have been trained to obey some easy commands: "
                "'Follow' makes them follow you. 'Front' makes them walk in "
                "front of you. With 'Back', they'll take one step back. They "
                "follow you more closely if you order a 'retreat'. And you can "
                "make them attack your enemy from 'behind'.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 3:
            say(cn,
                "It is up to you if you want their help on your missions. I "
                "recommend you take them along, the enemies are numerous, and "
          ...
          case 4:
            say(cn,
                "Alright, %s, your first mission is to reach the Ancient "
                "Defense Station number 1, and refuel it. You can find some of "
                "the power-crystals of the ancients in the big hall to the "
                "south-east. Take plenty, they're growing fast.",
          ...
          case 7:
            say(cn,
                "Your next mission is to activate Defense Station 3. That is "
                "the next station north-west of the one you activated in your "
                "last mission.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          ...
          case 10:
            say(cn,
                "I know, %s, you're tired of doing these simple missions, but "
                "they have to be done. We can't hold the demons back without "
                "those Defense Stations. %s, your mission is to activate "
                "Defense Station 2. It is located north-east of station 1.",
                ch[co].name, get_army_rank_string(co));
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 11:
          ...
          case 13:
            say(cn,
                "Our knowledge of the underground system here is pretty "
                "limited, %s. Your next mission is to find and activate the "
                "Defense Stations 4 and 5. I assume that these are pretty "
                "close to number 2 and 3.",
                ch[co].name);
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            ppd->boss_counter = 0;
          ...
          case 16:
            say(cn,
                "I've been getting reports about some beings we called 'Fire "
                "Golems', %s. It seems these beasts are very hard to kill. "
                "I've lost many good men to them. The few who made it back "
                "reported that only an attack in the back had any success. "
                "Your next mission is to slay one of those 'Fire Golems'. You "
                "can find them north-west of Defense Station 3. Oh, %s, may I "
                "remind you, that our soldiers have been trained to obey the "
                "commands 'follow', 'retreat', 'behind', 'front' and 'back'?",
                ch[co].name, get_army_rank_string(co));
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            ppd->boss_counter = 0;
          ...
          case 19:
            say(cn,
                "Scouts have found a room made by the ancients where a lot of "
                "small containers are stored. It is located in the "
                "north-western part of the underground. I want you to go "
                "there, aquire some of these containers and find out what they "
                "do. Good luck, %s.",
                ch[co].name);
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
          ...
          case 22:
            say(cn,
                "Now that we know what these containers can hold, we need to "
                "find out what to do with that golem blood, %s. Your mission, "
                "%s, is to find a use for it.",
                ch[co].name, get_army_rank_string(co));
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            ppd->boss_counter = 0;
          ...
          case 25:
            say(cn,
                "So that's how we can pass those lava fields. Well, since you "
                "can cross them now, %s, I want you to activate Defense "
                "Station 6. It is located north-east of number 5.",
                ch[co].name);
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            ppd->boss_counter = 0;
          ...
          case 29:
            say(cn,
                "I do not have a specific mission for you at the moment, %s, "
                "but I want you to scout the whole underground and find all "
                "the Defense Stations. When you find one, activate it, and "
                "report back from time to time. You don't have to come back "
                "for every single new station you find, though.",
                ch[co].name);
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 30:
            ...
            if (!cnt) break;
            say(cn,
                "Ah, %s. I hear you have found %d new Defense Stations. So "
                "you've found %d stations now.",
                ch[co].name, cnt, cnt2);
          ...
          case 31:
            say(cn,
                "It seems we know all stations here now. Thou wert most "
                "helpful, %s.",
                ch[co].name);
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
          ...
          case 32:
            say(cn,
                "I do not have any more missions for thee. But thou might want "
                "to explore the underworld further. This is but one part of "
                "it, and I feel that the cause of all this evil lies further "
                "to the north.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
    ...
    if (msg->type == NT_TEXT) {
      ...
      res = analyse_text_driver(cn, msg->dat1, (char *)msg->dat2, msg->dat3);
      if (res == 8 && (ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 &&
          char_see_char(cn, co) &&
          (ppd = (struct farmy_ppd *)set_data(co, DRD_FARMY_PPD,
                                              sizeof(struct farmy_ppd)))) {
        switch (ppd->boss_stage) {
      ...
      if (strcasestr((char *)msg->dat2, "take") && char_dist(cn, co) < 16) {
        drop_soldiers(co);
        take_soldiers(co);
      } else if (strcasestr((char *)msg->dat2, "drop") &&
                 char_dist(cn, co) < 16) {
        drop_soldiers(co);
      }
    }
...
static int find_loader(int in, int nr) {
  ...
  for (in2 = 1; in2 < MAXITEM; in2++) {
    if (it[in2].driver == IDR_FDEMONLOADER && it[in2].drdata[0] == nr &&
        (dist = abs(it[in].x - it[in2].x) + abs(it[in].y - it[in2].y)) <
            bestdist) {
      bestdist = dist;
      bestin = in2;
    }
...
void fdemon_light(int in, int cn) {
  ...
  if (!in2) {
    ...
    // xlog("light at %d,%d found loader %d at
    // %d,%d",it[in].x,it[in].y,in2,it[in2].x,it[in2].y);
  }
  if (!in2) return;

  pwr = max(max(*(unsigned short *)(it[in2].drdata + 1),
                *(unsigned short *)(it[in3].drdata + 1)),
            *(unsigned short *)(it[in4].drdata + 1));
...
void fdemon_loader(int in, int cn) {
  ...
  pwr2 = *(unsigned short *)(it[in].drdata +
                             4);  // power to use once animiation runs out
  ...
  if (cn) {  // player using item
    ...
    if (it[in].drdata[6] == 1 &&
        (ppd = (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD,
                                            sizeof(struct farmy_ppd))) &&
        ppd->boss_stage >= 0 && ppd->boss_stage <= 5) {
      log_char(cn, LOG_SYSTEM, 0,
               "You've solved your mission. Now head back to the commander to "
               "claim your reward!");
      ppd->boss_stage = 6;
    }
    if (it[in].drdata[6] == 3 &&
        (ppd = (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD,
                                            sizeof(struct farmy_ppd))) &&
        ppd->boss_stage >= 7 && ppd->boss_stage <= 8) {
      log_char(cn, LOG_SYSTEM, 0,
               "You've solved your mission. Now head back to the commander to "
               "claim your reward!");
      ppd->boss_stage = 9;
    ...
    if (it[in].drdata[6] == 2 &&
        (ppd = (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD,
                                            sizeof(struct farmy_ppd))) &&
        ppd->boss_stage >= 10 && ppd->boss_stage <= 11) {
      log_char(cn, LOG_SYSTEM, 0,
               "You've solved your mission. Now head back to the commander to "
               "claim your reward!");
      ppd->boss_stage = 12;
    ...
    if (it[in].drdata[6] == 4 &&
        (ppd = (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD,
                                            sizeof(struct farmy_ppd))) &&
        ppd->boss_stage >= 13 && ppd->boss_stage <= 14) {
      ppd->boss_counter |= 1;
      if ((ppd->boss_counter & 3) == 3) {
        log_char(cn, LOG_SYSTEM, 0,
                 "You've solved your mission. Now head back to the commander "
                 "to claim your reward!");
        ppd->boss_stage = 15;
      } else {
        log_char(cn, LOG_SYSTEM, 0,
                 "You've solved the first part of your mission. Now go find "
                 "that other station.!");
      }
    }
    if (it[in].drdata[6] == 5 &&
    ...
    if (it[in].drdata[6] == 6 &&
        (ppd = (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD,
                                            sizeof(struct farmy_ppd))) &&
        ppd->boss_stage >= 25 && ppd->boss_stage <= 26) {
      log_char(cn, LOG_SYSTEM, 0,
               "You've solved your mission. Now head back to the commander to "
               "claim your reward!");
      ppd->boss_stage = 27;
    }
    if (it[in].drdata[6] >= 7 && it[in].drdata[6] <= 35 &&
        (ppd = (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD,
                                            sizeof(struct farmy_ppd))) &&
        ppd->boss_stage >= 28) {
      int bit;
      ...
      if (!(ppd->boss_counter & bit)) {
        ...
        log_char(cn, LOG_SYSTEM, 0, "You've found Defense Station number %d.",
                 it[in].drdata[6]);
      }
    }
  } else {  // timer call
  ...
  *(unsigned short *)(it[in].drdata + 1) = pwr;
  ...
  if (pwr2)
    sprite = 59030 + 9 - (min(2880, pwr2) / 320);
  else
    sprite = 14234;
...
int find_target(int in, int x, int y, int dx, int dy, struct shot *shot) {
  ...
  for (n = 0; n < 25; n++) {
    for (co = getfirst_char_sector(x + offx[n], y + offy[n]); co;
         co = ch[co].sec_next) {
      ...
      if (!can_hit(in, co, it[in].x + ox, it[in].y + oy, ch[co].x, ch[co].y))
        continue;
  ...
  if (cn) {
    ...
    if (pwr) {
      ...
      if (find_target(in, it[in].x, it[in].y, dx, dy, &shot)) {
        ...
        create_edemonball(cn, it[in].x + shot.ox, it[in].y + shot.oy, shot.tx,
                          shot.ty, spwr, 2);
...
void fdemon_farm(int in, int cn) {
  ...
  if (cn) {
    ...
    if (!crystal) {
      log_char(cn, LOG_SYSTEM, 0, "There's nothing to take yet (%d of %d).",
               str, size);
      return;
    }
...
void find_waypoints(void) {
  ...
  for (n = 1; n < maxway; n++) {
    for (m = n + 1; m < maxway; m++) {
      ...
      if (dx > 35 && dx < 45 && abs(dy) < 10 &&
          pathfinder(wp[n].x, wp[n].y, wp[m].x, wp[m].y, 1, NULL, 400) != -1) {
        if (wp[n].left) {
          xlog("duplicated left %d,%d removed", wp[wp[n].left].x,
               wp[wp[n].left].y);
        }
        // xlog("%d %d,%d left to %d
        // %d,%d",n,wp[n].x,wp[n].y,m,wp[m].x,wp[m].y);
        wp[n].left = m;
        wp[m].right = n;
        continue;
      }

      if (dy > 35 && dy < 45 && abs(dx) < 10 &&
          pathfinder(wp[n].x, wp[n].y, wp[m].x, wp[m].y, 1, NULL, 400) != -1) {
        if (wp[n].up) {
...
int fww_flags(int next, int flags) {
  if ((flags & FWW_NOENEMY) && wp[next].last_enemy &&
      ticker - wp[next].last_enemy < TICKS * 60)
    return 0;
...
int find_way_to_waypoint(int from, int to, int flags) {
  ...
  while (maxnode < MAXNODE - 4) {
    ...
    if (maxnode > cnode)
      qsort(node + cnode, maxnode - cnode, sizeof(struct node), findwaycmp);
    else
      break;
  }
...
static int may_hunt_there(int cn, int x, int y) {
  if (x - (signed)ch[cn].tmpx > 30) {
    return 0;
  }
  if (y - (signed)ch[cn].tmpy > 30) {
    return 0;
  }

  if ((signed)ch[cn].tmpx - x > 70) {
    return 0;
  }
  if ((signed)ch[cn].tmpy - y > 70) {
    return 0;
  }
...
static int hunt_driver(int cn, struct fdemon_data *dat) {
  ...
  for (n = 1; n < maxway; n++) {
    if (wp[n].last_enemy && may_hunt_there(cn, wp[n].x, wp[n].y) &&
...
void fdemon_demon(int cn, int ret, int lastact) {
  ...
  dat = (struct fdemon_data *)set_data(cn, DRD_FDEMONDATA,
                                       sizeof(struct fdemon_data));
  if (!dat) return;  // oops...
  ...
  if (!may_hunt_there(cn, ch[cn].x, ch[cn].y)) {
    dat->gohome = 1;
    // say(cn,"gohome=1 (%d,%d -
    // %d,%d)",ch[cn].x-ch[cn].tmpx,ch[cn].y-ch[cn].tmpy,ch[cn].tmpx-ch[cn].x,ch[cn].tmpy-ch[cn].y);
  }

  if (abs(ch[cn].x - ch[cn].tmpx) < 15 && abs(ch[cn].y - ch[cn].tmpy) < 15) {
    dat->gohome = 0;
    // say(cn,"gohome=0
    // (%d,%d)",abs(ch[cn].x-ch[cn].tmpx),abs(ch[cn].y-ch[cn].tmpy));
  }
...
void fdemon_demon_dead(int cn, int co) {
  ...
  if ((ch[co].flags & CF_PLAYERLIKE) &&
      (dat = (struct farmy_data *)set_data(
           co, DRD_FARMYDATA, sizeof(struct farmy_data)))) {  // its a soldier
  ...
  if (!(ppd = (struct farmy_ppd *)set_data(cc, DRD_FARMY_PPD,
                                           sizeof(struct farmy_ppd))))
    return;
...
void fdemon_blood(int in, int cn) {
  ...
  if (it[in2].driver == IDR_FLASK) {
    ...
    if (ch[cn].flags & CF_PLAYER)
      dlog(cn, in2, "dropped flask (destroyed by golem blood)");
    destroy_item(in2);
    ch[cn].citem = 0;
    ch[cn].flags |= CF_ITEMS;
    return;
  ...
  if ((ppd = (struct farmy_ppd *)set_data(cn, DRD_FARMY_PPD,
                                          sizeof(struct farmy_ppd))) &&
  ...
  it[in2].drdata[0]++;
  it[in2].sprite++;
  sprintf(it[in2].description, "A container holding %d parts golem blood.",
          it[in2].drdata[0]);
  ch[cn].flags |= CF_ITEMS;
...
void fdemon_lava(int in, int cn) {
  ...
  if (!cn) {
    ...
    if (it[in].drdata[0] ==
        0) {  // last stage - set original sprite, set moveblock, remove flame
      ...
      return;  // stop timer calls
               ...
               20) {  // stage two - set flame and hurt characters
      ...
      if (co) hurt(co, 10 * POWERSCALE, 0, 1, 0, 50);
               ...
               60) {  // stage one - set next sprite and hurt characters
    ...
    if (!(in2 = ch[cn].citem)) {
      log_char(cn, LOG_SYSTEM, 0,
               "You do not want to touch burning lava with your bare hands, do "
               "you?");
      return;
    }
    if (it[in2].ID != IID_AREA8_BLOOD) {
