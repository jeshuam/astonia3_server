...
void ratchest_driver(int in, int cn)
{
  ...
  ppd = (struct ratchest_ppd*)set_data(cn, DRD_RATCHEST_PPD, sizeof(struct ratchest_ppd));
  if (!ppd) return;	// oops...
...
void ratling_died(int cn, int co)
{
  ...
  ppd = (struct ratchest_ppd*)set_data(co, DRD_RATCHEST_PPD, sizeof(struct ratchest_ppd));
  if (!ppd) return;	// oops...
