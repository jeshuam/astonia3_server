...
void count_chests(void) {
  ...
  for (in = 1; in < MAXITEM; in++) {
    ...
    switch (it[in].drdata[0]) {
      ...
      case 40:
        ...
        break;
      default:
        elog("unknown chest number %d in count_chests() in sewers.c",
             it[in].drdata[0]);
        continue;
    }
  ...
  xlog("sewers: %d/%d/%d/%d crates.", chest_cnt[0], chest_cnt[1], chest_cnt[2],
       chest_cnt[3]);
}
...
void set_treasure(int cn, struct ratchest_ppd *ppd) {
  ...
  if (ch[cn].level >= 37)
    nr = 3;  // 33,35,37,39
  else if (ch[cn].level >= 29)
    nr = 2;  // 25,27,29,31
  else if (ch[cn].level >= 21)
    nr = 1;  // 17,19,21,23
  else if (ch[cn].level >= 13)
    nr = 0;  // 9,11,13,15
  else
...
void give_treasure(int cn) {
  ...
  switch (RANDOM(3)) {
    ...
    case 1:
      name = "sewer_ring";
      if ((ch[cn].flags & (CF_MAGE | CF_WARRIOR)) == CF_MAGE) {
        if (ch[cn].value[1][V_FIREBALL] > ch[cn].value[1][V_FLASH])
          skl = V_FIREBALL;
        else
          skl = V_FLASH;
      } else
  ...
  if (ch[cn].level < 15)
    ...
    val = 5;
  else if (ch[cn].level < 20)
    val = 6;
  else if (ch[cn].level < 23)
    val = 7;
  else if (ch[cn].level < 26)
    val = 8;
  else if (ch[cn].level < 30)
    val = 9;
  else if (ch[cn].level < 33)
    val = 10;
  else if (ch[cn].level < 36)
    val = 11;
  else
    val = 12;
...
void ratchest_driver(int in, int cn) {
  ...
  ppd = (struct ratchest_ppd *)set_data(cn, DRD_RATCHEST_PPD,
                                        sizeof(struct ratchest_ppd));
  if (!ppd) return;  // oops...
...
void ratling_died(int cn, int co) {
  ...
  ppd = (struct ratchest_ppd *)set_data(co, DRD_RATCHEST_PPD,
                                        sizeof(struct ratchest_ppd));
  if (!ppd) return;  // oops...
  ...
  if (ppd->treasure_x && RANDOM(2)) {
    if (abs(ch[co].x - ppd->treasure_x) > abs(ch[co].y - ppd->treasure_y)) {
      if (ppd->treasure_x > ch[co].x)
        say(cn,
            "Arrgh. Not kill us. Is south-east. Go there. Take from grate. And "
            "leave.");
      if (ppd->treasure_x < ch[co].x)
        say(cn,
            "Arrgh. Not kill us. Is north-west. Go there. Take from grate. And "
            "leave.");
    } else {
      if (ppd->treasure_y > ch[co].y)
        say(cn,
            "Arrgh. Not kill us. Is south-west. Go there. Take from grate. And "
            "leave.");
      if (ppd->treasure_y < ch[co].y)
        say(cn,
            "Arrgh. Not kill us. Is north-east. Go there. Take from grate. And "
            "leave.");
    }
  }
}
