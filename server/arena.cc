...
void score_fight(int cn, int co, struct master_data *dat)
{
  ...
  p1 = (struct arena_ppd*)set_data(cn, DRD_ARENA_PPD, sizeof(struct arena_ppd)); if (!p1->fights) p1->score = -2000;
  p2 = (struct arena_ppd*)set_data(co, DRD_ARENA_PPD, sizeof(struct arena_ppd)); if (!p2->fights) p2->score = -2000;
...
void master_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct master_data*)set_data(cn, DRD_ARENAMASTER, sizeof(struct master_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      /*case 2:		ppd=set_data(co,DRD_AREA3_PPD,sizeof(struct area3_ppd));
      		...
      		break;*/
      case 3:		// register
        ppd = (struct arena_ppd*)set_data(co, DRD_ARENA_PPD, sizeof(struct arena_ppd)); if (!ppd->fights) ppd->score = -2000;
...
void fighter_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct fighter_data*)set_data(cn, DRD_ARENAFIGHTER, sizeof(struct fighter_data));
  if (!dat) return;	// oops...

  ppd = (struct arena_ppd*)set_data(cn, DRD_ARENA_PPD, sizeof(struct arena_ppd)); if (!ppd->fights) ppd->score = -2000;
  if (!ppd) return;
...
void toplist_driver(int in, int cn)
{
  ...
  ppd = (struct arena_ppd*)set_data(cn, DRD_ARENA_PPD, sizeof(struct arena_ppd));
  if (!ppd) return;
...
void manager_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct manager_data*)set_data(cn, DRD_ARENAMANAGER, sizeof(struct manager_data));
  if (!dat) return;	// oops...
