 ...
 * made small arena manager kick out all currently inside his arena when
 *executing 'rent' command
 *
 */
...
void add_contender(int cn, int co, struct master_data *dat,
                   struct arena_ppd *ppd) {
  int n, ID;

  /*if (realtime-ppd->lastfight<60*5) {
        say(cn,"Sorry, you have fought within the last five minutes already, you
  cannot fight again now, %s.",ch[co].name);
        return;
  }*/
  ...
  for (n = 0; n < MAXCONTENDER; n++) {
    if (dat->ct[n].ID == ID) {
      say(cn, "You're already registered for this tournament, %s.",
          ch[co].name);
  ...
  for (n = 0; n < MAXCONTENDER; n++) {
    if (!dat->ct[n].ID) {
      ...
      say(cn, "Good luck, %s. I will call you when your fight starts.",
          ch[co].name);
      notify_char(co, NT_NPC, NTID_ARENA, 3, 0);
      return;
    }
  ...
  say(cn,
      "I'm sorry, %s, but there are no free slots at the moment. Please try "
      "again after the next fight.",
      ch[co].name);
}
...
void find_contender(int cn, struct master_data *dat) {
  ...
  for (n = 0; n < MAXCONTENDER; n++) {
    ...
    if (charID(dat->ct[n].cn) != dat->ct[n].ID) {
      dat->ct[n].ID = 0;
      continue;
    }
    ...
    for (m = n + 1; m < MAXCONTENDER; m++) {
      ...
      if (charID(dat->ct[m].cn) != dat->ct[m].ID) {
        dat->ct[m].ID = 0;
        continue;
      }
  ...
  if (fight2) {
    ...
    say(cn,
        "Next fight is: °c6%s versus %s.°c0 Both participants please step "
        "forward and say: 'enter'. You have 30 seconds to enter the arena, "
        "otherwise you lose by default.",
        ch[dat->fight1_cn].name, ch[dat->fight2_cn].name);
...
void check_inside(int cn, struct master_data *dat) {
  ...
  if (ticker < dat->timeout) {
    if (ch[dat->fight1_cn].x <= 233 || ch[dat->fight1_cn].x >= 243 ||
        ch[dat->fight1_cn].y <= 132 || ch[dat->fight1_cn].y >= 142)
      return;
    if (ch[dat->fight2_cn].x <= 233 || ch[dat->fight2_cn].x >= 243 ||
        ch[dat->fight2_cn].y <= 132 || ch[dat->fight2_cn].y >= 142)
      return;
...
int toplist_cmp(const void *a, const void *b) {
  ...
  if (!((struct entry *)(a))->name[0] && !((struct entry *)(b))->name[0])
    return 0;

  return ((struct entry *)(b))->score - ((struct entry *)(a))->score;
}

void update_toplist(struct master_data *dat, int cn, int co, int cn_score,
                    int co_score) {
  int n;
  ...
  qsort(dat->storage_data.entry, 100, sizeof(dat->storage_data.entry[0]),
        toplist_cmp);
}
...
void score_fight(int cn, int co, struct master_data *dat) {
  ...
  p1 =
      (struct arena_ppd *)set_data(cn, DRD_ARENA_PPD, sizeof(struct arena_ppd));
  if (!p1->fights) p1->score = -2000;
  p2 =
      (struct arena_ppd *)set_data(co, DRD_ARENA_PPD, sizeof(struct arena_ppd));
  if (!p2->fights) p2->score = -2000;
  ...
  p1->score += worth;
  p2->score -= worth;
  p1->lastfight = realtime;
  p2->lastfight = realtime;
...
void check_fight(int cn, struct master_data *dat) {
  ...
  if (win1) {
    ...
    if (charID(dat->fight1_cn) == dat->fight1_ID &&
        charID(dat->fight2_cn) == dat->fight2_ID)
      score_fight(dat->fight1_cn, dat->fight2_cn, dat);
  } else if (win2) {
    say(cn, "And the winner is %s.", ch[dat->fight2_cn].name);
    if (charID(dat->fight1_cn) == dat->fight1_ID &&
        charID(dat->fight2_cn) == dat->fight2_ID)
      score_fight(dat->fight2_cn, dat->fight1_cn, dat);
  } else {
    say(cn, "Hu? No one won? Oh well...");
  }

  if (ch[dat->fight1_cn].flags && charID(dat->fight1_cn) == dat->fight1_ID)
    notify_char(dat->fight1_cn, NT_NPC, NTID_ARENA, 2, 0);
  if (ch[dat->fight2_cn].flags && charID(dat->fight2_cn) == dat->fight2_ID)
    notify_char(dat->fight2_cn, NT_NPC, NTID_ARENA, 2, 0);
...
void master_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct master_data *)set_data(cn, DRD_ARENAMASTER,
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      switch ((didsay =
        ...
        /*case 2:   ppd=set_data(co,DRD_AREA3_PPD,sizeof(struct
           area3_ppd));
                  if (ppd && ppd->crypt_state<=1) ppd->crypt_state=0;
                  break;*/
        case 3:  // register
          ppd = (struct arena_ppd *)set_data(co, DRD_ARENA_PPD,
                                             sizeof(struct arena_ppd));
  ...
  switch (dat->storage_state) {
    case 0:
      if (create_storage(dat->storage_ID, "Arena Toplist Data",
    ...
    case 3:
      ...
      if (res == 1 && size == sizeof(dat->storage_data)) {
        ...
        xfree(tmp);
        dat->storage_state++;
        // charlog(cn,"read storage: score=%d, wins=%d,
        // losses=%d",dat->storage_data.ppd.score,dat->storage_data.ppd.wins,dat->storage_data.ppd.losses);
    ...
    case 6:
      if (check_update_storage()) {
        dat->storage_state++;
        dat->storage_version++;
        // charlog(cn,"update storage: score=%d, wins=%d,
        // losses=%d",dat->storage_data.ppd.score,dat->storage_data.ppd.wins,dat->storage_data.ppd.losses);
      }
      break;
    case 7:
...
void fighter_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct fighter_data *)set_data(cn, DRD_ARENAFIGHTER,
                                        sizeof(struct fighter_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_NPC && msg->dat1 == NTID_ARENA && msg->dat2 == 0 &&
        dat->state == FS_WAIT) {
      dat->state = FS_ENTER;
      dat->lastact = -TICKS * 60 * 5;
    }
    if (msg->type == NT_NPC && msg->dat1 == NTID_ARENA && msg->dat2 == 1 &&
        dat->state == FS_WAIT2) {
      dat->state = FS_FIGHT;
      dat->enemy = msg->dat3;
    ...
    if (msg->type == NT_NPC && msg->dat1 == NTID_ARENA && msg->dat2 == 2 &&
        (dat->state == FS_FIGHT || dat->state == FS_WAIT2)) {
    ...
    if (msg->type == NT_NPC && msg->dat1 == NTID_ARENA && msg->dat2 == 3 &&
        dat->state == FS_REGISTER) {
      dat->state = FS_WAIT;
      dat->lastact = -TICKS * 60 * 5;
    }
    if (msg->type == NT_NPC && msg->dat1 == NTID_ARENA && msg->dat2 == 4 &&
        dat->state == FS_ENTER) {
      dat->state = FS_WAIT2;
      dat->lastact = -TICKS * 60 * 5;
    }
  ...
  if (dat->storage_state > 3) {
    switch (dat->state) {
      case FS_LEISURE:
        if ((abs(ch[cn].x - ch[cn].tmpx) > 2 ||
             abs(ch[cn].y - ch[cn].tmpy) > 2) &&
            move_driver(cn, ch[cn].tmpx, ch[cn].tmpy, 2))
          return;
        if (ticker - dat->lastact < TICKS * 60 * 3) break;
        dat->state++;
        break;
      case FS_START:
        if (abs(ch[cn].x - MASTER_POSX) < 5 && abs(ch[cn].y - MASTER_POSY) < 5)
          dat->state++;
        else if (move_driver(cn, MASTER_POSX, MASTER_POSY, 4))
          return;
        break;
      case FS_REGISTER:
        if (ticker - dat->lastact < TICKS * 30) break;
        say(cn, "register");
        dat->lastact = ticker;
        // dat->state++;
        break;
  ...
  switch (dat->storage_state) {
    case 0:
      if (create_storage(dat->storage_ID, "Arena Fighter Data",
                         &dat->storage_data, sizeof(dat->storage_data)))
        dat->storage_state++;
      break;
    case 1:
...
void toplist_driver(int in, int cn) {
  ...
  for (n = 0; n < 10; n++) {
    ...
    log_char(cn, LOG_SYSTEM, 0, "%d: %s %d", n + 1, tops->entry[n].name,
             tops->entry[n].score);
  }
  for (n = 10; n < 100; n++) {
    if (!tops->entry[n].name[0]) break;
  ...
  for (m = max(10, n - 5); m < min(100, n + 5); m++) {
    ...
    log_char(cn, LOG_SYSTEM, 0, "%d: %s %d", m + 1, tops->entry[m].name,
             tops->entry[m].score);
  }

  log_char(cn, LOG_SYSTEM, 0,
           "Your score is %d, you have won %d fights and lost %d fights.",
...
void manager_parse(int cn, struct manager_data *dat) {
  ...
  for (ptr = nextnv(ch[cn].arg, name, value); ptr;
       ptr = nextnv(ptr, name, value)) {
    if (!strcmp(name, "arenax"))
      dat->arena_x = atoi(value);
    else if (!strcmp(name, "arenay"))
      dat->arena_y = atoi(value);
    else if (!strcmp(name, "arenafx"))
      dat->arena_fx = atoi(value);
    else if (!strcmp(name, "arenafy"))
      dat->arena_fy = atoi(value);
    else if (!strcmp(name, "arenatx"))
      dat->arena_tx = atoi(value);
    else if (!strcmp(name, "arenaty"))
      dat->arena_ty = atoi(value);
    else
      elog("unknown arg for %s (%d): %s", ch[cn].name, cn, name);
  }
}
...
void manager_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct manager_data *)set_data(cn, DRD_ARENAMANAGER,
                                        sizeof(struct manager_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (co == dat->renter &&
          (ch[co].x < 232 || ch[co].x > 238 || ch[co].y < dat->arena_fy ||
           ch[co].y > dat->arena_ty)) {
        dat->renter = 0;
        dat->invite[0] = 0;
      }
      ...
      switch ((didsay =
        ...
        case 6:  // rent
          ...
          say(cn,
              "Say 'invite: <name>' to let someone in, or say 'leave' to leave "
              "the arena.");
          break;
        case 5:  // leave
          teleport_char_driver(co, ch[cn].x, ch[cn].y);
        ...
        case 4:  // enter
          if (!strcasecmp(ch[co].name, dat->invite)) {
            ...
            say(cn, "You have not been invited, %s.", ch[co].name);
          break;
      }
      if ((ptr = strcasestr((char *)msg->dat2, "invite:"))) {
        if (co != dat->renter) {
          ...
          say(cn, "%s, say 'enter' if you wish to enter the arena",
              dat->invite);
        }
      }
      if (didsay) {
...
int ch_driver(int nr, int cn, int ret, int lastact) {
  switch (nr) {
    case CDR_ARENAMASTER:
      master_driver(cn, ret, lastact);
      return 1;
    case CDR_ARENAFIGHTER:
      fighter_driver(cn, ret, lastact);
      return 1;
