...
void protect(void *mem, unsigned int size) {
  ...
  ptr = (void *)((unsigned long)(((unsigned char *)(mem) + PAGESIZE - 1)) &
                 (~(PAGESIZE - 1)));
...
static int load_zones(const char *dirname, const char *mask,
                      int (*process)(char *)) {
  DIR *dir;
  struct dirent *de;
...
static char *get_text(char *name, char *value, char *ptr) {
  ...
  // read name
  for (len = 0; len < 255 && (isalnum(*ptr) || *ptr == '_' || *ptr == '-');
       len++)
    *name++ = *ptr++;
  *name = 0;
...
static const char *V_tab[V_MAX] = {
    "V_HP",         "V_ENDURANCE", "V_MANA",

    "V_WIS",        "V_INT",       "V_AGI",         "V_STR",

    "V_ARMOR",      "V_WEAPON",    "V_LIGHT",       "V_SPEED",

    "V_PULSE",      "V_DAGGER",    "V_HAND",        "V_STAFF",
    "V_SWORD",      "V_TWOHAND",

    "V_ARMORSKILL", "V_ATTACK",    "V_PARRY",       "V_WARCRY",
    "V_TACTICS",    "V_SURROUND",  "V_BODYCONTROL", "V_SPEEDSKILL",

    "V_BARTER",     "V_PERCEPT",   "V_STEALTH",

    "V_BLESS",      "V_HEAL",      "V_FREEZE",      "V_MAGICSHIELD",
    "V_FLASH",      "V_FIREBALL",  "V_ARCANE",

    "V_REGENERATE", "V_MEDITATE",  "V_IMMUNITY",    "V_DEMON",
    "V_DURATION",   "V_RAGE",      "V_COLD",        "V_PROFESSION"};
...
static const char *WN_tab[] = {"WN_NECK",  "WN_HEAD",  "WN_CLOAK", "WN_ARMS",
                               "WN_BODY",  "WN_BELT",  "WN_RHAND", "WN_LEGS",
...
static const char *IF_tab[] = {
    "IF_USED",       "IF_MOVEBLOCK",   "IF_SIGHTBLOCK",   "IF_TAKE",
    "IF_USE",        "IF_WNHEAD",      "IF_WNNECK",       "IF_WNBODY",
    "IF_WNARMS",     "IF_WNBELT",      "IF_WNLEGS",       "IF_WNFEET",
    "IF_WNLHAND",    "IF_WNRHAND",     "IF_WNCLOAK",      "IF_WNLRING",
    "IF_WNRRING",    "IF_WNTWOHANDED", "IF_AXE",          "IF_DAGGER",
    "IF_HAND",       "IF_SHANON",      "IF_STAFF",        "IF_SWORD",
    "IF_TWOHAND",    "IF_DOOR",        "IF_QUEST",        "IF_SOUNDBLOCK",
    "IF_STEPACTION", "IF_MONEY",       "IF_NODECAY",      "IF_FRONTWALL",
    "IF_DEPOT",      "IF_NODEPOT",     "IF_NODROP",       "IF_NOJUNK",
    "IF_PLAYERBODY", "IF_BONDTAKE",    "IF_BONDWEAR",     "IF_LABITEM",
    "IF_VOID",       "IF_NOENHANCE",   "IF_BEYONDBOUNDS", "IF_BEYONDMAXMOD"};
...
int lookup_item(const char *name) {
  int n;
...
static int trans_ID(char *src) { return strtol(src, NULL, 16); }
...
static int process_item(char *ptr) {
  ...
  while ((ptr = get_text(name, value, ptr))) {
    ...
    if (!gotname && value[0] == ';') {
      elog("Error in line %d: Need name before semicolon (%100.100s).", line,
           ptr);
      return 0;
    }
    ...
    if (!strcasecmp(name, "name"))
      safecpy(itt.item.name, value) else if (!strcasecmp(name, "description"))
          safecpy(itt.item.description,
                  value) else if (!strcasecmp(name, "value"))
              itt.item.value = atoi(value);
    else if (!strcasecmp(name, "sprite"))
      itt.item.sprite = atoi(value);
    else if (!strcasecmp(name, "driver"))
      itt.item.driver = atoi(value);
    else if (!strcasecmp(name, "min_level"))
      itt.item.min_level = atoi(value);
    else if (!strcasecmp(name, "max_level"))
      itt.item.max_level = atoi(value);
    else if (!strcasecmp(name, "needs_class"))
      itt.item.needs_class = atoi(value);
    else if (!strcasecmp(name, "ID"))
      itt.item.ID = trans_ID(value);
    else if (!strcasecmp(name, "arg"))
      trans_arg(itt.item.drdata, value);
    else if (!strcasecmp(name, "mod_index")) {
      tmp = lookup_V(value);
      if (tmp == -1) {
        elog("Error in line %d: Unknown V_name \"%s\" (%100.100s).", line,
             value, ptr);
        return 0;
      }
      ...
      itt.item.mod_index[cmod] = -tmp;
    } else if (!strcasecmp(name, "mod_value") ||
               !strcasecmp(name, "req_value")) {
      if (cmod >= MAXMOD) {
        elog("Error in line %d: MAXMOD (%d) exceeded (%100.100s).", line,
             MAXMOD, ptr);
        return 0;
      }
      ...
      tmp = lookup_IF(value);
      if (tmp == -1) {
        elog("Error in line %d: Unknown IF \"%s\" (%100.100s).", line, value,
             ptr);
        return 0;
      }
      itt.item.flags |= (1ull << tmp);
    } else {
      elog("Item: Error in line %d: unknown name \"%s\" (%100.100s).", line,
           name, ptr);
      return 0;
    }
  }
  if (gotname) {
    elog(
        "Error in line %d: Premature end of input (or syntax error) "
        "(%100.100s).",
        line, ptr);
    return 0;
  }
...
static int add_char(struct ch_temp *cht) {
  ...
  // !(cht->ch.flags&(CF_IMMORTAL|CF_NOATTACK))) elog("******\nchar %s has
  // questitem but no immortal/noattack flag\n******",cht->ch.name);
...
int lookup_char(const char *name) {
  int n;
...
static int process_char(char *ptr) {
  ...
  while ((ptr = get_text(name, value, ptr))) {
    ...
    // parse values
    if (!strcasecmp(name, "name"))
      safecpy(cht.ch.name, value) else if (!strcasecmp(name, "description"))
          safecpy(cht.ch.description,
                  value) else if (!strcasecmp(name, "sprite"))
              cht.ch.sprite = atoi(value);
    else if (!strcasecmp(name, "sound"))
      cht.ch.sound = atoi(value);
    else if (!strcasecmp(name, "gold"))
      cht.ch.gold = atoi(value);
    else if (!strcasecmp(name, "driver"))
      cht.ch.driver = atoi(value);
    else if (!strcasecmp(name, "group"))
      cht.ch.group = atoi(value);
    else if (!strcasecmp(name, "class"))
      cht.ch.cls = atoi(value);
    else if (!strcasecmp(name, "respawn"))
      cht.ch.respawn = atoi(value) * TICKS;
    else if (!strcasecmp(name, "special_prob"))
      cht.special_prob = atoi(value);
    else if (!strcasecmp(name, "special_strength"))
      cht.special_str = atoi(value);
    else if (!strcasecmp(name, "special_base"))
      cht.special_base = atoi(value);
    else if (!strcasecmp(name, "gold_prob"))
      cht.gold_prob = atoi(value);
    else if (!strcasecmp(name, "gold_base"))
      cht.gold_base = atoi(value);
    else if (!strcasecmp(name, "gold_random"))
      cht.gold_random = atoi(value);
    else if (!strcasecmp(name, "arg")) {
      if (cht.ch.arg) {
        cht.ch.arg = (char *)xrealloc(
            cht.ch.arg, strlen(cht.ch.arg) + strlen(value) + 1, IM_CHARARGS);
        if (!cht.ch.arg) {
      ...
      if (tmp == 0) {
        elog("Error in line %d: Unknown item name \"%s\" (%100.100s).", line,
             value, ptr);
        return 0;
      }
      ...
      tmp = lookup_item(value);
      if (tmp == 0) {
        elog("Error in line %d: Unknown spell name \"%s\" (%100.100s).", line,
             value, ptr);
        return 0;
      }
      if (cspell >= 30) {
      ...
      tmp = lookup_item(value);
      if (tmp == 0) {
        elog("Error in line %d: Unknown r-item name \"%s\" (%100.100s).", line,
             value, ptr);
        return 0;
      }
      if (ritem >= MAXRANDITEM) {
        elog(
            "Error in line %d: Random carrying capacity (%d) exceeded "
            "(%100.100s).\n",
      ...
      tmp = atoi(value);
      if (tmp < 1 || tmp > 9999) {
        elog("Error in line %d: Illegal probability \"%s\". (%100.100s)", line,
             value, ptr);
        return 0;
      }
      ...
      tmp = lookup_CF(value);
      if (tmp == -1) {
        elog("Error in line %d: Unknown CF \"%s\" (%100.100s).", line, value,
             ptr);
        return 0;
      }
      ...
      elog("Char: Error in line %d: unknown name \"%s\" (%100.100s).", line,
           name, ptr);
      return 0;
    }
  }
...
int create_char_nr(int ctmp, int tmpa) {
  ...
  if (ch[n].flags & CF_PLAYER)
    ;
  else if (ch[n].level > 1)
    ch[n].level += 2;
  else
    ch[n].level += 1;

  if (ch[n].lifeshield < 0) {
    elog("lifeshield<0 while creating char %d (%s). fixing. (1)", ctmp,
         ch[n].name);
    ch[n].lifeshield = 0;
    ch_temp[ctmp].ch.lifeshield = 0;
  }

  // if (ch[n].flags&CF_DEMON) xlog("created %s (%s - %d), %d exp, level %d
  // (%.2f%%, %d exp to next
  // level)",ch[n].name,ch_temp[ctmp].name,ch[n].tmpa,ch[n].exp,ch[n].level,100.0/80977100*ch[n].exp,level2exp(ch[n].level-1)-ch[n].exp);
  ...
  for (m = 0, f = 30; m < MAXRANDITEM; m++) {
    if ((itmp = ch_temp[ctmp].rand_item[m]) &&
        RANDOM(10000) < ch_temp[ctmp].rand_prob[m]) {
      in = create_item_nr(itmp);
      if (!in) {
        elog("create_char(): create_item_nr() failed.");
        // FIXME: all items created so far are lost!
  ...
  if (ch_temp[ctmp].special_prob && RANDOM(ch_temp[ctmp].special_prob) == 0) {
    in = create_special_item(ch_temp[ctmp].special_str,
                             ch_temp[ctmp].special_base, 20, 10000);
    if (in) {
      ...
      if (f < INVENTORYSIZE) {
        ch[n].item[f] = in;
        it[in].carried = n;
        // xlog("created special item %s (%s) for
        // %s.",it[in].name,it[in].description,ch[n].name);
      } else {
        free_item(in);
      }
    }
  ...
  if (ch_temp[ctmp].gold_prob && RANDOM(ch_temp[ctmp].gold_prob) == 0) {
    ch[n].gold +=
        ch_temp[ctmp].gold_base + RANDOM(ch_temp[ctmp].gold_random + 1);
  }

  notify_char(n, NT_CREATE, ticker, 0, 0);

  if (ch[n].lifeshield < 0) {
    elog("lifeshield<0 while creating char %d (%s). fixing. (2)", ctmp,
         ch[n].name);
    ch[n].lifeshield = 0;
  }
...
int create_char(const char *name, int tmpa) {
  int tmp;
...
static int process_map(char *ptr) {
  ...
  while ((ptr = get_text(name, value, ptr))) {
    ...
    if (!strcasecmp(name, "origin")) {
    ...
    } else if (!strcasecmp(name, "gsprite"))
      map[x + y * MAXMAP].gsprite = atoi(value);
    else if (!strcasecmp(name, "fsprite"))
      map[x + y * MAXMAP].fsprite = atoi(value);
      ...
      if (ch[tmp].flags & CF_RESPAWN) {
        if ((ch[tmp].flags & CF_RESPAWN) &&
            (ch[tmp].flags & (CF_IMMORTAL | CF_NOATTACK)))
          elog("char %s has both, respawn and immortal/noattack", ch[tmp].name);
        register_npc_to_section(tmp);
        register_respawn_char(tmp);
      }
      if (!(ch[tmp].flags & (CF_RESPAWN | CF_IMMORTAL | CF_PLAYERLIKE))) {
        elog("char %s had neither respawn nor immortal nor playerlike",
             ch[tmp].name);
      }
      ...
      if (tmp == 0) {
        elog("Error in line %d: Unknown item name \"%s\" (%d,%d).", line, value,
             x, y);
        return 0;
      }
      map[x + y * MAXMAP].it = tmp;
      if (it[tmp].flags & IF_MOVEBLOCK)
        map[x + y * MAXMAP].flags |= MF_TMOVEBLOCK;
      if (it[tmp].flags & IF_SIGHTBLOCK)
        map[x + y * MAXMAP].flags |= MF_TSIGHTBLOCK;
      if (it[tmp].flags & IF_DOOR) map[x + y * MAXMAP].flags |= MF_DOOR;
      if (it[tmp].flags & IF_SOUNDBLOCK)
        map[x + y * MAXMAP].flags |= MF_TSOUNDBLOCK;
      it[tmp].x = x;
      it[tmp].y = y;
      if (it[tmp].flags & IF_TAKE) {
        elog("put IF_TAKE item %s on map at %d,%d during map load.",
             it[tmp].name, x, y);
      }
      ...
      if (((1ull << tmp) & MF_MOVEBLOCK) && map[x + y * MAXMAP].ch) {
        elog("placed char %s at %d,%d on MF_MOVEBLOCK",
             ch[map[x + y * MAXMAP].ch].name, x, y);
      }
    } else {
      elog("Map: Error in line %d: unknown name \"%s\" (%100.100s).", line,
           name, ptr);
      return 0;
    }
  }
...
int alloc_char(void) {
  ...
  // elog("alloc: %d, next=%d, prev=%d
  // uchar=%d",cn,ch[cn].next-ch,ch[cn].prev-ch,uchar-ch);

  // mark char as new in player cache
  for (n = 0; n < MAXPLAYER; n++) set_player_knows_name(n, cn, 0);
...
void free_item(int in) {
  ...
  if (it[in].content) {
    elog("trying to free item %s (%d) with content set (%d)", it[in].name, in,
         it[in].content);
    btrace("free_item");
    return;
  }
...
int init_create(void) {
  ...
  if (!map) return 0;
  xlog("Allocated map: %.2fM (%lu*%d)",
       sizeof(struct map) * MAXMAP * MAXMAP / 1024.0 / 1024.0,
       sizeof(struct map), MAXMAP * MAXMAP);
  mem_usage += sizeof(struct map) * MAXMAP * MAXMAP;

  it = (struct item *)xcalloc(sizeof(struct item) * MAXITEM, IM_BASE);
  if (!it) return 0;
  xlog("Allocated items: %.2fM (%lu*%d)",
       sizeof(struct item) * MAXITEM / 1024.0 / 1024.0, sizeof(struct item),
       MAXITEM);
  mem_usage += sizeof(struct item) * MAXITEM;

  ch =
      (struct character *)xcalloc(sizeof(struct character) * MAXCHARS, IM_BASE);
  if (!ch) return 0;
  xlog("Allocated characters: %.2fM (%lu*%d)",
       sizeof(struct item) * MAXCHARS / 1024.0 / 1024.0,
       sizeof(struct character), MAXCHARS);
  mem_usage += sizeof(struct item) * MAXCHARS;

  it_temp =
      (struct it_temp *)xcalloc(sizeof(struct it_temp) * MAXTITEM, IM_BASE);
  if (!it_temp) return 0;
  xlog("Allocated item templates: %.2fM (%lu*%d)",
       sizeof(struct it_temp) * MAXTITEM / 1024.0 / 1024.0,
       sizeof(struct it_temp), MAXTITEM);
  mem_usage += sizeof(struct it_temp) * MAXTITEM;

  ch_temp =
      (struct ch_temp *)xcalloc(sizeof(struct ch_temp) * MAXTCHARS, IM_BASE);
  if (!ch_temp) return 0;
  xlog("Allocated character templates: %.2fM (%lu*%d)",
       sizeof(struct ch_temp) * MAXTCHARS / 1024.0 / 1024.0,
       sizeof(struct ch_temp), MAXTCHARS);
...
void update_char(int cn) {
  int mod[V_MAX], base[V_MAX], beyond[V_MAX], n, in, m, v1, v2, v3, oldlight,
      fn, bless[V_MAX], infrared = 0, newlight, nonomagic = 0, oxygen = 0;
  unsigned int mmf;
  ...
  if (ch[cn].x > 0 && ch[cn].y > 0 && ch[cn].x < MAXMAP && ch[cn].y < MAXMAP)
    mmf = map[ch[cn].x + ch[cn].y * MAXMAP].flags;
  else
    mmf = 0;
  ...
  for (n = 0; n < 30; n++) {
    if ((in = ch[cn].item[n])) {
      if (in < 1 || in >= MAXITEM) {
        elog(
            "character %s (%d) has illegal item (nr %d out of bounds). "
            "Removing item.",
            ch[cn].name, cn, in);
        ch[cn].item[n] = 0;
        continue;
      ...
      for (m = 0; m < MAXMOD; m++) {
        ...
        if (v1 <= -V_MAX || v1 >= V_MAX) {
          elog(
              "item %s (%d) has illegal modifier (index %d out of bounds). "
              "Removing modifier.",
              it[in].name, in, v1);
          it[in].mod_value[m] = 0;
          it[in].mod_index[m] = 0;
        ...
        if (v1 >= 0) {  // negative indizes are requirements
          if (it[in].flags & IF_BEYONDMAXMOD)
            beyond[v1] += it[in].mod_value[m];
          else if (!(ch[cn].flags & CF_WARRIOR) && it[in].driver == IDR_BLESS)
            bless[v1] += it[in].mod_value[m];
          else
            mod[v1] += it[in].mod_value[m];
        }
      }
    }
  }
  ...
  if (nonomagic)
    ch[cn].flags |= CF_NONOMAGIC;
  else
    ch[cn].flags &= ~CF_NONOMAGIC;

  if (oxygen)
    ch[cn].flags |= CF_OXYGEN;
  else
    ch[cn].flags &= ~CF_OXYGEN;
  ...
  for (n = 0; n < 4; n++) {
    if ((fn = ch[cn].ef[n])) {
      if (fn < 1 || fn >= MAXEFFECT) {
        elog(
            "character %s (%d) has illegal effect (nr %d out of bounds). "
            "Removing effect.",
            ch[cn].name, cn, fn);
        ch[cn].ef[n] = 0;
        continue;
  ...
  for (n = 0; n < V_MAX; n++) {
    ...
    if (v1 != -1 && v2 != -1 && v3 != -1)
      base[n] +=
          (ch[cn].value[0][v1] + ch[cn].value[0][v2] + ch[cn].value[0][v3]) / 5;

    // Set total value
    if (!ch[cn].value[1][n] && n >= V_PULSE)
      ch[cn].value[0][n] = 0;
    else {
      if (n <= V_STR || n >= V_PULSE) {
        if ((ch[cn].flags & (CF_MAGE | CF_WARRIOR)) == (CF_MAGE | CF_WARRIOR))
          mod[n] = min((int)(ch[cn].value[1][n] * 0.725),
                       mod[n]);  // reduce item and spell mods to max 72.5% of
                                 // base value for seyans
        else
          mod[n] = min((int)(ch[cn].value[1][n] * 0.500),
                       mod[n]);  // reduce item and spell mods to max 50% of
                                 // base value for warrior/mage
      ...
      if (!(ch[cn].flags & CF_NOMAGIC) || (ch[cn].flags & CF_NONOMAGIC) ||
          n == V_WEAPON || n == V_ARMOR || n == V_LIGHT) {
        ch[cn].value[0][n] = base[n] + ch[cn].value[1][n] + mod[n] +
                             bless[n];  // boni from base attributes plus skill
                                        // value plus mods plus bless for mages
      } else {
        ch[cn].value[0][n] = base[n] + ch[cn].value[1][n];
      }

      if (ch[cn].prof[P_LIGHT] && n >= V_WIS && n <= V_STR && hour >= 6 &&
          hour < 18) {  // day bonus for master of light
        ch[cn].value[0][n] += ch[cn].prof[P_LIGHT] / 2;
      }
      if (ch[cn].prof[P_DARK] && n >= V_WIS && n <= V_STR &&
          (hour < 6 || hour >= 18)) {  // night bonus for master of dark
        ch[cn].value[0][n] += ch[cn].prof[P_DARK] / 2;
      }
      if (ch[cn].prof[P_CLAN] && n >= V_WIS && n <= V_STR &&
          (areaID == 13 || (mmf & MF_CLAN))) {  // bonus for clan master in
                                                // catacombs and in clan areas
    ...
    if (n == V_HP && ch[cn].value[0][n] < 0)
      ch[cn].value[0][n] = 1;
    else if (n <= V_STR && ch[cn].value[0][n] < 0)
      ch[cn].value[0][n] = 0;
  }

  if (ch[cn].value[1][V_SPEEDSKILL])
    ch[cn].value[0][V_SPEED] += ch[cn].value[0][V_SPEEDSKILL] / 2;
  if (ch[cn].prof[P_ATHLETE])
    ch[cn].value[0][V_SPEED] += ch[cn].prof[P_ATHLETE] * 3;
  if (ch[cn].prof[P_THIEF]) {
    if (ch[cn].flags & CF_THIEFMODE)
      ch[cn].value[0][V_STEALTH] += ch[cn].prof[P_THIEF] * 2;
    else
      ch[cn].value[0][V_STEALTH] += ch[cn].prof[P_THIEF];
    ch[cn].value[0][V_PERCEPT] += ch[cn].prof[P_THIEF] / 2;
  }

  if (ch[cn].prof[P_DEMON] && (ch[cn].flags & CF_DEMON)) {
    if (ch[cn].value[1][V_HAND])
      ch[cn].value[0][V_HAND] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_DAGGER])
      ch[cn].value[0][V_DAGGER] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_STAFF])
      ch[cn].value[0][V_STAFF] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_SWORD])
      ch[cn].value[0][V_SWORD] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_TWOHAND])
      ch[cn].value[0][V_TWOHAND] += ch[cn].prof[P_DEMON];

    if (ch[cn].value[1][V_ATTACK])
      ch[cn].value[0][V_ATTACK] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_PARRY])
      ch[cn].value[0][V_PARRY] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_TACTICS])
      ch[cn].value[0][V_TACTICS] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_IMMUNITY])
      ch[cn].value[0][V_IMMUNITY] += ch[cn].prof[P_DEMON];

    if (ch[cn].value[1][V_FLASH])
      ch[cn].value[0][V_FLASH] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_FIREBALL])
      ch[cn].value[0][V_FIREBALL] += ch[cn].prof[P_DEMON];
    if (ch[cn].value[1][V_FREEZE])
      ch[cn].value[0][V_FREEZE] += ch[cn].prof[P_DEMON];
  }
  ...
  if (ch[cn].value[1][V_BODYCONTROL]) {
    ...
    if ((!(in = ch[cn].item[WN_RHAND]) || !(it[in].flags & IF_WEAPON) ||
         (it[in].flags & IF_HAND)) &&
        (ch[cn].flags & CF_PLAYER))
      ch[cn].value[0][V_WEAPON] += min(90, ch[cn].value[0][V_BODYCONTROL] / 2);
  } else
    ch[cn].value[0][V_ARMOR] += get_spell_average(cn) * 17.5;

  if (ch[cn].value[1][V_ARMORSKILL])
    ch[cn].value[0][V_ARMOR] += armor_skill_bonus(cn);
  ...
  ch[cn].flags |=
      CF_UPDATE;  // we changed values, so update client side display
  ...
  if ((ch[cn].flags & CF_PLAYER) &&
      (!(ch[cn].flags & (CF_GOD)) ||
       (ch[cn].sprite >= 60 && ch[cn].sprite < 120) || ch[cn].sprite == 27 ||
       ch[cn].sprite == 157 || ch[cn].sprite == 39)) {
    ...
    if (!inr || !(it[inr].flags & IF_WEAPON)) {  // no weapon
      if (inl && it[inl].drdata[0])
        off = 3;  // torch
      else
        off = 0;  // nothing
    } else {      // weapon
      if (inl && it[inl].drdata[0])
        off = 4;  // torch and one-hand weapon
      else if ((it[inr].flags & IF_WNTWOHANDED))
        off = 2;  // two-handed weapon
      else
        off = 1;
    }

    switch (ch[cn].flags &
            (CF_WARRIOR | CF_MAGE | CF_MALE | CF_FEMALE | CF_ARCH)) {
    ...
    if ((in = ch[cn].item[WN_HEAD]) && it[in].ID == IID_DEMONSKIN1)
      demon1_count++;
    if ((in = ch[cn].item[WN_ARMS]) && it[in].ID == IID_DEMONSKIN1)
      demon1_count++;
    if ((in = ch[cn].item[WN_LEGS]) && it[in].ID == IID_DEMONSKIN1)
      demon1_count++;
    if ((in = ch[cn].item[WN_BODY]) && it[in].ID == IID_DEMONSKIN1)
      demon1_count++;
    if ((in = ch[cn].item[WN_CLOAK]) && it[in].ID == IID_DEMONSKIN1)
      demon1_count++;
    if ((in = ch[cn].item[WN_FEET]) && it[in].ID == IID_DEMONSKIN1)
      demon1_count++;

    if ((in = ch[cn].item[WN_HEAD]) && it[in].ID == IID_DEMONSKIN2)
      demon2_count++;
    if ((in = ch[cn].item[WN_ARMS]) && it[in].ID == IID_DEMONSKIN2)
      demon2_count++;
    if ((in = ch[cn].item[WN_LEGS]) && it[in].ID == IID_DEMONSKIN2)
      demon2_count++;
    if ((in = ch[cn].item[WN_BODY]) && it[in].ID == IID_DEMONSKIN2)
      demon2_count++;
    if ((in = ch[cn].item[WN_CLOAK]) && it[in].ID == IID_DEMONSKIN2)
      demon2_count++;
    if ((in = ch[cn].item[WN_FEET]) && it[in].ID == IID_DEMONSKIN2)
      demon2_count++;

    if ((in = ch[cn].item[WN_HEAD]) && it[in].ID == IID_DEMONSKIN3)
      demon3_count++;
    if ((in = ch[cn].item[WN_ARMS]) && it[in].ID == IID_DEMONSKIN3)
      demon3_count++;
    if ((in = ch[cn].item[WN_LEGS]) && it[in].ID == IID_DEMONSKIN3)
      demon3_count++;
    if ((in = ch[cn].item[WN_BODY]) && it[in].ID == IID_DEMONSKIN3)
      demon3_count++;
    if ((in = ch[cn].item[WN_CLOAK]) && it[in].ID == IID_DEMONSKIN3)
      demon3_count++;
    if ((in = ch[cn].item[WN_FEET]) && it[in].ID == IID_DEMONSKIN3)
      demon3_count++;
    ...
    if (sbase) {
      if (ch[cn].sprite != sbase + off) {
        if (ch[cn].sprite == 27 || sbase == 27 || ch[cn].sprite == 157 ||
            sbase == 257 || ch[cn].sprite == 39 || sbase == 39)
          reset_name(cn);  // must reset colors if changing to/from demon sprite
  ...
  if ((infrared && !(ch[cn].flags & CF_INFRAVISION)) ||
      (!infrared && (ch[cn].flags & CF_INFRAVISION))) {
