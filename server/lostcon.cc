...
void lostcon_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct lostcon_driver_data *)set_data(
      cn, DRD_LOSTCONDRIVER, sizeof(struct lostcon_driver_data));
  if (!dat) return;  // oops...
  ...
  if (ch[cn].x > 1 && ch[cn].x < MAXMAP && ch[cn].y > 1 && ch[cn].y < MAXMAP) {
    ...
    if (!ch[cn].player &&
        (map[m].flags & MF_RESTAREA)) {  // leave at once if at rest area
      exit_char(cn);
      return;
    }
    if (!ch[cn].player && (map[m].flags & MF_ARENA) &&
        areaID != 34) {  // leave at once if disconnected in an arena
      exit_char(cn);
      return;
    }
    if (ticker - 10 * TICKS + LAGOUT_TIME > dat->timeout &&
        (map[m].flags & MF_ARENA) &&
        areaID != 34) {  // leave after 10s if lagging in an arena
  ...
  if (ch[cn].karma <= -12 ||
      (!(ch[cn].flags & CF_PAID) && ch[cn].karma <= -5)) {
    exit_char(cn);
    return;
  }
  ...
  if (n == 4)
    create_show_effect(EF_LAG, cn, ticker, ticker + TICKS, 0, 1);
  else
    ef[fn].stop = ticker + TICKS;

  // low on hp?
  if (ch[cn].hp < ch[cn].value[1][V_HP] * (int)(POWERSCALE * 0.75)) {
    // try heal
    if (!ppd->noheal &&
        ch[cn].mana > ch[cn].value[1][V_MANA] * POWERSCALE / 2 &&
        do_heal(cn, cn))
      return;
  }
