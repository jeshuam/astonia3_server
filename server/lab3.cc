...
void lab3_passguard_driver(int cn, int ret, int lastact)
{
  ...
  // get data
  dat = (struct lab3_passguard_driver_data*)set_data(cn, DRD_LAB3_PASSGUARD, sizeof(struct lab3_passguard_driver_data));
  if (!dat) return; // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      // get lab ppd
      if (!(ppd = (struct lab_ppd*)set_data(co, DRD_LAB_PPD, sizeof(struct lab_ppd)))) { remove_message(cn, msg); continue; }
...
void lab3_prisoner_driver(int cn, int ret, int lastact)
{
  ...
  // get data
  dat = (struct lab3_prisoner_driver_data*)set_data(cn, DRD_LAB3_PRISONER, sizeof(struct lab3_prisoner_driver_data));
  if (!dat) return; // oops...
  ...
  if (dat->give_target) {
    if (!(ch[dat->give_target].flags) || ch[dat->give_target].serial != dat->give_serial || dist_from_home(cn, dat->give_target) > 8) {
      ...
      if (ch[cn].citem == 0) {
        if ((ppd = (struct lab_ppd*)set_data(dat->give_target, DRD_LAB_PPD, sizeof(struct lab_ppd)))) {
          ppd->prisoner_talkstep++;
...
{
  static const char *freshtext[5] = {" awful", "", " tasty", " delicious", " very delicious"};
...
void lab3_special(int in, int cn)
{
  ...
  if (drdata[0] == 1) {
    ...
    if (drdata[3] && (ppd = (struct lab_ppd*)set_data(cn, DRD_LAB_PPD, sizeof(struct lab_ppd))) && ppd->guard_talkstep < 20) {
  ...
  if (drdata[0] == 3) {
    ...
    case 20: if (!(ppd = (struct lab_ppd*)set_data(cn, DRD_LAB_PPD, sizeof(struct lab_ppd)))) break;
      lab3_init_password(ppd);
      log_char(cn, LOG_SYSTEM, 0, "Thou can read the incomplete word \"%s...\".", ppd->password1);
      break;

    case 21: if (!(ppd = (struct lab_ppd*)set_data(cn, DRD_LAB_PPD, sizeof(struct lab_ppd)))) break;
      lab3_init_password(ppd);
