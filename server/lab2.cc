...
void lab2_herald_driver(int cn, int ret, int lastact) {
  ...
  // get data
  dat = (struct lab2_herald_driver_data *)set_data(cn, DRD_LAB2_HERALD,
                                                   sizeof(*dat));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if ((ch[co].flags & CF_PLAYER) &&
          (ppd = (struct lab_ppd *)set_data(co, DRD_LAB_PPD, sizeof(*ppd)))) {
        if (it[ch[cn].citem].ID == IID_LAB2_ARATHASRING)
          ppd->herald_talkstep = 60;
      }
    ...
    if (msg->type == NT_CHAR) {
      ...
      switch (ppd->herald_talkstep) {
        case 0:  // INTRO
          say(cn,
              "Hello %s. I am Herald, the Keeper of this graveyard. I can't "
              "say how glad I am to see thee here. I assume thou wantst to "
              "pass this test. I need thine help urgently. Horrible things "
              "happen here, as thou hast probably noticed. I don't dare to "
              "leave the chapel since the dead are rising from their graves. "
              "°c4Arathas°c0 has caused this abomination, may his soul rest in "
              "peace.",
              ch[co].name);
          didsay = 1;
          ppd->herald_talkstep++;
        ...
        case 1:
          say(cn,
              "If thou wishest to help me, kill °c4Arathas°c0. Bring me his "
              "ring as proof, and I will open thee a gate leading out of this "
        ...
        case 10:  // ARATHAS
          say(cn,
              ...
              "°c4Elias°c0, his brother, wrote in his °c4diary°c0 when the "
              "skeletons attacked me in my study. I'm lucky I got out alive. "
              "Now I'm hiding here in the chapel. The undeads dare not enter "
              "it.");
          didsay = 1;
          ppd->herald_talkstep++;
          dat->next_talk = ticker + 10 * TICKS;
        ...
        case 11:
          say(cn,
              "But thou wished to hear about Arathas. He and his brother stem "
              "from a family of well renowed mages. They have their own "
              "°c4family vault°c0 on this graveyard. Arathas died during some "
              "kind of magical experiment, and he was buried in the family "
        ...
        case 20:  // ELIAS
          say(cn,
              "Elias is Arathas' brother. The last times I saw him, he seemed "
              "very frightened. He spoke about strange happenings in the "
              "crypt. Now we know what he was talking about.");
          didsay = 1;
          ppd->herald_talkstep++;
          dat->next_talk = ticker + 10 * TICKS;
          break;
        ...
        case 21:
          say(cn,
              "One day he entered the family vault. But he did not return, and "
              "after a while, his relatives divided his belongings among them. "
              "By now, they are all dead, too, and rest in this graveyard. If "
              "things were different, I'd show thee their graves, but with the "
              "undeads about I do not dare. Thou couldst check the books "
              "yourself, for the locations of their tombs. They are in the "
              "°c4administrative building°c0. But beware, lots of skeletons "
              "and undeads are there, too.");
          didsay = 1;
          ppd->herald_talkstep = 255;
          dat->next_talk = ticker + 10 * TICKS;
          break;
        ...
        case 40:  // ADMINISTRATIVE BUILDING
          say(cn,
              "The administrative building is located northwest of the chapel. "
              "All administrative records about the graveyard are stored "
              "there.");
          didsay = 1;
          ppd->herald_talkstep = 255;
          dat->next_talk = ticker + 5 * TICKS;
        ...
        case 50:  // DIARY
          say(cn,
              ...
              "°c4administrative building°c0. I left it there, in my study, "
              "when I fled from the undeads.");
          didsay = 1;
          ppd->herald_talkstep = 255;
          dat->next_talk = ticker + 5 * TICKS;
          break;
        ...
        case 60:  // AFTER RING
          say(cn,
              "I thank thee, %s. This ring proves that thou hast killed "
              "Arathas. I hope peace will return now.",
              ch[co].name);
          didsay = 1;
          ppd->herald_talkstep++;
          dat->next_talk = ticker + 5 * TICKS;
        ...
        case 61:  // AFTER RING
          say(cn,
              "And here, my friend, is the Gate, as I have promised thee. Thou "
              "hast been most resourceful, %s.",
              ch[co].name);
          didsay = 1;
          ppd->herald_talkstep++;
          dat->next_talk = ticker + 5 * TICKS;
    ...
    if (msg->type == NT_TEXT) {
      ...
      if (strcasestr(str, "ARATHAS")) {
        ...
        dat->next_talk = ticker + TICKS / 2;
      } else if (strcasestr(str, "ADMINISTRATIVE") &&
                 strcasestr(str, "BUILDING")) {
...
void lab2_deamon_create(int x, int y, int co) {
  ...
  if ((cn = getfirst_char())) do {
      if (ch[cn].flags && ch[cn].driver == CDR_LAB2DEAMON) {
        if (!(dat = (struct lab2_deamon_driver_data *)set_data(
                  cn, DRD_LAB2_DEAMON, sizeof(*dat))))
          return;  // oops
        if (dat->co == co && dat->serial == ch[dat->co].serial)
  ...
  // get data
  dat = (struct lab2_deamon_driver_data *)set_data(cn, DRD_LAB2_DEAMON,
                                                   sizeof(*dat));
  if (!dat) return;  // oops...
...
void lab2_deamon_driver(int cn, int ret, int lastact) {
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CREATE) {
      ...
      switch (lab2_deamon_is_elias(dat->co)) {
        ...
        case 1:
          ...
          dat->talkstep = 10;
          if (dat->co)
            player_dat = (struct lab2_player_data *)set_data(
                dat->co, DRD_LAB2_PLAYER, sizeof(*player_dat));
          else
            player_dat = NULL;
          if (!player_dat) break;
          if (player_dat->deamonchecked)
            dat->talkstep = 20;
          else
            player_dat->deamonchecked = 1;
          break;
      }
    }
    ...
    if (msg->type == NT_NPC && msg->dat1 == NTID_LAB2_DEAMONCHECK) {
      ...
      if (co == dat->co && ch[co].serial == dat->serial &&
          lab2_deamon_is_elias(co) != 1) {
        if (fight_driver_add_enemy(cn, co, 1, 1)) {
          dat->attacking = 1;
          dat->talkstep = 255;
          if (dat->observing)
            shout(cn, "Hey! Thou are not Elias. Now thou shalt die, %s!",
                  ch[co].name);
          else
            shout(cn, "I warned thee. Now thou shalt die, %s!", ch[co].name);
        }
      }
    }
  ...
  switch (dat->talkstep) {
    ...
    case 2:
      if (ticker < dat->talkticker) break;
      say(cn,
          "On behalf of Elias, mine Master, I shall not allow anyone but "
          "himself to enter this family vault. So try to reach that door and I "
          "will kill thee!");
      dat->talkstep++;
    ...
    case 4:
      if (ticker < dat->talkticker) break;
      say(cn,
          "My eyes tricked me again. Thou art not Elias. I remember excatly "
          "his black hat and cape, and also his belt and boots. So, again, go "
          "away or I will kill thee!");
      if (ch[dat->co].player) player_driver_halt(ch[dat->co].player);
      dat->talkstep = 255;
      break;
    ...
    case 12:
      if (ticker < dat->talkticker) break;
      say(cn,
          "Excuse me for coming out of my Dimension. I hadn't recognized "
          "thee.");
      dat->talkstep++;
    ...
    case 13:
      if (ticker < dat->talkticker) break;
      say(cn,
          "But 'tis good to see thee around again. Last time we met thou wert "
          "in a bad condition. Very bad, I must say, very bad indeed.");
      dat->talkstep++;
    ...
    case 15:
      if (ticker < dat->talkticker) break;
      say(cn,
          "But here I am, talking and talking. Thou sure hast more important "
          "things to do than listening to an old demon like me.");
      dat->talkstep++;
    ...
    case 52:
      if (ticker < dat->talkticker) break;
      say(cn,
          "What kind of masquerade is that. Thou are wearing some parts of "
          "Elias stuff, but thou are not Elias.");
      dat->talkstep++;
      dat->talkticker = ticker + 4 * TICKS;
      break;
    ...
    case 53:
      if (ticker < dat->talkticker) break;
      say(cn,
          "Do not try to get into the family vault, I won't let you in, and "
          "thou will have to die!");
      dat->talkstep = 255;
      break;
  }
  ...
  // do we need a teleport ? (validate enemy first, cause otherwise we'll
  // teleport into nirwana)
  if (dat->co && ch[dat->co].flags && ch[dat->co].serial == dat->serial) {
    if (dat->attacking &&
        pathfinder(ch[cn].x, ch[cn].y, ch[dat->co].x, ch[dat->co].y, 2, NULL,
                   0) == -1 &&
        pathfinder(ch[cn].x, ch[cn].y, ch[dat->co].x, ch[dat->co].y, 1, NULL,
                   0) == -1) {
      int oldx, oldy;
  ...
  // remove deamon
  if (!dat->co || (!dat->attacking && char_see_char(cn, dat->co) == 0) ||
      ch[dat->co].serial != dat->serial || !ch[dat->co].flags) {
    create_mist(ch[cn].x, ch[cn].y);
  ...
  // turn to char
  if (!dat->attacking)
    turn(cn, offset2dx(ch[cn].x, ch[cn].y, ch[dat->co].x, ch[dat->co].y));
...
struct lab2_undead_driver_data {
  ...
  int co;  // set by the grave. the enemy, just for the died driver and grave
           // bit logic
  int serial;  // set by the grave. the enemy, just for the died driver and
               // grave bit logic // short2int

  int nextwaitticker;  // used by patrol2 undead not to wait more than once
                       // behind the door
};

struct lab2_grave_data {
  char item;  // grave leads to an item (0=no, 1=hat, 2=cape, 3=boots, 4=belt,
              // 5=amulet (elias), 6=ring (arathas)
  char dummy[3];
  int co;  // 0 = grave closed, otherwise grave open and undead still walking
           // around // short2int
...
struct described_grave {
  ...
  const char *description;  // take care, a pointer
};
...
struct described_grave described_grave[MAX_DESCRIBED_GRAVE] = {
    ...
    {176, 194,
     "For his generosity %s is buried in the third grave of the first row next "
     "to the northwestern chapel aisle."},
    {176, 196, "%s is buried at the left side of her husband John."},
    {173, 191,
     "For his generosity %s is buried in the first grave of the second row "
    ...
    {196, 196,
     "For his generosity %s is buried in the first grave of the first row next "
     "to the southeastern chapel aisle."},
    {196, 194, "%s is buried at the left side of her husband John."},
    {160, 233,
     "%s is buried in the fifth grave of the second row in the southwest "
     "section of the graveyard."},
    {158, 233, "%s is buried at the left side of her husband John."},
    {162, 230,
     "%s is buried in the fourth grave of the third row in the southwest "
     "section of the graveyard."},
    {160, 230, "%s is buried at the left side of her husband John."},
    {210, 244,
     "%s is buried in the fourth grave of the second row in the southeast "
     "section of the graveyard."},
    {208, 244, "%s is buried at the left side of her husband John."},
    {206, 232,
     "%s is buried in the sixth grave of the sixth row in the southeast "
     "section of the graveyard."},
    {204, 232, "%s is buried at the left side of her husband John."},
    {172, 228,
     "%s is buried in the fifth grave of the first row in the northwest "
     "entrance section of the graveyard."},
    {172, 226, "%s is buried at the left side of her husband John."},
    {181, 224,
     "%s is buried in the seventh grave of the last row in the northwest "
     "entrance section of the graveyard."},
    {181, 222, "%s is buried at the left side of her husband John."},
    {191, 222,
     "%s is buried in the first grave of the last row in the southeast "
     "entrance section of the graveyard."},
    {191, 224, "%s is buried at the left side of her husband John."},
    {197, 232,
     "%s is buried in the sixth grave of the second row in the southeast "
     "entrance section of the graveyard."},
    {197, 234, "%s is buried at the left side of her husband John."},
    {155, 211,
     "%s is buried in the second grave of the first row in the section with "
     "the cross in front of the administrative building."},
    {155, 209, "%s is buried at the left side of her husband John."},
    {164, 201,
     "%s is buried in the seventh grave of the last row in the section with "
     "the cross in front of the administrative building."},
    {164, 199, "%s is buried at the left side of her husband John."},
    {158, 189,
     "%s is buried in the second grave of the second row in the section "
     "without the cross in front of the administravive building."},
    {158, 187, "%s is buried at the left side of her husband John."},
    {161, 189,
     "%s is buried in the second grave of the third row in the section without "
     "the cross in front of the administravive building."},
    {161, 187, "%s is buried at the left side of her husband John."},
    {208, 182,
     "%s is buried in the first grave in the northeastern part of the "
     "northeast section of the graveyard."},
    {210, 182, "%s is buried at the left side of her husband John."},
    {214, 191,
     "%s is buried in the fourth grave of the last row in the northeastern "
     "part of the northeast section of the graveyard."},
    {212, 191, "%s is buried at the left side of her husband John."}};

int get_grave_value(const char *array, int nr) {
  int byte, bit, cur;

  if (nr < 0 || nr >= max_grave) {
    xlog("fatal get_grave_bit parameter nr=%d in %s %d", nr, __FILE__,
         __LINE__);
    return -1;
  }
...
int inc_grave_value(char *array, int nr) {
  int byte, bit, old;

  if (nr < 0 || nr >= max_grave) {
    xlog("fatal set_grave_bit parameter nr=%d in %s %d", nr, __FILE__,
         __LINE__);
    return -1;
  }
  ...
  if (old == 0)
    array[byte] = (array[byte] & (~(3 << bit))) | (1 << bit);
  else if (old == 1)
    array[byte] = (array[byte] & (~(3 << bit))) | (2 << bit);
  else if (old == 2)
    array[byte] = (array[byte] & (~(3 << bit))) | (3 << bit);
#else
  byte = nr / 8;
  bit = nr % 8;
...
#ifdef GRAVE_TWO_BITS
  sizeof_player_data =
      sizeof(struct lab2_player_data) + (max_grave * 2 + 7) / 8;
#else
  sizeof_player_data = sizeof(struct lab2_player_data) + (max_grave + 7) / 8;
#endif

  xlog("found %d graves (crypt=%d yard=%d) in lab2 (%d bytes)", max_grave,
       max_crypt, max_yard, sizeof_player_data);
  graves_enumerated = 1;
}
...
int get_player_grave_value(int in, int cn, struct lab2_grave_data *grave_dat) {
  ...
  player_dat = (struct lab2_player_data *)set_data(cn, DRD_LAB2_PLAYER,
                                                   sizeof_player_data);
...
void set_player_described_graves(int cn, struct lab_ppd *ppd) {
  ...
  } while (ppd->graveindex[0] == ppd->graveindex[1] ||
           ppd->graveindex[1] == ppd->graveindex[2] ||
           ppd->graveindex[2] == ppd->graveindex[0]);
...
void lab2_undead_driver_parse(int cn, struct lab2_undead_driver_data *dat) {
  ...
  for (ptr = nextnv(ch[cn].arg, name, value); ptr;
    ...
    else if (!strcmp(name, "undead"))
      dat->undead = atoi(value);
    else
...
void lab2_undead_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct lab2_undead_driver_data *)set_data(cn, DRD_LAB2_UNDEAD,
                                                   sizeof(*dat));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CREATE) {
      ...
      if (dat->undead) {
        dat->regenit =
            create_lab2_regenerate_spell(cn, "lab2_regenerate_spell");
      }
      ...
      if (it[in].driver == IDR_LAB2_WATER && it[in].drdata[0] == 5) {
        ...
        if (ch[co].flags)
          log_char(co, LOG_SYSTEM, 0,
                   "You spill the holy water all over the %s.", ch[cn].name);

        // check if we are in the secure area
        mn = ch[cn].x + ch[cn].y * MAXMAP;
        if (((map[mn].flags & MF_NOMAGIC) && !(ch[co].flags & CF_NONOMAGIC)) ||
            dat->undead == 0) {
          say(cn, "Mwahahahaha...");
        } else {
          say(cn, "Arrgh!");
          ...
          remove_message(cn,
                         msg);  // we use return, so we remove the message here
    ...
    // the crypt patrol will remove everyone in the second corridor from his
    // enemy list
    if (dat->patrol == 2 && msg->type == NT_CHAR) {
      co = msg->dat1;
      if (cn != co && ch[co].x >= scx && ch[co].y >= scy && ch[co].x <= ecx &&
          ch[co].y <= ecy && char_see_char(cn, co))
  ...
  if (map[ch[cn].x + ch[cn].y * MAXMAP].gsprite == 20456 ||
      map[ch[cn].x + ch[cn].y * MAXMAP].gsprite == 17062) {
    say(cn, "Arrgh!");
    create_mist(ch[cn].x, ch[cn].y);
    kill_char(cn, 0);
    return;
  ...
  if (dat->patrol == 2 && (in = map[drx + dry * MAXMAP].it) &&
      it[in].driver == 2 && it[in].drdata[0] == 1 && abs(ch[cn].x - drx) < 3 &&
      abs(ch[cn].y - dry) < 3 && ch[cn].x < drx) {  // door is present and open
...
void lab2_undead_died_driver(int cn, int co) {
  ...
  undead_dat = (struct lab2_undead_driver_data *)set_data(cn, DRD_LAB2_UNDEAD,
                                                          sizeof(*undead_dat));
  ...
  player_dat = (struct lab2_player_data *)set_data(co, DRD_LAB2_PLAYER,
                                                   sizeof_player_data);
  ...
  if (val == 0) {
    if (map[it[undead_dat->graveit].x + it[undead_dat->graveit].y * MAXMAP]
            .flags &
        MF_NOMAGIC) {
      // crypta grave
      player_dat->numcrypt++;

      // log_char(co,LOG_SYSTEM,0,"%d of %d crypt graves
      // done.",player_dat->numcrypt,max_crypt);

      if (player_dat->numcrypt == max_crypt - 1) {
        ppd =
            (struct lab_ppd *)set_data(co, DRD_LAB_PPD, sizeof(struct lab_ppd));
        if (ppd && ppd->timesgotcryptgold == 0) {
          log_char(
              co, LOG_SYSTEM, 0,
              "Thou notice a silent jingling as thine enemy hits the ground.");
          ppd->timesgotcryptgold++;
          gold = 500;
        } else if (ppd && ppd->timesgotcryptgold == 1) {
          log_char(co, LOG_SYSTEM, 0,
                   "Again thou notice a silent jingling as thine enemy hits "
                   "the ground.");
          ppd->timesgotcryptgold++;
          gold = 500;
        } else
          log_char(
              co, LOG_SYSTEM, 0,
              "Thou notice nothing special as thine enemy hits the ground.");
      }
    } else {
      // yard grave
      player_dat->numyard++;

      // log_char(co,LOG_SYSTEM,0,"%d of %d yard graves
      // done.",player_dat->numyard,max_yard);
      ...
      if (player_dat->numyard == max_yard - 1) {
        ...
        if (ppd && ppd->timesgotyardgold == 0) {
          log_char(
              co, LOG_SYSTEM, 0,
              "Thou notice a loud jingling as thine enemy hits the ground.");
          ppd->timesgotyardgold++;
          gold = 7500;
        } else if (ppd && ppd->timesgotyardgold == 1) {
          log_char(co, LOG_SYSTEM, 0,
                   "Again thou notice a loud jingling as thine enemy hits the "
                   "ground.");
          ppd->timesgotyardgold++;
          gold = 7500;
        } else
...
void lab2_arathas_awakes_all(int in, int cn) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    // we won't need to, but we check the flags here cause it looks nicer if the
    // grave won't open
    grave_dat = (struct lab2_grave_data *)it[map[mn].it].drdata;
    if (get_grave_value(player_dat->bitarray, grave_dat->nr) == 0)
      lab2_grave(map[mn].it, cn);
  }
};
...
void lab2_grave(int in, int cn) {
  ...
  if (cn) {
    ...
    // get ppd
    ppd = (struct lab_ppd *)set_data(cn, DRD_LAB_PPD, sizeof(struct lab_ppd));
    if (!ppd) {
      log_char(cn, LOG_SYSTEM, 0,
               "Congratulations, you detected bug no. 12/HIHO/17. Please "
    ...
    switch (dat->item) {
      case 1:
        log_char(cn, LOG_SYSTEM, 0,
                 described_grave[ppd->graveindex[0]].description, "Henry");
        return;
      case 2:
        log_char(cn, LOG_SYSTEM, 0,
                 described_grave[ppd->graveindex[1]].description, "Eldrick");
        return;
      case 3:
        log_char(cn, LOG_SYSTEM, 0,
                 described_grave[ppd->graveindex[2]].description, "John");
        return;
      case 4:
        log_char(cn, LOG_SYSTEM, 0,
                 described_grave[ppd->graveindex[3]].description, "Mariah");
        return;
    }

    // check if it's one of the players graves (or one of the fixed item graves)
    if (it[in].x == described_grave[ppd->graveindex[0]].x &&
        it[in].y == described_grave[ppd->graveindex[0]].y)
      item = 1;
    else if (it[in].x == described_grave[ppd->graveindex[1]].x &&
             it[in].y == described_grave[ppd->graveindex[1]].y)
      item = 2;
    else if (it[in].x == described_grave[ppd->graveindex[2]].x &&
             it[in].y == described_grave[ppd->graveindex[2]].y)
      item = 3;
    else if (it[in].x == described_grave[ppd->graveindex[3]].x &&
             it[in].y == described_grave[ppd->graveindex[3]].y)
      item = 4;
    else if (dat->item)
      item = dat->item;
    else
    ...
    if (item == 0 && val > 0) {
      ...
      // set the timer for checking if the undead is still around (not true,
      // just to have it a few seconds open ;)
      call_item(it[in].driver, in, 0, ticker + TICKS * 5);

      return;
    }

    // create enemy (special items only on undead, except elias (5), he's a
    // skeleton )
    if (item != 5 && (item || RANDOM(100) <= 66))
      co = create_char("lab2_undead", 0);
    else
      co = create_char("lab2_skeleton", 0);
    if (!co) {
    ...
    if (item) {
      ...
      if (slot < INVENTORYSIZE) {
        ...
        if (in2) {
          ...
          xlog("failed to create item %d in %s %d", item, __FILE__, __LINE__);
      } else
        xlog("no free slot found, sorry in %s %d", __FILE__, __LINE__);
    }
    ...
    if ((cdat = (struct lab2_undead_driver_data *)set_data(co, DRD_LAB2_UNDEAD,
                                                           sizeof(*cdat)))) {
      cdat->graveit = in;
    ...
    // set enemy
    if (fight_driver_add_enemy(co, cn, 1, 1))
      say(co, "Thou woke me, %s. Now, thou die!", ch[cn].name);
    ...
    // and now to something completely different. arathas awakes the other
    // graves
    ...
    // he's gone
    if (dat->serial == -1 || !ch[dat->co].flags ||
        ch[dat->co].serial != dat->serial) {
      it[in].sprite--;
      dat->co = 0;
      dat->serial = 0;
      set_sector(it[in].x, it[in].y);
...
// drdata[0]:   0=not inited; 1=well; 2=altar; 3=bowl(unused); 4=waterbowl;
// 5=holy water bowl;
void lab2_water(int in, int cn) {
  ...
  if (cn) {
    if (it[in].drdata[0] == 1) {
      if (ch[cn].citem) {
        log_char(cn, LOG_SYSTEM, 0,
                 "You won't throw this into the water, will you?");
        return;
      }
      ...
      if (!ch[cn].citem || it[ch[cn].citem].driver != IDR_LAB2_WATER ||
          it[ch[cn].citem].drdata[0] != 4) {
        log_char(cn, LOG_SYSTEM, 0,
                 "You feel the holyness of the Altar. Water would be holy now, "
                 "if you had some.");
        return;
      }
    ...
    } else if (it[in].drdata[0] == 4 ||
               it[in].drdata[0] ==
                   5) {  // can't happen, item has no IF_USE flag ;-)
    ...
    if (it[in].drdata[0] == 0) {
      ...
      switch (it[in].sprite) {
        ...
        default:
          xlog("unknown sprite %d/%s in %s %d", it[in].sprite, it[in].name,
               __FILE__, __LINE__);
          break;
      }
    }
...
void lab2_stepaction(int in, int cn) {
  ...
  switch (type) {
    ...
    case 2:
      break;
    default:
      xlog("unknown lab2 stepaction type %d in %s %d", type, __FILE__,
           __LINE__);
      return;
  }
...
struct lab2_regenerate_data {
  ...
  unsigned int
      cn;       // cn to regenerate (won't need the serial, cause it's a spell)
  int startat;  // tickervalue when to start (modify this value and regeneration
                // will stop it startat>ticker)
};
