...
int driver(int type, int nr, int obj, long ret, int lastact) {
  switch (type) {
    case CDT_DRIVER:
    ...
    case CDT_SPECIAL:
      return special_driver(nr, obj, (char *)(ret));
    default:
      return 0;
  }
}
...
struct ai_preset preset[64] = {
    // name          worker level trspeed  income  endur   warc     speed
    // EGuards EGuardlevel
    {"", {0, 0, 0, 0, 0, 0, 0, 0, 0}},
...
void init_areas(void) {
  ...
  for (n = 1; n < MAXITEM; n++) {
    ...
    switch (it[n].driver) {
      case IDR_STR_SPAWNER:
        it[n].drdata[10] =
            area[slot].max_spawn;  // set slot number for npc sprite calculation
  ...
  for (n = 0; n < MAX_STR_AREA; n++) {
    if (area[n].used) {
      xlog("area %d: %d spawns, %d items", n, area[n].max_spawn,
           area[n].max_item);
    }
  }
}

int remove_party(int code, const char *msg) {
  int m, next, n, in;
  ...
  for (m = getfirst_char(); m; m = next) {
    ...
    if (ch[m].ID == code) {
      if (!teleport_char_driver(m, 15, 15) &&
          !teleport_char_driver(m, 20, 15) && !teleport_char_driver(m, 15, 20))
        teleport_char_driver(m, 20, 20);
    ...
    if (*(unsigned int *)(it[in].drdata + 0) == code) {
      if (it[in].driver == IDR_STR_SPAWNER) {
        if (*(unsigned int *)(it[in].drdata + 0) < 0xfffff000)
          ...
          *(unsigned int *)(it[in].drdata + 0) = 0xfffff000;
...
int init_mission(int n) {
  ...
  for (m = 0; m < area[ar].max_item; m++) {
    ...
    if (it[in].driver == IDR_STR_STORAGE) {
      *(unsigned int *)(it[in].drdata + 0) = 0;  // no owner
      *(unsigned int *)(it[in].drdata + 4) =
          mission[n].storage_size;                             // set gold
    ...
    if (it[in].driver == IDR_STR_SPAWNER) {
      ...
      if (*(unsigned int *)(it[in].drdata + 0) >= 0xfffff000) {
        *(unsigned int *)(it[in].drdata + 0) =
            0xfffff001 + mission[n].enemy[ai++];
...
int did_party_lose(int spawn) {
  ...
  for (m = getfirst_char(); m; m = getnext_char(m)) {
    if (!(dat = (struct strategy_data *)set_data(
              m, DRD_STRATEGYDRIVER, sizeof(struct strategy_data))) ||
...
void reward_winner(int code) {
  ...
  for (m = getfirst_char(); m; m = getnext_char(m)) {
    if (ch[m].ID == code) {
      if ((ppd = (struct strategy_ppd *)set_data(
               m, DRD_STRATEGY_PPD, sizeof(struct strategy_ppd)))) {
        ...
        if (n < 0 || n >= ARRAYSIZE(mission)) {
          ...
          if (mission[n].exp) {
            ...
            log_char(m, LOG_SYSTEM, 0,
                     "You received %d strategy experience points.",
                     mission[n].exp);
          }
        }
...
void str_ticker(int in, int cn) {
  ...
  for (n = 0; n < MAX_STR_AREA; n++) {
    ...
    for (ai = pl = m = 0; m < area[n].max_spawn; m++) {
      ...
      if (*(unsigned int *)(it[in].drdata + 0) &&
          *(unsigned int *)(it[in].drdata + 0) != 0xfffff000) {
        if (did_party_lose(in))
          remove_party(*(unsigned int *)(it[in].drdata + 0),
  ...
  for (dist = 1; dist < 20; dist++) {
    for (i = -dist; i <= dist; i++) {
      ...
      if ((in = map[x + y * MAXMAP].it) &&
          (it[in].driver == IDR_STR_DEPOT || it[in].driver == IDR_STR_STORAGE))
        return in;
    }
    for (i = -dist; i <= dist; i++) {
      x = xc + i;
...
void setname(int cn, struct strategy_data *dat) {
  ...
  sprintf(ch[cn].description, "Carrying %d Platinum, %d of %d exp", dat->platin,
...
int restplace(int cn, int m, struct strategy_data *dat) {
  static int restlist[] = {(-3 - MAXMAP * 5), (-4 - MAXMAP * 5),
                           (-5 - MAXMAP * 5), (-6 - MAXMAP * 5),

                           (-3 + MAXMAP * 5), (-4 + MAXMAP * 5),
                           (-5 + MAXMAP * 5), (-6 + MAXMAP * 5),
                           ...
                           (-3 - MAXMAP * 3), (-4 - MAXMAP * 3),
                           (-5 - MAXMAP * 3), (-6 - MAXMAP * 3),
  ...
  if (dat->restplace &&
      (!(map[m + dat->restplace].flags & (MF_MOVEBLOCK | MF_TMOVEBLOCK)) ||
       map[m + dat->restplace].ch == cn)) {
...
void strategy_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct strategy_data *)set_data(cn, DRD_STRATEGYDRIVER,
                                         sizeof(struct strategy_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      if ((ch[co].flags & CF_PLAYER) && ch[co].ID == ch[cn].group &&
          char_see_char(cn, co) && dat->order != OR_ETERNALGUARD) {
        ppd = (struct strategy_ppd *)set_data(co, DRD_STRATEGY_PPD,
                                              sizeof(struct strategy_ppd));
        ...
        if ((!me || me == cn) && strstr(text, "mine")) {
          if (!(in = finditem(co, IDR_STR_MINE))) {
            say(cn, "Sir, %s, sir, sorry sir, but I cannot find that mine.",
                get_army_rank_string(co));
            remove_message(cn, msg);
            continue;
          }
          if (!(in2 = finddepot(ch[co].x, ch[co].y, ch[co].ID))) {
            say(cn, "Sir, %s, sir, sorry sir, but I cannot find a depot.",
                get_army_rank_string(co));
            remove_message(cn, msg);
            continue;
          }
          if (abs(it[in].x - it[in2].x) > 18 ||
              abs(it[in].y - it[in2].y) > 18 ||
              abs(it[in].x - it[in2].x) + abs(it[in].y - it[in2].y) > 20) {
            say(cn, "Sir, %s, sir, sorry sir, but those are too far apart.",
                get_army_rank_string(co));
            remove_message(cn, msg);
            continue;
          }
        ...
        if ((!me || me == cn) && strstr(text, "take") &&
            (in = finditem(co, IDR_STR_DEPOT))) {
          dat->order = OR_TAKE;
          dat->or1 = in;
          dat->or2 = co;
        ...
        if ((!me || me == cn) && strstr(text, "transfer")) {
          int dx, dy;
          if (!(in = finditem(co, IDR_STR_DEPOT)) &&
              !(in = finditem(co, IDR_STR_STORAGE))) {
            say(cn,
                "Sir, %s, sir, sorry sir, but I cannot find the first depot.",
                get_army_rank_string(co));
            remove_message(cn, msg);
            continue;
          }
          dx2offset(ch[co].dir, &dx, &dy, NULL);
          if (!(in2 = finddepot(ch[co].x + dx * 16, ch[co].y + dy * 16,
                                ch[co].ID))) {
            say(cn,
                "Sir, %s, sir, sorry sir, but I cannot find the second depot.",
                get_army_rank_string(co));
            remove_message(cn, msg);
            continue;
          }
    ...
    if (dat->order == OR_GUARD || dat->order == OR_FIGHTER ||
        dat->order == OR_TRAIN || dat->order == OR_ETERNALGUARD)
  ...
  switch (dat->order) {
    case OR_MINE:
      if (!dat->platin) {
        if (!*(unsigned int *)(it[dat->or1].drdata + 4)) {
          if (abs(ch[cn].x - it[dat->or1].x) != 3 ||
              abs(ch[cn].y - it[dat->or1].y) != 3) {
    ...
    case OR_FIGHTER:
      ...
      if (abs(ch[cn].x - ch[dat->or1].x) > 2 ||
          abs(ch[cn].y - ch[dat->or1].y) > 2) {
    ...
    case OR_TRAIN:
      ...
      if (dat->platin >= dat->trainspeed) {
        ...
        xx = restplace(cn, it[dat->or1].x + it[dat->or1].y * MAXMAP, dat) %
             MAXMAP;
        yy = restplace(cn, it[dat->or1].x + it[dat->or1].y * MAXMAP, dat) /
             MAXMAP;
        ...
        if (abs(ch[cn].x - xx) + abs(ch[cn].y - yy) > 0) {
          ...
          fight_driver_set_home(cn, it[dat->or1].x, it[dat->or1].y);
        do_idle(cn, TICKS);
        return;
      }
...
void mine(int in, int cn) {
  ...
  if (ch[cn].flags & CF_PLAYER) {
    log_char(cn, LOG_SYSTEM, 0, "There are %d units of Platinum left.",
             *(unsigned int *)(it[in].drdata + 4));
    return;
  }
  dat = (struct strategy_data *)set_data(cn, DRD_STRATEGYDRIVER,
  ...
  *(unsigned int *)(it[in].drdata + 4) -= am;
  ...
  // say(cn,"Got %d gold, %d left, carrying %d",am,*(unsigned
  // int*)(it[in].drdata+4),dat->gold);
}
...
void storage(int in, int cn) {
  ...
  if (!cn) {
    if (*(unsigned int *)(it[in].drdata + 0) &&
        *(unsigned int *)(it[in].drdata + 0) != 0xfffff000) {
      *(unsigned int *)(it[in].drdata + 4) =
          min(*(unsigned int *)(it[in].drdata + 4) + it[in].drdata[9], 10000);
    }
  ...
  if (ch[cn].flags & CF_PLAYER) {
    if ((in2 = ch[cn].citem) && it[in2].driver == IDR_ENHANCE) {
      if (it[in2].drdata[0] == 1)
        am = (*(unsigned int *)(it[in2].drdata + 1)) / 50;
      else if (it[in2].drdata[0] == 2)
        am = (*(unsigned int *)(it[in2].drdata + 1)) / 5;
      else
        am = 0;
      ...
      if (am) {
        *(unsigned int *)(it[in].drdata + 4) += am;
        ...
        log_char(cn, LOG_SYSTEM, 0,
                 "Converted to %d units of Platinum and added to storage.", am);
        ...
        log_char(cn, LOG_SYSTEM, 0,
                 "You can only add mined gold or silver. The exchange rate is "
                 "5 to 1 for gold and 50 to 1 for silver.");
      }
    }
    log_char(cn, LOG_SYSTEM, 0, "This storage contains %d units of Platinum.",
             *(unsigned int *)(it[in].drdata + 4));
    return;
  }
  dat = (struct strategy_data *)set_data(cn, DRD_STRATEGYDRIVER,
...
void depot(int in, int cn) {
  ...
  if (ch[cn].flags & CF_PLAYER) {
    log_char(cn, LOG_SYSTEM, 0, "This depot contains %d units of Platinum.",
             *(unsigned int *)(it[in].drdata + 4));
    return;
  }
  dat = (struct strategy_data *)set_data(cn, DRD_STRATEGYDRIVER,
...
int spawner_sub(int in, int in2, int group, const char *name,
                struct strategy_ppd *ppd) {
  int co, cnt;
  struct strategy_data *dat;

  for (cnt = 0, co = getfirst_char(); co; co = getnext_char(co)) {
    if (ch[co].flags && ch[co].driver == CDR_STRATEGY &&
        ch[co].group == group &&
        (dat = (struct strategy_data *)set_data(
             co, DRD_STRATEGYDRIVER, sizeof(struct strategy_data))) &&
        dat->order != OR_ETERNALGUARD)
      cnt++;
  }
  ...
  *(unsigned int *)(it[in2].drdata + 4) -= NPCPRICE;
  ...
  if (item_drop_char(in, co)) {
    ...
    ch[co].group = group;
    dat = (struct strategy_data *)set_data(co, DRD_STRATEGYDRIVER,
                                           sizeof(struct strategy_data));
    if (dat) {
...
void take_spawner(int in, int cn, struct strategy_ppd *ppd) {
  ...
  if (!*(unsigned int *)(it[in].drdata + 0)) {
    ...
    *(unsigned int *)(it[in2].drdata + 0) = ch[cn].ID;
    ...
    sprintf(it[in2].name, "%.20s's Storage (%d)", ch[cn].name,
            it[in2].drdata[8]);
    log_char(
        cn, LOG_SYSTEM, 0,
        "You take control of this spawner. Use it again to create workers.");
...
void spawner(int in, int cn) {
  ...
  if (!cn) {
    ...
    if (code > 0xfffff000) {
      ...
      *(unsigned int *)(it[in2].drdata + 0) = code;
      sprintf(it[in].name, "%.20s's Spawner (%d)",
              preset[code - 0xfffff001].name, it[in].drdata[8]);
      sprintf(it[in2].name, "%.20s's Storage (%d)",
              preset[code - 0xfffff001].name, it[in2].drdata[8]);
  ...
  ppd = (struct strategy_ppd *)set_data(cn, DRD_STRATEGY_PPD,
                                        sizeof(struct strategy_ppd));
  if (!ppd) return;
...
void strategy_boss(int cn, int ret, int lastact) {
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 &&
          char_see_char(cn, co) && ch[cn].driver != CDR_LOSTCON &&
          (ppd = (struct strategy_ppd *)set_data(
               co, DRD_STRATEGY_PPD, sizeof(struct strategy_ppd))) &&
          realtime - ppd->boss_timer > 5) {
        if (!ppd->init_done) {
        ...
        switch (ppd->boss_stage) {
          case 0:
            if (get_army_rank_int(co) < 8) {
              say(cn,
                  "Ah, %s. The governer of Aston has some missions for you. "
                  "You'd better head back there and do those first.",
                  ch[co].name);
              ppd->boss_stage++;
            } else {
              say(cn,
                  "Welcome, %s, to the Ice Army's Caves. I am %s, the "
          ...
          case 2:
            say(cn,
                "We've discovered these caves a few weeks ago. Each cave seems "
                "to contain a network of depots, several castles and platinum "
                "mines. Some ancient magic is at work here, since each castle "
                "is able to create artificial creatures, which can be used as "
                "workers or fighters.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 3:
            say(cn,
                "Islena's Lieutenants are trying to gain control over the "
                "castles and the platinum mines. If they succeed, they could "
                "spawn a vast army of these artificial creatures, and overrun "
                "our newly established defenses around Aston.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 4:
            say(cn,
                "Unfortunately, we cannot spare any soldiers to fight the "
                "Lieutenants and their artificial armies. Therefore, we have "
                "to beat them using their own means.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 5:
            say(cn,
                "Your mission, %s, is to find out how to use the castles, the "
                "mines and the workers to raise an army of your own, and to "
                "defeat Islena's Lieutenants.",
                get_army_rank_string(co));
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 6:
            say(cn,
                "We have collected some information about the caves, and the "
                "Lieutenants you will encounter there. Type /mission to get a "
                "list of these caves, and the missions currently available.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 7:
            say(cn,
                "You can use /enter <number> to start any of the missions "
                "listed. With /info, you'll be able to get some information "
                "about your understanding of the Castle's magic, and with "
                "/raise <number> you can choose to research one of the topics "
                "listed with /info.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 8:
            say(cn,
                "You can also /surrender, /list, /jp and /eguard once you've "
                "started a mission. The artificial creatures obey specific "
                "spoken commands. So far, we discovered 'transfer', 'mine', "
                "'guard' and 'fight'. You will have to do some research of "
                "your own to utilize all the commands fully.");
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 9:
            say(cn, "Good luck, %s. And report back from time to time.",
                ch[co].name);
            ppd->boss_stage++;
            ppd->boss_timer = realtime;
            break;
          case 10:
            if (ppd->boss_exp > 0) {
              say(cn,
                  "Ah, %s. You made some progress defeating Islena's "
                  "Lieutenants, and I have orders to reward you. Do you prefer "
                  "°c4military rank°c0 or °c4levels and experience°c0?",
                  ch[co].name);
              ppd->boss_stage++;
      ...
      if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 &&
        ...
        if (strcasestr((char *)msg->dat2, "levels and experience")) {
          if (ppd->boss_exp > 0) {
            say(cn, "So be it.");
            give_military_pts(
                cn, co, ppd->boss_exp / 5 + 1,
                ppd->boss_exp * pow(get_army_rank_int(co) + 5, 4) / 24);
            ppd->boss_exp = ppd->boss_msg_exp = 0;
            ppd->boss_stage = 10;
          }
        }
...
void update_npc_place(int n) {
  ...
  if (abs(ad->an[n].x - ad->ap[t].x) < 10 &&
      abs(ad->an[n].y - ad->ap[t].y) < 10)
    return;

  for (m = 0; m < ad->max_ap; m++) {
    if (abs(ad->an[n].x - ad->ap[m].x) < 10 &&
        abs(ad->an[n].y - ad->ap[m].y) < 10) {
      ad->an[n].current = m;
      return;
    }
  }
  xlog("could not determine place for NPC %d at %d,%d", n, ad->an[n].x,
       ad->an[n].y);
}
...
void subtask_move(int n) {
  ...
  if (abs(ad->an[n].x - ad->ap[t].x) > 5 ||
      abs(ad->an[n].y - ad->ap[t].y) > 5) {  // to far away from target

    // can we go there without using waypoints?
    if (abs(ad->an[n].x - ad->ap[t].x) < 20 &&
        abs(ad->an[n].y - ad->ap[t].y) < 20 &&
        pathfinder(ad->an[n].x, ad->an[n].y, ad->ap[t].x, ad->ap[t].y, 1,
                   ai_check_target, 500) != -1) {
    ...
    for (last = t, m = ad->ap[t].parent; m != -1; m = ad->ap[m].parent) {
      if (m ==
          ad->an[n].current) {  // NPC is at this place, make him go up one step
        ad->an[n].order = OR_GUARD;
        ad->an[n].or1 = ad->ap[last].x;
        ad->an[n].or2 = ad->ap[last].y;
        ad->an[n].walktype = WT_DOWN;
        // xlog("to %d,%d (1) target=%d, last=%d, NPC is at
        // %d",ad->ap[last].x,ad->ap[last].y,t,last,ad->an[n].current);
        return;
      }
      last = m;
    }

    // NPC us not at any place on path from target to storage, make him go to
    // storage
    t = ad->an[n].current;
    t = ad->ap[t].parent;
    if (t == -1) {
    ...
    // xlog("to %d,%d (2) target=%d, NPC ist at
    // %d",ad->ap[t].x,ad->ap[t].y,t,ad->an[n].current);
    return;
  }
}
...
void task_idle(int n) {
  ...
  if (!(dat = (struct strategy_data *)set_data(ad->an[n].cn, DRD_STRATEGYDRIVER,
                                               sizeof(struct strategy_data))))
    return;
  ...
  ad->an[n].order = OR_GUARD;
  ad->an[n].or1 =
      restplace(ad->an[n].cn, ad->ap[t].x + ad->ap[t].y * MAXMAP, dat) % MAXMAP;
  ad->an[n].or2 =
      restplace(ad->an[n].cn, ad->ap[t].x + ad->ap[t].y * MAXMAP, dat) / MAXMAP;
...
int assign_npc(int n) {
  ...
  for (m = 0; m < ad->max_an; m++) {
    ...
    if (ad->ap[n].type == PT_MINE)
      ad->an[m].task = T_MINE;
    else
      ad->an[m].task = T_TRANSFER;
...
int assign_guards(int place, double count, int level, int ragnarok) {
  ...
  for (n = 0; n < MAXGUARD; n++) {
    ...
    // xlog("guard %d, target %d (%d %d), used
    // %d",ad->an[m].cn,ad->an[m].target,ad->ap[place].parent,place,ad->an[m].used);
    if (ad->an[m].ftarget == place) {
      ...
      if ((ad->an[m].level + 5 < level || have > count) && !ragnarok) {
        ...
        use[n] = 2;
        if (ad->an[m].current != ad->ap[place].parent &&
            ad->an[m].current != place)
          attarget = 0;
  ...
  for (n = 0; (ragnarok || have < count) && n < MAXGUARD; n++) {
    ...
    if (ad->an[m].used == 0 &&
        ((ad->nagguard != m && ad->an[m].level + 5 >= level &&
          ch[ad->an[m].cn].hp >=
              ch[ad->an[m].cn].value[0][V_HP] * POWERSCALE) ||
         ragnarok)) {
      have += THREAT(ad->an[m].cn);
      use[n] = 1;
      if (ad->an[m].current != ad->ap[place].parent &&
  ...
  if (have > count || ragnarok) {
    ...
    for (n = 0; (ragnarok || have < count) && n < MAXGUARD; n++) {
      if ((m = ad->guard[n]) != -1 && use[n]) {
        ...
        have += THREAT(ad->an[m].cn);
        // xlog("sending guard %d (%d) to
        // %d",ad->an[m].cn,ad->an[m].current,ad->an[m].target);
      }
    }
    return 1;
...
void nag_attack(void) {
  ...
  // attack if possible
  if (cnt > 1 && mindist < 99) {
    // xlog("we nag with guard %d, level %d, at place
    // %d",ad->an[guard].cn,ad->an[guard].level,place);
    ad->lastnag = ticker;
    ad->nagplace = place;
    ad->nagguard = guard;
...
void ai_init(int in, unsigned int code) {
  ...
  ad->ap[ad->max_ap].worker[0] = ad->ap[ad->max_ap].worker[1] =
      ad->ap[ad->max_ap].worker[2] = ad->ap[ad->max_ap].worker[3] = -1;
  ...
  for (n = 1; n < MAXITEM; n++) {
    ...
    if (it[n].driver == IDR_STR_DEPOT) {
      // xlog("%d: found depot %d %s at
      // %d,%d",ad->max_ap,n,it[n].name,it[n].x,it[n].y);
    ...
    for (n = 0; n < ad->max_ap; n++) {
      if (ad->ap[n].dist == cdepth) {
        ...
        for (i = 0; i < ad->max_ap; i++) {
          ...
          if (abs(ad->ap[i].x - ad->ap[n].x) < 20 &&
              abs(ad->ap[i].y - ad->ap[n].y) < 20 &&
              abs(ad->ap[i].x - ad->ap[n].x) + abs(ad->ap[i].y - ad->ap[n].y) <
                  25 &&
              pathfinder(ad->ap[i].x, ad->ap[i].y, ad->ap[n].x, ad->ap[n].y, 0,
                         ai_check_target, 200) != -1) {
            // xlog("%d/%d: path to %d,%d
            // found",cdepth,i,ad->ap[i].x,ad->ap[i].y);
            ad->ap[i].dist = cdepth + 1;
            ad->ap[i].parent = n;
            // xlog("place %d, dist %d, parent
            // %d",i,ad->ap[i].dist,ad->ap[i].parent);
          }
        }
      }
    }
  ...
  for (m = 0, n = getfirst_char(); n; n = getnext_char(n)) {
    if (ch[n].driver == CDR_STRATEGY && ch[n].group == code &&
        (dat = (struct strategy_data *)set_data(
             n, DRD_STRATEGYDRIVER, sizeof(struct strategy_data)))) {
      ad->an[m].cn = n;
...
int create_eguard(int x, int y, int group, int level, const char *name,
                  struct strategy_ppd *ppd);
...
void ai_main(int in, unsigned int code) {
  int n, m, i;
  struct strategy_data *dat;
  int cn, x, y, xs, ys, xe, ye, mindist = 0, missing = 0, ragnarok = 1,
                                nogoldleft = 1, cantrain = 0;
  ...
  for (ad->max_an = n = 0; n < MAXNPC; n++) {
    if (ad->an[n].cn) {
      if (!ch[ad->an[n].cn].flags ||
          ch[ad->an[n].cn].serial != ad->an[n].cserial) {
        ad->an[n].cn = 0;
        ...
        ad->an[n].used = -1;
        if ((dat = (struct strategy_data *)set_data(
                 ad->an[n].cn, DRD_STRATEGYDRIVER,
                 sizeof(struct strategy_data)))) {
          ad->an[n].platin = dat->platin;
        }
        if (ad->an[n].task == T_EGUARD && ad->an[n].level < ad->ppd.max_level)
          cantrain = 1;
  ...
  // update nag guard
  if ((i = ad->nagguard) != -1 &&
      (!ad->an[i].cn || ad->an[i].task != T_EGUARD ||
       ad->an[i].target != ad->nagplace || ticker - ad->lastnag > TICKS * 90))
    ad->nagguard = -1;
  ...
  for (n = 0, mindist = 99; n < ad->max_ap; n++) {
    ...
    // update platin
    ad->ap[n].platin =
        ad->ap[n].platin / 2 + *(unsigned int *)(it[ad->ap[n].in].drdata + 4);

    if (*(unsigned int *)(it[ad->ap[n].in].drdata + 0) == code)
      ad->ap[n].owned = 1;
    else
      ad->ap[n].owned = 0;
      ...
      for (x = xs; x <= xe; x += 8) {
        for (cn = getfirst_char_sector(x, y); cn; cn = ch[cn].sec_next) {
          if (ch[cn].driver == CDR_STRATEGY && ch[cn].group != code &&
              abs(ad->ap[n].x - ch[cn].x) < 10 &&
              abs(ad->ap[n].y - ch[cn].y) < 10) {
            if (seen[cn]) { /* xlog("ignoring %s ad dist
                               %d",ch[cn].name,ad->ap[n].dist); */
              continue;
            }
    ...
    if (ad->ap[n].parent != -1)
      ad->ap[n].threat += ad->ap[ad->ap[n].parent].threat / 2;

    // move threat one down the parent list
    if (ad->ap[n].threatcount && ad->ap[n].parent != -1)
      ad->ap[ad->ap[n].parent].threat = ad->ap[n].threat / 2;

    /*if (ad->ap[n].threatcount) {
    xlog("place %d at %d,%d, threat=%d, level=%d,
    count=%.0fM",n,ad->ap[n].x,ad->ap[n].y,ad->ap[n].threat,ad->ap[n].threatlevel,ad->ap[n].threatcount/1000);
    }*/

    if (*(unsigned int *)(it[ad->ap[n].in].drdata + 4) > 0 &&
        ad->ap[n].wcnt > 0) {
      for (m = ad->ap[n].parent; m != -1 && ad->ap[m].wcnt > 0;
           m = ad->ap[m].parent) {
    ...
    if (ad->ap[n].platin && !ad->ap[n].threat) {
      ...
      if (n == 0) {
        if (ad->ap[n].platin / 2 > ad->ppd.max_level && cantrain) {
          ragnarok = 0;
        }
      } else {
        ragnarok = 0;
  ...
  for (n = 0; n < ad->max_ap; n++) {
    if (ad->ap[n].threatcount && ad->ap[n].parent != -1) {
      ...
      ad->ap[ad->ap[n].parent].threatnlevel =
          max(ad->ap[ad->ap[n].parent].threatnlevel, ad->ap[n].threatlevel);
    }
    if (ad->ap[n].parent != -1 && ad->ap[ad->ap[n].parent].threatcount) {
      ad->ap[n].threatncount += ad->ap[ad->ap[n].parent].threatcount;
      ad->ap[n].threatnlevel =
          max(ad->ap[n].threatnlevel, ad->ap[ad->ap[n].parent].threatlevel);
    }
  }
  ...
  for (n = 0; n < ad->max_an; n++) {
    if (ad->an[n].cn && ad->an[n].task != T_IGNORE) {
      ad->npc_cnt++;
      if (ad->an[n].used == -1)
        ad->free_workers++;  // eguards have used!=-1 anyway
    }
  }

  // create new workers
  while (
      (ad->panic || !ad->free_workers) &&
      ad->npc_cnt <
          min(ad->ppd.max_worker,
              16 + (*(unsigned int *)(it[ad->storage_in].drdata + 4)) / 500)) {
    if (*(unsigned int *)(it[ad->storage_in].drdata + 4) >=
        NPCPRICE) {  // spawn new worker
      m = spawner_sub(in, ad->storage_in, code, preset[code - 0xfffff001].name,
                      &ad->ppd);
      if (m == 0) break;
  ...
  if (ad->panic) {
    ...
    for (n = 0; n < ad->max_an; n++) {
      ...
      if (ad->an[n].task == T_EGUARD && wantguardcnt() < ad->gcnt &&
          !ad->nogoldleft && !ad->ragnarok) {
        remove_guard(n);
      }
      ...
      if ((wantguardcnt() > ad->gcnt || ad->nogoldleft) &&
          ad->gcnt < MAXGUARD) {
        if (ad->an[n].used != -1) remove_worker(n);
      ...
      if (ad->an[n].used != -1 && ad->an[n].platin &&
          (ad->an[n].task == T_TRANSFER || ad->an[n].task == T_MINE) &&
          ad->ap[m].wcnt <= ad->worklevel && !ad->ap[m].threat &&
          ad->ap[m].dist <= mindist)
        continue;

      if (ad->an[n].used != -1 &&
          (ad->an[n].task == T_TAKE && !ad->ap[m].owned) && !ad->ap[m].threat &&
          ad->ap[m].dist <= mindist)
        continue;

      if (ad->ap[m].threat || ad->ap[i].threat || ad->ap[m].dist > mindist) {
        if (ad->an[n].used != -1) remove_worker(n);
      } else if (i > 0 && ad->ap[m].wcnt > ad->ap[i].wcnt &&
                 ad->ap[i].platin > (ad->ap[i].wcnt + 1) * WORKERPLATIN &&
                 ad->ap[i].wcnt < ad->worklevel) {
        if (ad->an[n].used != -1) remove_worker(n);
        add_worker(T_TRANSFER, n, i);
        continue;
      } else if (ad->an[n].used != -1 && ad->an[n].task == T_TRANSFER &&
                 ad->ap[m].platin && ad->ap[m].wcnt <= ad->worklevel)
        continue;
      else if (ad->an[n].used != -1 && ad->an[n].task == T_MINE &&
               ad->ap[m].platin && ad->ap[m].wcnt <= ad->worklevel)
        continue;
      ...
      for (m = 1; m < ad->max_ap; m++) {
        if (ad->ap[m].type == PT_DEPOT && !ad->ap[m].owned &&
            !ad->ap[m].threat && ad->ap[m].wcnt == 0 &&
            (bd > ad->ap[m].dist ||
             (bd == ad->ap[m].dist &&
              abs(ad->an[n].x - ad->ap[m].x) + abs(ad->an[n].y - ad->ap[m].y) <
                  bdiff))) {
          bm = m;
          bd = ad->ap[m].dist;
          bdiff =
      ...
      for (m = 1; m < ad->max_ap; m++) {
        if (!ad->ap[m].threat &&
            ad->ap[m].platin > (ad->ap[m].wcnt * WORKERPLATIN) &&
      ...
      if (bm && bd <= mindist) {
        remove_worker(n);
        if (ad->ap[bm].type == PT_MINE)
          add_worker(T_MINE, n, bm);
        else
          add_worker(T_TRANSFER, n, bm);
        continue;
      }
    ...
    for (n = 0; n < ad->max_ap; n++) {
      if (n != 0 && ad->ap[n].dist <= mindist && ad->ap[n].platin &&
          !ad->ap[n].threat) {
        // xlog("place %d, %d platin, %d
        // workers",n,ad->ap[n].platin,ad->ap[n].wcnt);
        if (ad->ap[n].wcnt < ad->worklevel - 1 &&
            ad->ap[n].wcnt * WORKERPLATIN < ad->ap[n].platin) {
          missing = 1;
        ...
        for (m = 0; m < ad->max_at; m++) {
          if (ad->at[m].place == n) {
            ...
            ad->at[m].count =
                ad->ap[n].threatcount +
                ad->ap[n]
                    .threatncount;  // max(ad->at[m].count,ad->ap[n].threatcount+ad->ap[n].threatncount);
            ad->at[m].level = max(
                ad->ap[n].threatlevel,
                ad->ap[n]
                    .threatnlevel);  // max(max(ad->at[m].level,ad->ap[n].threatlevel),ad->ap[n].threatnlevel);
            break;
          }
        }
        if (m == ad->max_at) {
    ...
    for (n = m = 0; m < ad->max_at; m++) {
      if (ad->at[m].place) {
        for (i = ad->ap[ad->at[m].place].parent; i != -1;
             i = ad->ap[i].parent) {
        ...
        if (i == -1) {
          // xlog("place %d at %d,%d, count=%.0fM, level=%d,
          // age=%.2fm",ad->at[m].place,ad->ap[ad->at[m].place].x,ad->ap[ad->at[m].place].y,ad->at[m].count/1000.0,ad->at[m].level,(ticker-ad->at[m].ticker)/((double)TICKS*60));
          if (assign_guards(ad->at[m].place, ad->at[m].count + 1,
                            ad->at[m].level, ad->ragnarok))
            ragnarok = ad->ragnarok;
        }
        n = m;
      }
    }
    ...
    if (!missing && ad->free_workers > 0 &&
        ticker - ad->lastchange > TICKS * 20) {
      ad->worklevel = min(MAXWORKER, ad->worklevel + 1);
      ad->lastchange = ticker;
    }
    if (missing && ad->free_workers == 0 &&
        ticker - ad->lastchange > TICKS * 10) {
    ...
    if (ad->ppd.eguards > ad->etguardcnt) {
      for (n = 0; n < ad->max_ap; n++) {
        if ((ad->ap[n].dist == ad->pdist && ad->ap[n].enemy_possible &&
             ad->ap[n].eguard == -1 &&
             *(unsigned int *)(it[ad->ap[n].in].drdata) == code)) {
          m = create_eguard(ad->ap[n].x + 2, ad->ap[n].y + 2, code,
                            ad->ppd.eguardlvl, preset[code - 0xfffff001].name,
                            &preset[code - 0xfffff001].ppd);
          for (i = 0; i < MAXNPC; i++) {
            if (!ad->an[i].cn) {
              ad->an[i].cn = m;
  ...
  // xlog("%s: dist=%d, free=%d, cnt=%d, level=%d, guards=%d, panic=%d, nogo=%d,
  // rag=%d,
  // ctrain=%d",preset[code-0xfffff001].name,mindist,ad->free_workers,ad->npc_cnt,ad->worklevel,ad->gcnt,ad->panic,nogoldleft,ragnarok,cantrain);

  // make NPCs do their jobs
  for (n = 0; n < ad->max_an; n++) {
    // xlog("NPC %d, target %d at
    // %d,%d",n,ad->an[n].target,ad->ap[ad->an[n].target].x,ad->ap[ad->an[n].target].y);
    switch (ad->an[n].task) {
      case T_IDLE:
        task_idle(n);
        break;
      ...
      case T_EGUARD:
        if (ad->an[n].target == 0) {
          if (ch[ad->an[n].cn].level < ad->ppd.max_level &&
              (ad->ap[0].platin > NPCPRICE * 2 || ad->free_workers ||
               ad->an[n].platin > ad->ppd.trainspeed * TRAINMULTI * 2 ||
    ...
    if ((dat = (struct strategy_data *)set_data(
             ad->an[n].cn, DRD_STRATEGYDRIVER, sizeof(struct strategy_data)))) {
...
void queue_validate(int ar) {
  ...
  for (n = 0; n < MAXQUEUE; n++) {
    if ((cn = area[ar].q_playercn[n]) &&
        (!ch[cn].flags || ch[cn].ID != area[ar].q_playerID[n])) {
...
void show_queue(int cn, int ar) {
  ...
  for (n = 0; n < MAXQUEUE; n++) {
    if (area[ar].q_playercn[n])
      log_char(cn, LOG_SYSTEM, 0, "%d: %s", n + 1,
               ch[area[ar].q_playercn[n]].name);
  }
}
...
int special_driver(int nr, int cn, char *ptr) {
  ...
  if (!special_init) {
    ...
    for (n = 1; n < MAXITEM; n++) {
      if (it[n].flags &&
          (it[n].driver == IDR_STR_DEPOT || it[n].driver == IDR_STR_STORAGE)) {
        jp[max_jp].in = n;
        jp[max_jp].x = it[n].x;
        jp[max_jp].y = it[n].y;
        if (it[n].driver == IDR_STR_DEPOT)
          sprintf(it[n].description,
                  "JP %d. A depot is used to store Platinum temporarily.",
                  max_jp);
        else
          sprintf(it[n].description,
                  "JP %d. The storage contains all Platinum collected so far.",
                  max_jp);
        max_jp++;
        if (max_jp == MAXJUMP) {
  ...
  if (*ptr == '#' || *ptr == '/') {
    ...
    if ((len = cmdcmp(ptr, "list", 4))) {
      ...
      for (val = 1; val < max_jp; val++) {
        if (*(unsigned int *)(it[jp[val].in].drdata + 0) == ch[cn].ID) {
          log_char(cn, LOG_SYSTEM, 0, "JP %d: \03%s \020%d Platinum.", val,
                   it[jp[val].in].name,
                   *(unsigned int *)(it[jp[val].in].drdata + 4));
          flag = 1;
        }
      }
    ...
    if ((len = cmdcmp(ptr, "eguard", 2))) {
      ...
      if (*(unsigned int *)(it[in].drdata + 0) != ch[cn].ID) {
        log_char(cn, LOG_SYSTEM, 0,
                 "You can only create guards for points you control.");
        return 2;
      }

      if (map_dist(ch[cn].x, ch[cn].y, it[in].x, it[in].y) > 10) {
        log_char(cn, LOG_SYSTEM, 0,
                 "You must be closer to the jump point you wish to create a "
                 "guard for.");
        return 2;
      }
      ...
      create_eguard(ch[cn].x, ch[cn].y, ch[cn].ID, ppd->eguardlvl, ch[cn].name,
                    ppd);
      ppd->eguards--;
    ...
    if ((len = cmdcmp(ptr, "info", 4))) {
      ...
      log_char(cn, LOG_SYSTEM, 0,
               "\001Name \010Value \015Exp Cost\21Increment");
      log_char(cn, LOG_SYSTEM, 0, "1 \001Base Income: \011%d \016%d \022%d",
               ppd->income, str_exp_cost(ppd, 1), str_increment(ppd, 1));
      log_char(cn, LOG_SYSTEM, 0, "2 \001Max Level: \011%d \016%d \022%d",
               ppd->max_level, str_exp_cost(ppd, 2), str_increment(ppd, 2));
      log_char(cn, LOG_SYSTEM, 0, "3 \001Max Worker: \011%d \016%d \022%d",
               ppd->max_worker, str_exp_cost(ppd, 3), str_increment(ppd, 3));
      log_char(cn, LOG_SYSTEM, 0, "4 \001Training Speed: \011%d \016%d \022%d",
               ppd->trainspeed, str_exp_cost(ppd, 4), str_increment(ppd, 4));
      log_char(cn, LOG_SYSTEM, 0, "5 \001Warcry Bonus: \011%d \016%d \022%d",
               ppd->warcry, str_exp_cost(ppd, 5), str_increment(ppd, 5));
      log_char(cn, LOG_SYSTEM, 0, "6 \001Endurance: \011%d \016%d \022%d",
               ppd->endurance, str_exp_cost(ppd, 6), str_increment(ppd, 6));
      log_char(cn, LOG_SYSTEM, 0, "7 \001Speed Bonus: \011%d \016%d \022%d",
               ppd->speed, str_exp_cost(ppd, 7), str_increment(ppd, 7));
      log_char(cn, LOG_SYSTEM, 0, "- \001Extra Guards: \011%d \016- \022-",
               ppd->eguards);
      log_char(cn, LOG_SYSTEM, 0, "8 \001Guard Level: \011%d \016%d \022%d",
               ppd->eguardlvl, str_exp_cost(ppd, 8), str_increment(ppd, 8));

      log_char(cn, LOG_SYSTEM, 0, "- \001Missions: \011%d \016-\022-",
               ppd->mis_cnt);
      log_char(cn, LOG_SYSTEM, 0, "- \001Victories: \011%d \016-\022-",
               ppd->won_cnt);
      log_char(cn, LOG_SYSTEM, 0, "- \001Experience: \011%d \016-\022-",
               ppd->exp);
    ...
    if ((len = cmdcmp(ptr, "mission", 4))) {
      ...
      log_char(cn, LOG_SYSTEM, 0,
               "°c3Nr \002Name \005Busy \007Solved \013Enemies ");
      for (n = 0; n < ARRAYSIZE(mission); n++) {
        if (mission[n].need_solve && !ppd->solve_cnt[mission[n].need_solve] &&
            !ppd->solve_cnt[mission[n].need_solve2])
          continue;
        if (mission[n].set_solve &&
            ppd->solve_cnt[mission[n].set_solve] >= MAXMISSIONTRY)
          continue;
        ...
        log_char(
            cn, LOG_SYSTEM, 0,
            "°c2 %d \002%s \006%d%c%c \010%d \013%s %s %s %s", n + 1,
            mission[n].name, area[mission[n].area].busy + cnt, self ? '/' : ' ',
            self ? self + '0' : ' ',
            mission[n].set_solve ? ppd->solve_cnt[mission[n].set_solve] : 0,
            preset[mission[n].enemy[0]].name, preset[mission[n].enemy[1]].name,
            preset[mission[n].enemy[2]].name, preset[mission[n].enemy[3]].name);
      }
      log_char(cn, LOG_SYSTEM, 0,
               "Use /enter <nr> to start a mission. If that mission is busy "
    ...
    if ((len = cmdcmp(ptr, "enter", 4))) {
      ...
      for (spawn = 0, in = 1; in < MAXITEM; in++) {
        ...
        if (it[in].drdata[8] == mission[n].area) {
          if (*(unsigned int *)(it[in].drdata + 0) == 0)
            spawn = in;
          else if (*(unsigned int *)(it[in].drdata + 0) < 0xfffff000)
            twoparty++;
        }
      ...
      if (!spawn) {
        queue_mission(cn, ar);
        log_char(cn, LOG_SYSTEM, 0,
                 "Mission area is busy. Request has been queued.");
        show_queue(cn, ar);
        return 2;
      }
    ...
    if ((len = cmdcmp(ptr, "queue", 5)) && (ch[cn].flags & CF_GOD)) {
      ...
      for (ar = 0; ar < MAX_STR_AREA; ar++) {
        for (n = 0; n < MAXQUEUE; n++) {
          ...
          log_char(cn, LOG_SYSTEM, 0, "Area %d, queue %d, cn=%d, ID=%d", ar, n,
                   area[ar].q_playercn[n], area[ar].q_playerID[n]);
        }
      }
