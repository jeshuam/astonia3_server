...
int did_party_lose(int spawn)
{
  ...
  for (m = getfirst_char(); m; m = getnext_char(m)) {
    if (!(dat = (struct strategy_data*)set_data(m, DRD_STRATEGYDRIVER, sizeof(struct strategy_data))) || dat->order == OR_ETERNALGUARD) continue;
...
void reward_winner(int code)
{
  ...
  for (m = getfirst_char(); m; m = getnext_char(m)) {
    if (ch[m].ID == code) {
      if ((ppd = (struct strategy_ppd*)set_data(m, DRD_STRATEGY_PPD, sizeof(struct strategy_ppd)))) {
...
void strategy_driver(int cn, int ret, int lastact)
{
  ...
  dat = (struct strategy_data*)set_data(cn, DRD_STRATEGYDRIVER, sizeof(struct strategy_data));
  if (!dat) return; // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_TEXT) {
      ...
      if ((ch[co].flags & CF_PLAYER) && ch[co].ID == ch[cn].group && char_see_char(cn, co) && dat->order != OR_ETERNALGUARD) {
        ppd = (struct strategy_ppd*)set_data(co, DRD_STRATEGY_PPD, sizeof(struct strategy_ppd));
...
int spawner_sub(int in, int in2, int group, const char *name, struct strategy_ppd *ppd)
{
  int co, cnt;
  struct strategy_data *dat;
  ...
  for (cnt = 0, co = getfirst_char(); co; co = getnext_char(co)) {
    if (ch[co].flags && ch[co].driver == CDR_STRATEGY && ch[co].group == group &&
        (dat = (struct strategy_data*)set_data(co, DRD_STRATEGYDRIVER, sizeof(struct strategy_data))) &&
...
void spawner(int in, int cn)
{
  ...
  ppd = (struct strategy_ppd*)set_data(cn, DRD_STRATEGY_PPD, sizeof(struct strategy_ppd));
  if (!ppd) return;
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 && char_see_char(cn, co) && ch[cn].driver != CDR_LOSTCON &&
          (ppd = (struct strategy_ppd*)set_data(co, DRD_STRATEGY_PPD, sizeof(struct strategy_ppd))) && realtime - ppd->boss_timer > 5) {
        if (!ppd->init_done) {
...
void task_idle(int n)
{
  ...
  if (!(dat = (struct strategy_data*)set_data(ad->an[n].cn, DRD_STRATEGYDRIVER, sizeof(struct strategy_data)))) return;
...
void ai_init(int in, unsigned int code)
{
  ...
  for (m = 0, n = getfirst_char(); n; n = getnext_char(n)) {
    if (ch[n].driver == CDR_STRATEGY && ch[n].group == code && (dat = (struct strategy_data*)set_data(n, DRD_STRATEGYDRIVER, sizeof(struct strategy_data)))) {
      ad->an[m].cn = n;
...
int create_eguard(int x, int y, int group, int level, const char *name, struct strategy_ppd *ppd);
  ...
  for (ad->max_an = n = 0; n < MAXNPC; n++) {
    if (ad->an[n].cn) {
      if (!ch[ad->an[n].cn].flags || ch[ad->an[n].cn].serial != ad->an[n].cserial) {
        ...
        ad->an[n].used = -1;
        if ((dat = (struct strategy_data*)set_data(ad->an[n].cn, DRD_STRATEGYDRIVER, sizeof(struct strategy_data)))) {
