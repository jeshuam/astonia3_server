...
int create_char(int user_ID, char *new_user, char *cls) {
  char data[80];
  ...
  *(unsigned int *)(ddata + 4) = add - 8;
  ...
  sprintf(buf,
          ...
          "1)",    // spacer
          new_user,
          (unsigned int)(flag & 0xffffffff), (int)time(NULL), user_ID, data,
          data, dbuf, mirror);
  ...
  if (mysql_query(&mysql, buf)) {
    ...
    sprintf(buf,
            ...
            new_user, (unsigned int)(flag & 0xffffffff), (int)time(NULL),
            user_ID);
    if (mysql_query(&mysql, buf)) {
      fprintf(stderr, "Failed to create account %s: Error: %s (%d)\n", new_user,
              mysql_error(&mysql), mysql_errno(&mysql));
...
int main(int argc, char **args) {
  if (argc != 4) {
    fprintf(stderr,
            "Usage: %s <accountID> <name> <genderandclass>\ngenderandclass: MW "
            "= male warrior, FM = female mage\n",
            args[0]);
    return 1;
  }
