...
struct qa qa[] = {
    ...
    {{"account", NULL},
     "If you want to open an account, you must first deposit (°c4explain "
     "deposit°c0) some money in it. After that, you can inquire for your "
     "balance (°c4explain balance°c0) or withdraw (°c4explain withdraw°c0) "
     "money.",
     0},
    {{"explain", "deposit", NULL},
     "To deposit 38 gold coins for example, just say: 'deposit 38'.",
     0},
    {{"explain", "withdraw", NULL},
     "To withdraw 38 gold coins for example, just say: 'withdraw 38'.",
     0},
    {{"explain", "balance", NULL},
     "To inquire about the balance of your account, just say: 'balance'",
     0}};
...
void bank_driver_parse(int cn, struct bank_driver_data *dat) {
  ...
  for (ptr = nextnv(ch[cn].arg, name, value); ptr;
    ...
    else if (!strcmp(name, "storefx"))
      dat->storefx = atoi(value);
    else if (!strcmp(name, "storefy"))
      dat->storefy = atoi(value);
    else if (!strcmp(name, "storetx"))
      dat->storetx = atoi(value);
    else if (!strcmp(name, "storety"))
      dat->storety = atoi(value);
    else if (!strcmp(name, "doorx"))
      dat->doorx = atoi(value);
    else if (!strcmp(name, "doory"))
      dat->doory = atoi(value);
    else if (!strcmp(name, "open"))
      dat->open = atoi(value);
    else if (!strcmp(name, "close"))
      dat->close = atoi(value);
    else
...
void bank_driver(int cn, int ret, int lastact) {
  ...
  dat = (struct bank_driver_data *)set_data(cn, DRD_BANKDRIVER,
                                            sizeof(struct bank_driver_data));
  if (!dat) return;  // oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      quiet_say(cn,
                "Hello %s! Would you like to open an °c4account°c0 with the "
                "Imperial Bank?",
                ch[co].name);
    ...
    if (msg->type == NT_TEXT) {
      ...
      if ((msg->dat1 == 1 || msg->dat1 == 2) &&
        ...
        ppd = (struct bank_ppd *)set_data(co, DRD_BANK_PPD,
        ...
        if (ppd && (ptr = strcasestr((char *)msg->dat2, "deposit"))) {
          ...
          if (val) {
            if (val > ch[co].gold || val < 0) {
              ...
              dlog(co, 0, "deposited %.2fG, new balance %.2fG.", val / 100.0,
                   ppd->imperial_gold / 100.0);
            }
          } else
            quiet_say(cn, "Thou must name an amount.");
          ...
          if (val) {
            if (val > ppd->imperial_gold || val < 0) {
              ...
              dlog(co, 0, "withdrew %.2fG, balance left %.2fG.", val / 100.0,
                   ppd->imperial_gold / 100.0);
            }
          } else
            quiet_say(cn, "Thou must name an amount.");
        } else if (ppd && strcasestr((char *)msg->dat2, "balance")) {
          if (ppd->imperial_gold > 100)
            quiet_say(cn, "Thou hast %d gold and %d silver in thine account.",
                      ppd->imperial_gold / 100, ppd->imperial_gold % 100);
          else if (ppd->imperial_gold)
            quiet_say(cn, "Thou hast %d silver in thine account.",
                      ppd->imperial_gold);
          else
            quiet_say(cn, "Thou dost not have any money in thine account.");
        }
      }
    }
  ...
  if (dat->dayx) {  // we have day / night positions
    if (!opening_time(dat->open, dat->close)) {  // we're closed
      if (dat->doorx &&
          !is_closed(dat->doorx, dat->doory)) {  // door is still open
        if (!is_room_empty(dat->storefx, dat->storefy, dat->storetx,
                           dat->storety)) {  // store is not empty
      ...
      if (dat->doorx &&
          is_closed(dat->doorx, dat->doory)) {  // door is still closed
        if (use_item_at(cn, dat->doorx, dat->doory, 0)) return;
        do_idle(cn, TICKS);
        return;
      }
