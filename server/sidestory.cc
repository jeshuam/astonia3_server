...
void ruby_driver(int cn, int retval, int lastact)
{
  ...
  dat = (struct ruby_driver_data*)set_data(cn, DRD_RUBYDRIVER, sizeof(struct ruby_driver_data));
  if (!dat) return;	// oops...
  ...
  for (msg = ch[cn].msg; msg; msg = next) {
    ...
    if (msg->type == NT_CHAR) {
      ...
      if ((ch[co].flags & CF_PLAYER) &&
          ...
          (ppd = (struct sidestory_ppd*)set_data(co, DRD_SIDESTORY_PPD, sizeof(struct sidestory_ppd)))) {
        switch (areaID) {
    ...
    if (msg->type == NT_GIVE) {
      ...
      if ((in = ch[cn].citem)) {
        if ((ch[co].flags & CF_PLAYER) && (ppd = (struct sidestory_ppd*)set_data(co, DRD_SIDESTORY_PPD, sizeof(struct sidestory_ppd)))) {
          switch (areaID) {
