...
int salesprice(int cn, int co, int nr) {
  ...
  if (store[s]->ware[nr].cnt) {
    ...
    price = max(price * 1.25,
                price * (double)(store[s]->pricemulti) /
                    (ch[co].value[0][V_BARTER] + 100 +
                     ch[co].prof[P_TRADER] * 5 + clan_trade_bonus(co)));
  } else
    price = 0;
...
int buyprice(int cn, int in) {
  ...
  if (!(it[in].flags & IF_MONEY))
    price = min(price * 0.80, price * (double)(ch[cn].value[0][V_BARTER] + 100 +
                                               ch[cn].prof[P_TRADER] * 5 +
                                               clan_trade_bonus(cn)) /
                                  400);
...
int buy(int cn, int co) {
  ...
  if (!(it[in].flags & (IF_QUEST | IF_NODEPOT | IF_BONDTAKE | IF_LABITEM |
                        IF_MONEY))) {  // only add non-quest items to store
    add_item_to_store(s, in);
  }
...
int create_store(int cn, int ignore, int pricemulti) {
  ...
  store[s] = (struct store *)xcalloc(sizeof(struct store), IM_STORE);
...
void player_store(int cn, int nr, int flag, int fast) {
  ...
  if (flag) {
    if (ch[cn].citem) {
      ...
      if (price) {
        if (price < 100)
          log_char(cn, LOG_SYSTEM, 0, "Sold for %dS", price);
        else
          log_char(cn, LOG_SYSTEM, 0, "Sold for %dG %dS", price / 100,
                   price % 100);
      }
    } else {
      price = sell(co, cn, nr);
      if (price) {
        if (price < 100)
          log_char(cn, LOG_SYSTEM, 0, "Bought for %dS", price);
        else
          log_char(cn, LOG_SYSTEM, 0, "Bought for %dG %dS", price / 100,
                   price % 100);
...
int init_store(void) {
  store = (struct store **)xcalloc(sizeof(struct store *) * MAXSTORE, IM_BASE);
  if (!store) return 0;
  xlog("Allocated store: %.2fM (%lu*%d)",
       sizeof(struct store *) * MAXSTORE / 1024.0 / 1024.0,
       sizeof(struct store *), MAXSTORE);
