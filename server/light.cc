...
to dark places, to make the outside light diminis slowly, the deeper into the
dark
area it gets.
*/
...
static inline void map_add_light(int x, int y, int v) {
  ...
  if (map[m].light < 0) {
    // elog("Error in light computations at %d,%d
    // (%d%+d=%d).",x,y,map[m].light-v,v,map[m].light);
...
static void add_light(int xc, int yc, int stren, int cn) {
  ...
  // if (cn) xlog("light: %03d, %03d: %+04d (%d, %s)",xc,yc,stren,cn,cn ?
  // ch[cn].name : "");

  prof = prof_start(16);

  // add light at center, we need to do this here as it blows the calculation
...
void add_char_light(int cn) {
  ...
  if (light > 0 && !(map[m].flags & MF_NOLIGHT))
    add_light(ch[cn].x, ch[cn].y, light, cn);
}
...
void add_item_light(int in) {
  ...
  if (it[in].x < 1 || it[in].x >= MAXMAP - 2 || it[in].y < 1 ||
      it[in].y >= MAXMAP - 2)
    return;
  m = it[in].x + it[in].y * MAXMAP;

  for (n = 0; n < MAXMOD; n++) {
    if (it[in].mod_index[n] == V_LIGHT) light += it[in].mod_value[n];
  }
...
void remove_item_light(int in) {
  ...
  if (light > 0 &&
      (!(it[in].flags & IF_TAKE) || !(map[m].flags & MF_NOLIGHT))) {
    add_light(it[in].x, it[in].y, -light, 0);
  }
}
...
void compute_groundlight(int x, int y) {
  ...
  if ((map[m].gsprite & 0xffff) == 14361 ||
      (map[m].gsprite & 0xffff) == 14353 ||
      ((map[m].gsprite & 0xffff) >= 12163 &&
       (map[m].gsprite & 0xffff) <= 12166)) {  // lava ground tiles
    add_light(x, y, 64, 0);
  }
}
...
void compute_shadow(int xc, int yc) {
  ...
  if (dorand)
    tmp = 512 / (shadow / 2 + RANDOM(shadow / 2 + 1) + 1);
  else
    tmp = 512 / (shadow + 1);
...
int compute_dlight(int xc, int yc) {
  ...
  if (!(map[xc + yc * MAXMAP].flags & MF_INDOORS))
    return 0;  // nothing to compute, dude!
...
int reset_dlight(int xc, int yc) {
  int xs, ys, xe, ye, x, y, m, have_indoors = 0, have_outdoors = 0, update = 0,
                               change = 0;
  unsigned long long prof;
...
int remove_lights(int xc, int yc) {
  ...
  for (y = ys; y < ye; y++) {
    ...
    for (x = xs; x < xe; x++, m++) {
      ...
      if ((fn = map[m].ef[0]) && ef[fn].light)
        remove_effect_light(x, y, ef[fn].light);
      if ((fn = map[m].ef[1]) && ef[fn].light)
        remove_effect_light(x, y, ef[fn].light);
      if ((fn = map[m].ef[2]) && ef[fn].light)
        remove_effect_light(x, y, ef[fn].light);
      if ((fn = map[m].ef[3]) && ef[fn].light)
        remove_effect_light(x, y, ef[fn].light);
    }
  }
...
int add_lights(int xc, int yc) {
  ...
  for (y = ys; y < ye; y++) {
    ...
    for (x = xs; x < xe; x++, m++) {
      ...
      if ((fn = map[m].ef[0]) && ef[fn].light)
        add_effect_light(x, y, ef[fn].light);
      if ((fn = map[m].ef[1]) && ef[fn].light)
        add_effect_light(x, y, ef[fn].light);
      if ((fn = map[m].ef[2]) && ef[fn].light)
        add_effect_light(x, y, ef[fn].light);
